...
// Checks that the seed is the same as the other seed when you input the seed.
#[test]
fn seed(){
    let code = BIP39API::generate(Language::English);
    let seed = code.derive_seed("Hello World.", Language::English);
    
    let code2 = SecretPhrase::new(&code.0);
    let seed2 = code2.derive_seed("Hello World.", Language::English);
