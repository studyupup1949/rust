...
struct Tab {
    ...
    active_pane_id: Option<PaneId>, // Swapped to Option to easily allow completely empty tabs
}
...
impl App {
    ...
    fn spawn_new_tab(&mut self) {
        // Starts completely empty with the scalable splash engine active
        self.tabs.push(Tab {
    ...
    fn initialize_empty_pane(&mut self) {
        // 1. Read cleanly from self without holding a long mutable reference
        if self.tabs[self.active_tab_index].layout_root == LayoutNode::Empty {
            let id = self.next_pane_id;
            self.next_pane_id += 1;
            
            // 2. self is completely free to be borrowed mutably here now
            let (writer, parser) = self.spawn_shell_for_pane(id);
            self.panes.insert(id, Pane { id, writer, parser });
            
            // 3. Now make the short mutable borrow to update the layout tree state
            let active_tab = &mut self.tabs[self.active_tab_index];
...
fn draw_scalable_splash(f: &mut Frame<'_>, area: Rect, is_focused: bool) {
    ...
    // Dynamic scale matrices based on area footprint
...
fn ui(f: &mut Frame<'_>, app: &App) -> Vec<(PaneId, Rect)> {
    ...
    let help_hint = match app.mode {
        Mode::Normal => "i: insert | n: split menu | t: tab menu | c: close pane | r: reset shell | q: quit",
                ...
                if let Some(pane) = app.panes.get_mut(&active_id) {
                    ...
                    if key.modifiers.contains(KeyModifiers::CONTROL) {
                        ...
                        match key.code {
                            ...
                            // Map all 4 arrow keys to standard ANSI / VT100 sequences
