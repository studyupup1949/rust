...
pub fn get_api() -> String {
    ...
    for (n,v) in env::vars() {
        ...
        if n == "CIRCLE_PROJECT_REPONAME" {
            project_repo_name = format!("{}/{}", &project_user_name, v.to_string());
        } else if n == "GITHUB_REPOSITORY" {
            project_repo_name = v.to_string();
        }
    }

    let api_endpoints = format!("https://api.github.com/repos/{}/pulls?head={}:{}&state=open", &project_repo_name,
        project_user_name, &project_branch_name);
