...
pub(crate) fn format_body(file_manager: &TempFs, save: bool) -> Value {
    ...
    if data.is_array() {
        ...
        writeln!(&mut w, "# {} issue{} found for {}", &count, seperator, &domain).unwrap();
        ...
        let json = if body.chars().count() > 65536 {
            json!({
                "body": format!("{} <p>This list has been truncated...</p>", truncate(body, 65536)),
            })
        } else {
            json!({
                "body": body,
        ...
        let mut w = Vec::new();
        writeln!(&mut w).unwrap();
        writeln!(&mut w, "# {} issue{} found for {}", &issues_length, seperator, &domain).unwrap();
        writeln!(&mut w, "<details>").unwrap();
        writeln!(&mut w, "<summary>").unwrap();
        writeln!(&mut w, "Details").unwrap();
        writeln!(&mut w, "</summary>").unwrap();
        ...
        let json = if body.chars().count() > 65536 {
            json!({
                "body": format!("{} <p>This list exceeds 65536 chars and has been truncated.</p>", truncate(body, 65326)),
            })
        } else {
            json!({
                "body": body,
