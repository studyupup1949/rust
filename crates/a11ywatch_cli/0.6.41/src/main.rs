    ...
    match &cli.command {
        ...
        Some(Commands::EXTRACT { platform, list }) => {
            if platform == "github" {
                ...
                if *list {
                    let mut report_message = String::from("<details><summary>A11yWatch testing results</summary><br>");
                    let results = results_list_to_string(&file_manager);
                    let mut replace_idx = results.lines();

                    replace_idx.next();
                    replace_idx.next();

                    let results = replace_idx.collect::<String>().to_string();

                    report_message.push_str(&format!(r#"
                    ...
                    file_manager.save_github_results(&json_data).unwrap_or_default();
