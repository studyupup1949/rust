...
The frontend when started locally (non docker) is done in the background with the main service. In order to stop the client on Unix run the following `kill $(lsof -t -i:3000)`.
To stop the primary service run the following across the ports on the [services section](https://docs.a11ywatch.com/documentation/services/) or use the command `a11ywatch stop` to kill all node processes atm.