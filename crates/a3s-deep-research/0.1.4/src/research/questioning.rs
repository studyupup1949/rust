...
const CLOSED_EVIDENCE_REASONING_GUARDRAILS: &str = "Keep every factual inference at the granularity supported by the cited evidence. Preserve the source's exact temporal, causal, comparative, quantitative, attribution, population, modality, and uncertainty scope. Co-occurring observations do not establish an unstated relation, order, cause, exclusivity, completeness, or general rule. Do not calculate or estimate a derived value from raw observations unless a cited claim states that derivation. Missing evidence establishes only the boundary of the reviewed evidence for the exact question; it does not establish the opposite proposition or absence outside that boundary. A partial excerpt does not establish anything about omitted material. Attributed evaluation remains attributed and is not converted into an objective conclusion. Recommendations may combine supported premises, but distinguish each normative recommendation from a sourced fact and keep its scope to the reviewed evidence.";
