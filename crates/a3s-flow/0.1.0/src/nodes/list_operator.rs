    ...
    async fn execute(&self, ctx: ExecContext) -> Result<Value> {
        ...
        let input_selector = ctx.data["input_selector"].as_str().ok_or_else(|| {
            FlowError::InvalidDefinition(
                "list-operator: missing data.input_selector".into(),
            )
        })?;
...
fn apply_sort(mut items: Vec<Value>, key: &str, order: &str) -> Vec<Value> {
    ...
    items.sort_by(|a, b| {
        let av = if key.is_empty() { Some(a) } else { get_path(a, key) };
        let bv = if key.is_empty() { Some(b) } else { get_path(b, key) };
...
fn compare_values(actual: &Value, op: &CondOp, expected: &Value) -> bool {
    match op {
        ...
        CondOp::Gt => numeric_cmp(actual, expected).map(|o| o.is_gt()).unwrap_or(false),
        CondOp::Lt => numeric_cmp(actual, expected).map(|o| o.is_lt()).unwrap_or(false),
        CondOp::Gte => {
            numeric_cmp(actual, expected).map(|o| o.is_ge()).unwrap_or(false)
        }
        CondOp::Lte => {
            numeric_cmp(actual, expected).map(|o| o.is_le()).unwrap_or(false)
        }
...
mod tests {
    ...
    async fn rejects_missing_input_selector() {
        let node = ListOperatorNode;
        let err = node
            .execute(ExecContext { data: json!({}), ..Default::default() })
            .await
            .unwrap_err();
