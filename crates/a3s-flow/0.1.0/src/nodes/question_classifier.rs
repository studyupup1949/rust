    ...
    async fn execute(&self, ctx: ExecContext) -> Result<Value> {
        ...
        let question_template = ctx.data["question"].as_str().ok_or_else(|| {
            FlowError::InvalidDefinition(
                "question-classifier: missing data.question".into(),
            )
        })?;

        let classes: Vec<ClassDecl> = serde_json::from_value(
            ctx.data["classes"].clone(),
        )
        .map_err(|e| {
        ...
        let messages = vec![
            ...
            ChatMessage { role: "user".into(),   content: question },
        ];
...
mod tests {
    ...
    fn make_classes() -> Vec<ClassDecl> {
        vec![
            ClassDecl { id: "tech".into(),    name: "Technical".into(), description: String::new() },
            ClassDecl { id: "billing".into(), name: "Billing".into(),   description: String::new() },
            ClassDecl { id: "general".into(), name: "General".into(),   description: String::new() },
        ]
    }
