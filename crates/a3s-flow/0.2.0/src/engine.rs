...
impl FlowEngine {
    ...
    pub fn validate(&self, definition: &Value) -> Vec<ValidationIssue> {
        ...
        let dag = match DagGraph::from_json(definition) {
            Ok(dag) => dag,
            Err(e) => {
                issues.push(ValidationIssue { node_id: None, message: e.to_string() });
                return issues;
            }
        };
        ...
        for node_def in dag.nodes_in_order() {
            ...
            if self.registry.get(&node_def.node_type).is_err() {
            ...
            if let Some(ref cond) = node_def.run_if {
                if !dag.nodes.contains_key(&cond.from) {
                    issues.push(ValidationIssue {
                        node_id: Some(node_def.id.clone()),
                        message: format!(
                            "run_if references unknown node '{}'",
                            cond.from
                        ),
                    });
                }
    ...
    pub async fn start(
        ...
        self.start_inner(definition, variables, Arc::clone(&self.emitter))
            .await
    }
    ...
    pub async fn start_streaming(
        ...
        let channel_emitter = Arc::new(ChannelEmitter::new(tx));
        let multicast: Arc<dyn EventEmitter> = Arc::new(MulticastEmitter {
            a: Arc::clone(&self.emitter),
            b: channel_emitter,
        });
        let id = self.start_inner(definition, variables, multicast).await?;
        Ok((id, rx))
    }

    /// Internal start implementation; accepts an explicit emitter so that both
    /// `start` and `start_streaming` can share the same spawn logic.
    async fn start_inner(
        ...
        let mut runner = FlowRunner::new(dag, (*self.registry).clone())
        ...
        tokio::spawn(async move {
            match runner
                .run_controlled(execution_id, variables, signal_rx, cancel)
                .await
            {
                Ok(result) => {
...
mod tests {
    ...
    async fn start_streaming_existing_emitter_also_fires() {
        ...
        let engine = FlowEngine::new(NodeRegistry::with_defaults())
            .with_event_emitter(Arc::new(CountEmitter(Arc::clone(&counter))) as Arc<dyn EventEmitter>);
