...
impl Node for AssignNode {
    ...
    async fn execute(&self, ctx: ExecContext) -> Result<Value> {
        let assigns = ctx.data["assigns"].as_object().ok_or_else(|| {
            FlowError::InvalidDefinition("assign: missing or non-object data.assigns".into())
        })?;
...
mod tests {
    ...
    async fn empty_assigns_returns_empty_object() {
        let node = AssignNode;
        let out = node.execute(ctx(json!({ "assigns": {} }))).await.unwrap();
        assert_eq!(out, json!({}));
    }
    ...
    async fn invalid_template_returns_error() {
        ...
        let err = node
            .execute(ctx(
                json!({ "assigns": { "x": "{{ bad | unknown_filter }}" } }),
            ))
            .await
            .unwrap_err();
