...
`a3s-flow` already exposes a naturally progressive API surface. The intended usage is:

| Level | Goal | API |
|-------|------|-----|
| L0 Discovery | Discover what this engine instance can do | `node_types()`, `node_descriptors()`, `capabilities()` |
| L1 Pre-flight | Check whether a flow is structurally valid before running it | `validate()` |
| L2 Fire-and-forget | Run a flow and wait for final state later | `start()`, `state()` |
| L3 Observable run | Run a flow and subscribe to progress events | `start_streaming()`, `FlowEvent` |
| L4 Runtime control | Intervene in a live execution | `pause()`, `resume()`, `terminate()` |
| L5 External context | Inspect or mutate shared execution context from outside the runner | `get_context()`, `set_context_entry()`, `delete_context_entry()` |
| L6 Named composition | Execute stored flows by stable name | `with_flow_store()`, `start_named()` |

This layering matters because callers do not need to adopt the full surface on day one:

- A CLI or test harness can stop at `validate()` + `start()`.
- A visual builder can start from `capabilities()` + `validate()`.
- A hosted workflow platform usually needs `start_streaming()` + lifecycle control + context APIs.

### L0 Discovery

Start with capability discovery rather than hard-coding node metadata.
...
- `node_types()` for simple allowlists and smoke checks.
- `node_descriptors()` for editor forms, node pickers, and grouped catalogs.
- `capabilities()` for transport-facing APIs because it carries a version marker and `progressive_disclosure = true`.
- `register_node_type()`, `register_node_type_with_descriptor()`, and `unregister_node_type()` when the engine must add, replace, or remove node types at runtime for future executions.
  `unregister_node_type()` only removes custom/runtime types; built-in node types are protected.
  ...
  "nodes": [
    {
      ...
      "default_data": { "method": "GET", "url": "", "headers": {} },
      "fields": [
        { "key": "method", "kind": "string", "required": true, "description": "HTTP method." },
        { "key": "url", "kind": "string", "required": true, "description": "Request URL." }
      ]
    }
  ]
}
...
Validate before execution when the caller is interactive or user-facing.

```rust
let issues = engine.validate(&definition);
if !issues.is_empty() {
    for issue in issues {
        eprintln!("{issue}");
    }
...
This stage is intentionally separate from execution:

- editors can validate on save
- servers can reject malformed flows early
- clients do not need to parse engine errors from `start()`

### L2 Execution

Use `start()` when you only need an execution ID and polling semantics.
...
This is the smallest practical runtime API:

- `start()` parses and validates synchronously, then spawns the run in the background
- `state()` is the pull-based snapshot API for terminal or current status

### L3 Streaming

Upgrade to `start_streaming()` when the caller needs progressive visibility.
...
- live progress UIs
- server-sent events or websocket wrappers
- metrics, audit, and operator consoles

### L4 Control

Add lifecycle control only when the product needs operator intervention.
...
- `pause()` takes effect at the next wave boundary
- `resume()` restarts a paused execution
- `terminate()` cancels between or within waves at the next cancellation checkpoint

### L5 Shared Context

Use shared context APIs when outside actors need to inspect or influence a run.

```rust
let ctx = engine.get_context(id).await?;
engine
    .set_context_entry(id, "approval".into(), serde_json::json!("granted"))
    .await?;
engine.delete_context_entry(id, "approval").await?;
```
...
- external coordination with long-running workflows

### L6 Named Flows

Add stored-flow indirection when execution should target stable names rather than inline JSON definitions.

```rust
let engine = FlowEngine::new(NodeRegistry::with_defaults())
    .with_flow_store(flow_store);

let id = engine.start_named("daily-briefing", variables).await?;
```

This is the right level for:

- reusable workflow catalogs
- tenant-managed saved flows
- composition via `"sub-flow"` nodes

### Suggested transport mapping

If you expose `a3s-flow` over HTTP or AHP/MCP-style transports, the progressive shape should stay the same:

| Intent | Suggested endpoint |
|--------|--------------------|
| discover engine | `GET /flow/capabilities` |
| validate definition | `POST /flow/validate` |
| start inline flow | `POST /flow/executions` |
| inspect state | `GET /flow/executions/:id` |
| stream events | `GET /flow/executions/:id/events` |
| pause / resume / terminate | `POST /flow/executions/:id/:action` |
| inspect / mutate context | `GET|PUT|DELETE /flow/executions/:id/context/...` |
| start named flow | `POST /flow/named/:name/executions` |
...
async fn main() -> a3s_flow::Result<()> {
    ...
    println!("available nodes: {:?}", engine.node_types()); // ["assign", "code", ...]
    println!("node catalog: {:?}", engine.node_descriptors());
...
- [HTTP API design](./DESIGN_HTTP_API.md)
