...
| Local and shared durability | `LocalFileEventStore`, `SqliteEventStore`, `PostgresEventStore`, `FlowHistoryRetentionPolicy`, `LocalFileFlowTaskQueue`, `PostgresFlowTaskQueue` | `examples/local_file_durability.rs`, `examples/sqlite_durability.rs`, `examples/sqlite_retention.rs`, `examples/postgres_durability.rs`, `examples/local_retention.rs`, `tests/sqlite_retention.rs`, `tests/postgres_retention.rs`, `tests/postgres_process_recovery.rs` | SQL stores and PostgreSQL compatibility queues use A3S ORM transactions, typed decoding, and checksummed migrations. SQLite and PostgreSQL share one retention planner that deletes complete eligible terminal components only, honors durable audit holds and parent-child references, leaves checksum tombstones, and never performs partial event-stream compaction. |
...
3. **Production storage and task management**
   ...
   - Keep SQLite and PostgreSQL whole-history retention aligned through the
     shared eligibility planner, durable audit holds, checksum tombstones, and
     parent-child reference protection. Partial event-stream compaction is
     intentionally unsupported because it would rewrite the append-only replay
     source of truth.
