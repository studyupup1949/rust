...
#[derive(Parser)]
#[command(name = "a8e-acp-server")]
#[command(about = "ACP server for a8e over HTTP and WebSocket")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let router = a8e_acp::transport::create_router(server);

    let addr: SocketAddr = format!("{}:{}", cli.host, cli.port).parse()?;
    info!("Starting a8e-acp-server on {}", addr);
