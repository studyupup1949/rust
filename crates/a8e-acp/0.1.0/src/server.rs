...
use a8e_core::mcp_utils::ToolResult;
...
impl GooseAcpAgent {
    ...
    async fn create_agent_for_session(&self) -> Arc<Agent> {
        ...
        if let Ok(config_file) = Config::new(&config_path, "a8e") {
...
impl GooseAcpAgent {
    ...
    async fn init_provider(&self, agent: &Agent, session: &Session) -> Result<Arc<dyn Provider>> {
        let model_config = match &session.model_config {
            ...
            None => {
                let config_path = self.config_dir.join(CONFIG_YAML_NAME);
                let config = Config::new(&config_path, "a8e")?;
                let model_id = config.get_a8e_model()?;
                let provider_name = config.get_a8e_provider()?;
                a8e_core::model::ModelConfig::new(&model_id)?.with_canonical_limits(&provider_name)
            }
        };
    ...
    async fn on_prompt(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(a8e_core::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions.get_mut(&session_id).ok_or_else(|| {
                        sacp::Error::invalid_params()
    ...
    async fn on_set_model(
        ...
        let config_path = self.config_dir.join(CONFIG_YAML_NAME);
        let config = Config::new(&config_path, "a8e").map_err(|e| {
            sacp::Error::internal_error().data(format!("Failed to read config: {}", e))
        })?;
        let provider_name = config.get_a8e_provider().map_err(|_| {
            sacp::Error::internal_error().data("No provider configured".to_string())
        })?;
        let model_config = a8e_core::model::ModelConfig::new(model_id)
            .map_err(|e| {
...
```",
    ...
    impl a8e_core::providers::base::Provider for MockModelProvider {
        ...
        async fn stream(
            &self,
            _model_config: &a8e_core::model::ModelConfig,
            _session_id: &str,
            _system: &str,
            _messages: &[a8e_core::conversation::message::Message],
            _tools: &[rmcp::model::Tool],
        ) -> Result<a8e_core::providers::base::MessageStream, ProviderError> {
            unimplemented!()
        }

        fn get_model_config(&self) -> a8e_core::model::ModelConfig {
            a8e_core::model::ModelConfig::new_or_fail("unused")
        }
