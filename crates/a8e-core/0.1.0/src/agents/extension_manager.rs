...
/// Manages a8e extensions / MCP clients and their interactions
...
const TOOL_EXTENSION_META_KEY: &str = "a8e_extension";
...
const A8E_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("a8e/", env!("CARGO_PKG_VERSION")));
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, A8E_USER_AGENT);
    ...
    if extract_auth_error(&client_res).is_some() {
        ...
        auth_headers.insert(reqwest::header::USER_AGENT, A8E_USER_AGENT);
    ...
    pub async fn get_extensions_info(&self, working_dir: &std::path::Path) -> Vec<ExtensionInfo> {
        ...
        self.extensions
            ...
            .map(|(name, ext)| {
                ...
                let instructions =
                    instructions.replace(a8e_mcp::WORKING_DIR_PLACEHOLDER, &working_dir_str);
