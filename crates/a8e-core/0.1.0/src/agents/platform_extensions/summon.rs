...
fn max_background_tasks() -> usize {
    std::env::var("A8E_MAX_BACKGROUND_TASKS")
        .ok()
...
impl SummonClient {
    ...
    async fn resolve_provider(
        ...
        let provider_name = params
            ...
            .or_else(|| {
                Config::global()
                    .get_param::<String>("A8E_SUBAGENT_PROVIDER")
                    .ok()
            })
...
You review code."#;
    ...
    async fn test_source_discovery_and_priority() {
        ...
        let a8e_skill = temp_dir.path().join(".a8e/skills/my-skill");
        fs::create_dir_all(&a8e_skill).unwrap();
        fs::write(
            a8e_skill.join("SKILL.md"),
            "---\nname: my-skill\ndescription: a8e version\n---\nContent",
        )
        .unwrap();
        ...
        let recipes = temp_dir.path().join(".a8e/recipes");
        ...
        assert_eq!(skill.description, "a8e version");
