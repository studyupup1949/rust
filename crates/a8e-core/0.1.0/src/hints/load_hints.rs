...
pub const A8E_HINTS_FILENAME: &str = ".a8ehints";
...
mod tests {
    ...
    #[test]
    fn test_a8ehints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(dir.path().join(A8E_HINTS_FILENAME), "Test hint content").unwrap();
        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(dir.path(), &[A8E_HINTS_FILENAME.to_string()], &gitignore);

        assert!(hints.contains("Test hint content"));
    }

    #[test]
    fn test_a8ehints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_a8ehints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        .unwrap();
        fs::write(
            dir.path().join(A8E_HINTS_FILENAME),
            "Custom hints file content from .a8ehints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), A8E_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .a8ehints"));
    }

    #[test]
    fn test_a8ehints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".a8ehints")); // Make sure it's not loading the default
    }

    #[test]
    fn test_nested_a8ehints_with_git_root() {
        ...
        fs::write(subdir.join(A8E_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(A8E_HINTS_FILENAME),
    ...
    #[test]
    fn test_nested_a8ehints_without_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let base_dir = temp_dir.path();

        fs::write(base_dir.join(A8E_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    #[test]
    fn test_nested_a8ehints_mixed_filenames() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();
        ...
        assert!(hints.contains("Subdir .a8ehints content"));
    }
...
Root level instructions"#;
        fs::write(project_root.join(A8E_HINTS_FILENAME), root_hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(A8E_HINTS_FILENAME),
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(A8E_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[A8E_HINTS_FILENAME.to_string()], &gitignore);
