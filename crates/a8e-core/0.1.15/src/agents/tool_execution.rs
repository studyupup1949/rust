...
pub const CHAT_MODE_TOOL_SKIPPED_RESPONSE: &str = "Let the user know the tool call was skipped in Articulate chat mode. \
                ...
                while let Some((req_id, confirmation)) = rx.recv().await {
                    if req_id == request.id {
                        ...
                        if let Some(finding_id) = get_security_finding_id_from_results(&request.id, inspection_results) {
                            tracing::info!(
                                monotonic_counter.a8e.prompt_injection_user_decisions = 1,
