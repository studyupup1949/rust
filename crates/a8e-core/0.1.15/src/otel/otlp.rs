...
/// Promotes a8e config-file OTel settings to env vars before exporter build.
...
fn create_resource() -> Resource {
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "a8e"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "a8e"),
        ])
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer = tracer_provider.tracer("a8e");
...
fn create_otlp_metrics_filter() -> FilterFn<impl Fn(&Metadata<'_>) -> bool> {
    FilterFn::new(|metadata: &Metadata<'_>| {
        ...
        if metadata.level() == &Level::DEBUG {
            let target = metadata.target();
            if target.starts_with("a8e::telemetry")
                || target.starts_with("a8e::metrics")
...
mod tests {
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "a8e"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "a8e")])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
        "no env vars uses a8e defaults"
    )]
    #[test_case(
    ...
    #[test_case(&[("RUST_LOG", "a8e=debug"), ("OTEL_LOG_LEVEL", "trace")], Level::TRACE; "directive RUST_LOG falls through to OTEL_LOG_LEVEL")]
    #[test_case(&[("RUST_LOG", "a8e=debug")], Level::INFO; "directive RUST_LOG falls through to default")]
