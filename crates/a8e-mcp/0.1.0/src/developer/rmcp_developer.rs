...
const DEFAULT_GOOSEIGNORE_CONTENT: &str = concat!(
    "# This file is created automatically if no .a8eignore exists.\n",
    "# Customize or uncomment the patterns below instead of deleting the file.\n",
    "# Removing it will simply cause a8e to recreate it on the next start.\n",
    "#\n",
...
impl DeveloperServer {
        ...
        for arg in &cmd_parts[1..] {
            ...
            if self.is_ignored(path) {
                return Err(ErrorData::new(
                    ErrorCode::INTERNAL_ERROR,
                    format!(
                        "The command attempts to access '{}' which is restricted by .a8eignore",
                        arg
                    ),
                    None,
                ));
    ...
    fn build_ignore_patterns(cwd: &PathBuf) -> Gitignore {
        ...
        let local_ignore_path = cwd.join(".a8eignore");

        let global_ignore_path = etcetera::choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.config_dir().join(".a8eignore"))
            .ok();
...
mod tests {
    ...
    async fn test_goose_ignore_basic_patterns() {
        ...
        // Create .a8eignore file with patterns
        fs::write(".a8eignore", "secret.txt\n*.env").unwrap();
    ...
    async fn test_text_editor_respects_ignore_patterns() {
        ...
        // Create .a8eignore file
        fs::write(".a8eignore", "secret.txt").unwrap();
    ...
    async fn test_text_editor_view_empty_directory() {
        ...
        let a8eignore_path = temp_path.join(".a8eignore");
        if a8eignore_path.exists() {
            fs::remove_file(&a8eignore_path).unwrap();
        }
    ...
    async fn test_default_patterns_when_no_ignore_files() {
        ...
        // Verify that .a8eignore is NOT created on disk (patterns applied in memory only)
        let a8eignore_path = temp_dir.path().join(".a8eignore");
        assert!(
            !a8eignore_path.exists(),
            ".a8eignore should NOT be created on disk"
        );
