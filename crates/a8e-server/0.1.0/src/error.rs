...
mod tests {
    ...
    fn test_env_var_conversion() {
        assert_eq!(to_env_var("type"), "A8E_PROVIDER__TYPE");
        assert_eq!(to_env_var("api_key"), "A8E_PROVIDER__API_KEY");
        assert_eq!(to_env_var("provider.host"), "A8E_PROVIDER__HOST");
        assert_eq!(to_env_var("provider.api_key"), "A8E_PROVIDER__API_KEY");
    }
}
