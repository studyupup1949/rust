...
use a8e_core::config::{Config, ConfigError};
use a8e_core::model::ModelConfig;
use a8e_core::providers::auto_detect::detect_provider_from_api_key;
use a8e_core::providers::base::{ProviderMetadata, ProviderType};
use a8e_core::providers::canonical::maybe_get_canonical_model;
use a8e_core::providers::create_with_default_model;
use a8e_core::providers::providers as get_providers;
use a8e_core::{
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, ErrorResponse> {
    let extensions = a8e_core::config::get_all_extensions();
    let warnings = a8e_core::config::get_warnings();
...
pub async fn add_extension(
    ...
    let extensions = a8e_core::config::get_all_extensions();
    let key = a8e_core::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    a8e_core::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, ErrorResponse> {
    let key = a8e_core::config::extensions::name_to_key(&name);
    a8e_core::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let provider = a8e_core::providers::create(&name, model_config, Vec::new()).await?;
...
pub async fn init_config() -> Result<Json<String>, ErrorResponse> {
    ...
    match a8e_core::config::base::load_init_config_from_workspace() {
        Ok(init_values) => {
...
pub async fn upsert_permissions(
    ...
    let permission_manager = a8e_core::config::PermissionManager::instance();
...
pub async fn create_custom_provider(
    ...
    let config = a8e_core::config::declarative_providers::create_custom_provider(
        a8e_core::config::declarative_providers::CreateCustomProviderParams {
    ...
    a8e_core::providers::refresh_custom_providers().await?;
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = a8e_core::config::declarative_providers::load_provider(id.as_str())
        .map_err(|e| {
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    a8e_core::config::declarative_providers::remove_custom_provider(&id)?;
...
pub async fn update_custom_provider(
    ...
    a8e_core::config::declarative_providers::update_custom_provider(
        a8e_core::config::declarative_providers::UpdateCustomProviderParams {
            id: id.clone(),
            engine: request.engine,
            display_name: request.display_name,
            api_url: request.api_url,
...
pub async fn set_config_provider(
    ...
    create_with_default_model(&provider, Vec::new())
        ...
        .and_then(|_| {
            let config = Config::global();
            config
                .set_a8e_provider(provider.clone())
                .and_then(|_| config.set_a8e_model(model.clone()))
