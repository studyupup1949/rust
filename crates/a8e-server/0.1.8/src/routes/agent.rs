...
use a8e_core::recipe_deeplink;
use a8e_core::session::session_manager::SessionType;
use a8e_core::session::{EnabledExtensionsState, ExtensionState, Session};
use a8e_core::{
...
async fn start_agent(
    ...
    a8e_core::posthog::set_session_context("desktop", false);
    ...
    let original_recipe = if let Some(deeplink) = recipe_deeplink {
        match recipe_deeplink::decode(&deeplink) {
            ...
            Err(err) => {
                ...
                a8e_core::posthog::emit_error("recipe_deeplink_decode_failed", &err.to_string());
    ...
    let mut session = manager
        ...
        .map_err(|err| {
            ...
            a8e_core::posthog::emit_error("session_create_failed", &err.to_string());
            ErrorResponse {
...
async fn resume_agent(
    ...
    a8e_core::posthog::set_session_context("desktop", true);
    ...
    let session = state
        ...
        .map_err(|err| {
            ...
            a8e_core::posthog::emit_error("session_resume_failed", &err.to_string());
            ErrorResponse {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_a8e_model().ok()) {
        Some(m) => m,
        None => {
