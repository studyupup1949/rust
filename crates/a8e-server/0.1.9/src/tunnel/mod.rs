...
fn try_acquire_tunnel_lock() -> anyhow::Result<File> {
    ...
    file.try_lock_exclusive()
        .map_err(|_| anyhow::anyhow!("Another a8e instance is already running the tunnel"))?;
...
impl TunnelManager {
    ...
    pub async fn check_auto_start(&self) {
        ...
        if auto_start && state == TunnelState::Idle {
            if is_tunnel_locked_by_another() {
                tracing::info!(
                    "Tunnel already running on another a8e instance, skipping auto-start"
                );
                return;
            }
    ...
    fn is_tunnel_disabled() -> bool {
        if let Ok(val) = std::env::var("A8E_TUNNEL") {
    ...
    pub async fn start(&self) -> anyhow::Result<TunnelInfo> {
        if Self::is_tunnel_disabled() {
            anyhow::bail!("Tunnel is disabled via A8E_TUNNEL environment variable");
        }
