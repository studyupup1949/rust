//! Generate manpages for the a8e CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man a8e`).
//!
//! When a8e is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Usage:
//!   cargo run -p a8e-cli --bin generate_manpages
//!   # or
...
fn generate_see_also(
    ...
    // Always reference the main a8e command if we're not it
    if current_name != "a8e" {
        references.push("a8e".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "a8e" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "a8e" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("a8e-{}", subcmd_name);
