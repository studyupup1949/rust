...
fn export_session_to_markdown(
    messages: Vec<a8e_core::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == rmcp::model::Role::User
            && message.content.iter().all(|content| {
                matches!(
                    content,
                    a8e_core::conversation::message::MessageContent::ToolResponse(_)
                )
            });
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                a8e_core::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {
