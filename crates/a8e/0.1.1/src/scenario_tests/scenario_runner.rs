...
{
    if let Ok(only_provider) = std::env::var("A8E_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    a8e_core::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn a8e_core::providers::base::Provider>,
            &session.id,
        )
        .await?;
