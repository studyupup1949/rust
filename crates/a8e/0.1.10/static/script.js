...
function getSessionId() {
    ...
    if (window.A8E_SESSION_NAME) {
        return window.A8E_SESSION_NAME;
    }
...
function addThinkingIndicator() {
    ...
    thinkingDiv.innerHTML = `
        ...
        <span class="thinking-text">a8e is thinking...</span>
    `;
...
function connectWebSocket() {
    ...
    const token = window.A8E_WS_TOKEN || '';
        ...
        if (response.ok) {
            ...
            if (sessionData.messages && sessionData.messages.length > 0) {
                ...
                if (sessionData.metadata && sessionData.metadata.description) {
                    document.title = `a8e chat - ${sessionData.metadata.description}`;
                }
...
function updateSessionTitle() {
    ...
    // Just show "a8e chat" - no need to show session ID
    titleElement.textContent = 'a8e chat';
}
