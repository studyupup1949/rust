...
pub const A8E_RECIPE_GITHUB_REPO_CONFIG_KEY: &str = "A8E_RECIPE_GITHUB_REPO";
...
fn discover_github_recipes(repo: &str) -> Result<Vec<RecipeInfo>> {
    ...
    let output = Command::new("gh")
        ...
        .set_no_window()
        .output()
        .map_err(|e| anyhow!("Failed to fetch repository contents using 'gh api' command (executed when A8E_RECIPE_GITHUB_REPO is configured). This requires GitHub CLI (gh) to be installed and authenticated. Error: {}", e))?;
