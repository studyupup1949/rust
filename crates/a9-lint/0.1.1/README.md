...
| `normalized-use-stmt` | Multiple separate `use root::...` statements with the same root must be merged into `use root::{...}`; also flags duplicate root segments inside a single `use { }` group |
