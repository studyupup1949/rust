...
const IDIOMATIC_NAMES: &[&str] = &[
    ...
    "is", "js", "ks", "ns", "xs", "ys", "zs", "es", "fs", "ss", "rs", "ws", "cs", "bs", "vs", "ms",
    "ps", "ts", "as", "tx", "rx", "ch", "db", "id", "fd", "io", "re", "wg", "cx", "cq", "op", "lk",
    "ok", "it", "idx", "buf", "ctx", "cfg", "src", "dst", "msg", "res", "req", "key", "val", "lhs",
    "rhs", "len", "cap", "pos", "ptr", "err", "opt", "tmp", "ret", "acc", "cmd", "env", "arg",
    "args", "out", "dir", "col", "row", "addr", "conn", "sock", "iter", "elem", "prev", "next",
    "min", "max", "sum", "cnt", "tok", "eof", "sep", "pat", "rng", "vec", "map", "set", "ast",
    "errs", "tree", "line", "expr", "node", "root", "path", "name", "span", "kind", "data", "body",
    "item", "file", "func", "spec", "flag", "mode", "type", "cwd", "cli", "segs",
];
...
impl ScopeVisitor {
    ...
    fn check_bindings_in_stmt(&mut self, stmt: &Stmt, scope: ScopeComplexity, scope_size: usize) {
        match stmt {
            Stmt::Local(local) => {
    ...
    fn check_pat_bindings(&mut self, pat: &Pat, scope: ScopeComplexity, scope_size: usize) {
        match pat {
    ...
    fn check_binding(&mut self, ident: &Ident, scope: ScopeComplexity, scope_size: usize) {
...
const fn classify_scope(stmt_count: usize) -> ScopeComplexity {
    match stmt_count {
        0..=3 => ScopeComplexity::Xs,
        4..=10 => ScopeComplexity::S,
        11..=30 => ScopeComplexity::M,
        31..=100 => ScopeComplexity::L,
        _ => ScopeComplexity::Xl,
    }
}

const fn classify_name(len: usize) -> IdentLen {
    match len {
        0..=1 => IdentLen::OneChar,
        2 => IdentLen::TwoChar,
        3..=4 => IdentLen::Short,
        _ => IdentLen::Descriptive,
    }
}

/// Minimum name class required for a given scope class.
const fn min_name_class(scope: ScopeComplexity) -> IdentLen {
    match scope {
        ScopeComplexity::Xs => IdentLen::OneChar,
        ScopeComplexity::S => IdentLen::TwoChar,
        ScopeComplexity::M => IdentLen::Short,
        ScopeComplexity::L | ScopeComplexity::Xl => IdentLen::Descriptive,
    }
}

/// Scope class name for diagnostic messages.
const fn scope_class_label(sc: ScopeComplexity) -> &'static str {
    match sc {
        ScopeComplexity::Xs => "XS (0-3 stmts)",
        ScopeComplexity::S => "S (4-10 stmts)",
        ScopeComplexity::M => "M (11-30 stmts)",
        ScopeComplexity::L => "L (31-100 stmts)",
        ScopeComplexity::Xl => "XL (101+ stmts)",
    }
}

const fn min_name_label(nc: IdentLen) -> &'static str {
    match nc {
        IdentLen::OneChar => "1-char",
        IdentLen::TwoChar => "2-char",
        IdentLen::Short => "short (3-4 chars)",
        IdentLen::Descriptive => "descriptive (5+ chars)",
    }
}
...
const fn effective_min_scope(name_class: IdentLen, idiomatic: bool) -> ScopeComplexity {
    let base = match name_class {
        IdentLen::OneChar => ScopeComplexity::Xs,
        IdentLen::TwoChar => ScopeComplexity::S,
        IdentLen::Short => ScopeComplexity::M,
        IdentLen::Descriptive => ScopeComplexity::L,
    };

    if idiomatic {
        match base {
            ScopeComplexity::Xs => ScopeComplexity::S,
            ScopeComplexity::S => ScopeComplexity::M,
            ScopeComplexity::M => ScopeComplexity::L,
            ScopeComplexity::L | ScopeComplexity::Xl => ScopeComplexity::Xl,
        }
    } else {
        base
    }
