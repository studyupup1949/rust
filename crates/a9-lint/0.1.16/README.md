...
These rules check project-level configuration (not individual source files). They are never auto-fixable.

| Rule | Description |
|---|---|
| `clippy-lints` | Cargo.toml must enable clippy all, pedantic, and nursery lint groups as warnings |
| `pre-commit-hook` | .githooks/pre-commit must exist with fmt, clippy, and a9-lint checks, and core.hooksPath must be set |
