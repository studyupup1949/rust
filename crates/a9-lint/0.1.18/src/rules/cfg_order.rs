...
mod tests {
    ...
    #[test]
    fn organon_derived_table_interleaved_pattern() {
        let src = concat!(
            "use organon_base::governance::AuthToken;\n",
            "use organon_data_source::DataSourceId;\n",
            "#[cfg(feature = \"spawn\")]\n",
            "use organon_governance::PdpService;\n",
            "use organon_named::GetName;\n",
            "use organon_pipeline::PipelineId;\n",
            "use organon_table::ColumnDef;\n",
            "#[cfg(feature = \"spawn\")]\n",
            "pub use private::args::DerivedTableServiceArgs;\n",
            "use serde::Serialize;\n",
            "use theta::prelude::*;\n",
            "use uuid::Uuid;\n",
        );
        ...
        assert!(
            !vs.is_empty(),
            "should detect unconditional uses after cfg-gated ones"
        );

        let fixed = fix(src);
        let gov_pos = fixed.find("organon_governance").unwrap();
        let named_pos = fixed.find("organon_named").unwrap();

        assert!(
            named_pos < gov_pos,
            "unconditional (named) should come before cfg-gated (governance)\nfixed:\n{fixed}"
        );
    }

    #[test]
    fn full_fix_api_interleaved_pattern() {
        let src = concat!(
            "use a::A;\n",
            "use b::B;\n",
            "#[cfg(feature = \"x\")]\n",
        ...
        let result = crate::fix(src, &[], &[]);
        let c_pos = result.source.find("c::C").unwrap();
        let d_pos = result.source.find("d::D").unwrap();

        assert!(
            d_pos < c_pos,
            "unconditional d::D should come before cfg-gated c::C\nsource:\n{}",
            result.source
        );
    }
}
