...
| `no-private-reexport` | public re-exports from `private::` module are forbidden; place the type in a public layer instead |
...
| `unnecessary-import-alias` | Flags `use A::B as C` when a direct import or 2-segment qualified path would suffice |
...
These rules check project-level configuration (not individual source files).

| Rule | Description |
|---|---|
| `clippy-lints` | [lints.clippy] exact: all=deny, pedantic/nursery=warn + 2 allow exceptions |
| `no-pub-in-private` | items in private/ modules must not have bare `pub` visibility; use `pub(crate)` or move the type out of `private/` |
| `pre-commit-hook` | .githooks/pre-commit must match canonical smart hook (auto-fixable) |
| `scan-coverage` | Every workspace member's src/ must be reachable by the effective scan dirs |
