...
| `pre-commit-hook` | pre-commit hook must exist with fmt --check, clippy -D warnings, and a9-lint check |
...
| Rule | Description |
|---|---|
| `theta-actor-at-bottom` | the `#[actor]` impl block must be the last item in the file |
