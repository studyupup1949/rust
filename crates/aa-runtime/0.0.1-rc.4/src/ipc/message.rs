...
pub enum IpcFrame {
    ...
    Heartbeat,
    /// The SDK's signed proof over the session nonce, proving it holds the
    /// agent's Ed25519 key (AAASM-3583). The first frame the SDK sends, in
    /// reply to the runtime's `HandshakeChallenge`.
...
/// - `5` = HandshakeChallenge (AAASM-3583)
#[derive(Debug)]
pub enum IpcResponse {
    ...
    /// The runtime's per-session nonce challenge, sent as the first frame on a
    /// freshly accepted connection (AAASM-3583). The SDK must reply with a
    /// `HandshakeProof` signing the nonce before any other frame is served.
