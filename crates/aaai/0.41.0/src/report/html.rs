...
pub fn build_html(
    ...
    for r in &result.results {
        ...
        let reason_raw = r.entry.as_ref().map(|e| masking.mask(&e.reason)).unwrap_or_default();
        ...
        let ticket = r.entry.as_ref().and_then(|e| e.ticket.as_ref())
            .map(|t| format!("<span class=\"ticket\">[{}]</span> ", html_escape(&masking.mask(t))))
            .unwrap_or_default();
...
<p style="margin-top:20px;font-size:0.75rem;color:#aaa">Generated by aaai vVERSION_PLACEHOLDER</p>
        ...
        def_line = definition_path.map(|p| format!(
            "&nbsp;·&nbsp; Definition: <code>{}</code>", html_escape(&masking.mask(&p.display().to_string()))
        )).unwrap_or_default(),
