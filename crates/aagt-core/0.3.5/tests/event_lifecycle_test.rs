...
use aagt_core::agent::core::AgentEvent;
use futures::stream;
use std::sync::Arc;
...
impl Provider for MockProvider {
    async fn stream_completion(&self, request: ChatRequest) -> aagt_core::error::Result<StreamingResponse> {
...
struct TestTool;
#[async_trait]
impl aagt_core::skills::tool::Tool for TestTool {
    ...
    loop {
        match rx.recv().await {
            Ok(event) => {
                ...
                if matches!(event, AgentEvent::Response { .. }) || matches!(event, AgentEvent::Error { .. }) {
