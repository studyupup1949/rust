...
use aagt_core::skills::SkillLoader;
use std::sync::Arc;
use tempfile::tempdir;
use async_trait::async_trait;
use aagt_core::agent::provider::{Provider, ChatRequest};
...
impl Provider for MockProvider {
    async fn stream_completion(&self, _request: ChatRequest) -> aagt_core::error::Result<StreamingResponse> {
...
async fn test_skill_forging_and_execution() {
    std::env::set_var("AAGT_UNSAFE_SKILL_EXEC", "true");
    ...
    let result = agent.call_tool("forge_skill", &forge_args.to_string()).await.expect("Forging failed");
    ...
    let hello_args = serde_json::json!({ "name": "AAGT" });
    let hello_result = agent.call_tool("hello_tool", &hello_args.to_string()).await.expect("Executing forged tool failed");
    
    assert_eq!(hello_result.trim(), "Hello, AAGT!");
}
