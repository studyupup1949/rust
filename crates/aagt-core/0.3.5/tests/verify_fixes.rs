...
use aagt_qmd::QmdMemory;
use aagt_core::trading::risk::{RiskManager, RiskConfig};
...
async fn test_verify_memory_tiering_compilation() -> anyhow::Result<()> {
    ...
    let cold = Arc::new(aagt_qmd::QmdMemory::new(Arc::new(aagt_qmd::QmdStore::new(db_path).expect("Failed to create QmdStore"))));
...
fn test_verify_context_safety() {
    ...
    let ctx = futures::executor::block_on(mgr.build_context(&history)).unwrap();
    assert_eq!(ctx.len(), 3);
    ...
    let ctx_pruned = futures::executor::block_on(mgr.build_context(&history_huge)).unwrap();
...
async fn test_verify_risk_zombie_cleanup() -> anyhow::Result<()> {
    use aagt_core::trading::risk::{FileRiskStore};
    ...
    let ctx = aagt_core::trading::risk::TradeContext {
