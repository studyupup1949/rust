//! Executer stage: materializes validated tasks into the final runtime state.
//!
...
/// Trait for executing the final materializationthe of configuration state.
///
...
impl DefaultExecuter {
    ...
    fn execute_inheritance(
        ...
        // Load the base file and merge its key-value pairs with child-wins
        // semantics: existing child keys are never overwritten.
        let raw_path = parts[0];
        let resolved = resolve_relative_path(raw_path, source_dir);
        let content = std::fs::read_to_string(&resolved).map_err(|e| AamlError::IoError {
            details: e.to_string(),
            diagnostics: Some(Box::new(ErrorDiagnostics::new(
                "I/O operation failed",
                format!(
                    "Could not read derived file '{}': {}",
                    resolved.display(),
                    e
                ),
        ...
        let sub_output = sub_pipeline
            .process_with_source_dir(&content, resolved.parent())
            .map_err(|errors| {
                errors
                    .into_iter()
                    .next()
                    .unwrap_or_else(|| AamlError::DirectiveError {
                        directive: "derive".to_string(),
                        message: "Unknown error in derived file".to_string(),
                        diagnostics: None,
                    })
            })?;
        ...
        // Apply defaults for explicitly selected schemas.
