...
//!
//! Pipeline stages:
//! 1. **Lexer** - tokenizes raw text into a ` Token ` stream
//! 2. **Parser** - builds AST from tokens, manages scope
//! 3. **Validator** - applies schema and type checks to AST
//! 4. **Executer** - executes directives and populates the final map
...
impl Pipeline {
    ...
    fn collect_referenced_schemas(context: &ExecutionContext) -> HashSet<SmolStr> {
        context
            .schemas
            .values()
    ...
    fn validate_schema_required_fields(context: &ExecutionContext) -> Vec<AamlError> {
        let referenced_schemas = Self::collect_referenced_schemas(context);
    ...
    fn validate_schema_field_types(context: &ExecutionContext) -> Vec<AamlError> {
        context
            ...
            .filter_map(|(key, value)| {
                ...
                utils::validate_type_value(value, type_name, context)
                    .err()
    ...
    fn process_with_tasks<'a>(
        ...
        let _guard = source_dir::SourceDirGuard::new(source_dir.map(std::path::Path::to_path_buf));
