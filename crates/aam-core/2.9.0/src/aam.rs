...
impl AAM {
    /// Attempts to read `text` as a file path. Returns `Some((content, source_dir))` if it is,
    /// or `None` if it should be treated as raw AAM content.
    fn try_read_file(text: &str) -> Result<Option<(String, Option<&Path>)>, Vec<AamlError>> {
    ...
    pub fn parse(text: &str) -> Result<Self, Vec<AamlError>> {
        if let Some((content, source_dir)) = Self::try_read_file(text)? {
