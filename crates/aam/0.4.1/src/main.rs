...
enum Commands {
    ...
    /// Split a multisection AAML file into separate AAM files
    Split {
        /// Path to the multisection AAML file
        file: PathBuf,
    ...
    /// Infer a @schema definition from one or more AAM files
    Schema {
        /// Name for the main schema
        name: String,
        /// One or more AAM files to analyze
        files: Vec<PathBuf>,
    },
    /// Run the LSP server
    Lsp,
...
fn run_schema(name: &str, files: &[PathBuf]) -> Result<()> {
    let mut file_sources: Vec<String> = Vec::new();
    for file in files {
        let content = fs::read_to_string(file)
            .with_context(|| format!("Failed to read file: {}", file.display()))?;
        file_sources.push(content);
    }
    let refs: Vec<&str> = file_sources.iter().map(String::as_str).collect();
    match reconstructer::reconstruct_schema(name, &refs) {
        ...
        Err(e) => {
            eprintln!("✗ Schema reconstruction failed: {e}");
            std::process::exit(1);
        }
    }
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Schema { name, files }) => run_schema(&name, &files),
