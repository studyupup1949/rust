...
pub const KNOWN_COMMANDS: &[&str] = &[
    "open ", "save", "check", "format", "quit", "help", "get ", "close", "mode", "split ", "toml ",
    "schema",
];
...
impl<'a> App<'a> {
    ...
    fn execute_input_command(&mut self, command: &str, args: &[&str]) {
        ...
        match command {
            ...
            "split" => {
                // If a path argument is provided, split that file; otherwise split the currently active file.
                if let Some(path) = args.first() {
                    self.split_aam_file(path);
                } else {
                    self.split_active_file();
                }
            }
            "toml" => {
                // If a path argument is provided, convert that file; otherwise convert the currently active file.
                if let Some(path) = args.first() {
                    self.convert_toml_file(path);
                } else {
                    self.convert_toml_active();
                }
            }
            "schema" => {
                let name = args.first().copied().unwrap_or("Schema");
                self.reconstruct_schema(name);
            }
    ...
    /// Split the currently active open file (wrapper around `split_aam_file` using empty path).
    pub fn split_active_file(&mut self) {
        self.split_aam_file("");
    }

    /// Convert the currently active open file from TOML to AAM modules.
    pub fn convert_toml_active(&mut self) {
        self.convert_toml_file("");
    }

    pub fn split_aam_file(&mut self, path_str: &str) {
        // When called from TUI: if path_str is empty, operate on the active file's content
        // Otherwise read from the provided path. Open resulting sections as new in-memory tabs
        // placed next to the source file. Do NOT write to disk here; saving a tab will
        // write it to its assigned path.
        let path = PathBuf::from(path_str);

        let use_active_file = if let Some(active_file) = self.get_active_file() {
            // use active if caller passed empty path or explicitly passed the active file path
            path_str.is_empty() || active_file.path == path
        } else {
            false
        };

        let (content, source_path) = if use_active_file {
            if let Some(file) = self.get_active_file() {
                (file.textarea.lines().join("\n"), file.path.clone())
            } else {
                self.status_message = "✗ No active file to split".to_string();
                return;
            }
        } else if path_str.is_empty() {
            self.status_message = "✗ Usage: split or split <file>".to_string();
            return;
        } else {
            match std::fs::read_to_string(&path) {
                Ok(c) => (c, path),
                Err(e) => {
                    self.status_message = "✗ Failed to read file".to_string();
                    self.error_message = Some(e.to_string());
                    return;
                }
            }
        ...
        if sections.is_empty() {
            self.status_message = "✗ No sections found".to_string();
            return;
        }

        // Determine insertion position: next to source file (after it). If no active file,
        // append at the end.
        let insert_base = self
            .active_file_index
        ...
        for (filename, builder) in sections {
            ...
            // If the section corresponds to the source file name and we're using the active file,
            // replace the active tab's content instead of creating a duplicate tab.
            if use_active_file
                && file_path == source_path
                && let Some(idx) = self.active_file_index
            {
                self.files[idx] = FileTab::new(file_path.clone(), aam_content);
                created += 1;
                continue;
            }

            // Avoid opening duplicate tabs for the same path
            if self.is_file_opened(&file_path) {
                continue;
            }

            let insert_pos = insert_base.saturating_add(offset).saturating_add(1);
            self.files
                .insert(insert_pos, FileTab::new(file_path.clone(), aam_content));
            offset += 1;
            created += 1;
        }

        self.status_message = format!("✓ Split: {created} files opened (in memory)");
    }

    pub fn convert_toml_file(&mut self, path_str: &str) {
        let path = PathBuf::from(path_str);
        ...
        let (content, source_path) = if use_active_file {
            if let Some(file) = self.get_active_file() {
                ...
                self.status_message = "✗ No active file to convert".to_string();
                return;
            }
        } else if path_str.is_empty() {
            self.status_message = "✗ Usage: toml or toml <file>".to_string();
            return;
        } else {
            match std::fs::read_to_string(&path) {
                Ok(c) => (c, path),
        ...
        // Try to convert TOML to modules; catch panics to avoid crashing the TUI
        ...
        match result {
            Ok(Ok(modules)) => {
                if modules.is_empty() {
                    self.status_message = "✗ No content generated".to_string();
                    return;
                }
                ...
                if use_active_file && main_path == source_path {
                    if let Some(idx) = self.active_file_index {
                        self.files[idx] = FileTab::new(main_path, modules[0].as_string());
                        created += 1;
                    }
                } else if !self.is_file_opened(&main_path) {
                    let insert_pos = self
                        .active_file_index
                        .unwrap_or_else(|| self.files.len().saturating_sub(1))
                        + 1;
                    self.files
                        .insert(insert_pos, FileTab::new(main_path, modules[0].as_string()));
                    created += 1;
                }
                ...
                for (idx, module) in modules.iter().enumerate().skip(1) {
                    let module_path = output_dir.join(format!("module_{idx}.aam"));
                    if self.is_file_opened(&module_path) {
                        continue;
                    }
                    let insert_base = self
                        .active_file_index
                    ...
                    self.files.insert(
                        insert_pos,
                        FileTab::new(module_path.clone(), module.as_string()),
                    );
                    offset += 1;
                    created += 1;
                }
                ...
                self.status_message =
                    format!("✓ TOML converted: {created} files opened (in memory)");
            }
            Ok(Err(e)) => {
                self.status_message = "✗ TOML conversion failed".to_string();
                self.error_message = Some(e.to_string());
            }
            Err(_) => {
                self.status_message = "✗ TOML parsing failed".to_string();
                self.error_message = Some("Invalid TOML format".to_string());
            }
        }
    }

    pub fn reconstruct_schema(&mut self, name: &str) {
        if self.files.is_empty() {
            self.status_message = "✗ No open files to infer schema from".to_string();
            return;
        }
        ...
        let sources: Vec<String> = self
            .files
            .iter()
            .map(|f| f.textarea.lines().join("\n"))
            .collect();
        let refs: Vec<&str> = sources.iter().map(String::as_str).collect();

        match reconstructer::reconstruct_schema(name, &refs) {
            Ok(formatted) => {
                let result_path = PathBuf::from(format!("{name}_inferred.aam"));
                self.files.push(FileTab::new(result_path, formatted));
                self.active_file_index = Some(self.files.len() - 1);
                self.status_message =
                    format!("✓ Schema '{name}' inferred from {} file(s)", sources.len());
            }
            Err(e) => {
                self.status_message = "✗ Schema reconstruction failed".to_string();
