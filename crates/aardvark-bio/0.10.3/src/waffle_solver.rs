/*!
# Waffle Solver
Contains the core logic for comparing truth and query variants relative to each other.
In Aardvark, this is the main entry point for comparing a set of truth variants against a set of query variants.
The regions are intended to be processed in parallel, and each region is compared independently.
