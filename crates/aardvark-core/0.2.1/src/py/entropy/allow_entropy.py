...
def allow_bad_entropy_calls(n):
    ...
    try:
        ...
        if ALLOWED_ENTROPY_CALLS[0] > 0:
            # In the local aardvark environment, certain imports (e.g. numpy.random)
            # do not always perform the expected number of entropy calls. Rather than
            # crashing the worker, degrade gracefully and just reset the counter.
