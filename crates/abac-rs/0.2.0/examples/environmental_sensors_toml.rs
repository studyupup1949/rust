//! Environmental sensor monitoring using TOML configuration from file.
//!
//! This example demonstrates loading sensor definitions and alert rules from
//! a TOML configuration file as an alternative to YAML.
//!
//! Run with: `cargo run --example environmental_sensors_toml -p abac-rs`
...
fn main() {
    println!("=== Environmental Sensor Monitoring System (TOML Config) ===");
    println!();

    // Load configuration from file
    let config_path = "crates/abac-rs/examples/sensor_config.toml";
    ...
    println!("=== Summary ===");
    println!("Configuration format: TOML");
    println!("Total sensor types: {}", config.sensor_types.len());
    for sensor_type in &config.sensor_types {
        println!(
            "  - {}: {} alerts configured",
            sensor_type.name,
