...
pub enum PipelineError {
    ...
    /// Refresh failed and backup restoration also failed — policy is now empty.
    #[error("refresh failed and backup restore also failed (policy is empty): refresh error: {refresh_error}, restore error: {restore_error}")]
    RefreshCatastrophic {
        /// The original error from load_rules.
        refresh_error: String,
        /// The error from restoring backup rules.
        ...
        if let Some(source) = &mut self.source {
            ...
            if let Err(e) = self.policy.load_rules(filtered) {
                ...
                if let Err(restore_err) = self.policy.load_rules(backup) {
                    log::error!(
                        "failed to restore backup rules after refresh failure. \
                         Policy is now empty (all requests will be denied). \
                         Restore error: {}, original error: {}",
                        restore_err,
                        e
                    );
                    return Err(PipelineError::RefreshCatastrophic {
                        refresh_error: e.to_string(),
