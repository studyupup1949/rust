...
pub trait Matcher: Send + Sync {
    ...
    /// Returns the name of this matcher for audit and explain output.
    ///
    /// The default returns `"custom"`. Custom implementations should override
    /// this to return a descriptive name (e.g., `"regex"`, `"glob"`, `"cidr"`).
    fn name(&self) -> &str {
        "custom"
    }
}
...
mod tests {
    ...
    #[test]
    fn test_default_matcher_name() {
        struct DefaultNameMatcher;

        impl Matcher for DefaultNameMatcher {
            fn matches(
                &self,
                _rule_value: &AttributeValue,
                _request_value: &AttributeType,
