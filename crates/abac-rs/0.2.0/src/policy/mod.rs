...
use crate::explain::{ExplainedDecision, RuleMatch};
    ...
    for AbacPolicyCore<L>
{
    ...
    fn current_count(&self) -> usize {
        self.rules.len() + self.temporal_rules.len()
    }
}
...
impl<L: CacheLock<LruCache<RequestKey, Decision>>> AbacPolicyCore<L> {
    ...
    /// Returns [`PolicyError::TooManyRules`] if adding this rule would exceed
    /// the configured maximum.
    pub fn add_temporal_rule(&mut self, rule: TemporalAbacRule) -> Result<(), PolicyError> {
        use acls_rs::policy::RuleLimitedPolicy;
        self.check_limit(1)?;
    ...
    pub fn evaluate_at(&mut self, request: &AbacRequest, timestamp: Timestamp) -> Decision {
        let valid_indices: Vec<usize> = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, tr)| tr.is_valid_at(timestamp))
            .map(|(i, _)| i)
            .collect();
        ...
        for &idx in &valid_indices {
            let rule = &self.temporal_rules[idx].rule;
            if rule.is_enabled() && rule.is_deny() && self.rule_matches(rule, request) {
        ...
        if regular_decision == Decision::Deny {
            for &idx in &valid_indices {
                let rule = &self.temporal_rules[idx].rule;
                if rule.is_enabled() && rule.is_allow() && self.rule_matches(rule, request) {
    ...
    /// Evaluate with full explanation for audit logging and diagnostics.
    ///
    /// Unlike `evaluate()`, this method:
    /// - Bypasses constant-result, deny-index, compiled-evaluator, and LRU cache layers
    /// - Collects ALL matching deny rules (no short-circuit on first deny)
    /// - Records the first matching allow rule
    /// - Includes temporal rule matches
    ///
    /// This is intentionally slower than `evaluate()` and is meant for:
    /// - Audit logging (called once per issuance)
    /// - Interactive `test-request` CLI commands
    pub fn evaluate_explained(&mut self, request: &AbacRequest) -> ExplainedDecision {
        use acls_rs::permission::current_timestamp_millis;
        self.evaluate_explained_at(request, current_timestamp_millis())
    }

    /// Evaluate with full explanation at a specific timestamp.
    ///
    /// Decision logic matches `evaluate_at` semantics:
    /// 1. Temporal deny overrides everything (including temporal allow)
    /// 2. Temporal allow overrides regular deny
    /// 3. Otherwise deny-override applies (any deny beats any allow)
    pub fn evaluate_explained_at(
        &mut self,
        request: &AbacRequest,
        timestamp: Timestamp,
    ) -> ExplainedDecision {
        ...
        let mut rules_evaluated: usize = 0;
        let mut has_regular_deny = false;
        let mut has_temporal_deny = false;
        ...
        // Phase 1: Check ALL regular deny rules (collect all, no short-circuit)
        for &rule_idx in &candidates {
            let rule = &self.rules[rule_idx];
            if !rule.is_enabled() || !rule.is_deny() {
                continue;
            }
            rules_evaluated += 1;
            if self.rule_matches(rule, request) {
                matched_rules.push(RuleMatch {
                    ...
                    rule_type: crate::RuleType::Deny,
                    temporal: false,
                });
                has_regular_deny = true;
            }
        }

        // Phase 2: Check temporal deny rules
        let valid_temporal: Vec<_> = self
            .temporal_rules
            .iter()
            .filter(|tr| tr.is_valid_at(timestamp))
            .collect();

        for tr in &valid_temporal {
            if tr.rule.is_enabled() && tr.rule.is_deny() {
                rules_evaluated += 1;
                if self.rule_matches(&tr.rule, request) {
                    matched_rules.push(RuleMatch {
                        id: tr.rule.id.clone(),
                        name: tr.rule.name.clone(),
                        rule_type: crate::RuleType::Deny,
                        temporal: true,
                    });
                    has_temporal_deny = true;
                }
            }
        }

        // Temporal deny overrides everything — skip allow checks
        if has_temporal_deny {
        ...
        // Phase 3: If regular deny matched, temporal allow can override
        if has_regular_deny {
            for tr in &valid_temporal {
                if tr.rule.is_enabled() && tr.rule.is_allow() {
                    rules_evaluated += 1;
                    if self.rule_matches(&tr.rule, request) {
                        matched_rules.push(RuleMatch {
                            id: tr.rule.id.clone(),
                            name: tr.rule.name.clone(),
                            rule_type: crate::RuleType::Allow,
                            temporal: true,
                        });
                        return ExplainedDecision {
        ...
        // Phase 4: Check regular allow rules (first match wins)
        for &rule_idx in &candidates {
            let rule = &self.rules[rule_idx];
            if !rule.is_enabled() || !rule.is_allow() {
                continue;
            }
            rules_evaluated += 1;
            if self.rule_matches(rule, request) {
        ...
        // Phase 5: Check temporal allow rules
        for tr in &valid_temporal {
            if tr.rule.is_enabled() && tr.rule.is_allow() {
                rules_evaluated += 1;
                if self.rule_matches(&tr.rule, request) {
        ...
        // Phase 6: No matching rules -> deny by default
        ExplainedDecision {
            decision: Decision::Deny,
            matched_rules,
            rules_evaluated,
...
/// Builder for constructing an [`AbacPolicyCore`] with all configuration in one step.
///
/// Avoids redundant index rebuilds from sequential `add_rule`/`register_matcher` calls
/// by collecting all rules, matchers, and settings before building the policy.
///
/// # Examples
///
/// ```
...
impl std::fmt::Debug for PolicyBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PolicyBuilder")
            ...
            .field(
                "matchers",
                &format_args!("<{} matchers>", self.matchers.len()),
            )
            .field("rules", &self.rules.len())
            .field("temporal_rules", &self.temporal_rules.len())
            .finish()
    }
}
...
impl PolicyBuilder {
    ...
    pub fn new() -> Self {
        Self {
            max_rules: AbacPolicy::DEFAULT_MAX_RULES,
            cache_size: AbacPolicy::DEFAULT_CACHE_SIZE,
    ...
    /// Set the maximum number of rules allowed (0 = no limit).
    pub fn max_rules(mut self, max_rules: usize) -> Self {
        self.max_rules = max_rules;
        self
    }

    /// Set the LRU cache size (recommended: 512-4096).
    pub fn cache_size(mut self, cache_size: usize) -> Self {
        self.cache_size = cache_size;
        self
    }

    /// Register a custom matcher for a dimension.
    pub fn matcher(mut self, dimension: impl Into<String>, matcher: Box<dyn Matcher>) -> Self {
        self.matchers.insert(dimension.into(), matcher);
        self
    }

    /// Set the rules for the policy.
    pub fn rules(mut self, rules: Vec<AbacRule>) -> Self {
        self.rules = rules;
        self
    }

    /// Add a single rule to the builder.
    pub fn rule(mut self, rule: AbacRule) -> Self {
        self.rules.push(rule);
        self
    }

    /// Set the temporal rules for the policy.
    pub fn temporal_rules(mut self, rules: Vec<TemporalAbacRule>) -> Self {
        self.temporal_rules = rules;
        self
    }

    /// Add a single temporal rule to the builder.
    pub fn temporal_rule(mut self, rule: TemporalAbacRule) -> Self {
        self.temporal_rules.push(rule);
        self
    }
    ...
    /// # Errors
    ///
    /// Returns [`PolicyError::TooManyRules`] if the number of rules exceeds
    /// the configured maximum.
    pub fn build<L: CacheLock<LruCache<RequestKey, Decision>>>(
        self,
    ) -> Result<AbacPolicyCore<L>, PolicyError> {
        let mut policy = AbacPolicyCore::with_capacity(self.rules.len().max(1), self.cache_size)?;
        policy.max_rules = self.max_rules;

        for (dimension, matcher) in self.matchers {
            policy.matchers.insert(dimension, matcher);
        }

        if !self.rules.is_empty() {
            policy.load_rules(self.rules)?;
        }

        for temporal_rule in self.temporal_rules {
            policy.add_temporal_rule(temporal_rule)?;
        }
...
impl<L: CacheLock<LruCache<RequestKey, Decision>>> AbacPolicyCore<L> {
    /// Creates a builder for constructing an ABAC policy.
