...
impl ExplainedDecision {
    /// Constructs an `ExplainedDecision` from an already-known decision.
    ///
    /// For callers that determine the decision via a cheaper `evaluate()`/
    /// `evaluate_at()` call (which uses the constant-result, deny-index,
    /// compiled-evaluator, and LRU cache fast paths this type's own
    /// evaluation bypasses) and only need `ExplainedDecision`'s shape to
    /// satisfy a uniform return type — e.g. when the caller can prove no
    /// rule explanation will ever be consulted for this result. `#[non_exhaustive]`
    /// otherwise prevents external crates from constructing this type directly.
    pub fn new(decision: Decision, matched_rules: Vec<RuleMatch>, rules_evaluated: usize) -> Self {
        Self {
            decision,
            matched_rules,
            rules_evaluated,
...
mod tests {
    ...
    #[test]
    fn test_explained_decision_new() {
        let ed = ExplainedDecision::new(Decision::Allow, vec![], 0);
        assert_eq!(ed.decision, Decision::Allow);
        assert!(ed.matched_rules.is_empty());
        assert_eq!(ed.rules_evaluated, 0);
    }
