...
mod tests {
    ...
    fn ln_branch() {
        ...
        // Known values: ln_1(1) = 2πi
        assert_ulps_eq!(Polar::ONE.ln_branch(1), Rectangular::new(0.0, TAU));
        // ln_(-1)(-1) = ln|-1| + i(π - 2π) = -πi
        assert_ulps_eq!(Polar::NEG_ONE.ln_branch(-1), Rectangular::new(0.0, -PI));
        // log2_branch and log10_branch consistency
        for z in random_samples::<Polar>() {
            assert_ulps_eq!(z.log2_branch(1), z.ln_branch(1) / LN_2);
            assert_ulps_eq!(z.log10_branch(1), z.ln_branch(1) / core::f64::consts::LN_10);
        }
    }
    ...
    fn sqrt_branch() {
        // k=0 matches principal value
        for z in random_samples::<Polar>() {
            assert_eq!(z.sqrt_branch(0), z.sqrt());
        }
        // k=1 gives the other square root: arg offset by π
        for z in random_samples::<Polar>() {
            assert_ulps_eq!(z.sqrt_branch(1).abs, z.sqrt().abs);
            assert_ulps_eq!(z.sqrt_branch(1).arg, z.sqrt().arg + PI);
        }
        // The two roots multiply to give z (after normalizing abs)
        assert_ulps_eq!(Polar::ONE.sqrt_branch(1), Polar::new(1.0, PI));
    }

    #[test]
    fn nth_root() {
        // k=0 matches powf(1/n) in terms of abs; arg = arg/n
        for z in random_samples::<Polar>() {
            let r = z.nth_root(3, 0);
            assert_ulps_eq!(r.abs, z.abs.powf(1.0 / 3.0));
            assert_ulps_eq!(r.arg, z.arg / 3.0);
        }
        // Cube roots of unity: nth_root(1+0i, 3, k) for k=0,1,2
        let one = Polar::ONE;
        let r0 = one.nth_root(3, 0);
        let r1 = one.nth_root(3, 1);
        let r2 = one.nth_root(3, 2);
        ...
        // Each root cubed gives the original
        for z in random_samples::<Polar>() {
            for k in 0..3_i32 {
                assert_ulps_eq!(z.nth_root(3, k).powi(3).abs, z.abs, epsilon = 1e-13);
            }
        }
    }
    ...
    #[test]
    fn pow_rational() {
        // p/q = 1/1: same as powi(1)
        for z in random_samples::<Polar>() {
            assert_ulps_eq!(z.pow_rational(1, 1, 0), z.powi(1));
        }
        // p/q = 1/2, k=0: same as sqrt
        for z in random_samples::<Polar>() {
            assert_eq!(z.pow_rational(1, 2, 0), z.sqrt());
        }
        // p/q = 1/3: cube root, k=0..2 give distinct values
        for z in random_samples::<Polar>() {
            for k in 0..3_i32 {
                let r = z.pow_rational(1, 3, k);
                assert_ulps_eq!(r.powi(3).abs, z.abs, epsilon = 1e-13);
                assert_ulps_eq!(r.abs, z.abs.powf(1.0 / 3.0), epsilon = 1e-13);
            }
        }
        // p/q = 2/3: two-thirds power — 3 distinct values
        let z = Polar::new(8.0, 0.0); // 8^(2/3) = 4
        assert_ulps_eq!(z.pow_rational(2, 3, 0).abs, 4.0);
        // 2/4 has only 2 distinct values (same as 1/2 after normalization)
        for z in random_samples::<Polar>() {
            assert_relative_eq!(
                z.pow_rational(2, 4, 0).normalize(),
                z.pow_rational(1, 2, 0).normalize(),
                max_relative = 1e-14
            );
            assert_relative_eq!(
                z.pow_rational(2, 4, 2).normalize(),
                z.pow_rational(1, 2, 0).normalize(),
                max_relative = 1e-14
            );
        }
