...
impl<FT: Number> ComplexPolar<FT> {
    ...
    pub fn ln_branch(self, k: i32) -> Rectangular<FT> {
        Rectangular::new(self.abs.ln(), self.arg + FT::TAU() * FT::from_i32(k))
    }
    ...
    pub fn sqrt_branch(self, k: i32) -> Self {
        ...
        Self::new(
            self.abs.sqrt(),
            (self.arg + FT::TAU() * FT::from_i32(k)) / two,
        )
    }
    ...
    pub fn nth_root(self, n: i32, k: i32) -> Self {
        Self::new(
            self.abs.powf(FT::from_i32(n).recip()),
            (self.arg + FT::TAU() * FT::from_i32(k)) / FT::from_i32(n),
        )
    }
