...
### Fixed
- **Run command config re-read**: `execute_run` now uses `Agent::new_from_config()`
  instead of `Agent::new_with_bases()`, eliminating the final disk read that caused
  "Failed to parse TOML config" errors when the on-disk config is a minimal overlay.
- **Remote checkpoint init from config**: Added `initialize_remote_checkpoint_backend_from_config()`
  so the run command initializes DocumentDB backend from the pre-parsed config.
