...
impl SessionManager {
    ...
    ///
    /// If a checkpoint for the current iteration already exists (e.g. the
    /// `_analyze_` file written by the workflow loop moments earlier), the write
    /// is skipped and resume info is built from that existing checkpoint,
    /// avoiding duplicate `_analyze_` / `_final_` file pairs per iteration.
    ///
    ...
    pub async fn create_final_checkpoint_and_get_resume_info<C: AgentContext>(
        ...
        // If a checkpoint for this iteration already exists, skip writing and
        // return resume info based on the existing file.
        if let Some(ref session_storage) = self.current_session {
            if let Some(existing_id) = session_storage.latest_checkpoint_id() {
                if existing_id.starts_with(&prefix) {
                    return Some(crate::cli::ResumeInfo {
