...
### Changed
- **orchestration: add `hints` field to `ToolsExecuting` event** — carries a
  short display hint per tool (file path for read/edit/write/multiedit, truncated
  command or description for bash) so TUI can show useful context in spinner lines
  without needing to parse arguments itself.
...
### Changed
- **deps: bump cats to 0.1.20** — picks up clearer `oldString not found` / multiedit
  atomicity error messages, EOF truncation detection with actionable guidance, and
  compact log truncation markers.
