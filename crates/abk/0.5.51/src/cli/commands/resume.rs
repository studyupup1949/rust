...
fn paths_match(a: &Path, b: &Path) -> bool {
    let a = crate::strip_unc_prefix(a.to_path_buf());
    let b = crate::strip_unc_prefix(b.to_path_buf());
...
fn path_starts_with(child: &Path, parent: &Path) -> bool {
    let child = crate::strip_unc_prefix(child.to_path_buf());
    let parent = crate::strip_unc_prefix(parent.to_path_buf());
    if cfg!(target_os = "windows") {
