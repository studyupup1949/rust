...
/// TokenRevocationRequest : A request to revoke previously-issued tokens.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TokenRevocationRequest {
    /// Up to 100 target specifiers, each a `type:value` string where `type` is `clientId`, `revocationKey`, or `channel`. 
    #[serde(rename = "targets")]
    pub targets: Vec<String>,
    /// Only tokens issued before this timestamp (epoch milliseconds) are revoked. Defaults to the current time when omitted. 
    #[serde(rename = "issuedBefore", skip_serializing_if = "Option::is_none")]
    pub issued_before: Option<i64>,
    /// When true, enforcement is postponed ~30 seconds and live connections are hinted to obtain a fresh token, avoiding an abrupt disconnect. 
...
impl TokenRevocationRequest {
    /// A request to revoke previously-issued tokens.
    pub fn new(targets: Vec<String>) -> TokenRevocationRequest {
