...
/// MessageReactionType : The reaction aggregation model. `unique`: at most one reaction per client. `distinct`: at most one of each named reaction per client. `multiple`: repeatable and counted. 
/// The reaction aggregation model. `unique`: at most one reaction per client. `distinct`: at most one of each named reaction per client. `multiple`: repeatable and counted. 
...
impl std::fmt::Display for MessageReactionType {
...
impl Default for MessageReactionType {
    fn default() -> MessageReactionType {
        Self::Unique
    }
}
