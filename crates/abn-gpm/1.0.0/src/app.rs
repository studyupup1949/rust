use std::{fs::File, io::{self, Write}, process::{Command, Output}, str::from_utf8};

use crate::{
    config::Config, multi_input::{MultiInput, MultiInputState}, project_item::{ProjectItem, ProjectItemType}, screen::Screen, switch_screen::{ScreenSwitcher, ScreenSwitcherState, ScreenSwitcherStateBuilder}
};

use ratatui::{
    crossterm::event::{self, Event, KeyCode, KeyEvent}, layout::{Constraint, Flex, Layout, Rect}, style::{Color, Modifier, Style}, text::Line, widgets::{Block, Clear, Paragraph, Scrollbar, ScrollbarOrientation, StatefulWidget}, DefaultTerminal, Frame
};
...
impl<'a> App<'a> {
    ...
    fn initialise_screen(&mut self) {
        match self.app_screen {
            ...
            Screen::WorktreeCreate => {
                if self.input_state.is_some() { return }
    ...
    fn handle_key_event(&mut self, k: KeyEvent) -> bool {
        match k.code {
            KeyCode::Esc => match self.app_screen {
                Screen::RepoCreate | Screen::WorktreeCreate => {
                    self.input_state = None;
            ...
            KeyCode::Char('x') => {
                ...
                if selected.project_type == ProjectItemType::Worktree {
                    ...
                    self.app_screen = Screen::RepoDelete
                }
                return false;
            }
            KeyCode::Char('y') => match self.app_screen {
                Screen::WorktreeDelete => self.delete_worktree(),
                Screen::RepoDelete => self.delete_repo(),
                _ => {}
            },
            KeyCode::Char('n') => match self.app_screen {
                Screen::WorktreeDelete | Screen::RepoDelete => self.app_screen = Screen::Main,
                _ => {}
            },
            KeyCode::Enter => {
                    ...
                    Screen::Main => match selected_proj.project_type {
                        ...
                        ProjectItemType::Worktree => {
                            self.screen_switch_state = Some(
                                ...
                                .with_option("Delete Project".to_string(), Screen::WorktreeDelete)
                                .build());
                            self.app_screen = Screen::ScreenSwitchMenu
                        },
                        ProjectItemType::WorktreeRepo => {
                            self.screen_switch_state = Some(
                                ScreenSwitcherStateBuilder::new(" Project Worktree Menu ".to_string())
                                .with_option("New Branch As Worktree".to_string(), Screen::WorktreeCreate)
                                .with_option("Delete Worktree".to_string(), Screen::RepoDelete)
                                .build());
                            self.app_screen = Screen::ScreenSwitchMenu
                        }
                        ProjectItemType::ProjectDirectory => {
                            self.screen_switch_state = Some(
                                ScreenSwitcherStateBuilder::new(" Project Worktree Menu ".to_string())
                                .with_option("Checkout New Repo".to_string(), Screen::RepoCreate)
                                .build());
                            self.app_screen = Screen::ScreenSwitchMenu
                        }
                    },
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        if self.app_screen == Screen::Summary {
            ...
            let mut lines = vec![
                Line::styled("Execution Summary", Style::default().add_modifier(Modifier::BOLD)),
            ];
            for l in self.summary_text.iter() {
                lines.push(Line::default());
                lines.push(Line::raw(l.clone()))
            }
            lines.push(Line::styled("Press q / Escape to exit, or any other key to continue editing.", Style::default().add_modifier(Modifier::BOLD)));
        ...
        match self.app_screen {
            Screen::RepoDelete | Screen::WorktreeDelete => {
                let to_delete = if self.app_screen == Screen::WorktreeDelete {
                    "Branch"
                } else {
                    "Worktree"
                };
    ...
    fn delete_worktree(&mut self) {
        let Some(wt) = self.get_selected_pt_item() else { return };
        ...
        self.generate_cmd_summary(&format!("Deleting Worktree {}", wt_name.to_string_lossy()), output);

        self.app_screen = Screen::Summary;
    }

    fn delete_repo(&mut self) {
        let Some(repo) = self.get_selected_pt_item() else { return };
        ...
        self.generate_cmd_summary(&format!("Deleting Repo {}", repo_name.to_string_lossy()), output);

        self.app_screen = Screen::Summary;
    }

    fn checkout_new_repo(&mut self) {
        if let Some(ref i_state) = self.input_state {
            ...
            let repo_dir_name = Self::sanitise_git_dir_name(if chosen_repo_name.is_empty() {
                let Some((_, after_slash)) = repo_link.rsplit_once('/') else {
                    self.summary_text = vec![
                        format!("Could not interpret '{}' as git repository link.", repo_link)
                    ];
                    self.app_screen = Screen::Summary;
                    return;
                };
            ...
            let Some(dir) = self.get_selected_pt_item() else { return };
            ...
            if !mkdir_output.status.success() {
                self.generate_cmd_summary(&format!("Mkdir {} at {:?}", &repo_dir_name, dir.path), mkdir_output);
                return;
            }
...

            {
                ...
                let Ok(mut f) = File::create(&file_path) else {
                    self.summary_text = vec![
                        format!("Failed to create .git file at {:?}", &file_path)
                    ];
                    self.app_screen = Screen::Summary;
                    return;
                };
            ...
            self.summary_text = vec![
                format!("Checked out new repo with name {} {:?}", repo_dir_name, &repo_path)
            ];
    ...
    fn checkout_new_worktree(&mut self) {
        if let Some(ref i_state) = self.input_state {
            ...
            self.generate_cmd_summary(&format!("Checking out new Worktree {} in repo {}", dir_name, repo.path.file_name().unwrap().to_string_lossy()),
                output);
    ...
    fn sanitise_git_dir_name(s: &str) -> String {
        s
        .replace("/", "_") // slashes will break the structure this project scans for dirs for.
    ...
    fn generate_cmd_summary(&mut self, desc: &str, output: Output) {
        if output.status.success() {
            ...
            self.summary_text = vec![
                format!("FAILURE: {}", desc),
                format!("STDOUT: {}", from_utf8(&output.stdout).unwrap_or("couldnt read stdout as utf-8")),
                format!("STDERR: {}", from_utf8(&output.stderr).unwrap_or("couldnt read stderr as utf-8")),
            ];
        }
    }
}
