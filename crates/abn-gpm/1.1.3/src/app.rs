...
impl<'a> App<'a> {
    ...
    fn get_repo_name_from_git_link(s: &str) -> Result<&str, String> {
        ...
        let Some((default_name, _)) = after_slash.rsplit_once('.') else {
            // special case for MS Azure :(
            if s.contains("azure") {
                return Ok(after_slash);
            }
            return Err(format!(
                "Could not interpret '{}' as git repository link.",
                s
...
mod test {
    ...
    #[rstest]
    #[case("git@github.com:anna-singleton/gpm.git")]
    #[case("https://github.com/anna-singleton/gpm.git")]
    #[case("git@gitlab.com:abn3213932/gpm.git")]
    #[case("https://gitlab.com/abn3213932/gpm.git")]
    #[case("https://annabeths111@dev.azure.com/anna/anna/_git/gpm")]
    #[case("git@ssh.dev.azure.com:v3/anna/anna/gpm")]
    fn valid_link_gets_correct_name(#[case] link: &str) {
        let name = App::get_repo_name_from_git_link(link).expect("Should not error.");
        assert_eq!("gpm", name);
    }
}
