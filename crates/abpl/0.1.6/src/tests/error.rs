...
fn sanity_check() {
	...
	enum TestKind {
		...
		#[cause(std::num::TryFromIntError, unserializable)]
		#[cause(std::num::ParseIntError, unserializable)]
		#[cause(std::ffi::FromBytesWithNulError, unserializable)]
		Five,
	}
	impl core::fmt::Display for TestKind {
		fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
			match self {
				...
				TestKind::Five => f.write_str("the fifth"),
			}
		}
	}
	...
	fn _test() -> Result<(), Test> {
		...
		let _ = u32::try_from(69u64).map_err_five()?;
		let _ = "fhf".parse::<u32>().map_err_five()?;
		let _ = core::ffi::CStr::from_bytes_with_nul(b"f\0oo").map_err_five()?;
