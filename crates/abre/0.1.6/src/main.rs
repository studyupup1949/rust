...
struct Args {
    ...
    /// Segment separator characters — each char is a separator (e.g. "/." splits on / and .)
    /// Use "common" as shorthand for /_-
...
fn resolve_seps(s: &str) -> Vec<char> {
    match s {
        "common" => vec!['/', '_', '-'],
...
fn main() {
    ...
    let seps = resolve_seps(&args.separator);
    ...
    if args.json {
        process_json(&lines, &args, &capture_re, &strategy, &seps, &mut out);
    } else {
        process_plain(&lines, &capture_re, &strategy, &seps, &args.ellipsis, &mut out);
    }
}
...
fn process_plain(
    ...
    let split_lines: Vec<(Vec<&str>, Vec<char>)> =
        targets.iter().map(|t| shorten::split_multi(t, seps)).collect();

    let shortened = shorten::shorten(&split_lines, ellipsis, strategy);
...
fn process_json(
    ...
    let shortened = shorten::shorten(&split_lines, &args.ellipsis, strategy);
