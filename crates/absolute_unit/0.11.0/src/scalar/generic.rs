...
macro_rules! impl_value_type_conversions {
    ...
    (Real, $TypeName:ty, $UnitA:path, $UnitB:path) => {
        impl<A, B> From<Real> for $TypeName
        where
            A: $UnitA,
            B: $UnitB,
        {
    ...
    (Real, $TypeName:ty, $UnitA:path) => {
        impl<A> From<Real> for $TypeName
        where
            A: $UnitA,
        {
            fn from(v: Real) -> Self {
