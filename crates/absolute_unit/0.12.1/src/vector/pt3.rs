use crate::{Length, LengthUnit, Quantity, Scalar, V3};
    ...
    Unit: LengthUnit,
{
    ...
    #[cfg(feature = "mint")]
    #[inline]
    pub fn mint_dvec3(&self) -> mint::Vector3<f64> {
        let v = Pt3::<crate::Meters>::from(self);
        mint::Vector3 {
            x: v.vec.x,
            y: v.vec.y,
            z: v.vec.z,
...
impl<A, B> SubAssign<Pt3<B>> for Pt3<A>
where
    A: LengthUnit,
    B: LengthUnit,
{
    fn sub_assign(&mut self, rhs: Pt3<B>) {
        let rhs = Pt3::<A>::from(&rhs);
        self.vec -= rhs.vec;
    }
}

impl<A, B> Sub<V3<Length<B>>> for Pt3<A>
where
    A: LengthUnit,
    B: LengthUnit,
{
    ...
    fn sub(self, rhs: V3<Length<B>>) -> Self::Output {
        let rhs = Pt3::<B>::new(rhs.x(), rhs.y(), rhs.z());
        let rhs = Pt3::<A>::from(&rhs);
        Pt3::new_dvec3(self.vec - rhs.vec)
    }
...
#[cfg(feature = "mint")]
impl From<&mint::Vector3<f64>> for Pt3<crate::Meters> {
    fn from(value: &mint::Vector3<f64>) -> Self {
        Self::new(
            crate::meters!(value.x),
            crate::meters!(value.y),
            crate::meters!(value.z),
        )
    }
}
