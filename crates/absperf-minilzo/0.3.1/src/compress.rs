...
pub trait CompressInto<T: ?Sized> {
    ...
    fn compress_into<'comp>(&self, destination: &'comp mut T) -> Result<&'comp [u8]>;
}

impl CompressInto<[u8]> for [u8] {
    fn compress_into<'comp>(&self, destination: &'comp mut [u8]) -> Result<&'comp [u8]> {
...
impl CompressInto<Vec<u8>> for [u8] {
    ...
    fn compress_into<'comp>(&self, destination: &'comp mut Vec<u8>) -> Result<&'comp [u8]> {
        WRKMEM.with(move |wrkmem| {
            let mut wrkmem = wrkmem.borrow_mut();
