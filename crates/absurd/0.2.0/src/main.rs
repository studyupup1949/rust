...
fn handle_model(args: ComponentModelArgs) -> Result<()> {
    ...
    let model_path = ComponentName(args.name.clone());
    let base_path = Path::new("src/model");
    let component_path = base_path.join(&model_path.snake_case()).with_extension("rs");

    if component_path.exists() {
        return Err(Error::ModelAlreadyExists(model_path.pascal_case()));
    }
    ...
    let schema = if args.schemaless || (!args.schemafull && args.fields.is_none() && model_path.0.contains("/")) {
    ...
    let fields = if args.fields.is_some() {
        ...
        } else if args.fields.is_none() && model_path.0.contains("/") {
    ...
    let table = if args.fields.is_some() || schema == SchemaType::Schemafull {
            // If fields are passed, then we just want to take the final value
            // from the path.
            // Or if the table is schemafull, then we don't need a separate
            // model for each field on a table.
            model_path.0
                .split("/")
                .last()
                .ok_or(Error::FailedToHandleComponent("Could not detect table name from path."))
        } else {
            // No fields passed, or schemaless so get each split and get the
            // second to last.
            // This is because the format is `Path/To/Table/Field`.
            model_path.0
                .split("/")
                // Collect, because we cannot reverse split for some reason.
                .collect::<Vec<&str>>()
                .into_iter()
                .rev()
                .nth(1)
    ...
    let table = ComponentName(table.to_string());
    ...
    match schema {
        SchemaType::Schemafull => {
            let model_contents = if component_path.exists() {
                let model_contents = fs::read_to_string(&component_path)?;
                // Add the `|fields|` marker in so that we can replace it
                // regardless of which branch we took.
                let model_contents = append_after_marker(model_contents, "pub id: Option<Thing>,", "|fields|".to_string());
                model_contents
            } else {
            ...
            let fields = fields
                ...
                .collect::<Vec<String>>()
                .join("\n");
            let model_contents = model_contents.replace("|fields|", &fields);
        ...
        SchemaType::Schemaless => {
            for field in &fields {
                ...
                // We can unwrap because we have controlled the number of folders.
                let table_path = component_path.parent().unwrap().with_extension("rs");
                let table_component: ComponentName = table_path.to_owned().into();
                // This needs to match the stub.
                let use_string = format!("pub use {}::{}{};", field.snake_case(), table_component.pascal_case(), field.pascal_case());
