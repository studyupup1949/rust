...
pub fn send_change_notification(notification: &BroadcastNotification) -> Result<(), DatabaseError> {
    ...
    web_sys::console::log_1(&format!("DEBUG: Sending notification on channel: {}", channel_name).into());
    ...
    let json = serde_json::to_string(notification)
        .map_err(|e| DatabaseError::new("SERIALIZATION_ERROR", &format!("Failed to serialize notification: {}", e)))?;
    ...
    let js_value = js_sys::JSON::parse(&json)
        .map_err(|e| DatabaseError::new("JSON_PARSE_ERROR", &format!("Failed to parse JSON: {:?}", e)))?;
    ...
    web_sys::console::log_1(&format!("DEBUG: Notification sent successfully on {}", channel_name).into());
...
pub fn register_change_listener(
    ...
    let channel_name = format!("datasync_changes_{}", db_name);
    web_sys::console::log_1(&format!("DEBUG: Registering listener on channel: {}", channel_name).into());
    ...
    web_sys::console::log_1(&format!("DEBUG: Listener registered successfully on {}", channel_name).into());
...
pub fn send_change_notification(_notification: &BroadcastNotification) -> Result<(), DatabaseError> {
    Err(DatabaseError::new("NOT_SUPPORTED", "BroadcastChannel only available in WASM"))
}
