...
async fn test_sync_frequency_is_excessive() {
    ...
    web_sys::console::log_1(&" If you see 10 'VFS sync: Persisting' messages, the bug is confirmed!".into());
...
async fn test_sync_overhead_performance_impact() {
    ...
    web_sys::console::log_1(&format!(" 50 inserts with sync-per-insert: {:.2}ms", time_with_excessive_syncs).into());
    web_sys::console::log_1(&format!(" Per-insert cost: {:.2}ms", time_with_excessive_syncs / 50.0).into());
...
async fn test_deferred_sync_behavior_after_fix() {
    ...
    web_sys::console::log_1(&format!(" 10 inserts: {:.2}ms ({:.2}ms per insert)", insert_time, insert_time / 10.0).into());
...
async fn test_data_persistence_with_deferred_sync() {
    ...
    // Read data back in new connection - should work now!
    {
        ...
        match db.execute("SELECT * FROM test WHERE id = 1").await {
            Ok(_) => {
                web_sys::console::log_1(&"PASS: Schema and data persisted correctly across connections!".into());
            }
            Err(e) => {
