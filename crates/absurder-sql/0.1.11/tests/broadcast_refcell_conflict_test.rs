...
async fn test_broadcast_channel_refcell_conflict() {
    ...
    let mut db = Database::new_wasm(db_name.to_string()).await.expect("create db");
    sleep_ms(100).await;
    ...
    let callback = wasm_bindgen::closure::Closure::wrap(Box::new(move |_change_type: wasm_bindgen::JsValue| {
        web_sys::console::log_1(&"📢 Data change notification received".into());
    }) as Box<dyn FnMut(wasm_bindgen::JsValue)>);
    
    db.on_data_change_wasm(callback.as_ref().unchecked_ref()).expect("register callback");
        ...
        match db.execute_with_params(
            ...
            Ok(_) => {
                ...
                match db.is_leader().await {
                    Ok(is_leader) => {
                        web_sys::console::log_1(&format!("Leader status check: {}", is_leader).into());
                    },
                    Err(e) => {
                        let err_str = format!("{:?}", e);
                        if err_str.contains("recursive use") || err_str.contains("borrow") {
                            web_sys::console::log_1(&"🐛 REPRODUCED: RefCell conflict during isLeader check!".into());
                ...
                if let Err(e) = db.sync().await {
                    let err_str = format!("{:?}", e);
                    if err_str.contains("recursive use") || err_str.contains("borrow") {
                        web_sys::console::log_1(&"🐛 REPRODUCED: RefCell conflict during sync!".into());
                ...
                match db.is_leader().await {
                    Ok(is_leader) => {
                        web_sys::console::log_1(&format!("Post-sync leader check: {}", is_leader).into());
                    },
                    Err(e) => {
                        let err_str = format!("{:?}", e);
                        if err_str.contains("recursive use") || err_str.contains("borrow") {
                            web_sys::console::log_1(&"🐛 REPRODUCED: RefCell conflict in post-sync isLeader!".into());
            ...
            Err(e) => {
                ...
                if err_str.contains("recursive use") || err_str.contains("borrow") {
                    web_sys::console::log_1(&"🐛 REPRODUCED: RefCell borrow conflict from BroadcastChannel!".into());
