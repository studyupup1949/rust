    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Error should mention localStorage unavailability: {}", error_msg
            );
            log::info!("Correctly returned error for localStorage unavailable: {:?}", e);
        }
    }
}
            ...
            match sync_result {
                ...
                Err(e) => {
                    ...
                    assert!(
                        ...
                        error_msg.contains("private browsing"),
                        "Error should mention IndexedDB unavailability: {}", error_msg
                    );
                    log::info!("Correctly returned error for IndexedDB unavailable: {:?}", e);
                }
            }
        }
    }
...
async fn test_window_access_safety() {
    ...
    assert!(result.is_ok() || result.is_err(), "Should return Ok or Err, not panic");
}
...
async fn test_private_browsing_user_friendly_error() {
    ...
    if let Err(e) = result {
        ...
        assert!(
            ...
            !error_str.contains("rust_panic"),
            "Error should be user-friendly, not a panic message: {}", error_str
        );
        
        // Should mention one of these user-understandable terms
        let is_user_friendly = 
            error_str.contains("private browsing") ||
        ...
        assert!(
            is_user_friendly,
            "Error should mention what's unavailable in user-friendly terms: {}", error_str
        );
    }
}
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Should not expose internal unwrap errors: {}", error_msg
            );
        }
    }
}
