...
mod telemetry_tracing_tests {
    ...
    async fn test_query_span_has_attributes() {
        ...
        let span = recorder.get_latest_span().expect("Should have a span");
        assert_eq!(span.attributes.get("query_type").map(|s| s.as_str()), Some("INSERT"));
        assert!(span.attributes.contains_key("duration_ms"), "Should have duration_ms");
        assert!(span.attributes.contains_key("affected_rows"), "Should have affected_rows");
    ...
    async fn test_error_span_status() {
        ...
        db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY)").await.expect("Failed to create table");
        recorder.clear(); // Clear the CREATE span
        
        // Action: Try to insert duplicate primary key (should fail at execute time)
        db.execute("INSERT INTO test VALUES (1)").await.expect("First insert");
        ...
        assert!(recorder.span_count() >= 1, "Should have at least one span for the failed insert");
