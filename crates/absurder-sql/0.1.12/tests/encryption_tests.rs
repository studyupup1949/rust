...
async fn test_create_encrypted_database() {
    ...
    let mut db = Database::new_encrypted(config, key).await.expect("Failed to create encrypted database");
    
    // Should be able to execute SQL on encrypted database
    let result = db.execute("CREATE TABLE test (id INTEGER, data TEXT)").await;
    assert!(result.is_ok(), "Should be able to create table in encrypted database");
    
    // Insert some data
    let result = db.execute("INSERT INTO test VALUES (1, 'secret data')").await;
    assert!(result.is_ok(), "Should be able to insert into encrypted database");
    
    // Query the data
    let result = db.execute("SELECT * FROM test").await.expect("Should be able to query");
...
async fn test_encrypted_database_wrong_key_fails() {
    ...
    let mut db = Database::new_encrypted(config.clone(), key1).await.expect("Failed to create database");
    
    // Create a table
    db.execute("CREATE TABLE secrets (id INTEGER, data TEXT)").await.expect("Should create table");
    db.execute("INSERT INTO secrets VALUES (1, 'secret')").await.expect("Should insert");
...
async fn test_rekey_changes_encryption_key() {
    ...
    let mut db = Database::new_encrypted(config.clone(), old_key).await.expect("Failed to create database");
    
    // Create and insert data
    db.execute("CREATE TABLE rekey_test (id INTEGER, value TEXT)").await.expect("Should create table");
    db.execute("INSERT INTO rekey_test VALUES (1, 'test data')").await.expect("Should insert");
    ...
    let result = db.execute("SELECT * FROM rekey_test").await.expect("Should query after rekey");
    assert_eq!(result.rows.len(), 1);
    
    // Close and reopen with new key
    drop(db);
    
    let mut db2 = Database::new_encrypted(config.clone(), new_key).await.expect("Should open with new key");
    let result = db2.execute("SELECT * FROM rekey_test").await.expect("Should query with new key");
    assert_eq!(result.rows.len(), 1);
...
async fn test_encrypted_database_persistence() {
    ...
    {
        let mut db = Database::new_encrypted(config.clone(), key).await.expect("Failed to create database");
        db.execute("CREATE TABLE persist_test (id INTEGER PRIMARY KEY, value TEXT)").await.unwrap();
        db.execute("INSERT INTO persist_test VALUES (1, 'data1')").await.unwrap();
        db.execute("INSERT INTO persist_test VALUES (2, 'data2')").await.unwrap();
        // db is dropped here
    }
    
    // Reopen and verify data persists
    {
        let mut db = Database::new_encrypted(config, key).await.expect("Should reopen encrypted database");
        let result = db.execute("SELECT * FROM persist_test ORDER BY id").await.expect("Should query");
...
async fn test_encryption_key_length_validation() {
    ...
    assert!(result.is_err(), "Should reject key shorter than 8 characters");
}
