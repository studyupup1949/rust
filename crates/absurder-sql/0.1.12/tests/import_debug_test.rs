...
async fn test_import_simple() {
    ...
    db1.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, data TEXT)").await
        .expect("Should create table");
    
    db1.execute("INSERT INTO test (data) VALUES ('hello')").await
    ...
    let _result = db3.execute("SELECT * FROM test").await
