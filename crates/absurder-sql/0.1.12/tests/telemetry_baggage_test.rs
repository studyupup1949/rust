...
mod telemetry_baggage_tests {
    ...
    async fn test_baggage_creation() {
        ...
        assert_eq!(context.get_baggage("tenant_id"), Some("tenant456".to_string()));
    ...
    async fn test_baggage_propagates_to_child_spans() {
        ...
        assert_eq!(parent_span.baggage.get("request_id"), Some(&"req_abc123".to_string()));
        assert_eq!(parent_span.baggage.get("user_id"), Some(&"user789".to_string()));
        ...
        assert_eq!(child_span.baggage.get("request_id"), Some(&"req_abc123".to_string()));
        assert_eq!(child_span.baggage.get("user_id"), Some(&"user789".to_string()));
    ...
    async fn test_baggage_in_database_operations() {
        ...
        assert_eq!(span.baggage.get("request_id"), Some(&"req_xyz789".to_string()));
    ...
    async fn test_baggage_propagates_through_nested_operations() {
        ...
        assert_eq!(execute_spans[0].baggage.get("trace_id"), Some(&"trace_123".to_string()));
        assert_eq!(execute_spans[0].baggage.get("session_id"), Some(&"session_abc".to_string()));
        
        // Sync span (child) should have inherited baggage
        assert_eq!(sync_spans[0].baggage.get("trace_id"), Some(&"trace_123".to_string()));
        assert_eq!(sync_spans[0].baggage.get("session_id"), Some(&"session_abc".to_string()));
    ...
    async fn test_baggage_isolation_between_contexts() {
        ...
        // Verify isolation
        assert_eq!(context1.get_baggage("user_id"), Some("user_context1".to_string()));
        assert_eq!(context2.get_baggage("user_id"), Some("user_context2".to_string()));
    }
}
