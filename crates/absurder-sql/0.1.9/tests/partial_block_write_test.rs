...
async fn test_partial_block_write_with_small_page_size() {
    ...
    db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, data TEXT)").await
        .expect("Should create table with partial block writes");
    
    // This INSERT will also require partial block writes
    let insert_result = db.execute("INSERT INTO test (data) VALUES ('test_data')").await;
    ...
    assert_eq!(result["rows"].as_array().unwrap().len(), 1, "Should have 1 row");
