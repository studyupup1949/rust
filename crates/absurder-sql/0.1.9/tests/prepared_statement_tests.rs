...
async fn test_prepare_select_statement() {
    ...
    let result = stmt.execute(&[ColumnValue::Integer(1)])
        .await
    ...
    assert_eq!(result.rows[0].values[1], ColumnValue::Text("Alice".to_string()));
    assert_eq!(result.rows[0].values[2], ColumnValue::Integer(30));
    
    // Test: Execute same statement with different parameter
    let result2 = stmt.execute(&[ColumnValue::Integer(2)])
        .await
        .expect("Failed to execute prepared statement second time");
    
    assert_eq!(result2.rows.len(), 1);
    assert_eq!(result2.rows[0].values[1], ColumnValue::Text("Bob".to_string()));
...
async fn test_prepare_insert_statement() {
    ...
    let mut stmt = db.prepare("INSERT INTO products (name, price) VALUES (?, ?)")
        .expect("Failed to prepare statement");
...
async fn test_prepare_statement_reuse() {
    ...
    let mut insert_stmt = db.prepare("INSERT INTO numbers VALUES (?)")
        .expect("Failed to prepare");
    
    // Execute 100 times - this should be fast because statement is already parsed
    let start = std::time::Instant::now();
    for i in 1..=100 {
        insert_stmt.execute(&[ColumnValue::Integer(i)])
            .await
    ...
    let result = db.execute("SELECT COUNT(*) FROM numbers")
        .await
        .expect("Failed to count");
    assert_eq!(result.rows[0].values[0], ColumnValue::Integer(100));
    
    // Duration should be reasonable (less than 1 second for 100 inserts)
    assert!(duration.as_millis() < 1000, "100 prepared inserts took too long: {:?}", duration);
}
...
async fn test_named_parameters() {
    ...
    let mut stmt = db.prepare("SELECT * FROM employees WHERE department = :dept AND salary > :min_salary")
        .expect("Failed to prepare statement");
    ...
    assert_eq!(result.rows[0].values[1], ColumnValue::Text("Alice".to_string()));
    assert_eq!(result.rows[1].values[1], ColumnValue::Text("Charlie".to_string()));
...
async fn test_numbered_positional_parameters() {
    ...
    let mut stmt = db.prepare("INSERT INTO coords VALUES (?1, ?2, ?1)")
        .expect("Failed to prepare statement");
    ...
    // Verify insert
    let result = db.execute("SELECT * FROM coords")
        .await
...
async fn test_parameter_count_mismatch() {
    ...
    let mut stmt = db.prepare("INSERT INTO test_table VALUES (?, ?, ?)")
        .expect("Failed to prepare statement");
    ...
    assert!(result.is_err(), "Expected error for parameter count mismatch");
