...
async fn profile_insert_performance() {
    ...
    web_sys::console::log_1(&format!(" BATCH 10 INSERTS: {:.2}ms ({:.2}ms per insert)", batch_10_time, batch_10_time / 10.0).into());
    ...
    web_sys::console::log_1(&format!(" BATCH 10 INSERTS (explicit txn): {:.2}ms ({:.2}ms per insert)", batch_txn_time, batch_txn_time / 10.0).into());
    ...
    if batch_10_time / 10.0 > single_insert_time * 0.8 {
        web_sys::console::log_1(&" WARNING: Each insert in batch takes similar time to single insert".into());
    ...
    if batch_txn_time < batch_10_time * 0.5 {
        ...
        web_sys::console::log_1(&" This suggests we're syncing on every operation regardless of transaction!".into());
    }
...
async fn test_journal_mode_impact() {
    ...
    web_sys::console::log_1(&format!(" Default journal mode: {:.2}ms for 50 inserts", default_time).into());
    ...
    web_sys::console::log_1(&format!(" MEMORY journal mode: {:.2}ms for 50 inserts", memory_time).into());
    ...
    if speedup > 1.5 {
        ...
        web_sys::console::log_1(&" MEMORY journal mode doesn't help much - bottleneck is elsewhere".into());
    }
}
