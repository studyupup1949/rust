...
impl ResolutionCache {
    ...
    pub fn get(&self, key: &CacheKey) -> Option<Arc<ResolvedVariable>> {
        ...
        // Check TTL cache - return value if not expired
        ...
        // Remove expired entry atomically - this avoids TOCTOU race where another
        // thread might insert fresh data between our check and remove
        self.ttl_cache.remove_if(key, |_, cached| now.duration_since(cached.cached_at) >= ttl);
...
impl ResolutionEngine {
    ...
    /// Update the resolution config at runtime (for source precedence changes).
    /// This clears the cache to ensure fresh resolution with new config.
    ...
    /// Update the interpolation config at runtime.
    /// This clears the cache to ensure fresh resolution with new config.
    ...
    /// Get the current resolution config precedence.
    ...
    ///
    /// Semantics:
    /// - `None` = no filtering, return all sources
    /// - `Some(non_empty)` = filter file sources to only those in the set, keep non-file sources
    /// - `Some(empty)` = no file sources allowed, only return non-file sources (shell, etc.)
    fn filter_snapshots_ref<'a>(
        ...
        match file_source_filter {
            // Filter provided with specific file sources - include only these files + non-file sources
            Some(filter) if !filter.is_empty() => snapshots
                ...
                .collect(),
            // Empty filter provided - exclude ALL file sources, only return non-file sources
            Some(_) => snapshots
                .iter()
                .filter(|snapshot| {
                    let source_id_str = snapshot.source_id.as_str();
    ...
    /// Filter snapshots by source type based on precedence config.
    /// Only sources whose type is listed in precedence are included.
    /// If precedence is empty, no sources are enabled, so return empty.
    fn filter_by_source_type<'a>(
        ...
        // If precedence is empty, no sources are enabled - return empty
        ...
        snapshots
            ...
            .filter(|snapshot| {
                ...
                let source_type = if source_id_str.starts_with("file:") {
                    ...
                    // Memory sources or unknown - always include
                    return true;
                };

                // Only include if source type is in precedence list
    ...
    fn all_variables_inner(
        ...
        // Filter by source type based on precedence config
        let type_filtered = self.filter_by_source_type(filtered_snapshots);

        // Sort by file order for proper override behavior
    ...
    pub async fn resolve_with_filter(
        ...
        // Filter by source type (Shell/File/Remote) based on precedence config
