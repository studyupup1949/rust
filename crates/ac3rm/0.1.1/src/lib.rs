...
type Callback = Box<dyn Fn(VarId, Vec<i32>)>;
...
impl Engine {
    ...
    /// let mut engine = ac3rm::Engine::new();
    /// let x = engine.add_var([1, 2, 3]);
    /// engine.set_listener(x, |var_id, vals| {
    ///     println!("Variable {} domain changed to {:?}", var_id, vals);
    /// });
    /// ```
    pub fn set_listener<F>(&mut self, var: VarId, callback: F)
    where
        F: Fn(VarId, Vec<i32>) + 'static,
    {
...
mod tests {
    ...
    fn test_listener_notification() {
        ...
        ac.set_listener(a, move |var_id, _vals| {
