...
pub struct Fill {
    ...
    pub ts: i64,         // exchange time message sent (sent_ts) (indexed)
    pub lat: i64,        // latency in ms, recv_ts - sent_ts
    pub side: TradeSide, // buy / sell (indexed)
    pub tif: String,     // time in force
    pub sym: String,     // market symbol (indexed)
    pub ins: String,     // instrument type (indexed)
    pub p: f64,          // price
    pub fq: f64,         // filled quantity
    pub rq: f64,         // remaining quantity
}
...
pub enum PositionSide {
    #[serde(alias = "LONG", alias = "buy")]
    Long,
    #[serde(alias = "SHORT", alias = "sell")]
    Short,
    #[serde(alias = "BOTH")]
    Both,
    Unknown,
