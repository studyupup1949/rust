...
pub fn parse_statics_map(reader: &SharedMemoryReader) -> Result<StaticsMap> {
    ...
    let sm_version = read_string(&mut offset, 15, 0)?;
    let ac_version = read_string(&mut offset, 15, 0)?;
    let number_of_sessions = read_value!(i32);
    let num_cars = read_value!(i32);
    let car_model = read_string(&mut offset, 33, 0)?;
    let track = read_string(&mut offset, 33, 0)?;
    let player_name = read_string(&mut offset, 33, 0)?;
    let player_surname = read_string(&mut offset, 33, 0)?;
    let player_nick = read_string(&mut offset, 33, 2)?;
    ...
    let dry_tyres_name = read_string(&mut offset, 33, 0)?;
    let wet_tyres_name = read_string(&mut offset, 33, 0)?;
