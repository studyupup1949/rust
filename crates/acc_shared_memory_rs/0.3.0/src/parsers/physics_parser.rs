...
pub fn parse_physics_map(reader: &SharedMemoryReader) -> Result<PhysicsMap> {
    ...
    macro_rules! read_value {
        ($type:ty) => {{
            let value = reader.read_at::<$type>(offset)?;
    ...
    macro_rules! read_array {
        ($type:ty, $count:expr) => {{
            let values = reader.read_array_at::<$type>(offset, $count)?;
            offset += std::mem::size_of::<$type>() * $count;
            values
        }};
    }
    ...
    let velocity = read_vector3f(&mut offset)?;
    let g_force = read_vector3f(&mut offset)?;

    // Wheels data
    let wheel_slip = read_wheels(&mut offset)?;
    let _wheel_load = read_wheels(&mut offset)?; // Not used in our structure
    let wheel_pressure = read_wheels(&mut offset)?;
    let wheel_angular_speed = read_wheels(&mut offset)?;
    let _tyre_wear = read_wheels(&mut offset)?; // Not used
    let _tyre_dirty_level = read_wheels(&mut offset)?; // Not used
    let tyre_core_temp = read_wheels(&mut offset)?;
    let _camber_rad = read_wheels(&mut offset)?; // Not used
    let suspension_travel = read_wheels(&mut offset)?;
    ...
    let local_angular_vel = read_vector3f(&mut offset)?;
    ...
    let brake_temp = read_wheels(&mut offset)?;
    let clutch = read_value!(f32);

    // Skip tyre temps (inner, middle, outer)
    let _tyre_temp_i = read_wheels(&mut offset)?;
    let _tyre_temp_m = read_wheels(&mut offset)?;
    let _tyre_temp_o = read_wheels(&mut offset)?;

    let is_ai_controlled = read_value!(i32) != 0;

    // Tyre contact points (3 sets of 4x3 arrays)
    let tyre_contact_point = read_contact_point(&mut offset)?;
    let tyre_contact_normal = read_contact_point(&mut offset)?;
    let tyre_contact_heading = read_contact_point(&mut offset)?;

    let brake_bias = read_value!(f32);
    let local_velocity = read_vector3f(&mut offset)?;
    ...
    // Skip force values
    let _mz = read_wheels(&mut offset)?;
    let _fz = read_wheels(&mut offset)?;
    let _my = read_wheels(&mut offset)?;

    let slip_ratio = read_wheels(&mut offset)?;
    let slip_angle = read_wheels(&mut offset)?;

    let _tc_in_action = read_value!(i32); // Not used
    let _abs_in_action = read_value!(i32); // Not used
    let suspension_damage = read_wheels(&mut offset)?;
    let _tyre_temp = read_wheels(&mut offset)?; // Duplicate, not used
    let water_temp = read_value!(f32);

    let brake_pressure = read_wheels(&mut offset)?;
    let front_brake_compound = read_value!(i32);
    let rear_brake_compound = read_value!(i32);
    let pad_life = read_wheels(&mut offset)?;
    let disc_life = read_wheels(&mut offset)?;
