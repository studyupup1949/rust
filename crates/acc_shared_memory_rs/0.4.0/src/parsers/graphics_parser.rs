...
pub fn parse_graphics_map(reader: &SharedMemoryReader) -> Result<GraphicsMap> {
    ...
    let current_time_str = read_string(&mut offset, 15, 0)?;
    let last_time_str = read_string(&mut offset, 15, 0)?;
    let best_time_str = read_string(&mut offset, 15, 0)?;
    let last_sector_time_str = read_string(&mut offset, 15, 0)?;
    ...
    let tyre_compound = read_string(&mut offset, 33, 2)?;
    ...
    let delta_lap_time_str = read_string(&mut offset, 15, 2)?;
    let delta_lap_time = read_value!(i32);
    let estimated_lap_time_str = read_string(&mut offset, 15, 2)?;
    ...
    let track_status = read_string(&mut offset, 33, 2)?;
