...
use crate::utils::{
    assert_no_close_or_delegate, calc_reward_fp32, check_account_key, check_account_owner,
     check_and_retrieve_royalty_account
};
...
pub struct Accounts<'a, T> {
    ...
    ///  The Sysvar instructions account to check if the CPI caller is allowed
     pub sysvar_instructions_info: &'a T,
...
impl<'a, 'b: 'a> Accounts<'a, AccountInfo<'b>> {
    pub fn parse(
        ...
        let accounts = Accounts {
            ...
            sysvar_instructions_info: next_account_info(accounts_iter)?,
        ...
        check_account_key(
            accounts.sysvar_instructions_info,
...
// todo refine this check
    let current_ix =
        get_instruction_relative(0, accounts.sysvar_instructions_info).unwrap();

    msg!("Current instruction: {:?}", current_ix);
