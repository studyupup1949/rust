...
impl<I2C: I2c> ADXL345I2c<I2C> {
    ...
    pub fn read_rawaccel(&mut self) -> Result<Point3<i16>, Error> {
        self.0
            .comm
    ...
    pub fn read_accel(&mut self) -> Result<Acceln, Error> {
        let mut range = [0u8];
        self.0
            .comm
        ...
        Ok(self
            .read_rawaccel()?
            .map(|x| x as f32 * f32::from(Range::from(range[0]))))
    }
}
...
impl<SPI: SpiBus, CS: OutputPin> ADXL345SPI<SPI, CS> {
    ...
    pub fn write_register(&mut self, register: u8, value: u8) -> Result<(), Error> {
        self.1.set_low().map_err(|_| CSLowError)?;
        self.0
            .comm
            .write(&[register | 0x40, value])
            .map_err(|_| SPIError)?;
        self.1.set_high().map_err(|_| CSHighError)?;

        Ok(())
    }

    pub fn read_register(&mut self, register: u8, read: &mut [u8]) -> Result<(), Error> {
        self.1.set_low().map_err(|_| CSLowError)?;
        self.0
            .comm
