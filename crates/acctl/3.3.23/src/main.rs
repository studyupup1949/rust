...
async fn cmd_codegen(config: &Config, force: bool) -> Result<()> {
    ...
    if response.success {
        ...
        // Extract gm.rs and (if present) www/src/autocore/results.ts. The
        // server bundles both so a single `acctl codegen` keeps the Rust
        // mapping and the TS test-definition schema in sync.
        ...
        for i in 0..archive.len() {
            ...
            } else if name.ends_with("www/src/autocore/results.ts") || name == "www/src/autocore/results.ts" {
                Some(PathBuf::from("www/src/autocore/results.ts"))
            } else {
                None
            };
