...
//! (as a side effect) `www/src/autocore/results.ts` — keeping the two
...
pub async fn cmd_codegen_tags(force: bool) -> Result<()> {
    ...
    // www/src/autocore/results.ts. Non-fatal: if the server is unreachable,
    // the tags file on disk is still good — just warn and exit success.
    println!();
    println!("{} server to refresh gm.rs + results.ts…", "Asking".bold());
    ...
    match crate::cmd_codegen(&config, false).await {
        ...
        Err(e) => {
            ...
            println!(
                "  {}",
                "(run `acctl codegen` once the server is reachable to refresh results.ts)"
                    .dimmed(),
            );
        }
    }
