...
async fn sync_datastore(config: &Config, dry_run: bool) -> Result<()> {
    ...
    // Pull-only paths: files the server owns as live state. Sync may
    // pull a fresher server copy down, but never the other way — even
    // if local mtime is newer, that's almost always stale or a restore
    // scenario the operator should be doing deliberately via the
    // matching `acctl push <thing>` command. Filtering happens here
    // (after the mtime comparison) rather than at list time so that
    // server→local pulls still work.
    //
    // autocore_gnv.ini holds NV writes from the running control
    // program. Use `acctl push gnv` to restore from a local snapshot.
    const PULL_ONLY: &[&str] = &["autocore_gnv.ini"];
    let push_blocked: Vec<String> = to_push
        .iter()
        .filter(|p| PULL_ONLY.iter().any(|po| p.as_str() == *po))
        .cloned()
        .collect();
    to_push.retain(|p| !PULL_ONLY.iter().any(|po| p.as_str() == *po));
