...
/// True if `path` (a datastore-relative path, forward-slashed) is pull-only
/// per `SYNC_PULL_ONLY`. Directory entries (trailing `/`) match the dir
/// itself and anything under it; other entries match exactly.
fn is_pull_only(path: &str) -> bool {
    SYNC_PULL_ONLY.iter().any(|entry| match entry.strip_suffix('/') {
