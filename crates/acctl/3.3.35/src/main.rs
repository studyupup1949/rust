...
enum Commands {
    ...
    Sync {
        ...
        /// `datastore`) for the full datastore tree.
        #[arg(value_parser = ["all", "datastore"])]
        scope: Option<String>,
...
//   - `config.halt_blocks_setpoint_ack` (renamed field) → 3.3.54 (when used)
//   - `autocore_std::motion::SimDrive` (virtual axes)    → 3.3.55 (when used)
const STD_MIN_GMCOMPAT: (u32, u32, u32) = (3, 3, 52);
const STD_MIN_HALT_FIELD: (u32, u32, u32) = (3, 3, 54);
const STD_MIN_SIMDRIVE: (u32, u32, u32) = (3, 3, 55);
const STD_MIN_CSI_FIELD: (u32, u32, u32) = (3, 3, 57);
...
fn required_min_autocore_std(project: &serde_json::Value) -> (u32, u32, u32) {
    ...
    for home in ["ethercat", "motion"] {
        ...
        for axis in axes {
            if axis.get("options").and_then(|o| o.get("halt_blocks_setpoint_ack")).is_some() {
                min = min.max(STD_MIN_HALT_FIELD);
            }
            if axis.get("options").and_then(|o| o.get("change_set_immediately_blocks_ack")).is_some() {
                min = min.max(STD_MIN_CSI_FIELD);
            }
...
/// links. Currently the same set as `SYNC_PULL_ONLY`, kept separate because
/// the two lists answer different questions (what to sync vs which way).
const SYNC_CRITICAL: &[&str] = &["autocore_gnv.ini", "assets/"];
...
async fn cmd_push_gnv(config: &Config, no_reinit: bool) -> Result<()> {
    let project_root = find_project_root()?;
    let local_datastore = project_root.join("datastore");
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Push { what } => match what {
            ...
            PushCommands::Gnv { no_reinit } => cmd_push_gnv(&config, no_reinit).await,
        },
        ...
        Commands::Sync { scope, dry_run } => cmd_sync(&config, scope.is_some(), dry_run).await,
...
///
/// Drive-behavior defaults (e.g. `halt_blocks_setpoint_ack`) are NOT seeded
...
async fn cmd_add_axis(name: &str, link: &str, axis_type: &str, backend: &str) -> Result<()> {
    ...
    if backend == "ethercat" {
        ...
        println!("     like `halt_blocks_setpoint_ack`. Or set them by hand under `options`.");
...
mod tests {
    ...
    fn required_min_std_scales_with_project_features() {
        ...
        // An ethercat axis seeding the halt field → 3.3.54.
        let halt = serde_json::json!({ "modules": { "ethercat": { "config": { "axes": [
            { "name": "Press", "link": "AKD_3",
              "options": { "halt_blocks_setpoint_ack": true } }
        ] } } } });
        assert_eq!(required_min_autocore_std(&halt), STD_MIN_HALT_FIELD);

        // A virtual axis in the neutral home → 3.3.55 (SimDrive), the highest.
        ...
        // Mixed → the max wins.
        let mixed = serde_json::json!({ "modules": {
            "ethercat": { "config": { "axes": [
                { "name": "Press", "options": { "halt_blocks_setpoint_ack": true } } ] } },
            "motion": { "config": { "axes": [
                { "name": "Sim", "backend": { "kind": "virtual" } } ] } }
        } });
        assert_eq!(required_min_autocore_std(&mixed), STD_MIN_SIMDRIVE);
    }
