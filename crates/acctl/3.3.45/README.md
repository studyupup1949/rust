...
### remote-backup / remote-restore

Take a whole-system snapshot **on the target machine** before a risky upgrade,
and roll back to it in one command if the upgrade goes wrong. The tar-ball is
created and stored on the box itself (`/srv/autocore/system_backup/`), so the
operation is fast over the link regardless of project size.

A backup captures everything needed to bring the autocore ecosystem back without
the original `.deb`:

- the autocore binaries and modules (`/opt/autocore`)
- the server config (`/srv/autocore/config`)
- **all** projects (`project.json` + datastore: GNV vars, assets, scripts)
- the web console bundle (`/srv/autocore/console`)
- the systemd unit (`autocore_server.service`)

It does **not** include test `results/`/`captures/` (large; opt in with
`--include-results`), server logs, or OS-level tuning (polkit/sysctl/RT limits,
Tailscale). The newest 5 backups are kept; the rest are pruned automatically.

```bash
# Snapshot the target before an upgrade (results/captures excluded by default)
acctl remote-backup --note "before 3.4 upgrade"

# Include test results and raw captures too
acctl remote-backup --include-results

# List the backups already on the server
acctl remote-backup --list

# Restore interactively: pick from a numbered list (date/time, version, size)
acctl remote-restore

# Restore a specific backup without the confirmation prompt
acctl remote-restore backup_20260601T143022_v3.3.135.tar.gz --yes
```

`remote-restore` shows the `current → restore` version transition, then
overwrites binaries/config/projects, re-applies file capabilities, reloads the
systemd unit and restarts the server. The WebSocket connection drops as the
server restarts — reconnect after a few seconds.

To copy backups off the box onto this machine (e.g. for safekeeping), use
`acctl sync backups`, which pulls every archive (+ manifest) into a local
`backups/` directory, skipping any already present. Plain `acctl sync` and
`acctl sync all` never touch system backups.
