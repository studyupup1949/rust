//! Documentation subcommands: `acctl doc init|build|serve|generate-vars|clean`.
//!
...
use autocore_util::templates::{
    DOC_BOOK_TOML, DOC_CONTROL_API_MD, DOC_INTRO_MD, DOC_SUMMARY_MD, DOC_VARIABLES_MD,
};
use colored::*;
use std::fs;
...
pub async fn cmd_doc(cmd: &DocCommand) -> Result<()> {
    match cmd {
        DocCommand::Init { force } => cmd_init(*force),
    ...
    if !d.join("book.toml").exists() {
        return Err(anyhow!(
            "doc/book.toml not found — this project was created before `acctl doc` support.\n\
             Run `acctl doc init` to scaffold the doc/ directory."
        ));
    }
    Ok(d)
}

/// Read the `name` field from project.json for template substitution. Falls
/// back to the current directory name if the field is missing or malformed.
fn project_name(root: &Path) -> String {
    ...
    let content = match fs::read_to_string(root.join("project.json")) {
        Ok(c) => c,
...
/// Write a scaffolded file, creating parent dirs. Returns `Ok(true)` if the
/// file was written, `Ok(false)` if it existed and was left untouched.
fn scaffold_write(path: &Path, content: &str, force: bool) -> Result<bool> {
...
fn cmd_init(force: bool) -> Result<()> {
    ...
    println!(
        "{} '{}' (using project name: {})",
        "Scaffolding doc/ in".bold(),
        root.canonicalize().unwrap_or_else(|_| root.clone()).display(),
        name.as_str().cyan(),
    );

    let sub = |tmpl: &str| tmpl.replace("{name}", &name);

    let entries: &[(&str, String)] = &[
        ("book.toml",            sub(DOC_BOOK_TOML)),
        ("src/SUMMARY.md",       DOC_SUMMARY_MD.to_string()),
        ("src/introduction.md",  sub(DOC_INTRO_MD)),
        ("src/control_api.md",   DOC_CONTROL_API_MD.to_string()),
        ("src/variables.md",     DOC_VARIABLES_MD.to_string()),
    ];

    let mut written = 0;
    let mut skipped = 0;
    for (rel, content) in entries {
        let path = doc.join(rel);
        if scaffold_write(&path, content, force)? {
    ...
    println!();
    if skipped == 0 {
        println!("{}", "doc/ scaffolded. Next steps:".green());
    } else if written == 0 {
        println!(
            "{}",
            "doc/ already fully scaffolded — nothing to do.".yellow(),
        );
        return Ok(());
    } else {
        println!(
            "{} ({} created, {} left untouched)",
            "doc/ scaffolded.".green(),
            written,
            skipped,
        );
    }
    println!("  acctl doc serve   # live-reload preview at http://localhost:4444");
    println!("  acctl doc build   # one-shot static HTML at doc/book/");
    Ok(())
}
...
fn run_cargo_doc(root: &Path, doc: &Path) -> Result<()> {
    ...
    // `cargo doc --no-deps` for a single-crate project leaves the actual
    // API page at <crate_name>/index.html and does not always emit a
    // top-level index.html. Write a redirect so the link in
    // doc/src/control_api.md (rustdoc/index.html) lands somewhere useful.
    ensure_rustdoc_index(&control_manifest, &dst)?;

    Ok(())
}

/// Ensure `rustdoc/index.html` exists by writing a meta-refresh redirect to
/// the control crate's actual API page (`<crate_name>/index.html`). If a
/// real index.html already exists (newer cargo emits one for single-crate
/// projects), leave it alone.
fn ensure_rustdoc_index(control_manifest: &Path, rustdoc: &Path) -> Result<()> {
    let index = rustdoc.join("index.html");
    if index.exists() {
        return Ok(());
    }

    let crate_name = read_crate_name(control_manifest)?;
    // rustdoc replaces hyphens with underscores in the on-disk directory name.
    let crate_dir_name = crate_name.replace('-', "_");
    let crate_index = rustdoc.join(&crate_dir_name).join("index.html");
    if !crate_index.exists() {
        // Defensive: if cargo doc produced something unexpected, write a
        // helpful placeholder rather than 404 silently.
        let body = format!(
            r#"<!DOCTYPE html><html><head><meta charset="utf-8"><title>Rustdoc</title></head>
<body style="font-family:system-ui;max-width:640px;margin:4em auto;padding:0 1em;color:#333">
<h1>Rustdoc index missing</h1>
<p><code>cargo doc</code> ran but no <code>{crate_dir_name}/index.html</code> was produced.
Try running <code>acctl doc clean</code> followed by <code>acctl doc build</code>.</p>
</body></html>"#,
        );
        fs::write(&index, body).with_context(|| format!("writing {}", index.display()))?;
        return Ok(());
    }
...
<head>
  <meta charset="utf-8">
  <meta http-equiv="refresh" content="0; url={crate_dir_name}/index.html">
  <title>Redirecting to {crate_name} API docs</title>
</head>
<body style="font-family:system-ui;padding:2em">
  <p>Redirecting to <a href="{crate_dir_name}/index.html">{crate_name} API docs</a>…</p>
</body>
</html>
"#,
    );
...
/// Read the `package.name` field from a Cargo.toml file.
fn read_crate_name(manifest: &Path) -> Result<String> {
    ...
    parsed
        ...
        .and_then(|n| n.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| anyhow!("{}: package.name not found", manifest.display()))
}
