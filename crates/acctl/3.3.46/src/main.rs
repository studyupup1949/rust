...
fn ssh_probe_installed(target: &str, port: u16) -> std::collections::HashMap<String, String> {
...
fn ssh_run_interactive(target: &str, port: u16, remote: &str) -> Result<()> {
    let status = std::process::Command::new("ssh")
        .args(["-p", &port.to_string(), "-t", target, remote])
        .status()
...
fn scp_files(target: &str, port: u16, files: &[PathBuf], remote_dir: &str) -> Result<()> {
...
async fn cmd_remote_install(a: RemoteInstallArgs) -> Result<()> {
    ...
    let target_pkgs = ssh_probe_installed(&ssh_target, a.ssh_port);
    ...
    scp_files(&ssh_target, a.ssh_port, &files, remote_dir)?;
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::RemoteInstall { host, deb_dir, ssh_port, all, only, no_start, dry_run, yes } =>
            cmd_remote_install(RemoteInstallArgs { host, deb_dir, ssh_port, all, only, no_start, dry_run, yes }).await,
