...
enum Commands {
    ...
    /// Manage the autocore IDE (VS Code / VSCodium extension) from the CLI. §15.
    Ide {
        #[command(subcommand)]
        cmd: IdeCommand,
    },
    ...
    /// Push-install AutoCore .deb packages onto a target machine over SSH — the
    /// commissioning / bench workflow (no repo, no Internet, no autocore-server
    /// needed on the target beforehand). Selects the packages the active project
    /// needs, shows what will change, transfers them, and runs the install
    /// (stop → apt install → restart) with a health check. §14.
    RemoteInstall {
        /// SSH target: `[user@]host` of the machine to install onto.
        host: String,

        /// Directory holding the built .deb files.
        ...
        /// Install every .deb in --deb-dir (instead of the project-derived set).
        /// Use with care on an offline box — unrelated modules may need base-OS
        /// deps that aren't present.
        #[arg(long)]
        all: bool,

        /// Only these packages/modules (comma-separated; bare `python` or full
        /// `autocore-python`). Overrides the project-derived set.
        #[arg(long)]
        only: Option<String>,

        /// Don't (re)start autocore_server after installing.
        #[arg(long)]
        no_start: bool,

        /// Show the plan + generated script; transfer and install nothing.
        #[arg(long)]
        dry_run: bool,
        ...
        /// Don't verify or store the target's SSH host key. Use for shared
        /// commissioning IPs (e.g. every unit answers on 192.168.127.1), where
        /// each machine presents a different key for the same address and strict
        /// checking rejects the "changed" key. Trusted/direct link only — this
        /// drops MITM protection.
        #[arg(long)]
...
#[derive(Subcommand)]
enum IdeCommand {
    /// Download the autocore IDE `.vsix` from the private repo and install it into
    /// your local VS Code / VSCodium. Use it for the FIRST install on a dev box
    /// (after that the extension keeps itself updated). Reuses the same repo +
    /// credential as apt.
    Install {
        /// Channel to pull from (stable | beta | dev).
        #[arg(long, default_value = "dev")]
        channel: String,
        /// Base URL of the update server (default: https://apt.automateddesign.com/apt).
        #[arg(long)]
        repo_url: Option<String>,
        /// Repository username (adc-sn-NNNN). Prompted if omitted.
        #[arg(long)]
        user: Option<String>,
        /// Repository token/password. Prompted if omitted (pass it to avoid the
        /// prompt echoing your token).
        #[arg(long)]
        token: Option<String>,
        /// Editor CLI to install into (code | codium | code-insiders).
...
enum PushCommands {
    ...
    /// snapshot.
    ///
    /// The pushed values become AUTHORITATIVE. The server snapshots the
    /// machine's current GNV first (recover it with `list-gnv-snapshots` /
    /// `restore-gnv-snapshot`), then drops any `[__meta__] layout_hash` the
    /// backup carried, so the values are not judged against the layout they
    /// were captured under. `gm.reinitialize` then loads them and stamps the
    /// current layout, making them persistent in one step; pass --no-reinit
    /// to skip that (use only if you plan to restart the server yourself).
    Gnv {
...
// ============================================================================
// Remote install — PUSH .deb packages from this (engineering) machine onto a
// target over SSH/scp (UPDATE_SYSTEM_PLAN.md §14). The commissioning / bench
// workflow: no repo, no Internet, and the target needs nothing pre-installed
// except SSH. Runs ON THE ENGINEER'S PC (unlike update/snapshot/modules, which
// run on the target). Closest thing to TwinCAT "Activate Configuration" for the
// OS-package layer.
// ============================================================================
...
/// The websocket host (strip any `user@`) for the optional server-state checks.
fn ws_host_of(ssh_target: &str) -> &str {
    ssh_target.rsplit('@').next().unwrap_or(ssh_target)
}

/// Parse `<name>_<version>_<arch>.deb`. Name may contain `_` (e.g.
/// `autocore_server`), so split from the RIGHT: arch, version, then name.
fn parse_deb_filename(fname: &str) -> Option<(String, String)> {
    let stem = fname.strip_suffix(".deb")?;
...
/// The .deb files present in `dir`, as (package_name, version, path).
fn available_debs(dir: &Path) -> Result<Vec<(String, String, PathBuf)>> {
    let mut v = Vec::new();
    for entry in std::fs::read_dir(dir).with_context(|| format!("cannot read {}", dir.display()))? {
        ...
        if path.extension().map_or(false, |e| e == "deb") {
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                if let Some((n, ver)) = parse_deb_filename(name) {
                    v.push((n, ver, path));
                }
            }
        }
...
/// Packages the active project needs: always server + tis + ams, plus one per
/// enabled project module. `modbus` is bundled in the server deb (no standalone),
/// and ethercat pulls its `-esi` data package.
fn project_needed_packages() -> Result<Vec<String>> {
    let mut needed = vec![
        "autocore_server".to_string(),
        "autocore-tis".to_string(),
        "autocore-ams".to_string(),
    ];
    let pj = Path::new("project.json");
    if pj.exists() {
        let v: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(pj)?)?;
        if let Some(mods) = v.get("modules").and_then(|m| m.as_object()) {
            for (domain, cfg) in mods {
                if !cfg.get("enabled").and_then(|x| x.as_bool()).unwrap_or(true) {
                    continue;
                }
                let exe = cfg.get("executable").and_then(|x| x.as_str());
                let pkg = module_package_name(domain, exe);
                if pkg == "autocore-modbus" {
                    continue; // bundled inside the server deb
                }
                needed.push(pkg);
            }
        }
    }
    if needed.iter().any(|p| p == "autocore-ethercat") {
        needed.push("autocore-ethercat-esi".to_string());
    }
    needed.sort();
    needed.dedup();
    Ok(needed)
...
/// Best-effort: read the target's installed autocore package versions over SSH.
/// Uses BatchMode so it fails fast (rather than hanging on a password prompt)
/// when key-based auth isn't set up — the version diff is a nicety, not required.
/// Extra ssh/scp options for shared commissioning IPs (e.g. every unit answers on
/// 192.168.127.1) where each machine presents a DIFFERENT host key for the SAME
/// address — strict checking would reject the "changed" key. Skips verification and
/// avoids writing the key to known_hosts (so it never conflicts next time). Trusted
/// direct/commissioning link only; it drops MITM protection.
fn host_key_opts(insecure: bool) -> Vec<&'static str> {
    if insecure {
        vec![
...
fn ssh_probe_installed(
    target: &str,
    port: u16,
    insecure: bool,
) -> std::collections::HashMap<String, String> {
    let mut map = std::collections::HashMap::new();
    let mut args: Vec<String> = vec!["-p".into(), port.to_string()];
    args.extend(host_key_opts(insecure).into_iter().map(String::from));
    args.extend(
        [
            "-o", "BatchMode=yes",
            "-o", "ConnectTimeout=8",
            target,
            "dpkg-query -W -f='${Package} ${Version}\\n' 'autocore*' 2>/dev/null || true",
        ]
        .into_iter()
        .map(String::from),
    );
    let out = std::process::Command::new("ssh").args(&args).output();
    match out {
        ...
        _ => {
            eprintln!(
                "{}",
                "(couldn't read the target's current versions over SSH — set up key-based auth \
                 for a version diff; continuing without it)"
                    .dimmed()
            );
        }
    }
...
/// Run an SSH command with inherited stdio (so sudo / password / host-key
/// prompts work). Errors on non-zero exit.
fn ssh_run_interactive(target: &str, port: u16, remote: &str, insecure: bool) -> Result<()> {
    let mut args: Vec<String> = vec!["-p".into(), port.to_string()];
    args.extend(host_key_opts(insecure).into_iter().map(String::from));
    args.push("-t".into());
    ...
    let status = std::process::Command::new("ssh")
        .args(&args)
        .status()
        .context("failed to run ssh (is the OpenSSH client installed?)")?;
    if !status.success() {
        return Err(anyhow!("remote command failed over SSH: {}", remote));
    }
    Ok(())
}

/// scp files to `target:remote_dir/` with inherited stdio.
fn scp_files(
    target: &str,
    port: u16,
    files: &[PathBuf],
    ...
    if !status.success() {
        return Err(anyhow!("scp transfer to {} failed", target));
    }
    Ok(())
}

fn generate_install_script(selected: &[(String, String, PathBuf)], no_start: bool) -> String {
    let list = selected
        .iter()
        .map(|(n, v, _)| format!("{} {}", n, v))
        .collect::<Vec<_>>()
        .join(", ");
    let start = if no_start {
        ""
    } else {
        "if command -v systemctl >/dev/null 2>&1; then\n  \
           echo 'Starting autocore_server...'\n  \
           systemctl restart autocore_server || true\nfi\n"
    };
    format!(
        "#!/bin/sh\n\
         # Generated by `acctl remote-install`. Runs on the TARGET as root.\n\
         # Packages: {list}\n\
         set -e\n\
         cd \"$(dirname \"$0\")\"\n\
         echo '== autocore remote-install =='\n\
         if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet autocore_server 2>/dev/null; then\n  \
           echo 'Stopping autocore_server...'\n  \
           systemctl stop autocore_server || true\n\
         fi\n\
...
async fn cmd_remote_install(a: RemoteInstallArgs) -> Result<()> {
    let deb_dir = Path::new(&a.deb_dir);
    let available = available_debs(deb_dir)?;
    if available.is_empty() {
        return Err(anyhow!(
            "no .deb files in {} — build them first (e.g. `sh build_debian.sh`).",
            a.deb_dir
        ));
    }

    // Which package names do we want?
    let needed: Vec<String> = if a.all {
        available.iter().map(|(n, _, _)| n.clone()).collect()
    } else if let Some(only) = &a.only {
        only.split(',')
            .map(|t| t.trim())
            .filter(|t| !t.is_empty())
            .map(|t| {
                if t == "server" || t == "autocore_server" {
                    "autocore_server".to_string()
                } else {
                    to_module_package(t)
                }
        ...
        if !Path::new("project.json").exists() {
            println!(
                "{}",
                "No project.json here — installing only the base (server, tis, ams). Run from \
                 your project directory, or use --only / --all, to include modules."
                    .yellow()
            );
        }
        project_needed_packages()?
    };

    // Match needed → available debs; note any needed package with no deb present.
    let mut selected: Vec<(String, String, PathBuf)> = Vec::new();
    let mut missing: Vec<String> = Vec::new();
    for name in &needed {
        match available.iter().find(|(n, _, _)| n == name) {
    ...
    if selected.is_empty() {
        return Err(anyhow!(
            "none of the needed packages ({}) were found in {}.",
            needed.join(", "),
            a.deb_dir
        ));
    }

    let ssh_target = a.host.clone();
    let ws_host = ws_host_of(&ssh_target).to_string();

    println!("{} {} (ssh port {})", "Target:".bold(), ssh_target, a.ssh_port);
    let target_pkgs = ssh_probe_installed(&ssh_target, a.ssh_port, a.insecure_host_key);

    println!("{} {}", "Packages from".bold(), a.deb_dir);
    for (name, ver, _) in &selected {
        let status = match target_pkgs.get(name) {
            None => "new".green().to_string(),
            Some(cur) if cur == ver => format!("up to date ({})", cur).dimmed().to_string(),
            Some(cur) => format!("{} → {}", cur, ver).yellow().to_string(),
        };
        println!("   {:<26} {:<14} [{}]", name, ver, status);
    }
    if !missing.is_empty() {
        println!(
            "{} {}",
            "Needed but no .deb found (skipped):".yellow(),
            missing.join(", ")
        );
    }

    let script = generate_install_script(&selected, a.no_start);
    if a.dry_run {
        println!("\n{}\n{}", "--- generated install script (dry run) ---".bold(), script);
        println!("{}", "Dry run — nothing transferred.".dimmed());
        return Ok(());
    }

    if !a.yes && !confirm("Transfer and install on the target?")? {
        println!("Aborted.");
        return Ok(());
    }

    // RT safety: if the target server is up, refuse mid-test. If it's not
    // reachable (fresh box), this passes (nothing to protect).
    let target_cfg = Config {
        server: Some(ServerConfig {
            host: Some(ws_host.clone()),
            port: Some(11969),
            ..Default::default()
        }),
        ..Default::default()
    };
    gate_not_mid_test(&target_cfg).await?;

    // Stage locally, prep a clean remote dir, transfer, run.
    let local_script =
        std::env::temp_dir().join(format!("autocore-install-{}.sh", std::process::id()));
    std::fs::write(&local_script, script)?;
    let remote_dir = "/tmp/autocore-install";

    println!("{}", "Preparing target directory...".bold());
    ssh_run_interactive(
        &ssh_target,
        a.ssh_port,
        &format!("rm -rf {d} && mkdir -p {d}", d = remote_dir),
        a.insecure_host_key,
    )?;

    println!("{}", "Transferring packages...".bold());
    let mut files: Vec<PathBuf> = selected.iter().map(|(_, _, p)| p.clone()).collect();
    files.push(local_script.clone());
    scp_files(&ssh_target, a.ssh_port, &files, remote_dir, a.insecure_host_key)?;

    let script_name = local_script
        .file_name()
        .map(|s| s.to_string_lossy().into_owned())
        .unwrap_or_else(|| "install.sh".into());
    println!("{}", "Running install on the target (you may be prompted for sudo)...".bold());
    ssh_run_interactive(
        &ssh_target,
        a.ssh_port,
        &format!("sudo sh {}/{}", remote_dir, script_name),
        a.insecure_host_key,
    )?;
    let _ = std::fs::remove_file(&local_script);

    if a.no_start {
        println!("{}", "Installed. Server not started (--no-start).".green());
        return Ok(());
    }

    println!("{}", "Waiting for the target server to come back healthy...".bold());
    let (ok, reason) = wait_healthy(&target_cfg, HEALTH_TIMEOUT).await;
    if ok {
        println!("{}", "Remote install complete — server healthy.".green());
        Ok(())
    } else {
        Err(anyhow!(
            "Installed, but the target server is {}. Check `acctl --host {} status` and its logs.",
            reason, ws_host
        ))
    }
}
...
// ============================================================================
// IDE install — fetch the autocore-ide .vsix from the private repo and install
// it into the local editor (UPDATE_SYSTEM_PLAN.md §15). First-install bootstrap
// for a dev box; the extension's own self-updater takes over afterward. Shells
// out to curl / sha256sum / the editor CLI (all present on a dev box) so acctl
// gains no new HTTP dependency.
// ============================================================================

struct IdeInstallArgs {
    channel: String,
    repo_url: Option<String>,
    user: Option<String>,
    token: Option<String>,
...
/// Which editor CLI to install into: explicit flag, else first of code / codium /
/// code-insiders found on PATH.
fn resolve_editor_cli(flag: Option<String>) -> Result<String> {
    if let Some(e) = flag {
        return Ok(e);
    }
    for cand in ["code", "codium", "code-insiders"] {
    ...
    Err(anyhow!(
        "no editor CLI found on PATH (code / codium). Install one, or pass --editor."
    ))
}

async fn cmd_ide_install(a: IdeInstallArgs) -> Result<()> {
    let repo = a
        .repo_url
        .unwrap_or_else(|| "https://apt.automateddesign.com/apt".to_string())
        .trim_end_matches('/')
        .to_string();

    let user = match a.user {
        Some(u) => u,
        None => prompt_line("AutoCore repository username (e.g. adc-sn-0041): ")?,
    };
    if user.is_empty() {
        return Err(anyhow!("a username is required"));
    }
    let token = match a.token {
        Some(t) => t,
        None => prompt_line("Repository token/password: ")?,
    };
    let cred = format!("{}:{}", user, token);

    // 1. Fetch the channel manifest. The vsix has a versioned filename so it's
    // never served stale, but latest.json is a FIXED url — a web cache in front of
    // the repo will happily hand back an old one (this is exactly what made a
    // freshly published version look un-updated). Defeat it with a per-request
    // cache-buster query param plus a no-cache request header.
    let manifest_url = format!("{}/ide/{}/latest.json", repo, a.channel);
    let nonce = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let manifest_fetch_url = format!("{}?t={}", manifest_url, nonce);
    ...
    if !out.status.success() {
        return Err(anyhow!(
            "could not fetch the IDE manifest ({}). Check the URL, channel, and credentials.",
            manifest_url
        ));
    }
    let manifest: serde_json::Value = serde_json::from_slice(&out.stdout)
        .context("IDE manifest was not valid JSON")?;
    let version = manifest["version"].as_str().ok_or_else(|| anyhow!("manifest missing version"))?;
    let vsix = manifest["vsix"].as_str().ok_or_else(|| anyhow!("manifest missing vsix"))?;
    let sha = manifest["sha256"].as_str().ok_or_else(|| anyhow!("manifest missing sha256"))?;

    // 2. Download the vsix.
    let tmp = std::env::temp_dir().join(vsix);
    let vsix_url = format!("{}/ide/{}/{}", repo, a.channel, vsix);
    println!("Downloading autocore IDE {}...", version);
    let dl = std::process::Command::new("curl")
        .args(["-fsSL", "--user", &cred, &vsix_url, "-o"])
        .arg(&tmp)
        .status()
        .context("failed to run curl")?;
    if !dl.success() {
        return Err(anyhow!("failed to download {}", vsix_url));
    }
    ...
    if !got.eq_ignore_ascii_case(sha) {
        ...
        return Err(anyhow!(
            "checksum mismatch — download may be corrupt or tampered with (expected {}, got {})",
            sha, got
        ));
    }

    // 4. Install into the editor.
    let editor = resolve_editor_cli(a.editor)?;
    println!("Installing into '{}'...", editor);
    let st = std::process::Command::new(&editor)
    ...
    if !st.success() {
        return Err(anyhow!(
            "'{} --install-extension' failed. You can install the .vsix manually.",
            editor
        ));
    }
    ...
    println!(
        "{}",
        format!("Installed autocore IDE {}.", version).green()
    );
    println!(
        "Tip: in the editor, run \"autocore IDE: Set Update Credential\" once so it keeps itself \
         updated from now on."
    );
    println!(
        "{}",
        format!(
            "Then set `autocore.ide.updateChannel` to \"{}\" (Settings), so the IDE self-updates \
             from the same channel it was installed from. Otherwise update checks will fail — \
             nothing is published on the other channels.",
            a.channel
        )
        .yellow()
    );
...
async fn cmd_push_gnv(config: &Config, no_reinit: bool, target: Option<&str>) -> Result<()> {
    ...
    // The server drops any layout stamp the backup carried, so these values
    // become the machine's truth rather than being judged against the layout
    // they were captured under. Older servers do not report this field — say so
    // rather than implying a guarantee they do not make.
    match resp.data.get("gnv_stamp_stripped").and_then(|v| v.as_bool()) {
        Some(true) => println!(
            "{} stale layout stamp dropped — these values are now authoritative",
            "✓".green()
        ),
        Some(false) => println!(
            "{} no layout stamp in the pushed file — values accepted as-is",
            "✓".green()
        ),
        None => println!(
            "{} server did not report on the layout stamp (older build): if the pushed file \
             carries a [__meta__] layout_hash from a different layout it may be rejected on \
             load. Remove that section and re-push if the values do not appear.",
            "⚠".yellow()
        ),
    }
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Ide { cmd } => match cmd {
            IdeCommand::Install { channel, repo_url, user, token, editor } =>
                cmd_ide_install(IdeInstallArgs { channel, repo_url, user, token, editor }).await,
        },
        Commands::RemoteRestore { name, yes } => cmd_remote_restore(&config, name, yes).await,
        Commands::RemoteInstall { host, deb_dir, ssh_port, all, only, no_start, dry_run, yes, insecure_host_key } =>
            cmd_remote_install(RemoteInstallArgs { host, deb_dir, ssh_port, all, only, no_start, dry_run, yes, insecure_host_key }).await,
