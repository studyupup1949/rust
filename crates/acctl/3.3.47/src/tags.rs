...
//!
//! After the local tag file is written, this command automatically follows
//! up by invoking [`crate::cmd_codegen`] so the server rebuilds `gm.rs` and
//! (as a side effect) `www/src/autocore/tis.ts` — keeping the two
//! generated TypeScript files in sync with a single command. If the server
//! is unreachable or the follow-up fails, a warning is printed and this
//! command still exits successfully; the tags file itself is already saved.
//!
//! # Output file layout
//!
...
//!
//! A full regen on a project whose tag file was hand-written (the common case
//! the first time this command runs against an older project) replaces that
//! spec with the generated one, which is usually far smaller — so it prints a
//! prominent warning naming the number of tags that now live only in the `.bak`.
//! Both paths guarantee the file ends with the `TagName` type export, since
//! `www/src/AutoCore.ts` imports it.
...
pub async fn cmd_codegen_tags(force: bool) -> Result<()> {
    ...
    // Methods live in the test_methods.json sidecar next to project.json
    // (legacy embedded block as fallback) — resolve them the same way the
    // server does before deciding whether to inject the tis_* scalars.
    let tis_enabled = crate::test_methods::has_test_methods(&project_json_path, &project)?;

    let tags = collect_tags(&project, tis_enabled);
    println!(
    ...
    match action {
        Action::ReplaceInPlace { existing } => {
            let spliced = splice_generated_block(&existing, &generated_block)?;
            let new_contents = ensure_tag_name_export(&spliced);
            fs::write(&tags_path, new_contents)
                .with_context(|| format!("writing {}", tags_path.display()))?;
            let kept = count_tag_entries(&existing).saturating_sub(tags.len());
            println!(
        ...
        Action::FullRegen { backed_up, displaced_tags } => {
            if let Some(bak) = backed_up {
                ...
                // The pre-codegen file had no sentinels and no acTagSpecCustom, so
                // its whole hand-written spec is being replaced by the (usually
                // much smaller) generated one. Say so loudly: silently dropping
                // tags here compiles as a `never`-typed acTagSpec and surfaces
                // later as unrelated-looking tsc errors in AutoCore.ts.
                if displaced_tags > 0 {
                    println!();
                    println!(
                        "  {} this file held {} hand-written tag{} that are NOT in project.json",
                        "WARNING:".red().bold(),
                        displaced_tags,
                        if displaced_tags == 1 { "" } else { "s" },
                    );
                    println!(
                        "  {}",
                        "They now live ONLY in the .bak above. Copy them into the".yellow(),
                    );
                    println!(
                        "  {}",
                        "acTagSpecCustom array of the new file, or set \"ux\": true on the".yellow(),
                    );
                    println!(
                        "  {}",
                        "matching project.json variables and re-run — otherwise the UI".yellow(),
                    );
                    println!(
                        "  {}",
                        "build will fail with a `never` type on every tag hook.".yellow(),
                    );
                    println!();
                }
            }
    ...
    // Chain to `acctl codegen` so the server rebuilds gm.rs and re-emits
    // www/src/autocore/tis.ts. Non-fatal: if the server is unreachable,
    // the tags file on disk is still good — just warn and exit success.
    println!();
    println!("{} server to refresh gm.rs + tis.ts…", "Asking".bold());
    let config = crate::Config::load().unwrap_or_default();
    match crate::cmd_codegen(&config, false).await {
        Ok(()) => {}
        Err(e) => {
            println!(
                "  {} server codegen skipped: {}",
                "warning:".yellow(),
                e,
            );
            println!(
                "  {}",
                "(run `acctl codegen` once the server is reachable to refresh tis.ts)"
                    .dimmed(),
            );
        }
    }
...
fn collect_tags(project: &Value, tis_enabled: bool) -> Vec<Tag> {
    ...
    // Mirror the server-side `Project::normalize()` injection: when the
    // project declares any `test_methods`, the server auto-adds nine
    // `tis_*` readiness scalars at load time so the codegen can emit
    // them into gm.rs and the IPC bus can broadcast them. The HMI also
    // needs them in `AutoCoreTags.ts` to resolve `useAutoCoreTag("tisStaged")`
    // and friends — but `acctl codegen-tags` works against the raw JSON
    // and doesn't run normalize, so we replicate the injection here. The
    // caller resolves `tis_enabled` from the test_methods.json sidecar
    // (falling back to a legacy embedded block) via
    // `crate::test_methods::has_test_methods`.
    if tis_enabled {
        const TIS_SCALARS: &[(&str, &str)] = &[
            ("tis_staged",            "boolean"),
            ("tis_staged_project_id", "string"),
            ("tis_staged_method_id",  "string"),
            ("tis_staged_sample_id",  "string"),
            ("tis_active",            "boolean"),
            ("tis_active_project_id", "string"),
            ("tis_active_method_id",  "string"),
            ("tis_active_sample_id",  "string"),
            ("tis_active_run_id",     "string"),
        ];
        for (name, value_type) in TIS_SCALARS {
...
"#;
    ...
    out.push_str(FOOTER);
    ensure_tag_name_export(&out)
}

/// The `TagName` type export. The per-project hook layer (`www/src/AutoCore.ts`)
/// does `import type { TagName } from "./AutoCoreTags"`, so every tag file must
/// carry it — both freshly rendered ones and older files being spliced in place
/// that were written before this export existed. Omitting it breaks `tsc` in a
/// way that looks unrelated to codegen (TS2614 on AutoCore.ts).
const TAG_NAME_EXPORT: &str = "\n\
// Export tag names as a type for IntelliSense support.\n\
export type TagName = typeof acTagSpec[number][\"tagName\"];\n";

/// Append [`TAG_NAME_EXPORT`] unless the file already declares it. Idempotent.
fn ensure_tag_name_export(contents: &str) -> String {
    if contents.contains("export type TagName") {
...
enum Action {
    ...
    /// Rewrite the whole file. If the old file existed, it was backed up and
    /// `displaced_tags` counts the hand-written tags that only live in the
    /// backup from now on.
    FullRegen {
        backed_up: Option<PathBuf>,
        displaced_tags: usize,
    },
}

fn decide_action(tags_path: &Path, force: bool) -> Result<Action> {
    if !tags_path.exists() {
        return Ok(Action::FullRegen { backed_up: None, displaced_tags: 0 });
    }
    ...
    Ok(Action::FullRegen {
        backed_up: Some(bak),
        displaced_tags: count_tag_entries(&existing),
    })
}

/// Number of `"tagName"` entries in a tag file — used only to tell the operator
/// how much hand-written spec a full regen just moved into the `.bak`.
fn count_tag_entries(contents: &str) -> usize {
    contents.matches("\"tagName\"").count() + contents.matches("tagName:").count()
}
...
mod tests {
    ...
    fn collect_tags_filters_and_sorts() {
        ...
        let tags = collect_tags(&project, false);
    ...
    #[test]
    fn collect_tags_injects_tis_scalars_when_enabled() {
        // `tis_enabled` comes from the test_methods.json sidecar (or a
        // legacy embedded block) — collect_tags itself no longer looks at
        // the raw project value for it.
        let project = serde_json::json!({
            "variables": {
                "tis_staged": { "type": "bool", "ux": true }, // explicit wins, not duplicated
            }
        });
        let tags = collect_tags(&project, true);
        let names: Vec<_> = tags.iter().map(|t| t.tag_name.as_str()).collect();
        assert!(names.contains(&"tisActive"));
        assert!(names.contains(&"tisActiveRunId"));
        assert_eq!(names.iter().filter(|n| **n == "tisStaged").count(), 1);

        let none = collect_tags(&project, false);
        assert!(!none.iter().any(|t| t.tag_name == "tisActive"));
    }
    ...
    #[test]
    fn full_file_always_exports_tag_name() {
        // www/src/AutoCore.ts imports `TagName`; a template without it breaks
        // `tsc` on every project the first time codegen-tags runs.
        let full = render_full_file(&render_generated_block(&[]));
        assert!(full.contains("export type TagName = typeof acTagSpec[number][\"tagName\"];"));
        assert!(full.contains("export const acTagSpecCustom = ["));
    }

    #[test]
    fn ensure_tag_name_export_is_idempotent_and_backfills() {
        // Already present → byte-identical.
        let with = "export type TagName = typeof acTagSpec[number][\"tagName\"];\n";
        assert_eq!(ensure_tag_name_export(with), with);

        // Missing (a file generated before this export existed) → appended once.
        let without = "export default acTagSpec;\n";
        let fixed = ensure_tag_name_export(without);
        assert!(fixed.starts_with(without));
        assert!(fixed.contains("export type TagName"));
        assert_eq!(ensure_tag_name_export(&fixed), fixed);
    }

    #[test]
    fn count_tag_entries_handles_both_quoting_styles() {
        assert_eq!(count_tag_entries("{ \"tagName\": \"a\" }, { \"tagName\": \"b\" }"), 2);
        assert_eq!(count_tag_entries("{ tagName: \"a\" }"), 1);
        assert_eq!(count_tag_entries("no tags at all"), 0);
    }
}
