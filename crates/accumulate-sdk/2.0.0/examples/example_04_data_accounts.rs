//! Example 4: Data Accounts and Entries (Kermit Testnet)
//!
...
//!
//! Run with: cargo run --example example_04_data_accounts
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== SDK Example 4: Data Accounts and Entries ===\n");
    println!("Endpoint: {}\n", KERMIT_V3);
    ...
    let timestamp = SystemTime::now()
        ...
        .as_millis();
    let adi_name = format!("sdk-data-{}", timestamp);
    ...
    // =========================================================
    println!("--- Step 6: Create ADI Data Account ---\n");

    let mut adi_signer = SmartSigner::new(&client, adi_keypair.clone(), &key_page_url);

    let data_account_url = format!("{}/data", identity_url);
    println!("Data Account URL: {}", data_account_url);

    let create_data_body = TxBody::create_data_account(&data_account_url);

    let result = adi_signer.sign_submit_and_wait(
        &identity_url,
        &create_data_body,
        Some("Create ADI data account"),
        30,
    ).await;

    if result.success {
        println!("CreateDataAccount SUCCESS - TxID: {:?}\n", result.txid);
    } else {
        println!("CreateDataAccount FAILED: {:?}", result.error);
        return Ok(());
    }
    ...
    // =========================================================
    println!("--- Step 7: Write Data Entries ---\n");

    // Prepare data entries
    let data_entry1 = "Hello, Accumulate!";
    let data_entry2 = "Data entry from Rust SDK";
    let timestamp_str = format!("Timestamp: {}", chrono::Utc::now().to_rfc3339());
    let data_entry3 = &timestamp_str;

    // Convert to hex strings
    let entries_hex = vec![
        hex::encode(data_entry1.as_bytes()),
        hex::encode(data_entry2.as_bytes()),
        hex::encode(data_entry3.as_bytes()),
    ];

    println!("Writing {} data entries:", entries_hex.len());
    println!("  1: {}", data_entry1);
    println!("  2: {}", data_entry2);
    println!("  3: {}\n", data_entry3);

    let write_data_body = TxBody::write_data(&[data_entry1, data_entry2, data_entry3]);

    let result = adi_signer.sign_submit_and_wait(
        &data_account_url,
        &write_data_body,
        Some("Write data entries"),
        30,
    ).await;

    if result.success {
        println!("WriteData SUCCESS - TxID: {:?}\n", result.txid);
    } else {
        println!("WriteData FAILED: {:?}", result.error);
    }

    // =========================================================
    // Step 8: Query Data Account
    // =========================================================
    println!("--- Step 8: Query Data Account ---\n");

    tokio::time::sleep(std::time::Duration::from_secs(5)).await;

    match query_account(&client, &data_account_url).await {
        Ok(account) => {
            println!("Data Account Query:");
        ...
        Err(e) => println!("Could not query data account: {}\n", e),
    }
    ...
    println!("Created Data Account: {}", data_account_url);
    println!("Wrote {} data entries", entries_hex.len());
