//! Example 7: Custom Tokens (Kermit Testnet)
//!
...
//!
//! Run with: cargo run --example example_07_custom_tokens
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== SDK Example 7: Custom Tokens ===\n");
    println!("Endpoint: {}\n", KERMIT_V3);
    ...
    for i in 1..=5 {
        match client.faucet(&lite_token_account).await {
            Ok(response) => {
                println!("  Faucet {}/5: {:?}", i, response.txid);
            }
            Err(e) => {
    ...
    let timestamp = SystemTime::now()
        ...
        .as_millis();
    let adi_name = format!("sdk-token7-{}", timestamp);
    ...
    let custom_token_url = format!("{}/my-custom-token", identity_url);
    let symbol = "MYT2";
    let precision = 2u64;
    ...
    let account1_url = format!("{}/myCustomTokenAccount1", identity_url);
    let account2_url = format!("{}/myCustomTokenAccount2", identity_url);
    ...
    let result = adi_signer.sign_submit_and_wait(
        ...
        Some("Create custom token account 1"),
        30,
    ).await;
    ...
    let result = adi_signer.sign_submit_and_wait(
        ...
        Some("Create custom token account 2"),
        30,
    ).await;
    ...
    let issue_amount = 100000u64; // 1000.00 MYT2 (2 decimal precision)
    println!("Issuing {} tokens (1000.00 MYT2) to {}", issue_amount, account1_url);

    let issue_body = TxBody::issue_tokens_single(&account1_url, &issue_amount.to_string());
    ...
    let send_amount = 6000u64; // 60.00 MYT2
    println!("Sending {} tokens (60.00 MYT2) from account 1 to account 2", send_amount);

    let send_body = TxBody::send_tokens_single(&account2_url, &send_amount.to_string());
    ...
    println!("Token Account 1: {}", account1_url);
    println!("Token Account 2: {}", account2_url);
    println!("\nOperations:");
    println!("  - Issued 1000.00 {} to Account 1", symbol);
    println!("  - Transferred 60.00 {} to Account 2", symbol);
    println!("  - Account 1 balance: 940.00 {}", symbol);
    println!("  - Account 2 balance: 60.00 {}", symbol);
