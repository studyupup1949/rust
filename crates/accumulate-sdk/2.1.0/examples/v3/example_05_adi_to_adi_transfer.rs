//! Example 5: ADI-to-ADI Token Transfer (Kermit Testnet)
//!
//! This example demonstrates:
//! - Creating ADI Token Accounts with specific purposes (tokens, staking)
//! - ADI-to-ADI token transfers
//! - Using SmartSigner API for auto-version tracking
//!
//! Run with: cargo run --example example_05_adi_to_adi_transfer
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== SDK Example 5: ADI-to-ADI Token Transfer ===\n");
    println!("Endpoint: {}\n", KERMIT_V3);
    ...
    let timestamp = SystemTime::now()
        ...
        .as_millis();
    let adi_name = format!("sdk-transfer-{}", timestamp);
    ...
    println!("Creating tokens account: {}", tokens_account_url);
    ...
    if result.success {
        println!("CreateTokenAccount (tokens) SUCCESS - TxID: {:?}", result.txid);
    } else {
        println!("CreateTokenAccount (tokens) FAILED: {:?}", result.error);
    }

    // Create staking account
    println!("Creating staking account: {}", staking_account_url);
    let create_staking_body = TxBody::create_token_account(&staking_account_url, "acc://ACME");
    let result = adi_signer.sign_submit_and_wait(
        &identity_url,
        &create_staking_body,
        Some("Create staking account"),
        30,
    ).await;

    if result.success {
        println!("CreateTokenAccount (staking) SUCCESS - TxID: {:?}\n", result.txid);
    } else {
        println!("CreateTokenAccount (staking) FAILED: {:?}", result.error);
    }
    ...
    // =========================================================
    println!("--- Step 7: Fund ADI tokens account from lite ---\n");

    let fund_amount = "1000000000"; // 10 ACME (8 decimal places)
    println!("Sending 10 ACME from lite to {}", tokens_account_url);

    let fund_body = TxBody::send_tokens_single(&tokens_account_url, fund_amount);
    ...
    match query_balance(&client, &tokens_account_url).await {
        Ok(balance) => println!("Tokens account balance: {}\n", balance),
        Err(e) => println!("Could not query tokens balance: {}\n", e),
    }

    // =========================================================
    // Step 8: ADI-to-ADI Transfer (Main Event!)
    // =========================================================
    println!("--- Step 8: ADI-to-ADI Transfer ---\n");

    let transfer_amount = "500000000"; // 5 ACME
    println!("Sending 5 ACME from {} to {}", tokens_account_url, staking_account_url);
    println!("(ADI-to-ADI transfer using SmartSigner)\n");

    let transfer_body = TxBody::send_tokens_single(&staking_account_url, transfer_amount);

    let result = adi_signer.sign_submit_and_wait(
        &tokens_account_url,
        &transfer_body,
        Some("ADI-to-ADI transfer"),
        30,
    ).await;

    if result.success {
        println!("ADI-to-ADI Transfer SUCCESS!");
        println!("TxID: {:?}\n", result.txid);
    } else {
        println!("ADI-to-ADI Transfer FAILED: {:?}", result.error);
    }

    // =========================================================
    // Step 9: Verify balances
    // =========================================================
    println!("--- Step 9: Verify Balances ---\n");

    tokio::time::sleep(std::time::Duration::from_secs(5)).await;

    match query_balance(&client, &tokens_account_url).await {
        Ok(balance) => println!("Tokens account balance: {}", balance),
        Err(e) => println!("Could not query tokens balance: {}", e),
    }

    match query_balance(&client, &staking_account_url).await {
        Ok(balance) => println!("Staking account balance: {}\n", balance),
        Err(e) => println!("Could not query staking balance: {}\n", e),
    }
    ...
    println!("Tokens Account: {}", tokens_account_url);
    println!("Staking Account: {}", staking_account_url);
    println!("\nToken transfers:");
    println!("  - 10 ACME: lite -> tokens account");
    println!("  - 5 ACME: tokens account -> staking account (ADI-to-ADI)");
