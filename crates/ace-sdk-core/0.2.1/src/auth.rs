...
pub async fn login(
    ...
    // Show code to user
    on_user_code(&device_code.user_code, &device_code.verification_uri_complete);
    ...
    loop {
        ...
        match poll_for_token(&device_code.device_code).await? {
            PollResult::Success(token_response) => {
                let user_data = token_response.resolved_user()
                    .map_err(|e| AceError::Other(e))?;
...
pub fn get_token_status() -> TokenStatus {
    ...
    let has_token = auth.get("token").and_then(|t| t.as_str()).map(|t| !t.is_empty()).unwrap_or(false);
    if !has_token {
        return TokenStatus {
            has_token: false,
            is_access_expired: false,
    ...
    let access_expires = auth.get("expires_at").and_then(|v| v.as_str()).map(|s| s.to_string());
    let refresh_expires = auth.get("refresh_expires_at").and_then(|v| v.as_str()).map(|s| s.to_string());
    let absolute_expires = auth.get("absolute_expires_at").and_then(|v| v.as_str()).map(|s| s.to_string());
    let has_refresh = auth.get("refresh_token").and_then(|t| t.as_str()).map(|t| !t.is_empty()).unwrap_or(false);
...
pub fn is_token_locally_expired() -> Option<bool> {
    ...
    let has_absolute = auth.get("absolute_expires_at").and_then(|v| v.as_str()).is_some();
    let has_refresh = auth.get("refresh_expires_at").and_then(|v| v.as_str()).is_some();
...
pub fn get_current_user() -> Option<crate::types::CurrentUser> {
    ...
    Some(crate::types::CurrentUser {
        ...
        default_org_id: config.get("default_org_id").and_then(|v| v.as_str()).map(|s| s.to_string()),
