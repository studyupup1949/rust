...
impl Default for ThoroughnessLevel {
    fn default() -> Self {
        Self::Medium
    }
}
...
impl TokenResponse {
    ...
    pub fn resolved_user(&self) -> Result<TokenUser, String> {
        ...
        let user_id = self.user_id.clone().ok_or_else(||
            "TokenResponse missing both nested user and flat user_id".to_string()
        )?;
...
pub fn get_usage_percentage(metric: &UsageMetric) -> u32 {
    ...
    std::cmp::min(100, (metric.used as f64 / metric.limit as f64 * 100.0).round() as u32)
}
...
/// Available time windows for usage history queries.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum UsageHistoryWindow {
    #[serde(rename = "1h")]
    OneHour,
    #[serde(rename = "6h")]
    SixHours,
...
impl std::fmt::Display for UsageHistoryWindow {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::OneHour => write!(f, "1h"),
            Self::SixHours => write!(f, "6h"),
...
/// Granularity of usage history buckets.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum UsageHistoryGranularity {
    Hourly,
    Daily,
}
...
pub struct UsageHistoryBucket {
    ...
    #[serde(default)]
    pub api_calls_patterns: u32,
    #[serde(default)]
    pub api_calls_traces: u32,
    #[serde(default)]
    pub api_calls_playbook: u32,
    #[serde(default)]
    pub patterns_created: u32,
    #[serde(default)]
    pub patterns_updated: u32,
    #[serde(default)]
    pub patterns_deleted: u32,
    #[serde(default)]
    pub patterns_searched: u32,
    #[serde(default)]
    pub traces_submitted: u32,
    #[serde(default)]
...
/// Full usage history response from GET /api/v1/usage/history.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UsageHistoryResponse {
    pub org_id: String,
    pub project_id: Option<String>,
    pub window: UsageHistoryWindow,
    pub granularity: UsageHistoryGranularity,
    #[serde(default)]
...
mod tests {
    ...
    fn test_usage_percentage() {
        ...
        let m2 = UsageMetric { used: 100, limit: 100 };
