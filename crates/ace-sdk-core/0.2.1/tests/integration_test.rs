...
fn test_playbook_bullet_json_roundtrip() {
    let bullet = PlaybookBullet {
        ...
        last_used: "2025-04-01T00:00:00Z".to_string(),
    ...
    assert_eq!(deserialized.helpful, 10);
...
fn test_playbook_bullet_minimal_json() {
    ...
    assert_eq!(bullet.helpful, 0); // default
    assert_eq!(bullet.harmful, 0); // default
...
fn test_usage_info_construction() {
    let info = UsageInfo {
        ...
        patterns: UsageMetric { used: 50, limit: 100 },
        patterns_total: UsageMetric { used: 200, limit: 500 },
        projects: UsageMetric { used: 3, limit: 10 },
        domains: UsageMetric { used: 0, limit: 5 },
        templates: UsageMetric { used: 0, limit: 10 },
        api_calls: UsageMetric { used: 1000, limit: 10000 },
...
fn test_delta_operation_update_json() {
    let op = DeltaOperation {
        ...
        helpful_delta: Some(1),
        harmful_delta: Some(0),
        confidence: None,
        evidence: None,
...
fn test_usage_percentage_normal() {
    let m = UsageMetric { used: 80, limit: 100 };
...
fn test_is_near_limit_below_80() {
    let m = UsageMetric { used: 79, limit: 100 };
    assert!(!is_near_limit(&m));
}
...
fn test_is_over_limit_below() {
    let m = UsageMetric { used: 99, limit: 100 };
    assert!(!is_over_limit(&m));
}
...
fn test_default_runtime_settings() {
    ...
    assert_eq!(
        settings.bootstrap_default_mode,
...
fn test_usage_history_window_display() {
    assert_eq!(UsageHistoryWindow::OneHour.to_string(), "1h");
    assert_eq!(UsageHistoryWindow::SevenDays.to_string(), "7d");
    assert_eq!(UsageHistoryWindow::ThirtyDays.to_string(), "30d");
}
...
fn test_usage_history_response_json() {
    ...
    assert_eq!(response.window, UsageHistoryWindow::OneDay);
    assert_eq!(response.granularity, UsageHistoryGranularity::Hourly);
...
fn test_is_interesting_result() {
    assert!(ace_sdk_core::utils::is_interesting("fn foo() -> Result<T, E>"));
}
...
fn test_detect_primary_language_empty() {
    ...
    assert_eq!(ace_sdk_core::services::detect_primary_language(&files), None);
}
...
fn test_error_from_http_response_insufficient_permissions() {
    ...
    let err = ace_sdk_core::errors::AceError::from_http_response(403, body);
    match err {
        ace_sdk_core::errors::AceError::InsufficientPermissions {
            required_role, ..
        } => {
