...
pub struct TrajectoryStep {
    ...
    /// Absolute Unix epoch ms (UTC) when this step started.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub start_ms: Option<i64>,
    /// Absolute Unix epoch ms (UTC) when this step ended.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub end_ms: Option<i64>,
    /// Ground-truth step duration in ms. May differ from `end_ms - start_ms`
    /// due to client clock skew.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub duration_ms: Option<i64>,
}
```

The three timing fields (`start_ms`, `end_ms`, `duration_ms`) are optional,
additive, and omitted from the JSON when `None`. Server-side validation is
lenient (Pydantic `extra='ignore'`), so writing traces from an older SDK
that does not populate them remains fully compatible.
...
pub struct TraceStep {
    ...
    /// Step duration in ms. `None` on responses from servers that have not
    /// yet adopted the matching read-side schema extension.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub duration_ms: Option<i64>,
}
```

`duration_ms` will be `None` on responses from servers that have not yet
adopted the matching read-side schema extension. In that case, compute the
duration on the client with `end_ms - start_ms`.
...
Use `client.get_last_usage().await` to access the most recent snapshot
(`None` until the first request returns an `X-ACE-Plan` header).

```rust
pub struct UsageInfo {
    pub plan: String,                          // "team/pro", "individual/free", ...
    pub subscription_type: SubscriptionType,   // Individual | Team
    pub plan_tier: PlanTier,                   // Free | Basic | Pro
    pub status: SubscriptionStatus,            // Active | Trialing | ReadOnly | Blocked
    pub patterns: UsageMetric,                 // per-project
    pub patterns_total: UsageMetric,           // org-wide
    pub projects: UsageMetric,
    pub domains: UsageMetric,
    pub templates: UsageMetric,
    pub api_calls: UsageMetric,                // billing-cycle counter
    pub traces_today: UsageMetric,             // resets daily UTC
    pub subscription_updated_at: Option<String>, // RFC3339
}
```
...
pub struct UsageMetric {
    pub used: u32,
    pub limit: i32,   // -1 == unparseable / unlimited sentinel
}
```

The parser short-circuits and returns `None` when `X-ACE-Plan` is
absent — responses without subscription headers leave `last_usage`
untouched (parity with TS, Kotlin, Go, Python).
