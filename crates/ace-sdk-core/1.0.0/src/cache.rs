...
impl GraphCache {
    ...
    // ── Server refresh hook (TODO: wire to CO_APPLIED graph endpoint) ─────────
    //
    // TODO(#104): When the server CO_APPLIED graph endpoint is available,
    // implement `refresh_from_server(&client, org_id, project_id)` here.
    //
    // Pseudo-implementation:
    //   1. GET /graph/co-applied?org_id=...&project_id=...
    //   2. For each node in response:
    //        self.upsert_pattern(node.id, node.payload_json, node.cumulative_reward)?;
    //   3. For each edge in response:
    //        self.upsert_edge(edge.src, edge.dst, edge.weight)?;
    //   4. self.prune()?;
    //
    // This is a clearly-marked refresh hook (CONTRACT §5c "if the edges endpoint
    // is not yet wired, leave a clearly-marked refresh hook + unit-test the
