...
| Search request | Optional `task_intent`, `exploration_enabled`, `exploration_rate`, and `session_id` body fields. |
...
| Graph cache | New `GraphCache` (SQLite, 7-day TTL, 2-hop neighbours, per-`(org,project)` isolation). Replaces the old 5-min KV cache. Same schema as all other language SDKs — local data source for ace-desktop Brain-Graphs. `refresh_from_server` populates edges from `GET /patterns/graph` (verified live: ~11.5k edges). |

## Features

- **ACE 1.5 Reward Model** — tier counters, `cumulative_v15_reward`, `PatternEffectiveness`, `is_at_risk()`
- **`search_patterns15`** — native 1.5 search with `match_factors` + top-level `retrieval_id` + optional `session_id`
...
- **SQLite Graph Cache** — `GraphCache` with 7-day TTL, flat 2-hop neighbours (p95 ~0.17 ms), per-project isolation; `refresh_from_server` pulls edge topology from `GET /patterns/graph`
