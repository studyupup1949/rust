...
impl AceClient {
    ...
    ///
    /// After a successful search, each returned pattern is upserted into the
    /// local `GraphCache` (7-day TTL). This wires the search read-path to the
    /// graph cache so patterns are available for 2-hop neighbourhood queries.
    ///
    ...
    pub async fn search_patterns15(
        ...
        let response: SearchResponse15 = self
            .request(
                "/patterns/search?include_metadata=true",
                reqwest::Method::POST,
                Some(&body),
        ...
        // ── Graph cache population (CONTRACT §5c "wire on search refresh") ────
        // Upsert every pattern into the local graph cache so it is available
        // for `get_pattern(id)` and `neighbors(id, hops, min_weight)`.
        // Failures are silently swallowed — cache miss is never fatal.
        if let Some(ref gc_mutex) = self.graph_cache {
            let now_ms = chrono::Utc::now().timestamp_millis();
            if let Ok(gc) = gc_mutex.lock() {
                for pattern in &response.similar_patterns {
                    // Serialize the pattern as the payload_json stored in the cache.
                    if let Ok(payload_json) = serde_json::to_string(pattern) {
                        let reward = pattern.cumulative_v15_reward.unwrap_or(0.0);
                        let _ = gc.upsert_pattern_at(&pattern.id, &payload_json, reward, now_ms);
                    }
                }
            }
        }
