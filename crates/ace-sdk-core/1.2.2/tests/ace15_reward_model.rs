...
// Helper: build a minimal Pattern with the given cumulative_v15_reward value.
fn make_pattern_with_reward(reward: Option<f64>) -> Pattern {
    let reward_json = match reward {
        Some(v) => format!("{}", v),
        None => "null".to_string(),
    };
    serde_json::from_str(&format!(
        r#"{{"id":"test","content":"x","section":"s","created_at":"2026-01-01T00:00:00Z","cumulative_v15_reward":{}}}"#,
        reward_json
    ))
    .expect("minimal Pattern must parse")
}
...
// =============================================================================
// (extra) is_at_risk() semantics: at-risk iff cumulative_v15_reward < 0.0
//
// Correct semantics (server-confirmed):
...
//   reward == None -> NOT at-risk (legacy row, no reward data)
// =============================================================================

#[test]
fn zero_reward_is_not_at_risk_uncredited_neutral() {
    // cumulative_v15_reward == 0.0 means uncredited/neutral -- NOT at-risk.
    let resp = load_fixture();
    ...
    assert!(
        !p.is_at_risk(),
        "reward == 0.0 is uncredited/neutral and must NOT be flagged at-risk"
    );
}

#[test]
fn negative_reward_is_at_risk() {
    // cumulative_v15_reward < 0.0 -> net-negative / harmful -> at-risk.
    let p = make_pattern_with_reward(Some(-0.5));
    assert!(p.is_at_risk(), "reward < 0.0 must be flagged at-risk");
}

#[test]
fn positive_reward_is_not_at_risk() {
    // pattern[0]: cumulative_v15_reward == 0.8
    let resp = load_fixture();
    let p = &resp.similar_patterns[0];
    assert_eq!(p.cumulative_v15_reward, Some(0.8));
    assert!(!p.is_at_risk(), "reward > 0.0 must NOT be flagged at-risk");
}

#[test]
fn nil_reward_is_not_at_risk_legacy() {
    // None reward (legacy row) -> NOT at-risk.
    let p = make_pattern_with_reward(None);
    assert!(
        !p.is_at_risk(),
        "None reward (legacy row) must NOT be flagged at-risk"
    );
}
