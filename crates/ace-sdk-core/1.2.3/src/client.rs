...
/// Structured dead-letter event emitted when POST /traces exhausts all retries.
#[derive(Debug, Clone, serde::Serialize)]
pub struct TraceDeadLetterEvent {
    /// Always `"ace_trace_dead_letter"`.
    pub event: &'static str,
    /// Total number of attempts made (initial + retries).
    pub attempts: u32,
    /// HTTP status of the last failure, or `null` for network errors.
    pub status: Option<u16>,
    /// Error message from the last failure.
    pub error: String,
    /// ISO-8601 timestamp of the dead-letter emission.
...
pub struct AceClientOptions {
    ...
    /// Optional callback invoked when POST /traces exhausts all retries.
    /// Receives the dead-letter event as a `serde_json::Value`.
    /// When absent, the event is written as a JSON line to stderr.
    pub on_dead_letter: Option<Box<dyn Fn(serde_json::Value) + Send + Sync>>,
}

impl std::fmt::Debug for AceClientOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AceClientOptions")
            .field("auto_refresh", &self.auto_refresh)
            .field("custom_headers", &self.custom_headers)
            .field("graph_edges_throttle_ms", &self.graph_edges_throttle_ms)
...
pub struct AceClient {
    ...
    /// Optional callback for structured dead-letter events on POST /traces exhaustion.
    on_dead_letter: Option<Arc<dyn Fn(serde_json::Value) + Send + Sync>>,
}

/// Maximum number of retries for POST /traces (transient errors only).
const TRACE_MAX_RETRIES: u32 = 2;
/// Base backoff in milliseconds (attempt 1 ~250ms, attempt 2 ~750ms).
const TRACE_BASE_BACKOFF_MS: u64 = 250;
...
impl AceClient {
    ...
    pub fn new(config: AceConfig, options: AceClientOptions) -> Result<Self, AceError> {
        ...
        let on_dead_letter: Option<Arc<dyn Fn(serde_json::Value) + Send + Sync>> =
            options.on_dead_letter.map(|f| {
                let arc: Arc<dyn Fn(serde_json::Value) + Send + Sync> = Arc::from(f);
                arc
            });
    ...
    /// Returns true when an HTTP status code is transient and should be retried.
    fn is_trace_retryable_status(status: u16) -> bool {
        matches!(status, 502..=504)
    }

    /// Returns true when an `AceError` is transient and warrants a retry.
    fn is_trace_retryable_error(err: &AceError) -> bool {
        match err {
            AceError::Http { status, .. } => Self::is_trace_retryable_status(*status),
            AceError::Network(_) => true,
            _ => false,
        }
    }

    /// Deterministic jitter backoff: attempt 1 → ~250ms, attempt 2 → ~750ms.
    /// `attempt` is 1-based (first retry = attempt 1).
    fn trace_backoff_ms(attempt: u32) -> u64 {
        // base * 3^(attempt-1) + small deterministic jitter derived from attempt
        let base = TRACE_BASE_BACKOFF_MS * (3u64.pow(attempt - 1));
        let jitter = (attempt as u64 * 37) % 50; // deterministic, 0–49 ms
        base + jitter
    }

    /// Emit a dead-letter event to the registered callback or stderr.
    fn emit_trace_dead_letter(&self, attempts: u32, status: Option<u16>, error: &str) {
        let ev = serde_json::json!({
            "event": "ace_trace_dead_letter",
            "attempts": attempts,
            "status": status,
            "error": error,
    ...
    /// Execute a single POST /traces HTTP call and return the raw result.
    /// Handles graceful 429 (quota_exceeded) → Ok(quota_exceeded:true).
    async fn post_traces_once(
        &self,
        body: &serde_json::Value,
    ) -> Result<LearningResponse, AceError> {
        let url = format!("{}/traces", self.config.server_url);
        let headers = self.build_headers()?;
        ...
        if status >= 400 {
            ...
            // Graceful quota handling — return Ok(quota_exceeded:true) rather than Err.
            if status == 429 {
        ...
        Ok(response.json::<LearningResponse>().await?)
    }

    /// Store execution trace for server-side analysis (with bounded retry).
    ///
    /// Retries at most `TRACE_MAX_RETRIES` times (total attempts = 3) on
    /// transient failures: HTTP 502/503/504 and network/connection errors.
    /// Never retries on 4xx or `QuotaExceeded`.
    /// On final exhaustion, emits a structured dead-letter event.
    pub async fn store_execution_trace(
        &self,
        trace: &ExecutionTrace,
    ) -> Result<LearningResponse, AceError> {
        let body = serde_json::to_value(trace)?;
        let mut last_err: Option<AceError> = None;

        for attempt in 0..=(TRACE_MAX_RETRIES) {
            if attempt > 0 {
                let backoff = Self::trace_backoff_ms(attempt);
                tokio::time::sleep(std::time::Duration::from_millis(backoff)).await;
            }

            match self.post_traces_once(&body).await {
                Ok(resp) => return Ok(resp),
                Err(err) if Self::is_trace_retryable_error(&err) => {
                    self.logger.debug(&format!(
                        "POST /traces attempt {} failed (retryable): {}",
                        attempt + 1,
                        err
                    ));
                    last_err = Some(err);
        ...
        // All retries exhausted — emit dead-letter event.
        let err = last_err.unwrap();
        let status = match &err {
            AceError::Http { status, .. } => Some(*status),
            _ => None,
        };
        let total_attempts = TRACE_MAX_RETRIES + 1;
        self.emit_trace_dead_letter(total_attempts, status, &err.to_string());
        Err(err)
    }
    ...
    /// Execute a single POST /traces/stream HTTP call.
    /// Returns `Ok(response)` on success, `Err` on any failure.
    async fn post_traces_stream_once(
        &self,
        body: &serde_json::Value,
    ) -> Result<reqwest::Response, AceError> {
    ...
    /// Retries at most `TRACE_MAX_RETRIES` times on transient errors (502/503/504/network)
    /// before falling back to `store_execution_trace` (when `fallback_on_error` is true).
    pub async fn store_execution_trace_stream(
        ...
        for attempt in 0..=(TRACE_MAX_RETRIES) {
            ...
            let response = match self.post_traces_stream_once(&body).await {
                Ok(r) => r,
                Err(e) => {
                    if Self::is_trace_retryable_error(&e) {
                        last_err = Some(e);
                        continue;
                    }
            ...
            if status >= 400 {
                ...
                let err = AceError::from_http_response(status, &body_text);
                if Self::is_trace_retryable_status(status) {
                    last_err = Some(err);
                    continue;
                }
            ...
            return Ok(last_response.unwrap_or(LearningResponse {
                stored: true,
        ...
        if fallback_on_error {
            self.logger
                .info("SSE stream retries exhausted, falling back to /traces endpoint");
            return self.store_execution_trace(trace).await;
        }
        Err(err)
    }
