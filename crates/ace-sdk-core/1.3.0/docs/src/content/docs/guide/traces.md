...
### Using `anchor_trace` instead of manual wiring

`anchor_trace` (module-level or as a method on `TaskSession`) is the
**recommended** way to attach F-080 fields to a trace. It reads the pinned
`retrieval_id` and `retrieval_log_ids` from the file-based anchor store, unions
them (accumulating across multiple `pin_search` calls and across processes), and
reaps the pin files atomically.
...
// ── In process A (SubagentStart hook) ─────────────────────────────────────
...
ts.pin_search(&response); // persists anchor to disk; safe to call from any process

// ── In process B (SubagentStop hook) ──────────────────────────────────────
// Module-level anchor_trace: derives session_id from trace.session_id.
use ace_sdk_core::anchor_trace;
...
// anchor_trace fills retrieval_id + applied_log_ids and reaps the pin files.
let trace = anchor_trace("org_123", "prj_abc", trace, None);
client.store_trace(&trace).await?;
```

See the [ACE Client guide — TaskSession section](/ace-sdk/rust/core/guide/ace-client/#tasksession-f-080-helper)
for the full API including injected session IDs and non-reaping `read_f080`.
