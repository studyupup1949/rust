...
impl LocalCacheService {
    ...
    /// Return the current busy_timeout in ms (test helper).
    pub fn get_busy_timeout(&self) -> Option<i64> {
        self.db
            .lock()
            .ok()
            .and_then(|db| db.query_row("PRAGMA busy_timeout", [], |r| r.get(0)).ok())
    }
...
impl ProjectIndex {
    ...
    pub fn new(config: ProjectIndexConfig) -> Result<Self, rusqlite::Error> {
        ...
        // Set busy_timeout BEFORE journal_mode=WAL so the WAL pragma itself can
        // wait/retry on lock contention instead of failing with SQLITE_BUSY immediately.
...
impl GraphCache {
    ...
    pub fn new_with_ttl(
        ...
        // wait/retry on lock contention instead of failing with SQLITE_BUSY immediately.
        conn.pragma_update(None, "busy_timeout", 5000i32)?;
        // WAL mode (must be set before schema creation for full effect)
        conn.pragma_update(None, "journal_mode", "WAL")?;
    ...
    /// Return the current busy_timeout in ms (test helper for busy-timeout verification).
    pub fn get_busy_timeout(&self) -> Result<i64, rusqlite::Error> {
        let db = self.lock()?;
...
mod tests {
    ...
    // =========================================================================
    // busy_timeout hardening tests
    // =========================================================================

    #[test]
    fn local_cache_busy_timeout_is_5000() {
        let tmp = TempDir::new().unwrap();
        let cache =
            LocalCacheService::new("org", "proj", 5, Some(tmp.path().to_path_buf())).unwrap();
        let timeout = cache.get_busy_timeout().unwrap();
        assert_eq!(
            timeout, 5000,
            "LocalCacheService busy_timeout must be 5000 ms"
        );
    }

    #[test]
    fn session_storage_busy_timeout_is_5000() {
        let tmp = TempDir::new().unwrap();
        let storage = SessionStorage::new(Some(SessionStorageConfig {
            cache_dir: Some(tmp.path().to_path_buf()),
        }))
        .unwrap();
        let timeout = storage.get_busy_timeout().unwrap();
        assert_eq!(timeout, 5000, "SessionStorage busy_timeout must be 5000 ms");
    }

    #[test]
    fn project_index_busy_timeout_is_5000() {
        let tmp = TempDir::new().unwrap();
        let idx = ProjectIndex::new(ProjectIndexConfig {
            org_id: "org".to_string(),
            project_id: "proj".to_string(),
        ...
        .unwrap();
        let timeout = idx.get_busy_timeout().unwrap();
        assert_eq!(timeout, 5000, "ProjectIndex busy_timeout must be 5000 ms");
    }

    #[test]
    fn graph_cache_busy_timeout_is_5000() {
        let tmp = TempDir::new().unwrap();
        let gc = GraphCache::new("org", "proj", Some(tmp.path().to_path_buf())).unwrap();
        let timeout = gc.get_busy_timeout().unwrap();
        assert_eq!(timeout, 5000, "GraphCache busy_timeout must be 5000 ms");
    }
}
