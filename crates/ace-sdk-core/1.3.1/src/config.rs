            ...
            Ok(contents) => match serde_json::from_str::<serde_json::Value>(&contents) {
                Ok(file_config) => {
                    ...
                    if let Some(v) = file_config.get("verbosity").and_then(|v| v.as_str()) {
                        config.verbosity =
                            serde_json::from_value(serde_json::Value::String(v.to_lowercase()))
                                .ok();
                    }
                    ...
                    if let Some(org_id) = file_config.get("default_org_id").and_then(|v| v.as_str())
                    {
                        config.default_org_id = Some(org_id.to_string());
                    }
                    if let Some(device_id) = file_config.get("device_id").and_then(|v| v.as_str()) {
...
pub fn list_organizations(config: &crate::types::AceConfig) -> Vec<(String, String, usize)> {
    ...
    if let Some(ref org_map) = config.orgs {
        ...
        let org_id =
            extract_org_id_from_token(&config.api_token).unwrap_or_else(|| "default".to_string());
...
pub fn is_multi_org_mode(config: &crate::types::AceConfig) -> bool {
    config.orgs.as_ref().map(|o| !o.is_empty()).unwrap_or(false)
}
...
pub fn validate_config(config: &crate::types::AceConfig) -> Vec<String> {
    ...
    let has_multi_org = config.orgs.as_ref().map(|o| !o.is_empty()).unwrap_or(false);
    let has_single_org = !config.api_token.is_empty();

    if !has_multi_org && !has_single_org && config.auth.is_none() {
        errors.push("No API token configured (neither multi-org nor single-org mode)".to_string());
    }
...
pub fn config_to_context(config: &crate::types::AceConfig) -> crate::types::AceContext {
    let org_id = config.default_org_id.clone().or_else(|| {
        config
            .auth
            .as_ref()
...
pub fn save_auth_credentials(
    ...
    auth_obj.insert(
        "organizations".into(),
        serde_json::json!(auth.organizations),
    );
    auth_obj.insert(
        "authenticated_at".into(),
        serde_json::json!(auth.authenticated_at.as_deref().unwrap_or("unknown")),
    );
...
pub fn set_default_org(config_path: &std::path::Path, org_id: &str) -> Result<(), AceError> {
    ensure_config_directory(config_path)?;
    ...
    if let Some(auth) = config.get("auth") {
        if let Some(orgs) = auth.get("organizations").and_then(|v| v.as_array()) {
            let org_exists = orgs
                .iter()
                .any(|org| org.get("org_id").and_then(|v| v.as_str()) == Some(org_id));
            if !org_exists {
...
fn migrate_config_impl(from: &std::path::Path, to: &std::path::Path) -> Result<(), AceError> {
...
fn generate_device_id() -> String {
    ...
    let encoded: String = raw.chars().take(22).collect();
    format!("dev_{}", encoded)
}
...
fn write_config_json(
    ...
    let contents = serde_json::to_string_pretty(config)
        .map_err(|e| AceError::Config(format!("Failed to serialize config: {}", e)))?;
    fs::write(config_path, contents)
        .map_err(|e| AceError::Config(format!("Failed to write config file: {}", e)))
}
...
mod tests {
    ...
    // between parallel test threads (ACE_ORG_ID, ACE_PROJECT_ID, ACE_SERVER_URL,
    // ACE_API_TOKEN, ACE_CONFIG_PATH, ACE_TEST_EXPAND, ACE_TEST_BRACED,
    // ACE_TEST_MISSING_VAR).
    ...
    fn test_save_auth_credentials() {
        ...
        assert_eq!(
            config["auth"]["absolute_expires_at"],
