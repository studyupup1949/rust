    ...
    thread::spawn(move || -> io::Result<()> {
      loop {
        ...
        for (id, datagram) in c2_pending_in_datagrams.lock().clone().into_iter() {
          println!("Wating for datagram {:?}. Total: {}, Current: {}", id, datagram.segments_count, datagram.segments.len());
    ...
    thread::spawn(move || -> io::Result<()> {
      loop {
        ...
        for (id, datagram) in c2_pending_out_datagrams.lock().clone().into_iter() {
          println!("Sent datagram {:?}, Segments: {}, Acks: {}", id, datagram.segments_count, datagram.segments_acks.len());
          let start_time = datagram.last_active.time();
          let end_time = Utc::now().time();
          let diff = end_time - start_time;
