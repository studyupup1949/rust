  ...
  pub fn new(address: SocketAddr) -> io::Result<AckUdp> {
    ...
    thread::spawn(move || -> io::Result<()> {
      loop {
        ...
        // println!("In datagrams: {}", c2_pending_in_datagrams.lock().len());
        for (id, datagram) in c2_pending_in_datagrams.lock().clone().into_iter() {
          let start_time = datagram.last_active.time();
          let end_time = Utc::now().time();
          let diff = end_time - start_time;
          // println!("Wating for datagram {:?}. Total: {}, Current: {}, Diff time: {}", id, datagram.segments_count, datagram.segments.len(), diff.num_seconds());
    ...
    thread::spawn(move || -> io::Result<()> {
      loop {
        ...
        // println!("Out datagrams: {}", c2_pending_out_datagrams.lock().len());
        for (id, datagram) in c2_pending_out_datagrams.lock().clone().into_iter() {
          let start_time = datagram.last_active.time();
          let end_time = Utc::now().time();
          let diff = end_time - start_time;
          
          // println!("Sent datagram {:?}, Segments: {}, Acks: {}, Diff time: {}", id, datagram.segments_count, datagram.segments_acks.len(), diff.num_seconds());
          if diff.num_seconds() >= 5 {
            if datagram.checks_failure_count < 3 {
              ...
              thread::spawn(move || {
                ...
                // println!("Resending packets: {}", non_ack_segments.len());
                for packet in non_ack_segments {
                  // println!("resending packet: {}", packet.seg_index);
