    ...
    thread::spawn(move || -> io::Result<()> {
      loop {
        ...
        for (id, datagram) in datagrams.into_iter() {
          ...
          println!("failures: {}, sent: {}, total_segments: {}, acks: {}", datagram.checks_failure_count, datagram.is_fully_sent, datagram.segments_count, datagram.segments_acks.lock().len());
          if diff.num_seconds() >= 1 && datagram.is_fully_sent {
            if datagram.checks_failure_count < 1122 {
        ...
        if packet.total_segments > 1 && packet.ack == 0 {
          if cc_pending_in_datagrams.lock().contains_key(&packet.datagram_id) {
            ...
            if packet.total_segments > 100 {
              if got_segments % 100 == 0 || got_segments > (datagram.segments_count - 100) as usize {
                let last_packets = {
