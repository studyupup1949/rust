use acktor::{ActorId, Address};

use super::Session;

/// A session reference which can be used to identify a session in a node by either its address,
/// index, or label.
#[derive(Debug, Clone)]
pub enum SessionRef {
    Address(Address<Session>),
    Index(ActorId),
    Label(String),
}

impl From<Address<Session>> for SessionRef {
    fn from(addr: Address<Session>) -> Self {
        Self::Address(addr)
    }
}

impl From<ActorId> for SessionRef {
    fn from(index: ActorId) -> Self {
        Self::Index(index)
    }
}

impl From<String> for SessionRef {
    fn from(label: String) -> Self {
        Self::Label(label)
    }
}

impl From<&str> for SessionRef {
    fn from(label: &str) -> Self {
        Self::Label(label.to_string())
    }
}
