...
pub struct AttributePermission {
    /// The permission guarded by attribute predicates.
    pub permission: AtomicPermission,
    /// Required attribute key-value pairs that must match the context.
...
pub struct AttributeRule {
    /// Permissions granted when their attribute predicates match.
    pub grants: Vec<AttributePermission>,
    /// Permissions denied when their attribute predicates match.
