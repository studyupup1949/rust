...
pub struct Role {
    /// Name of this role.
    pub name: String,
    /// Grants and denials directly assigned to this role.
...
pub enum RbacError {
    ...
    CyclicDependency {
        ...
        role: String,
        /// Chain of roles forming the cycle.
        chain: Vec<String>,
    },
    /// Role hierarchy exceeds maximum depth.
    MaxDepthExceeded {
        /// Role that exceeded the depth limit.
        role: String,
        /// Depth reached before the limit was hit.
        depth: usize,
    },
}
...
impl RbacPolicy {
    ...
    fn clear_cache(&self) {
        match self.cache.write() {
            ...
            Err(e) => {
                log::warn!("RBAC cache lock was poisoned, recovering");
                e.into_inner().clear();
            }
        }
    }
    ...
    pub fn resolve_permissions(&self, role_names: &[String]) -> Result<GrantDenialPair, RbacError> {
        // Try cache first (read lock)
        {
            let cache = match self.cache.read() {
                Ok(c) => c,
                Err(e) => {
                    log::warn!("RBAC cache read lock was poisoned, recovering");
                    e.into_inner()
                }
            };
        ...
        match self.cache.write() {
            ...
            Err(e) => {
                log::warn!("RBAC cache write lock was poisoned, recovering");
