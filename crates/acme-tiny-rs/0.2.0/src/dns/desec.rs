...
impl DnsProvider for DesecDns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        let resp = self
            .client
            .post(format!("https://desec.io/api/v1/domains/{domain}/rrsets/"))
            .header("Authorization", format!("Token {}", self.token))
            .header("Content-Type", "application/json")
            .json(&body)
            .send()?;
        if !resp.status().is_success() {
            bail!("deSEC error: {}", resp.text()?);
        }
        Ok(())
    }
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        let resp = self
            .client
            .get(format!(
                "https://desec.io/api/v1/domains/{domain}/rrsets/{sub}/TXT/"
            ))
            .header("Authorization", format!("Token {}", self.token))
            .send()?;
        if resp.status().is_success() {
