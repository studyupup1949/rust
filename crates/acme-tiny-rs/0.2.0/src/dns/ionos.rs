...
impl IonosDns {
    pub fn new() -> Result<Self> {
        Ok(Self {
            key: format!(
                "{}.{}",
                env::var("IONOS_PREFIX")?,
                env::var("IONOS_SECRET")?
            ),
...
impl DnsProvider for IonosDns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        let r: serde_json::Value = self
            .client
            .get(format!("https://api.hosting.ionos.com/dns/v1/zones/{zid}"))
            .header("X-API-Key", &self.key)
            .send()?
            .json()?;
        ...
        self.client
            .put(format!("https://api.hosting.ionos.com/dns/v1/zones/{zid}"))
            .header("X-API-Key", &self.key)
    ...
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        let r: serde_json::Value = self
            ...
            .send()?
            .json()?;
        let records: Vec<serde_json::Value> = r["records"].as_array().map_or(vec![], |a| {
            a.iter()
                .filter(|r| {
