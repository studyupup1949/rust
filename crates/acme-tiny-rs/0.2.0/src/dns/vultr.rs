...
impl VultrDns {
    ...
    fn get_domain(&self, domain: &str) -> Result<String> {
        ...
        for i in 1..parts.len() {
            ...
            if r["domains"]
                .as_array()
                .is_some_and(|a| a.iter().any(|d| d["domain"].as_str() == Some(&root)))
            {
                return Ok(root);
            }
        }
...
impl DnsProvider for VultrDns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        self.client
            .post(format!("https://api.vultr.com/v2/domains/{root}/records"))
            .header("Authorization", format!("Bearer {}", self.key))
    ...
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        let r: serde_json::Value = self
            .client
            .get(format!("https://api.vultr.com/v2/domains/{root}/records"))
            .header("Authorization", format!("Bearer {}", self.key))
            .send()?
            .json()?;
        if let Some(records) = r["records"].as_array() {
            for rec in records {
                if rec["name"].as_str() == Some("_acme-challenge")
                    && rec["type"].as_str() == Some("TXT")
                {
                    if let Some(id) = rec["id"].as_str() {
                        let _ = self
                            .client
                            .delete(format!(
                                "https://api.vultr.com/v2/domains/{root}/records/{id}"
                            ))
                            .header("Authorization", format!("Bearer {}", self.key))
                            .send();
                    }
