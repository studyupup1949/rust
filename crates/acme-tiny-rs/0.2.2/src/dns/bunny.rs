...
impl DnsProvider for BunnyDns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        let resp = self
            .client
            .put(format!("https://api.bunny.net/dnszone/{domain}/records"))
    ...
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        let r: serde_json::Value = self
            .client
            .get(format!("https://api.bunny.net/dnszone/{domain}"))
            .header("AccessKey", &self.key)
            .send()?
            .json()?;
        if let Some(records) = r["Records"].as_array() {
            for rec in records {
                if rec["Name"].as_str() == Some("_acme-challenge")
                    && rec["Type"].as_u64() == Some(0)
                {
                    if let Some(id) = rec["Id"].as_u64() {
                        let _ = self
                            .client
                            .delete(format!(
                                "https://api.bunny.net/dnszone/{domain}/records/{id}"
                            ))
                            .header("AccessKey", &self.key)
                            .send();
                    }
