...
impl LinodeV4Dns {
    pub fn new() -> Result<Self> {
        Ok(Self {
            token: env::var("LINODE_V4_API_KEY")?,
...
impl DnsProvider for LinodeV4Dns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        self.client
            .post(format!("{LINODE_V4_API}/{domain_id}/records"))
            .header("Authorization", self.headers())
    ...
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        let resp: serde_json::Value = self
            .client
            .get(format!("{LINODE_V4_API}/{domain_id}/records"))
            .header("Authorization", self.headers())
            .send()?
            .json()?;
            ...
            for r in records {
                if r["name"].as_str() == Some(&sub) && r["type"].as_str() == Some("TXT") {
                    if let Some(id) = r["id"].as_u64() {
                        let _ = self
                            .client
                            .delete(format!("{LINODE_V4_API}/{domain_id}/records/{id}"))
                            .header("Authorization", self.headers())
                            .send();
                    }
                }
...
impl LinodeV3Dns {
    pub fn new() -> Result<Self> {
        Ok(Self {
            api_key: env::var("LINODE_API_KEY")
                .map_err(|_| anyhow!("LINODE_API_KEY required (v3 deprecated, prefer v4)"))?,
            client: reqwest::blocking::Client::new(),
        })
    }
    fn url(&self, action: &str, params: &str) -> String {
        format!(
            "{LINODE_V3_API}?api_key={}&api_action={action}{params}",
            self.api_key
        )
    }
    fn get_domain_id(&self, domain: &str) -> Result<(u64, String)> {
...
impl DnsProvider for LinodeV3Dns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        let _ = self
            .client
            .get(self.url("domain.resource.create", &params))
            .send()?;
        Ok(())
    }
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        let resp: serde_json::Value = self
            .client
            .get(self.url("domain.resource.list", &params))
            .send()?
            .json()?;
        if let Some(data) = resp["DATA"].as_array() {
            for r in data {
                if r["NAME"].as_str() == Some(&sub) && r["TYPE"].as_str() == Some("TXT") {
                    if let Some(rid) = r["RESOURCEID"].as_u64() {
                        ...
                        let _ = self
                            .client
                            .get(self.url("domain.resource.delete", &dp))
                            .send();
                    }
                }
            }
