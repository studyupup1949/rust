...
impl NetlifyDns {
    pub fn new() -> Result<Self> {
        Ok(Self {
            token: env::var("NETLIFY_ACCESS_TOKEN")?,
    ...
    fn get_record_id(&self, zone_id: &str, hostname: &str) -> Result<Option<String>> {
        ...
        Ok(records
            .iter()
            .find(|r| r["hostname"].as_str() == Some(hostname) && r["type"].as_str() == Some("TXT"))
...
impl DnsProvider for NetlifyDns {
    fn present(&self, domain: &str, value: &str) -> Result<()> {
        ...
        let resp = self
            ...
            .send()?;
        if !resp.status().is_success() {
            bail!("Netlify error: {}", resp.text().unwrap_or_default());
        }
        Ok(())
    }
    fn cleanup(&self, domain: &str, _value: &str) -> Result<()> {
        ...
        if let Ok(Some(id)) = self.get_record_id(&zone_id, &hostname) {
            let _ = self
                .client
                .delete(format!(
                    "https://api.netlify.com/api/v1/dns_zones/{zone_id}/dns_records/{id}"
                ))
                .header("Authorization", format!("Bearer {}", self.token))
                .send();
        }
