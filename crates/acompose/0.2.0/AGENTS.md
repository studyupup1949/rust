...
1. Spawns persistent `kimi acp` agents in arbitrary working directories.
...
- `create_session(name, cwd, charter, allowed_tool_kinds = [])` — create a sub-agent.
...
Note: whether the new `kimi acp` process can restore session history is an implementation detail of `kimi acp`. acompose itself only persists the name-to-id mapping.

## Tool / permission sandboxing

- `SessionConfig.allowed_tool_kinds` enforces an allowlist for permission requests.
- `kimi acp` does **not** ask permission before executing `Bash`/`Execute` tools; it only reports them as `SessionUpdate` notifications. Real isolation requires OS-level sandboxing.
