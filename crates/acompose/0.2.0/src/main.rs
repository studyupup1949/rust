...
async fn main() -> anyhow::Result<()> {
    ...
    let proxy_handle = if config.acp_proxy.enabled {
        let bind_address = config
            .acp_proxy
            .bind_address
            .parse()
            .context("parsing acp_proxy bind address")?;
        let proxy = Server::new(Arc::clone(&compositor), bind_address);
        let proxy_ct = main_cancel.child_token();
        ...
        let handle = tokio::spawn(async move {
            ...
            tokio::select! {
                ...
                () = proxy_ct_clone.cancelled_owned() => {
                    info!("Compose server cancelled");
                }
            }
            Ok::<(), anyhow::Error>(())
        });
        Some((handle, proxy_ct))
    } else {
        None
    };
    ...
    if let Some((handle, ct)) = proxy_handle {
        ct.cancel();
        if let Err(e) = handle.await {
