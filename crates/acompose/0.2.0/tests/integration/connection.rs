...
use crate::common::mocks::{MockSessionFactory, run_proxy_client};
...
async fn test_initialize() {
    ...
    let client = run_proxy_client(compositor).await;
...
async fn test_new_session() {
    let factory = MockSessionFactory::new();
    let compositor = test_compositor(factory.clone());
    let client = run_proxy_client(compositor.clone()).await;
    client.initialize().await.unwrap();
...
async fn test_load_session_replays_history() {
    ...
    let first = run_proxy_client(compositor.clone()).await;
    ...
    let second = run_proxy_client(compositor).await;
...
async fn test_prompt_suppressed_for_sender() {
    ...
    let sender = run_proxy_client(compositor.clone()).await;
    let observer = run_proxy_client(compositor.clone()).await;
...
async fn test_cancel_is_forwarded_to_agent() {
    ...
    let sender = run_proxy_client(compositor.clone()).await;
    let canceler = run_proxy_client(compositor.clone()).await;
    sender.initialize().await.unwrap();
...
async fn test_two_clients_receive_same_live_update() {
    ...
    let first = run_proxy_client(compositor.clone()).await;
    let second = run_proxy_client(compositor.clone()).await;
    first.initialize().await.unwrap();
    second.initialize().await.unwrap();
