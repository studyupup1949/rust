...
Spawn persistent acp agents, keep them alive, and expose them through an MCP server.
...
| `model` | Optional model to select via ACP session config options (e.g. `kimi-code/k3`). Must be one of the values offered by the agent's `model` config option; applied on create, recreate and load |
...
- `create_session` — spawn a new agent (accepts an optional `model` to select via ACP config options)
