...
impl SessionActor {
        ...
        T: agent_client_protocol::ConnectTo<Client> + Send + 'static,
    {
        ...
        let (ready_tx, ready_rx) = oneshot::channel::<anyhow::Result<Self>>();
        ...
        let client = Client
            ...
            agent_client_protocol::on_receive_request!(),
                ...
                // Run session setup in an inner block so that setup errors
                // (e.g. an unavailable model) are delivered to the caller
                // through the ready channel instead of being lost when the
                ...
                let setup = async {
                    ...
                    let (session_id, config_options) = if let Some(load_id) = load_session_id {
                        debug!(%name, %load_id, "loading session");
                        let loaded = connection.send_request(
                        ...
                        (session_id, new_session.config_options)
                    };
                    ...
                    Ok::<_, anyhow::Error>((actor, connection_cancel))
                };

                match setup.await {
                    Ok((actor, connection_cancel)) => {
                        let _ = ready_tx.send(Ok(actor));
                        // Keep the ACP connection handler alive until the actor
    ...
    async fn handle_recreate(
        ...
        // The recreated session starts with default config options, so the
        // configured model has to be selected again.
        if let Some(model) = self.model.clone()
            && let Err(e) = apply_model_option(
                &self.name,
                &self.connection,
                &self.session_id,
            ...
            .await
        {
            let _ = respond_to.send(Err(e.context("failed to apply model after recreate")));
            return;
        }
...
/// Select `model` on the ACP session via the agent's `model` config option
/// (`session/set_config_option`).
///
/// The model option is located by `category: "model"`, falling back to the
/// option id `model`. If the agent does not report config options, a warning
/// is logged and the agent's default model is kept. If the agent reports the
/// option but does not offer the requested value, an error listing the
/// available values is returned.
async fn apply_model_option(
    name: &str,
    connection: &ConnectionTo<Agent>,
    session_id: &str,
    ...
    let options = config_options.unwrap_or(&[]);
    ...
    let Some(option) = option else {
        warn!(
            %name,
            %model,
            "agent did not report a model config option; keeping the default model"
        );
        return Ok(());
    };
    let SessionConfigKind::Select(select) = &option.kind else {
        warn!(
            %name,
            %model,
            "model config option is not a select; keeping the default model"
        );
        return Ok(());
    };
    if select.current_value.0.as_ref() == model {
        info!(%name, %model, "model already selected");
        return Ok(());
    }

    let available: Vec<&str> = match &select.options {
    ...
    if !available.is_empty() && !available.contains(&model) {
        anyhow::bail!(
            "model '{}' is not offered by the agent (available: {})",
            model,
            available.join(", ")
        );
    }
    ...
    connection
        ...
        .block_task()
        .await
        .map_err(|e| anyhow::anyhow!("failed to set model '{}': {}", model, e))?;
    info!(%name, %model, "model selected");
    Ok(())
}
