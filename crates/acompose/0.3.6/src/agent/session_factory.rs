...
///
/// Production implementations spawn real `acp_command` processes; test
...
/// Production session factory that spawns `acp_command` processes.
#[derive(Debug, Clone)]
...
impl StdioFactory {
    #[must_use]
    pub fn new(acp_command: impl Into<String>) -> Self {
        Self {
            acp_command: acp_command.into(),
        }
    }

    /// Spawn a `acp_command` process and connect a session actor to it.
    ...
    pub async fn spawn(
        ...
        let words = shell_words::split(acp_command)?;
        let Some((cmd, args)) = words.split_first() else {
            anyhow::bail!("empty acp_command");
        };
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| anyhow::anyhow!("failed to spawn '{}': {}", acp_command, e))?;
