...
pub struct Config {
    /// Agent acp command. Defaults to "kimi acp" (resolved via PATH).
...
pub struct SessionConfig {
    ...
    /// Model to select for this session via ACP session config options.
    /// Must be one of the values offered by the agent's `model` config option.
    /// If unset, the agent's default model is used.
    #[serde(default)]
