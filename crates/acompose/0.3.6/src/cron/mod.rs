...
pub fn next_run_utc(cron: &Cron, timezone: Tz, after: DateTime<Utc>) -> Result<DateTime<Utc>> {
    ...
    // croner preserves the nanoseconds of the input `after` time when it snaps
    // to the next cron boundary. Truncate them so stored `next_run_at` values
    // land exactly on the scheduled minute/second.
    let truncated = next_in_tz
        .with_nanosecond(0)
        .ok_or_else(|| anyhow::anyhow!("failed to truncate nanoseconds in next run"))?;
    Ok(truncated.with_timezone(&Utc))
}
