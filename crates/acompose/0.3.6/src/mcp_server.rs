...
pub struct CreateSessionRequest {
    ...
    pub charter: String,
    /// Model to select for the session via ACP session config options.
    /// Must be one of the values offered by the agent's `model` config option.
    #[serde(default)]
...
impl ComposeMcpServer {
    ...
    async fn create_session(
        ...
        info!(name = %req.name, cwd = %req.cwd, model = ?req.model, ?req.allowed_tool_kinds, "handling create_session");
...
impl ServerHandler for ComposeMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_instructions("Compose compositor MCP server. Manage persistent ACP sessions.")
    }
}
