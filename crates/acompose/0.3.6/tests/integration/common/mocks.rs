...
/// Default model reported by the mock agent's `model` config option.
pub const MOCK_DEFAULT_MODEL: &str = "mock-model-a";
/// Alternative model offered by the mock agent's `model` config option.
pub const MOCK_OTHER_MODEL: &str = "mock-model-b";

/// Build the mock agent's session config options: a single `model` select
/// offering [`MOCK_DEFAULT_MODEL`] and [`MOCK_OTHER_MODEL`].
fn mock_config_options(current: &str) -> Vec<SessionConfigOption> {
    vec![
        SessionConfigOption::select(
            ...
            current.to_string(),
            vec![
                SessionConfigSelectOption::new(MOCK_DEFAULT_MODEL, "Mock Model A"),
                SessionConfigSelectOption::new(MOCK_OTHER_MODEL, "Mock Model B"),
            ],
        )
...
pub struct InMemoryAgent {
    ...
    recorded_model_changes: Arc<Mutex<Vec<String>>>,
...
impl InMemoryAgent {
    pub fn with_response_delay(session_id: impl Into<String>, delay: std::time::Duration) -> Self {
        ...
        Self {
            ...
            current_model: Arc::new(Mutex::new(MOCK_DEFAULT_MODEL.to_string())),
            recorded_model_changes: Arc::new(Mutex::new(Vec::new())),
            live_tx,
    ...
    /// Return the model values passed to `session/set_config_option`, in order.
    #[must_use]
    pub fn recorded_model_changes(&self) -> Vec<String> {
        self.recorded_model_changes.lock().unwrap().clone()
    }

    /// Return the mock agent's currently selected model.
    #[must_use]
    ...
    pub async fn run(self, channel: agent_client_protocol::Channel) {
        ...
        let current_model_for_new = current_model.clone();
        let current_model_for_load = current_model.clone();
        let recorded_model_changes = self.recorded_model_changes.clone();
        ...
        let result = Agent
            ...
            .on_receive_request(
                async move |_req: NewSessionRequest, responder, _cx| {
                    ...
                    *session_id.lock().unwrap() = new_session_id.clone();
                    ...
                    // A fresh ACP session starts with default config options.
                    *current_model_for_new.lock().unwrap() = MOCK_DEFAULT_MODEL.to_string();
                    let resp = NewSessionResponse::new(SessionId::new(new_session_id))
                        .config_options(mock_config_options(MOCK_DEFAULT_MODEL));
                    responder.respond(resp)
                },
                agent_client_protocol::on_receive_request!(),
            )
            .on_receive_request(
                async move |_req: LoadSessionRequest, responder, _cx| {
                    let current = current_model_for_load.lock().unwrap().clone();
                    let resp =
                        LoadSessionResponse::new().config_options(mock_config_options(&current));
                    responder.respond(resp)
                },
                agent_client_protocol::on_receive_request!(),
            )
            .on_receive_request(
                async move |req: SetSessionConfigOptionRequest, responder, _cx| {
                    ...
                    if req.config_id.0.as_ref() == "model" {
                        *current_model.lock().unwrap() = value.clone();
                    }
                    recorded_model_changes.lock().unwrap().push(value);
                    let current = current_model.lock().unwrap().clone();
                    responder.respond(SetSessionConfigOptionResponse::new(mock_config_options(
                        &current,
                    )))
                },
                agent_client_protocol::on_receive_request!(),
...
/// A typed ACP client mock that drives the server over a `tokio::io::duplex` pipe.
///
/// The client speaks real typed ACP directly to `ClientConnection`.
#[derive(Clone, Debug)]
...
impl TypedClient {
    ...
    /// Create a new session through the server.
...
/// Spawn `ClientConnection` over an in-memory transport and return a [`TypedClient`] handle.
pub async fn run_client(compositor: Arc<Compositor>) -> TypedClient {
