...
use crate::common::mocks::{MOCK_DEFAULT_MODEL, MOCK_OTHER_MODEL, MockSessionFactory};
...
async fn setup_test_session(comp: &Compositor, name: &str) -> String {
    let (info, _charter_prompt_id) = comp
        .create_session(name, PathBuf::from("/tmp"), "", None, vec![], vec![])
        .await
...
#[tokio::test]
async fn create_session_selects_configured_model() {
    let (comp, _store, factory) = build_compositor_with_state(State::default()).await;
    ...
    comp.create_session(
        ...
        PathBuf::from("/tmp"),
        "",
        Some(MOCK_OTHER_MODEL.to_string()),
        vec![],
        vec![],
    )
    .await
    ...
    let agent = factory.agent("model-session").expect("agent should exist");
    assert_eq!(
        agent.recorded_model_changes(),
        vec![MOCK_OTHER_MODEL.to_string()],
        "the configured model should be set via session/set_config_option"
    );
    assert_eq!(agent.current_model(), MOCK_OTHER_MODEL);
}

#[tokio::test]
async fn create_session_without_model_keeps_agent_default() {
    let (comp, _store, factory) = build_compositor_with_state(State::default()).await;
    ...
    let agent = factory
        .agent("default-model-session")
        .expect("agent should exist");
    assert!(
        agent.recorded_model_changes().is_empty(),
        "no set_config_option call is expected without a configured model"
    );
    assert_eq!(agent.current_model(), MOCK_DEFAULT_MODEL);
}

#[tokio::test]
async fn create_session_with_default_model_does_not_set_config_option() {
    let (comp, _store, factory) = build_compositor_with_state(State::default()).await;

    comp.create_session(
        "already-default-session",
        PathBuf::from("/tmp"),
        "",
        Some(MOCK_DEFAULT_MODEL.to_string()),
        vec![],
        vec![],
    )
    .await
    ...
    let agent = factory
        .agent("already-default-session")
        .expect("agent should exist");
    assert!(
        agent.recorded_model_changes().is_empty(),
        "set_config_option is skipped when the agent already uses the requested model"
    );
}

#[tokio::test]
async fn create_session_with_unavailable_model_fails() {
    let (comp, _store, _factory) = build_compositor_with_state(State::default()).await;
    ...
    let err = comp
        ...
        .await
        .expect_err("create_session should fail for an unknown model");

    let message = format!("{err:#}");
    assert!(
        message.contains("no-such-model"),
        "error should name the requested model, got: {message}"
    );
    assert!(
        message.contains(MOCK_DEFAULT_MODEL) && message.contains(MOCK_OTHER_MODEL),
        "error should list the available models, got: {message}"
    );
}

#[tokio::test]
async fn recreate_session_reapplies_configured_model() {
    let (comp, _store, factory) = build_compositor_with_state(State::default()).await;
    ...
    comp.recreate_session("recreate-model-session", None)
        .await
        .expect("recreate_session should succeed");

    let agent = factory
        .agent("recreate-model-session")
        .expect("agent should exist");
    assert_eq!(
        agent.recorded_model_changes(),
        vec![MOCK_OTHER_MODEL.to_string(), MOCK_OTHER_MODEL.to_string()],
        "the model should be re-applied after recreate resets config options"
    );
    assert_eq!(agent.current_model(), MOCK_OTHER_MODEL);
}

#[tokio::test]
async fn loaded_session_reapplies_persisted_model() {
    let mut state = State::default();
    state.sessions.insert(
        "persisted-model-session".to_string(),
        SessionState {
            session_id: "persisted-model-sid".to_string(),
            cwd: PathBuf::from("/tmp"),
            charter: Some("charter".to_string()),
            model: Some(MOCK_OTHER_MODEL.to_string()),
            allowed_tool_kinds: vec![],
            mcp_servers: vec![],
            cron_jobs: HashMap::new(),
            jobs: vec![],
    ...
    let (comp, _store, factory) = build_compositor_with_state(state).await;
    drop(comp);

    let agent = factory
        .agent("persisted-model-session")
        .expect("agent should exist");
    assert_eq!(
        agent.recorded_model_changes(),
        vec![MOCK_OTHER_MODEL.to_string()],
        "the persisted model should be re-applied when the session is loaded"
    );
}
