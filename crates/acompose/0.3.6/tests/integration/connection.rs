...
async fn test_list_sessions() {
    ...
    compositor
        .create_session("alpha", PathBuf::from("/tmp/a"), "", None, vec![], vec![])
        .await
        .unwrap();
    compositor
        .create_session("beta", PathBuf::from("/tmp/b"), "", None, vec![], vec![])
        .await
        .unwrap();
...
async fn test_load_session_replays_history() {
    ...
    let compositor = test_compositor(factory.clone());
    compositor
        .create_session("s", PathBuf::from("/tmp"), "", None, vec![], vec![])
        .await
        .unwrap();
...
#[tokio::test]
async fn test_duplicate_load_on_same_connection_is_rejected() {
    let factory = MockSessionFactory::new();
    let compositor = test_compositor(factory.clone());
    compositor
        .create_session("s", PathBuf::from("/tmp"), "", None, vec![], vec![])
    ...
    assert!(
        result.is_err(),
        "second session/load for the same session on the same connection should fail"
    );
}

#[tokio::test]
async fn test_duplicate_load_on_same_connection_does_not_duplicate_updates() {
    let factory = MockSessionFactory::new();
    let compositor = test_compositor(factory.clone());
    compositor
        .create_session("s", PathBuf::from("/tmp"), "", None, vec![], vec![])
    ...
    // A second load would duplicate updates, so it must be rejected.
    let result = client
        .load_session("s", &PathBuf::from("/tmp"), vec![])
        .await;
    assert!(result.is_err());
    ...
    assert!(
        result.is_err(),
        "only one forward task should be active for the session"
    );
}
