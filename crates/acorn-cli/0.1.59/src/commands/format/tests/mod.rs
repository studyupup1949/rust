...
async fn test_format_dry_run() -> Result<()> {
    ...
    let result = run(&path, &None, &None, &None, &None, true, &false, &Verbosity::new(0, 0), OFFLINE).await;
...
async fn test_format_no_changes_needed() -> Result<()> {
    ...
    let result = run(&path, &None, &None, &None, &None, false, &false, &Verbosity::new(0, 0), OFFLINE).await;
