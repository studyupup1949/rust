...
impl ResearchActivity {
    ...
    pub fn analyze(paths: Vec<PathBuf>, is_offline: bool) -> usize {
        let config = ValeConfig::default().save();
        let init = Vale::init().build();
        let vale = if command_exists("vale") {
            init.with_config(config).with_system_command()
        } else if is_offline {
            todo!("Support pointing to local vale executable");
        } else {
    ...
    pub fn calculate_readability(paths: Vec<PathBuf>, readability_type: ReadabilityType) -> usize {
        paths
            .par_iter()
            .map(|path| match ResearchActivity::read(path.into()) {
                | Some(data) => {
                    let index = readability_type.calculate(&data.extract_prose());
                    let maximum = readability_type.maximum_allowed();
                    debug!("{} Readability index = {index}", Label::using());
                    if index > maximum {
                        error!(
                            "=> {} {} has a readability index of {} (less than {} is recommended for {} metric)",
                            Label::fail(),
                            path.display(),
                            index.red().bold(),
                            maximum.cyan(),
                        ...
                        let score = format!("({} = {}/{})", readability_type.to_string().to_uppercase(), index, maximum);
                        info!(
                            "=> {} {} has {} {}",
                            Label::pass(),
                            path.display(),
    ...
    pub fn check(paths: Vec<PathBuf>, is_offline: bool) -> usize {
        let runtime = tokio_runtime();
        paths
            .par_iter()
            .map(|path| match ResearchActivity::read(path.into()) {
                | Some(data) => {
                    let offline_issues = data
                        .clone()
                        .validation_issues()
                        .into_iter()
                        .map(|issue| issue.with_path(path.clone()))
                        .collect::<Vec<_>>();
                    let online_issues = runtime.block_on(async {
                        let mut issues: Vec<LinkCheck> = vec![];
                        if !is_offline {
                            ...
                            for url in dois.into_iter().chain(websites.into_iter()) {
                                let result = LinkCheck::run(Some(url)).await;
                                issues.push(result);
                            }
                        }
                        issues
                    });
                    let offline_issue_count: usize = offline_issues
                        .into_iter()
                        .inspect(|issue| issue.clone().print())
                        .map(|issue| issue.issue_count())
                        .sum();
                    let online_issue_count: usize = online_issues
                        .into_iter()
                        .inspect(|issue| issue.clone().print())
                        .filter(|issue| !issue.success)
                        .count();
                    offline_issue_count + online_issue_count
                }
                | None => {
...
{}"#,
    ...
    pub fn format(self, path: Option<PathBuf>) -> ResearchActivity {
        ...
        debug!(path = path_to_string(path_parent), "=> {} Parent directory", Label::using());
    ...
    fn read_json(path: PathBuf) -> serde_json::Result<ResearchActivity> {
        ...
        trace!("=> {} RAD content = {:#?}", label, data.dimmed());
        data
    }
    ...
    fn read_yaml(path: PathBuf) -> serde_yml::Result<ResearchActivity> {
        ...
        debug!("=> {} RAD content = {:#?}", label, data.dimmed());
        data
    }
    ...
    fn validation_issues(self) -> Vec<SchemaCheck> {
        fn errors_collect<T: Validate>(attribute: T) -> Option<Vec<SchemaCheck>> {
            match attribute.validate() {
                | Ok(_) => None,
                | Err(err) => Some(
                    err.into_errors()
                        .into_iter()
                        .map(|(key, value)| SchemaCheck::init().success(false).errors(value).message(key.to_string()).build())
                        .collect::<Vec<SchemaCheck>>(),
                ),
            }
        }
