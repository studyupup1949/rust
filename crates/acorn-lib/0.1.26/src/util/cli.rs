...
use crate::util::{files_all, files_from_git_branch, files_from_git_commit, git_branch_name};
use bon::Builder;
use clap::ValueEnum;
...
pub fn paths_from_options(path: &Option<PathBuf>, options: &Option<Options>) -> Vec<PathBuf> {
    ...
    match options {
                ...
                match commit {
                    ...
                    | None => match branch {
                        ...
                        | None => {
                            ...
                            files_all(value, extensions, ignore.clone())
                        }
                    },
                }
            }
        ...
        | None => {
            ...
            files_all(value, extensions, None)
        }
    }
}
