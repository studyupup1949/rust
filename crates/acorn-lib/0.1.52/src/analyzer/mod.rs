...
impl Analysis for ResearchActivity {
    ...
    fn prose_analysis(paths: Vec<PathBuf>, is_offline: bool, skip_verify_checksum: bool) -> Vec<Check> {
        ...
        match vale.clone().sync(is_offline) {
            | Ok(_) => {
                let results = paths.iter().map(|path| match ResearchActivity::read(path) {
                    | Ok(data) => vale.clone().run(data.clone().meta.identifier, data.to_markdown(), Some("JSON".into())),
                    | Err(why) => {
            ...
            | CheckCategory::Prose => {
                ...
                match &errors {
                    | Some(ErrorKind::Vale(values)) => {
                        ...
                        let title = self.uri.unwrap_or("index.json".to_string());
                        if let Some(text) = &context {
                            println!();
                            values.iter().enumerate().for_each(|(index, item)| {
                                let ValeOutputItem {
                                    check,
                                    line,
                                    message,
                                ...
                                let _ = Report::build(severity.into(), (&title, 1..=1))
                                    ...
                                    .with_message(check)
                                    .with_label(
                                        ariadne::Label::new((&title, hightlighted_span(source.clone(), span, *line)))
...
impl StaticAnalyzer<ValeConfig> for Vale {
    ...
    fn run(&self, id: String, content: String, output: Option<String>) -> Check {
        ...
        match create_dir_all(root.clone()) {
            | Ok(_) => {}
            | Err(why) => error!(path = root.clone().to_absolute_string(), "=> {} Create - {why}", Label::fail()),
        }
        let path = root.join(&id);
        let mut file = match File::create(&path) {
            | Ok(file) => file,
            | Err(why) => panic!("=> {} Create file {} - {why}", Label::fail(), path.display()),
        };
                    ...
                    | Ok(output) => {
                        ...
                        if parsed.is_empty() {
                            ...
                            Check::init()
                                ...
                                .context(content)
                                .uri(path.file_name().unwrap().to_str().unwrap().to_string())
                                .build()
                        }
                    }
                    | Err(output) => {
        ...
        match result {
            | Ok(_) => {
                ...
                match File::create(format!("{parent}/reject.txt")) {
                    ...
                    | Err(why) => panic!("=> {} Create reject.txt - {why}", Label::fail()),
                }
                Ok(())
            }
            | Err(why) => {
                error!(config = config_path.to_absolute_string(), "=> {} Vale sync - {why}", Label::fail());
                Err(why)
            }
        }
    }
...
pub fn convert_lychee_response(value: Response) -> Check {
    match value.status() {
        ...
        | Status::UnknownStatusCode(code) => Check::init()
            ...
            .build(),
        | Status::Timeout(_) => Check::init().category(CheckCategory::Link).success(false).message("HTTP timeout").build(),
        | _ => Check::init()
            .category(CheckCategory::Link)
            .success(true)
...
pub async fn link_check(uri: Option<String>) -> Check {
    match uri {
        ...
        | None => Check::init().category(CheckCategory::Link).success(false).message("missing URL").build(),
    }
}
