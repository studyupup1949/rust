...
#[cfg(feature = "std")]
use crate::prelude::{io::Error, PathBuf};
use crate::schema::constants::{CODEMETA_CONTEXT, DCAT_CONTEXT, SCHEMA_ORG_CONTEXT};
#[cfg(feature = "std")]
use crate::schema::validate::format_phone_number;
use crate::schema::validate::{is_kebabcase, is_list_url, validate_attribute_books, validate_attribute_doi, validate_attribute_ror_list};
#[cfg(feature = "std")]
use crate::schema::Organization;
use crate::schema::{ClassificationLevel, ContactPoint, ImageObject, Keyword, MediaObject, OrganizationType, Other, Sections, Status, Website};
#[cfg(feature = "std")]
...
use crate::util::{frontmatter_and_body, LinkedData, ToMarkdown};
#[cfg(feature = "std")]
...
pub struct ResearchActivityMetadata {
    ...
    #[validate(custom(function = "validate_attribute_books"))]
    #[serde(default)]
...
impl Default for ResearchActivityContext {
    fn default() -> Self {
        ResearchActivityContext::init()
            ...
            .contact(format!("{DCAT_CONTEXT}/contactPoint"))
...
impl Default for ResearchActivityMetadataContext {
    fn default() -> Self {
        ResearchActivityMetadataContext::init()
            ...
            .keywords(format!("{DCAT_CONTEXT}/keyword"))
...
impl InputOutput for ResearchActivity {
    fn read(path: impl Into<PathBuf>) -> Result<ResearchActivity, Error> {
        let source = path.into().clone();
        let result = match MimeType::from_path(source.clone()) {
            | MimeType::Json => ResearchActivity::read_json(source.clone()),
            | MimeType::Yaml => ResearchActivity::read_yaml(source.clone()),
            | _ => Err(Error::other("Unsupported research activity data file extension")),
        };
        ...
        match &result {
            | Ok(data) => {
                debug!(path = source.to_str().unwrap(), "=> {}", label);
                debug!("=> {} Research activity data = {:#?}", label, data.dimmed().cyan());
            }
            | Err(_) => {
                error!(path = source.to_str().unwrap(), "=> {}", label);
            }
        }
        result
    }
    fn read_json(path: PathBuf) -> Result<ResearchActivity, Error> {
        read_file(path.clone())
            .and_then(|content| serde_json::from_str(&content).map_err(|why| Error::other(format!("Failed to parse JSON RAD: {}", why))))
    }
    fn read_markdown(path: PathBuf) -> Option<ResearchActivity> {
        ...
        let meta: ResearchActivityMetadata = match frontmatter {
            | Some(value) => match serde_yml::from_str(&value) {
                | Ok(data) => data,
                | Err(why) => {
        ...
        dbg!(&meta);
        // TODO: Parse body into title, subtitle, sections, and contact
        None
    }
    fn read_yaml(path: PathBuf) -> Result<ResearchActivity, Error> {
        read_file(path.clone())
            .and_then(|content| serde_yml::from_str(&content).map_err(|why| Error::other(format!("Failed to parse YAML RAD: {}", why))))
    }
    fn write(&self, path: impl Into<PathBuf>) -> Result<(), Error> {
        let output = path.into();
        let mime = MimeType::from_path(output.clone());
        match mime {
            | MimeType::Json => self.write_json(output),
            | MimeType::Markdown => self.write_markdown(output),
            | MimeType::Yaml => self.write_yaml(output),
            | _ => {
                unimplemented!("Unsupported research activity data file extension for writing")
            }
        }
    }
    ...
    fn write_markdown(&self, path: impl Into<PathBuf>) -> Result<(), Error> {
...
impl ToMarkdown for ResearchActivity {
    // TODO: Implement to_markdown for ASPECT
    fn to_markdown(&self) -> String {
        ...
        let frontmatter = serde_yml::to_string(&meta).unwrap();
