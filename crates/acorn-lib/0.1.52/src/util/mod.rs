...
// TODO: Unify with io::api::spdx::License
/// # License
...
impl From<&str> for SemanticVersion {
    ...
    fn from(value: &str) -> Self {
        let pattern = r"(?<major>\d+)\.?(?<minor>\d+)?\.?(?<patch>\d+)?";
        ...
        match parts.clone().collect::<Vec<_>>().len() {
            | 3 => {
                ...
                SemanticVersion::init().major(major).minor(minor).patch(patch).build()
            }
            | 2 => {
                let major = parts.next().unwrap().parse::<u32>().unwrap();
                let minor = parts.next().unwrap().parse::<u32>().unwrap();
                SemanticVersion::init().major(major).minor(minor).build()
            }
            | 1 => {
                let major = parts.next().unwrap().parse::<u32>().unwrap();
                SemanticVersion::init().major(major).build()
            }
...
pub fn base32_crockford_encode(value: u128) -> String {
    if value == 0 {
        ...
        successors(Some(value), |&n| (n >= MODULUS).then_some(n / MODULUS))
            .map(|n| CROCKFORD_BASE32_ALPHABET[(n % MODULUS) as usize] as char)
            .collect::<Vec<_>>()
            .into_iter()
            .rev()
            .collect::<String>()
...
pub fn detect_json(text: impl ToString) -> bool {
    serde_json::from_str::<serde_json::Value>(&text.to_string()).is_ok()
}
