    ...
    pub fn key(&self) -> Option<String> {
        match &self.event {
            ...
            | HookPayload::Note {
                ...
                let check_requested = body.lines().any(|line| line.trim() == "/acorn check");
                (human_authored && work_item && check_requested).then(|| format!("work-item-check:{}:{iid}:{note_id}", actor.project_id))
            }
            | _ => None,
        }
    }
