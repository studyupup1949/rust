...
//!
//! This module provides database connection helpers guarded by a process-global
//! mutex so concurrent operations do not race while still respecting per-handle
//! database paths.
...
use crate::util::{to_rfc3339, Label};
    ...
    S: TableSchemaProvider + Sync,
{
    ...
    async fn populate(&self, table: impl TableSchemaProvider + Send) -> ApiResult<PersistStatus> {
        match self.row_count(table) {
            ...
            | Ok(_) => table.into().populate(self.path.clone()).await.map(PersistStatus::Downloaded),
            | Err(why) => Err(why),
        }
    }
    ...
    T: TableSchemaProvider,
{
    ...
    pub fn with_connection<U>(&self, callback: impl FnOnce(&Connection) -> ApiResult<U>) -> ApiResult<U> {
        CONNECTION_LOCK
            ...
            .and_then(|guard| {
                let result = resolve_database_path(self.path.as_ref())
                    .and_then(|db_path| Connection::open(db_path).map_err(|why| eyre!("Failed to open database connection — {why}")))
                    .and_then(|connection| callback(&connection));
                drop(guard);
                result
            })
    }
        ...
        #[cfg(feature = "std")]
        {
            if let Ok(env_ttl) = dotenvy::var(CACHE_TTL) {
                if let Ok(seconds) = env_ttl.parse::<i64>() {
                    if seconds > 0 {
                        return SignedDuration::from_secs(seconds);
                    }
                }
            }
        }
        let default_ttl = i64::try_from(DEFAULT_CACHE_TTL_SECONDS).map_or(SignedDuration::MAX, SignedDuration::from_secs);
    ...
    pub fn migrate(&self) -> ApiResult<()> {
        T::all().iter().copied().try_fold((), |_, table| self.migrate_table(table)).inspect(|_| {
            debug!("{} Database migration", Label::pass());
        })
    }
    ...
    pub fn cleanup_expired_cache(&self) -> ApiResult<usize> {
        ...
        let cutoff = to_rfc3339(now.checked_sub(ttl).unwrap_or(now));
