...
/// Tokenizes text by splitting on whitespace, deunicode-normalizing each
/// word to its closest ASCII representation (so Unicode punctuation like
/// curly quotes U+2018/2019/201C/201D and em-dashes U+2014 collapse to
/// their ASCII equivalents before the punctuation filter below runs --
/// `is_ascii_punctuation` alone never matches those, so a source/ASR-
/// transcript pair differing only in quote-mark style previously produced
/// spurious WER errors), stripping ASCII punctuation, and lowercasing.
