...
/// stripping remaining ASCII punctuation, and lowercasing.
...
mod tests {
    ...
    fn word_error_rate_treats_hyphens_as_word_separators() {
        // Real-world case (moss-tts's onboarding1-en fixture): ASR never
        // reconstructs hyphens, so a hyphenated compound in written source
        // text ("AI-narrated") is always transcribed as separate
        ...
        let reference = "Transform content into personalised, AI-narrated audio scrolls.";
        let hypothesis = "Transform content into personalized AI narrated audio scrolls.";
        let wer = word_error_rate(reference, hypothesis).expect("non-empty reference");
        // Only "personalised"/"personalized" (a genuine spelling variant)
        // should count as an error once hyphens are treated as separators:
        // 1 edit out of 8 reference words = 0.125.
        assert!(
            (wer - 0.125).abs() < 1e-4,
            "expected only the spelling-variant substitution to count as an error, got WER {wer}"
        );
    }
