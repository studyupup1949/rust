...
// ACROFORM-RS: Removed image decoding param structs - not needed for form filling
// Removed: DCTDecodeParams, CCITTFaxDecodeParams, JBIG2DecodeParams
...
pub enum StreamFilter {
    ...
    // ACROFORM-RS: Removed image decoding filters
    // JPXDecode, DCTDecode, CCITTFaxDecode, JBIG2Decode, Crypt
    RunLengthDecode
}
impl StreamFilter {
    pub fn from_kind_and_params(kind: &str, params: Dictionary, r: &impl Resolve) -> Result<StreamFilter> {
       ...
       match kind {
           ...
           // ACROFORM-RS: Removed image decoding and encryption filters
           ...
           ty => bail!("Unsupported filter type {:?} (image/encryption filters removed in acroform-rs)", ty),
       } 
       )
    }
}
...
// ACROFORM-RS: Removed image decoding functions
// Removed: dct_decode, fax_decode, jpx_decode, jbig2_decode, set_jpx_decoder, set_jbig2_decoder
...
// ACROFORM-RS: Removed fax_decode, jpx_decode, jbig2_decode - image decoding not needed
...
// ACROFORM-RS: Removed JPX and JBIG2 decoder infrastructure - image decoding not needed
...
pub fn decode(data: &[u8], filter: &StreamFilter) -> Result<Vec<u8>> {
    match *filter {
        ...
        // ACROFORM-RS: Removed image decoding filters
        // DCTDecode, CCITTFaxDecode, JPXDecode, JBIG2Decode, Crypt
    }
}
