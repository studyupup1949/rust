...
// ACROFORM-RS: Stub for removed CryptDict type
#[derive(Debug, Clone, DataSize)]
pub struct CryptDict;
impl Object for CryptDict {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        Err(PdfError::Other { msg: "CryptDict not supported - encryption removed in acroform-rs".into() })
    }
}
impl ObjectWrite for CryptDict {
...
pub struct Storage<B, OC, SC, L> {
    ...
    // ACROFORM-RS: Removed decoder field - only supporting decrypted PDFs
    ...
    L: Log,
{
    pub fn empty(object_cache: OC, stream_cache: SC, log: L) -> Self {
        Storage {
            ...
            // ACROFORM-RS: Removed decoder - only supporting decrypted PDFs
    ...
    L: Log,
{
    ...
    fn decode(&self, id: PlainRef, range: Range<usize>, filters: &[StreamFilter]) -> Result<Arc<[u8]>> {
        ...
        // ACROFORM-RS: Removed decryption - only supporting decrypted PDFs
    ...
    pub fn load_storage_and_trailer_password(&mut self, password: &[u8]) -> Result<Dictionary> {
        ...
        // ACROFORM-RS: Removed encryption handling - only supporting decrypted PDFs
        // If the PDF has an /Encrypt entry, we cannot handle it
        if trailer.get("Encrypt").is_some() {
            return Err(PdfError::Other { msg: "Encrypted PDFs are not supported. Please decrypt the PDF first.".into() });
        }
    ...
    pub fn scan(&self) -> impl Iterator<Item = Result<ScanItem>> + '_ {
        ...
        std::iter::from_fn(move || {
            loop {
                ...
                // ACROFORM-RS: Removed decoder parameter - only supporting decrypted PDFs
                match parse_indirect_object(&mut lexer, &resolver, ParseFlags::all()) {
        ...
        match self.changes.get(&r.id) {
            ...
            None => match t!(self.refs.get(r.id)) {
                XRef::Raw {pos, ..} => {
                    ...
                    // ACROFORM-RS: Removed decoder parameter - only supporting decrypted PDFs
                    let p = t!(parse_indirect_object(&mut lexer, resolve, flags)).1;
                    Ok(p)
                }
