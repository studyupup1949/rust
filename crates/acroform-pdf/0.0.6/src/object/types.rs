...
use crate as pdf;
// ACROFORM-RS: Removed content module - stubbing required types
// use crate::content::deep_clone_op;
use crate::object::*;
use crate::error::*;
// ACROFORM-RS: Removed content and font modules - stubbing required types  
// use crate::content::{Content, FormXObject, Matrix, parse_ops, serialize_ops, Op};
// use crate::font::Font;
use crate::enc::StreamFilter;

// ACROFORM-RS: Stub types for removed modules (content, font, color) that are referenced in this file
// These types are defined but not actually used by acroform functionality
#[derive(Debug, Clone, DataSize)]
pub struct Content;
#[derive(Debug, Clone, DataSize)]
pub struct FormXObject {
    pub stream: PdfStream, // Stub field needed by XObject::Form
}
#[derive(Debug, Clone, DataSize)]
pub struct Matrix;
#[derive(Debug, Clone, DataSize)]
...
impl Object for Content {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Content parsing not supported - removed in acroform-rs".into() })
    }
}
impl ObjectWrite for Content {
    fn to_primitive(&self, _update: &mut impl Updater) -> Result<Primitive> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Content serialization not supported - removed in acroform-rs".into() })
    }
}
impl Object for FormXObject {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "FormXObject parsing not supported - removed in acroform-rs".into() })
    }
}
impl ObjectWrite for FormXObject {
    fn to_primitive(&self, _update: &mut impl Updater) -> Result<Primitive> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "FormXObject serialization not supported - removed in acroform-rs".into() })
    }
}
impl DeepClone for FormXObject {
    fn deep_clone(&self, _cloner: &mut impl Cloner) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "FormXObject deep_clone not supported - removed in acroform-rs".into() })
    }
}
impl Object for Matrix {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Matrix parsing not supported - removed in acroform-rs".into() })
    }
}
impl ObjectWrite for Matrix {
    fn to_primitive(&self, _update: &mut impl Updater) -> Result<Primitive> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Matrix serialization not supported - removed in acroform-rs".into() })
    }
}
impl DeepClone for Matrix {
    fn deep_clone(&self, _cloner: &mut impl Cloner) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Matrix deep_clone not supported - removed in acroform-rs".into() })
    }
}
impl Object for Font {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Font parsing not supported - removed in acroform-rs".into() })
    }
}
impl ObjectWrite for Font {
    fn to_primitive(&self, _update: &mut impl Updater) -> Result<Primitive> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Font serialization not supported - removed in acroform-rs".into() })
    }
}
impl DeepClone for Font {
    fn deep_clone(&self, _cloner: &mut impl Cloner) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "Font deep_clone not supported - removed in acroform-rs".into() })
    }
}
impl Object for ColorSpace {
    fn from_primitive(_p: Primitive, _resolve: &impl Resolve) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "ColorSpace parsing not supported - removed in acroform-rs".into() })
    }
}
impl ObjectWrite for ColorSpace {
    fn to_primitive(&self, _update: &mut impl Updater) -> Result<Primitive> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "ColorSpace serialization not supported - removed in acroform-rs".into() })
    }
}
impl DeepClone for ColorSpace {
    fn deep_clone(&self, _cloner: &mut impl Cloner) -> Result<Self> {
        // ACROFORM-RS: Stub - not implemented
        Err(PdfError::Other { msg: "ColorSpace deep_clone not supported - removed in acroform-rs".into() })
    }
}
// Stub function for deep_clone_op - now takes only 2 arguments
fn deep_clone_op(_op: &Op, _cloner: &mut impl Cloner) -> Result<Op> {
    // ACROFORM-RS: Stub - not implemented
    Err(PdfError::Other { msg: "Op deep_clone not supported - removed in acroform-rs".into() })
}
fn parse_ops(_data: &[u8], _resolve: &impl Resolve) -> Result<Vec<Op>> {
    // ACROFORM-RS: Stub - not implemented
    Err(PdfError::Other { msg: "Op parsing not supported - removed in acroform-rs".into() })
}
fn serialize_ops(_ops: &[Op]) -> Result<Vec<u8>> {
    // ACROFORM-RS: Stub - not implemented
    Err(PdfError::Other { msg: "Op serialization not supported - removed in acroform-rs".into() })
}
    ...
    fn deep_clone(&self, cloner: &mut impl Cloner) -> Result<Self> {
        match *self {
            ...
            Pattern::Stream(ref dict, ref ops) => {
                ...
                let ops: Vec<Op> = ops.iter().map(|op| deep_clone_op(op, cloner)).collect::<Result<Vec<_>>>()?;
                // ACROFORM-RS: Explicitly clone matrix field since Matrix doesn't implement Copy
                let matrix = dict.matrix.as_ref().map(|m| m.deep_clone(cloner)).transpose()?;
                let dict = PatternDict {
                    paint_type: dict.paint_type,
                    tiling_type: dict.tiling_type,
                    bbox: dict.bbox,
                    x_step: dict.x_step,
                    y_step: dict.y_step,
...
impl ObjectWrite for XObject {
    fn to_primitive(&self, update: &mut impl Updater) -> Result<Primitive> {
        let (subtype, mut stream) = match self {
            ...
            // ACROFORM-RS: FormXObject stub - return stream directly
            XObject::Form(s) => ("Form", s.stream.clone()),
...
impl ImageXObject {
        ...
        match self.inner.inner_data {
            ...
            StreamData::Original(ref file_range, id) => {
                ...
                let end = filters.iter().rposition(|f| match f {
                    ...
                    // ACROFORM-RS: Removed image and encryption filters
                ...
                match image_filters {
                    [] => Ok((data, None)),
                    // ACROFORM-RS: Removed image filter patterns
                    // [StreamFilter::DCTDecode(_)] |
                    // [StreamFilter::CCITTFaxDecode(_)] |
                    // [StreamFilter::JPXDecode] |
                    [StreamFilter::FlateDecode(_)] => Ok((data, Some(&image_filters[0]))),
                    // [StreamFilter::JBIG2Decode(_)] => Ok((data, Some(&image_filters[0]))),
                    _ => bail!("Unsupported image filters (removed in acroform-rs): {:?}", image_filters)
                }
            }
        }
    }
    ...
    pub fn image_data(&self, resolve: &impl Resolve) -> Result<Arc<[u8]>> {
        ...
        // ACROFORM-RS: Removed image decoding - not needed for form filling
        // The code below tried to decode image formats but we've removed those functions
        bail!("Image decoding not supported (removed in acroform-rs). Filter: {:?}", filter)
    }
}
