...
// ACROFORM-RS: Moved UTF-16BE helper functions here from removed font module
...
fn utf16be_to_string_lossy(data: &[u8]) -> String {
    utf16be_to_char(data)
        .map(|r| r.unwrap_or(std::char::REPLACEMENT_CHARACTER))
        .collect()
}
/// converts UTF16-BE to a string errors out in illegal/unknown characters
fn utf16be_to_string(data: &[u8]) -> Result<SmallString> {
    utf16be_to_char(data)
...
impl PdfString {
    ...
    pub fn to_string(&self) -> Result<String> {
        if self.data.starts_with(&[0xfe, 0xff]) {
            Ok(String::from(std::str::from_utf8(utf16be_to_string(&self.data[2..])?.as_bytes())
