...
use acroform_pdf::file::FileOptions;
use acroform_pdf::object::*;
use acroform_pdf::parser::{parse, ParseFlags};
use glob::glob;
...
fn read_pages() {
    for path in dir_pdfs(files()) {
        ...
        // ACROFORM-RS: Skip encrypted PDFs since encryption support was removed
        if path_str.contains("encrypted") {
            println!("Skipping encrypted PDF (encryption support removed)");
            continue;
        }
        
        let file = run!(FileOptions::cached().open(path_str));
...
// ACROFORM-RS: Removed user_password and owner_password tests
// Encryption support was removed as the library now focuses on decrypted AcroForm PDFs only
// 
// #[test]
// fn user_password() { ... }
// 
...
fn parse_objects_from_stream() {
    use acroform_pdf::object::NoResolve;
