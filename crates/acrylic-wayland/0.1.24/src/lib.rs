...
impl WaylandApp {
		...
		xdg_toplevel.quick_assign(|_iface, event, mut data| {
			...
			if let XdgToplevelEvent::Configure { width, height, .. } = event {
				if app.configured {
					...
					app.size = Size::new(w as usize, h as usize);
					app.acrylic_app.update_spot((Point::zero(), app.size));
	...
	pub fn frame(&mut self, assets: &str) {
		...
		for i in 0..self.frame_buffer.data.len() {
			self.frame_buffer.data[i] = bg_color[i % RGBA];
		}
