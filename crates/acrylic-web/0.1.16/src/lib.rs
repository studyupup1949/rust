use acrylic::flexbox::compute_tree;
...
pub extern fn process_response(app: &mut Application) {
	...
	let widget = app.tree.get_node_widget(request.node).unwrap();
	let mut widget = widget.lock().unwrap();
	let data = unsafe {
		RESPONSE_BYTES.as_ref().unwrap()
	};
	widget.loaded(app, request.node, &request.name, 0, data);
}
...
pub extern fn set_output_size(app: &mut Application, w: usize, h: usize) -> *const u8 {
	let (p, _) = app.tree.get_node_spot(app.view_root).unwrap();
	app.tree.set_node_spot(&mut app.view_root, Some((p, Size::new(w, h))));
	compute_tree(&mut app.tree, app.view_root);
	app.render();
