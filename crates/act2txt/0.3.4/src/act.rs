...
impl core::fmt::Display for ReadError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", match self {
            Self::InvalidFileLength => "Invalid file length",
...
impl Palette {
    pub fn read(reader: &mut impl Read, all: bool) -> Result<Self, ReadError> {
        ...
        Ok(Self { colors, transparent_index })
    }
}

fn read_extra_data(reader: &mut (impl Read + ?Sized), all: bool) -> (Option<u16>, u16) {
    let mut extra_buf = [0; EXTRA_DATA_SIZE];
    let mut transparent_index = None;
    let mut num_colors = MAX_COLORS;
    if reader.read_exact(&mut extra_buf).is_ok() {
        if !all {
            num_colors = u16::from_le_bytes([extra_buf[0], extra_buf[1]]);
        }
