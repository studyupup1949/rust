...
fn main() -> Result<(), anyhow::Error> {
    ...
    let mut in_file = File::open(&args.input_filename)
        .inspect_err(|_| error!("Could not open input file: {}", args.input_filename.display()))?;

    let palette = act::Palette::read(&mut in_file, args.all)
        .inspect_err(|e| error!("{}: {}", e, args.input_filename.display()))?;
    ...
    let mut out_file = result
        .inspect_err(|_| error!("Could not create output file: {}", args.output_filename.display()))?;

    palette.write_pdn_txt(&mut out_file)
        .inspect_err(|_| error!("Could not write palette to output file: {}", args.output_filename.display()))?;
