...
fn test_includes_generates_like_with_wildcards() {
    ...
    if let SqlValue::String(s) = &bindings[0] {
        assert_eq!(s, "%test%");
    } else {
...
fn test_uniq_generates_count_distinct() {
    ...
    let (sql, _) = d.compile(&ir);
    assert!(sql.contains("COUNT(DISTINCT `wallet`) AS `__uniq`"));
}
...
fn test_deeply_nested_and_or() {
    ...
    let (sql, bindings) = d.compile(&ir);
    assert!(sql.contains("(`a` = ? AND ((`b` > ? AND `c` < ?) OR `d` = ?))"));
...
fn test_is_null_operator() {
    ...
    let (sql, bindings) = d.compile(&ir);
    assert!(sql.contains("`name` IS NULL"), "got: {sql}");
...
fn test_is_not_null_operator() {
    ...
    let (sql, bindings) = d.compile(&ir);
    assert!(sql.contains("`email` IS NOT NULL"), "got: {sql}");
    assert!(bindings.is_empty());
}
...
fn test_count_star_aggregate() {
    ...
    let ir = QueryIR {
        ...
        selects: vec![
            ...
            SelectExpr::Aggregate { function: "COUNT".into(), column: "*".into(), alias: "__count".into() },
        ],
        filters: FilterNode::Empty, having: FilterNode::Empty,
        group_by: vec!["category".into()], order_by: vec![], limit: 100, offset: 0,
    };
    let (sql, _) = d.compile(&ir);
    assert!(sql.contains("COUNT(*) AS `__count`"), "got: {sql}");
    assert!(sql.contains("GROUP BY `category`"), "got: {sql}");
}
...
fn test_multiple_order_by() {
    ...
    let (sql, _) = d.compile(&ir);
    assert!(sql.contains("ORDER BY `created_at` DESC, `id` ASC"), "got: {sql}");
}
