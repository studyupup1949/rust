...
activecube-rs provides a declarative way to expose analytical databases (StarRocks, ClickHouse, etc.) through GraphQL. Define your data model as **Cubes** with dimensions, metrics, and selectors — the library generates a full GraphQL schema and compiles queries into parameterized SQL.
...
| **SqlDialect** | Pluggable SQL generation backend (StarRocks included) |
...
- **SqlDialect trait** — pluggable SQL generation (StarRocks included, ClickHouse extensible)
...
activecube-rs/
  src/
    ...
    sql/            # SqlDialect trait + StarRocks implementation
