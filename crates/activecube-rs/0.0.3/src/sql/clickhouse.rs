        ...
        let select_parts: Vec<String> = ir.selects.iter().map(|s| match s {
            ...
            SelectExpr::Aggregate { function, column, alias } => {
                ...
                if func == "COUNT" && column == "*" {
                    ...
                    format!("uniq(`{column}`) AS `{alias}`")
                } else {
                    format!("{func}(`{column}`) AS `{alias}`", func = func.to_lowercase())
                }
            }
        }).collect();
...
mod tests {
    ...
    fn test_uniq_uses_native_function() {
        let ir = QueryIR {
            ...
            selects: vec![
                SelectExpr::Aggregate { function: "UNIQ".into(), column: "wallet".into(), alias: "__uniq".into() },
            ],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 10, offset: 0,
        };
    ...
    fn test_aggregate_lowercase() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
                SelectExpr::Aggregate { function: "SUM".into(), column: "amount".into(), alias: "__sum".into() },
                SelectExpr::Aggregate { function: "AVG".into(), column: "price".into(), alias: "__avg".into() },
            ],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 10, offset: 0,
        };
    ...
    fn test_having_with_aggregate_expr() {
        let ir = QueryIR {
            ...
            selects: vec![
                ...
                SelectExpr::Aggregate { function: "SUM".into(), column: "amount_usd".into(), alias: "__sum".into() },
            ],
