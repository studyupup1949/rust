...
fn build_cube_field(
    ...
    field = field
        ...
        .argument(InputValue::new("limitBy", TypeRef::named(format!("{}LimitByInput", cube.name)))
...
fn build_cube_types(cube: &CubeDefinition) -> CubeTypes {
    ...
    let limitby_input_name = format!("{}LimitByInput", cube.name);
    ...
    match node {
        DimensionNode::Leaf(dim) => {
            ...
            if matches!(dim.dim_type, DimType::Bool) {
                c.filter_fields.push(InputValue::new(&dim.graphql_name, TypeRef::named(TypeRef::BOOLEAN)));
            } else {
...
fn dim_type_to_typeref(dt: &DimType) -> TypeRef {
    match dt {
        DimType::String | DimType::DateTime | DimType::Decimal => TypeRef::named(TypeRef::STRING),
