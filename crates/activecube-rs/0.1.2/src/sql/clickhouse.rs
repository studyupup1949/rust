...
impl SqlDialect for ClickHouseDialect {
    fn compile(&self, ir: &QueryIR) -> CompileResult {
        ...
        // Build augmented selects: aggregate expressions used in HAVING need
        // aliases in SELECT so ClickHouse can resolve them (especially -Merge combinators).
        ...
        if has_having_agg {
            // Only alias columns that are BOTH aggregate AND referenced in HAVING.
            ...
            // Add missing HAVING columns not in SELECT
            for col in &having_cols {
                if col.contains('(') && !agg_alias_map.contains_key(col.as_str()) {
                    ...
                    // No remap needed: this column wasn't requested by user
                }
            }
        }
        ...
        if has_having_agg {
            let having_clause = compile_filter_with_aliases(&ir.having, &mut bindings, &agg_alias_map);
...
mod tests {
    ...
    fn test_having_with_aggregate_expr() {
        ...
        // HAVING references sum(`amount_usd`) which is not a SelectExpr::Column,
        // so it gets added as a new aliased column for HAVING reference
    ...
    fn test_having_appends_missing_agg_column() {
        // HAVING references an aggregate column NOT in SELECT — it should be
        // auto-appended to SELECT with an alias.
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
