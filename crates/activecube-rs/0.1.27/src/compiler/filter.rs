...
fn parse_leaf_filter(
    ...
    let ops: &[(&str, CompareOp)] = match dim_type {
        ...
        DimType::StringArray | DimType::IntArray => &[],
    };

    for (key, op) in ops {
        if let Ok(val) = obj.try_get(key) {
            // Skip null values from nullable GraphQL variables — treat as "filter not set"
            if is_null_accessor(&val) {
                continue;
            }
...
/// Detect null `ValueAccessor` — when a nullable GraphQL variable is not provided,
/// all typed accessors fail. We check the most common ones to identify null.
fn is_null_accessor(val: &ValueAccessor) -> bool {
    val.string().is_err()
        && val.i64().is_err()
        && val.f64().is_err()
        && val.boolean().is_err()
        && val.enum_name().is_err()
        && val.object().is_err()
        && val.list().is_err()
}
...
fn accessor_to_sql(
    ...
    match dim_type {
        ...
        DimType::StringArray | DimType::IntArray => {
            Err(async_graphql::Error::new("Array dimensions do not support scalar filters"))
        }
    }
}
