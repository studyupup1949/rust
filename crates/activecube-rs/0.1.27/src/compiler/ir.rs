...
impl FilterNode {
    ...
    /// `In`/`NotIn` values are stored as comma-joined strings, so each element
    /// is transformed individually and re-joined. Empty and non-String values
    /// are left untouched.
    pub fn apply_filter_value_transforms(&mut self, rules: &[FilterValueTransform]) {
        ...
        match self {
            FilterNode::Condition { column, op, value } => {
                let SqlValue::String(s) = value else { return };
                if s.is_empty() {
                    return;
                }
                let is_list = matches!(op, CompareOp::In | CompareOp::NotIn);
                for rule in rules {
                    let FilterValueTransform::StripPrefix { column: rule_col, prefix } = rule;
