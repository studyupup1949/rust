    ...
    fn compile_inner(
        ...
        if let Some(ref lb) = ir.limit_by {
            for col in &lb.columns {
                if !augmented_selects.iter().any(|s| match s {
                    SelectExpr::Column { column, .. } => column == col,
                    _ => false,
                }) {
                    augmented_selects.push(SelectExpr::Column { column: col.clone(), alias: None });
                }
            }
        }
        ...
        let select_parts: Vec<String> = augmented_selects.iter().map(|s| match s {
            SelectExpr::Column { column, alias } => {
                let is_expr = column.contains('(');
                let col = if is_expr { column.clone() } else { format!("`{column}`") };
                match alias {
                    Some(a) => format!("{col} AS `{a}`"),
                    // Auto-alias expression columns by their raw form so the response mapper
                    // (which keys on `dim.column`) can locate them in the ClickHouse result.
                    // Plain column references keep CH's native naming.
                    None if is_expr => format!("{col} AS `{column}`"),
                    None => col,
                }
            },
            SelectExpr::Aggregate { function, column, alias, condition } => {
        ...
        if let Some(ref subquery) = ir.from_subquery {
            let has_group_by = subquery.contains(" GROUP BY ");
            if has_group_by {
                let (agg_aliases, passthrough_cols) = extract_subquery_agg_aliases(subquery);
                let (pushdown, outer) = split_filter_for_subquery(ir.filters.clone(), &agg_aliases);

                let pushdown_clause = if !passthrough_cols.is_empty() {
                    let qualify_map: HashMap<String, String> = passthrough_cols.iter()
                        .map(|col| (col.clone(), format!("_src.{col}")))
                        .collect();
                    compile_filter_with_aliases(&pushdown, bindings, &qualify_map)
                } else {
                    compile_filter(&pushdown, bindings)
                };

                if !pushdown_clause.is_empty() {
                    let gb_pos = subquery.find(" GROUP BY ").unwrap();
                    let (before_gb, after_gb) = subquery.split_at(gb_pos);
                    let before_with_alias = if !passthrough_cols.is_empty() {
                        inject_table_alias(before_gb, "_src")
                    } else {
                        before_gb.to_string()
                    };
                    sql.push_str(&format!(" FROM ({before_with_alias} WHERE {pushdown_clause}{after_gb}) AS _t"));
                } else {
                    sql.push_str(&format!(" FROM ({}) AS _t", subquery));
                }

                let outer_clause = compile_filter(&outer, bindings);
                if !outer_clause.is_empty() {
                    sql.push_str(" WHERE ");
                    sql.push_str(&outer_clause);
                }
            } else {
                sql.push_str(&format!(" FROM ({}) AS _t", subquery));
        ...
        let mut effective_group_by = if !ir.group_by.is_empty() {
            let mut gb = ir.group_by.clone();
            for col in &ir.required_group_by {
                if !gb.contains(col) {
                    gb.push(col.clone());
                }
            }
            ...
            if has_agg_cols {
                let mut gb: Vec<String> = augmented_selects.iter().filter_map(|s| match s {
        ...
        if !effective_group_by.is_empty() {
            if let Some(ref lb) = ir.limit_by {
                for col in &lb.columns {
                    if !effective_group_by.contains(col) && !contains_aggregate_expr(col) {
                        effective_group_by.push(col.clone());
                    }
                }
            }
            sql.push_str(" GROUP BY ");
        ...
        let has_aggregate_in_select = augmented_selects.iter().any(|s| match s {
            SelectExpr::Column { column, .. } => column.contains("Merge("),
            SelectExpr::Aggregate { .. } | SelectExpr::DimAggregate { .. } => true,
        });
        let user_having_empty = matches!(ir.having, FilterNode::Empty);
        ...
        } else if !user_having_empty {
            let having_clause = compile_filter(&ir.having, bindings);
            if !having_clause.is_empty() {
                sql.push_str(" HAVING ");
                sql.push_str(&having_clause);
            }
        } else if has_aggregate_in_select {
            // ClickHouse returns a single "default" row for global aggregates on zero input
            // rows; drop that pseudo-row so GraphQL can return an empty list (Bitquery-like).
        ...
        if let Some(ref lb) = ir.limit_by {
            ...
            if lb.offset > 0 {
                sql.push_str(&format!(" LIMIT {}, {} BY {}", lb.offset, lb.count, by_cols.join(", ")));
            } else {
...
fn compile_filter_with_aliases(
    ...
    match node {
        ...
        FilterNode::Condition { column, op, value } => {
            if let Some(aliased) = aliases.get(column) {
                return compile_condition(aliased.as_str(), op, value, bindings);
            }
            if let Some((wrapper, inner)) = split_wrapped_column(column) {
                if let Some(aliased) = aliases.get(inner) {
                    let requalified = format!("{wrapper}({aliased})");
                    return compile_condition_raw(&requalified, op, value, bindings);
                }
            }
...
fn compile_filter_with_param_remap(
    ...
    match node {
        ...
        FilterNode::Condition { column, op, value } => {
            ...
            } else if let Some((wrapper, inner)) = split_wrapped_column(column) {
                if let Some(&param) = col_to_param.get(inner) {
                    let wrapped = format!("{wrapper}({param})");
                    compile_condition_raw(&wrapped, op, value, bindings)
                } else {
                    compile_condition(column, op, value, bindings)
                }
            } else {
...
/// Analyse a subquery SELECT list and return:
///   - `agg_aliases`: columns that are true aggregates (must stay in outer WHERE)
///   - `passthrough_cols`: `any(col) AS col` columns (safe for pushdown, but
///     need table-qualified names in the WHERE to avoid alias shadowing)
///
/// **Passthrough rule**: `any(col) AS col` (alias == inner column name) is
/// treated as a passthrough — the filter can be pushed into the subquery
/// WHERE.  This is safe because `any()` wraps dimension columns that are
/// functionally determined by the GROUP BY keys; filtering inside the
/// subquery narrows the scan without changing per-group aggregation results.
///
/// ClickHouse resolves bare column names in WHERE against SELECT aliases,
/// so pushed-down filters on passthrough columns must use table-qualified
/// references (`schema.table.column`) to avoid `ILLEGAL_AGGREGATION` errors.
fn extract_subquery_agg_aliases(subquery: &str) -> (HashSet<String>, HashSet<String>) {
    const AGG_PATTERNS: &[&str] = &[
        "Merge(",
        "anyLast(",
        "anyHeavy(",
        "sum(",
    ...
    let mut passthrough_cols = HashSet::new();
    let select_end = subquery.find(" FROM ").unwrap_or(subquery.len());
    let select_part = &subquery[..select_end];
    for part in select_part.split(',') {
        let trimmed = part.trim();

        if let Some(col) = extract_any_passthrough(trimmed) {
            passthrough_cols.insert(col);
            continue;
        }

        let is_agg = AGG_PATTERNS.iter().any(|p| trimmed.contains(p))
            || trimmed.contains("any(");
        if is_agg {
            if let Some(as_pos) = trimmed.to_uppercase().find(" AS ") {
                let alias = trimmed[as_pos + 4..].trim().to_string();
                agg_aliases.insert(alias);
            }
        }
    }
    (agg_aliases, passthrough_cols)
}

/// If `expr` matches `any(column_name) AS column_name` (alias == inner
/// column), return the column name.  Otherwise return `None`.
fn extract_any_passthrough(expr: &str) -> Option<String> {
    let start = expr.find("any(")?;
    if start > 0 && expr.as_bytes()[start - 1].is_ascii_alphanumeric() {
        return None;
    }
    let inner_start = start + 4;
    let close = expr[inner_start..].find(')')?;
    let inner_col = expr[inner_start..inner_start + close].trim();
    if inner_col.is_empty() || inner_col.contains('(') {
        return None;
    }
    let as_pos = expr.to_uppercase().find(" AS ")?;
    let alias = expr[as_pos + 4..].trim();
    if alias == inner_col { Some(inner_col.to_string()) } else { None }
}

/// Insert a table alias right after the FROM `schema.table` reference.
/// `"SELECT ... FROM schema.table GROUP BY ..."` becomes
/// `"SELECT ... FROM schema.table _src GROUP BY ..."`.
fn inject_table_alias(before_group_by: &str, alias: &str) -> String {
    if let Some(from_pos) = before_group_by.rfind(" FROM ") {
        let after_from = &before_group_by[from_pos + 6..];
        let table_end = after_from.find(" WHERE ")
            .or_else(|| after_from.find(" PREWHERE "))
            .unwrap_or(after_from.len());
        let insert_at = from_pos + 6 + table_end;
        format!("{} {alias}{}", &before_group_by[..insert_at], &before_group_by[insert_at..])
    } else {
        before_group_by.to_string()
    }
}

/// Check if a filter condition column references a subquery aggregate alias.
fn references_agg_alias(column: &str, agg_aliases: &HashSet<String>) -> bool {
    let base = column.trim_matches('`');
    agg_aliases.contains(base)
}

/// Split a FilterNode into (pushdown, outer) parts for subquery WHERE injection.
/// Conditions on aggregate columns stay on the outer query; all others are pushed down.
fn split_filter_for_subquery(
    node: FilterNode,
    ...
    match node {
        ...
        FilterNode::Condition { ref column, .. } => {
            if references_agg_alias(column, agg_aliases) {
        ...
        FilterNode::And(children) => {
            ...
            for child in children {
                let (p, o) = split_filter_for_subquery(child, agg_aliases);
                if !p.is_empty() { pushdown.push(p); }
                if !o.is_empty() { outer.push(o); }
            }
            let p_node = match pushdown.len() {
                0 => FilterNode::Empty,
                1 => pushdown.into_iter().next().unwrap(),
                _ => FilterNode::And(pushdown),
            };
            let o_node = match outer.len() {
                0 => FilterNode::Empty,
                1 => outer.into_iter().next().unwrap(),
                _ => FilterNode::And(outer),
            };
            (p_node, o_node)
        }
        FilterNode::Or(children) => {
            let any_agg = children.iter().any(|c| filter_references_agg(c, agg_aliases));
            if any_agg {
...
/// Check if any condition in a FilterNode tree references an aggregate alias.
fn filter_references_agg(node: &FilterNode, agg_aliases: &HashSet<String>) -> bool {
    match node {
        FilterNode::Empty => false,
        FilterNode::Condition { column, .. } => references_agg_alias(column, agg_aliases),
        FilterNode::And(children) | FilterNode::Or(children) => {
            children.iter().any(|c| filter_references_agg(c, agg_aliases))
        }
...
/// Detect wrapped columns of the form `FN(`inner`)` and return `("FN", "inner")`.
/// Used inside ArrayIncludes lambda remap so that hex-normalized columns like
/// `lower(`log_arg_values`)` get compiled as `lower(_pN)` instead of being passed
/// through verbatim (which would apply `lower()` to an `Array(String)` column).
fn split_wrapped_column(column: &str) -> Option<(&str, &str)> {
    if !column.ends_with("`)") {
        return None;
    }
    let open_paren = column.find("(`")?;
    let wrapper = &column[..open_paren];
    if wrapper.is_empty() || !wrapper.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return None;
    }
    let inner_start = open_paren + 2;
    let inner_end = column.len() - 2;
    if inner_end <= inner_start {
        return None;
    }
    let inner = &column[inner_start..inner_end];
    if inner.contains('`') {
        return None;
    }
    Some((wrapper, inner))
...
mod tests {
    ...
    fn test_uniq_uses_native_function() {
        let ir = QueryIR {
            ...
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 10, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
    ...
    #[test]
    fn test_auto_having_drops_zero_row_global_aggregate() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
                SelectExpr::Aggregate { function: "MAX".into(), column: "p".into(), alias: "m".into(), condition: None },
            ],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 1, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        assert!(r.sql.contains("HAVING count() > 0"), "expected auto HAVING, got: {}", r.sql);
    }

    #[test]
    fn test_auto_having_drops_on_group_by_too() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
                SelectExpr::Column { column: "k".into(), alias: None },
                SelectExpr::Aggregate { function: "SUM".into(), column: "v".into(), alias: "s".into(), condition: None },
            ],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec!["k".into()], order_by: vec![], limit: 10, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        assert!(r.sql.contains(" GROUP BY `k`"), "got: {}", r.sql);
        assert!(r.sql.contains("HAVING count() > 0"), "expected auto HAVING after GROUP BY, got: {}", r.sql);
    }

    #[test]
    fn test_auto_having_skipped_for_simple_user_having() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
                SelectExpr::Column { column: "g".into(), alias: None },
                SelectExpr::Aggregate { function: "SUM".into(), column: "a".into(), alias: "s".into(), condition: None },
            ],
            filters: FilterNode::Empty,
            having: FilterNode::Condition { column: "g".into(), op: CompareOp::Gt, value: SqlValue::Int(0) },
            group_by: vec!["g".into()], order_by: vec![], limit: 5, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        assert!(!r.sql.contains("HAVING count() > 0"), "user HAVING should replace auto HAVING, got: {}", r.sql);
        assert!(r.sql.contains("HAVING `g` > ?"), "expected user HAVING, got: {}", r.sql);
    }

    #[test]
    fn test_no_auto_having_without_aggregation() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![SelectExpr::Column { column: "x".into(), alias: None }],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 10, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        assert!(!r.sql.contains("HAVING"), "non-aggregate select should not get HAVING, got: {}", r.sql);
    }

    #[test]
    fn test_auto_having_on_merge_function_column() {
        let ir = QueryIR {
            cube: "T".into(), schema: "db".into(), table: "t".into(),
            selects: vec![
                SelectExpr::Column { column: "sumMerge(vol_state)".into(), alias: None },
            ],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec!["d".into()], order_by: vec![], limit: 1, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        assert!(r.sql.contains("HAVING count() > 0"), "Merge() column is aggregate, got: {}", r.sql);
    }
    ...
    fn test_limit_by() {
        let ir = QueryIR {
            ...
            use_final: false, joins: vec![], custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
        let sql = &r.sql;
    ...
    fn test_limit_by_with_offset() {
        ...
        let r = ch().compile(&ir);
        assert!(r.sql.contains("LIMIT 2, 5 BY `token`, `wallet`"), "multi-column LIMIT BY with offset, got: {}", r.sql);
    }
    ...
    fn test_array_includes_single_condition() {
        let ir = QueryIR {
            ...
            group_by: vec![], order_by: vec![], limit: 10, offset: 0,
            limit_by: None, use_final: false, joins: vec![],
            custom_query_builder: None, from_subquery: None, required_group_by: vec![],
        };
        let r = ch().compile(&ir);
    ...
    /// When `normalize_hex_addresses` wraps an ArrayIncludes child column as
    /// `lower(`col`)`, the compile step must still remap it to `lower(_pN)`
    /// instead of emitting `lower(`col`)` verbatim (which would fail at
    /// ClickHouse with "Illegal type Array(String) of argument of function lower").
    #[test]
    fn test_array_includes_hex_normalized_column_remaps_to_wrapped_param() {
        let ir = QueryIR {
            cube: "Events".into(), schema: "dexes_dwd2".into(),
            table: "bsc_logs".into(),
            selects: vec![SelectExpr::Column { column: "ts".into(), alias: None }],
            filters: FilterNode::ArrayIncludes {
                array_columns: vec![
                    "log_arg_names".into(),
                    "log_arg_types".into(),
                ...
                element_conditions: vec![vec![
                    FilterNode::Condition {
                        column: "log_arg_names".into(),
                        op: CompareOp::Eq,
                        value: SqlValue::String("token1".into()),
                    },
                    FilterNode::Condition {
                        column: "log_arg_types".into(),
                        op: CompareOp::Eq,
                        value: SqlValue::String("address".into()),
                    },
                    FilterNode::Condition {
                        column: "log_arg_values".into(),
                        op: CompareOp::Eq,
                        value: SqlValue::String("0x5a49ce64a1e44f6fce07e9ff38f54dde8a8a0e94".into()),
                    },
                ]],
            },
            having: FilterNode::Empty,
        ...
        let mut filters = ir.filters.clone();
        filters.normalize_hex_addresses();
        let ir = QueryIR { filters, ..ir };
        let sql = ch().compile(&ir).sql;
        assert!(sql.contains("lower(_p2)"),
            "hex-normalized array element should become lower(_pN), got: {sql}");
        assert!(!sql.contains("lower(`log_arg_values`)"),
            "must not apply lower() to the Array(String) column itself, got: {sql}");
    }
}
