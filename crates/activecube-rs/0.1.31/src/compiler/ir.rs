            ...
            FilterNode::Condition { column, op, value } => {
                ...
                for rule in rules {
                    match rule {
                        ...
                        FilterValueTransform::FormatAddress { column: rule_col, format } => {
                            if rule_col != column {
                                continue;
                            }
                            let SqlValue::String(s) = value else { continue };
                            ...
                            let apply = |part: &str| -> String {
                                let trimmed = part.trim();
                                use crate::cube::definition::AddressFormat::*;
                                let encoded = match format {
                                    EvmEip55 => crate::compiler::address::to_eip55(trimmed),
                                    EvmLower => crate::compiler::address::to_evm_lower(trimmed),
                                    Base58 => crate::compiler::address::normalize_base58(trimmed),
                                    Base58Check => crate::compiler::address::normalize_base58check(trimmed),
                                    Upper => Some(trimmed.to_uppercase()),
                                    Lower => Some(trimmed.to_lowercase()),
                                };
                                encoded.unwrap_or_else(|| part.to_string())
                            };
                            if is_list {
                                let joined = s
                                    .split(',')
