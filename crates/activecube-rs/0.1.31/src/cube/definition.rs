...
/// Address encoding format used by `FilterValueTransform::FormatAddress`.
/// Controls how a filter value is re-encoded to match the canonical form
/// stored in the underlying database.
///
/// Design rationale: chains have different canonical address encodings
/// (EVM ≠ Solana ≠ Bitcoin). Rather than case-insensitive comparison
/// (which loses checksum info and fights ETL storage), each cube declares
/// the expected format per column and the filter value is normalized
/// before SQL compilation.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AddressFormat {
    /// EVM EIP-55 checksum: `0x` + 40 hex with per-nibble case derived
    /// from keccak-256(lowercase_address). Standard for Ethereum ecosystem.
    EvmEip55,
    /// EVM lower-case: `0x` + 40 lower hex. Legacy / non-checksum.
    EvmLower,
    /// Solana / Bitcoin base58: pass-through when valid base58, otherwise
    /// left unchanged (case-sensitive alphabet so no normalization needed).
    Base58,
    /// Base58Check (with 4-byte sha256d checksum): re-encode when valid,
    /// otherwise left unchanged.
    Base58Check,
    /// Plain upper-case on the entire string (non-address use cases).
    Upper,
    /// Plain lower-case on the entire string (non-address use cases).
    Lower,
}

/// Per-column filter value transformation applied before SQL compilation.
/// Generic string manipulation for upstream-compat prefixes, unit conversions,
/// or address encoding normalization.
#[derive(Debug, Clone)]
pub enum FilterValueTransform {
    ...
    /// Re-encode filter value to a canonical address format matching the
    /// DB storage convention. Invalid values (not recognizable as the
    /// declared format) pass through unchanged so non-address filters on
    /// the same column are not mangled.
    ///
    /// Example: `FormatAddress { column: "pool_smart_contract", format: EvmEip55 }`
    /// rewrites `0x5d4f3c6fa...` and `0x5D4F3C6fA...` both into the
    /// canonical EIP-55 form `0x5d4F3C6fA16908609BAC31Ff148Bd002AA6b8c83`.
    FormatAddress { column: String, format: AddressFormat },
}
