...
pub fn parse_cube_query(
    ...
    // Note: `normalize_hex_addresses` (auto-detecting 0x+40hex and forcing
    // `lower(col) = lower(value)` comparison) was removed in Round 18 —
    // it conflicts with the EIP-55 storage convention used by ChainStream's
    // ETL. Address normalization is now opt-in per column via
    // `FilterValueTransform::FormatAddress { column, format }`.
    ...
    for ti in time_intervals {
        ...
        let alias = dim_agg_key(&ti.field_path);
