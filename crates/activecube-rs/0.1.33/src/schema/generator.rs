    ...
    let mut field = Field::new(
        ...
        move |ctx| {
            ...
            FieldFuture::new(async move {
                ...
                let table_transform = ctx.ctx.data::<TableNameTransform>().ok();
                    ...
                    if let Some(jd) = join_def {
                        if let Some(target_cube) = registry.get(&jd.target_cube) {
                            let join_expr = build_join_expr(
                                jd, target_cube, &sub_field, network, join_idx, chain_ctx,
                                table_transform,
                            );
                ...
                if let (Some(cctx), Some(transform)) = (chain_ctx, table_transform) {
...
fn build_join_expr(
    ...
    network: &str,
    join_idx: usize,
    chain_ctx: Option<&ChainContext>,
    table_transform: Option<&TableNameTransform>,
) -> JoinExpr {
    ...
    let requested_columns: Vec<String> = target_flat.iter()
        .filter(|(path, _)| requested_paths.contains(path))
        .map(|(_, dim)| dim.column.clone())
        .collect();

    let (target_schema, target_table) = target_cube.resolve_table(network, &requested_columns);
    let target_table = if let (Some(cctx), Some(transform)) = (chain_ctx, table_transform) {
        transform(&target_table, cctx)
    } else {
        target_table
    };
...
mod tests {
    use super::*;
    use crate::cube::definition::{dim, join_def, CubeBuilder, DimType, TableRoute};
    use crate::cube::registry::CubeRegistry;
    use crate::sql::clickhouse::ClickHouseDialect;
    use std::sync::{Arc, Mutex};

    #[tokio::test]
    async fn join_targets_use_table_routes_and_table_name_transform() {
        let source = CubeBuilder::new("Source")
            .schema("db")
            .table("{chain}_source")
            .dimension(dim("Id", "source_id", DimType::String))
            .join(join_def("joinTarget", "Target", &[("source_id", "target_id")]))
            .table_route(TableRoute {
                schema: "db".into(),
                table_pattern: "{chain}_source_alt".into(),
                available_columns: vec!["source_id".into()],
                priority: 1,
            })
            .chain_groups(vec![ChainGroup::Evm])
            .build();

        let target = CubeBuilder::new("Target")
            .schema("db")
            .table("{chain}_target")
            .dimension(dim("Id", "target_id", DimType::String))
            .table_route(TableRoute {
                schema: "db".into(),
                table_pattern: "{chain}_target_alt".into(),
                available_columns: vec!["target_id".into()],
                priority: 1,
            })
            .chain_groups(vec![ChainGroup::Evm])
            .build();
        ...
        let captured_sql_for_executor = captured_sql.clone();
        let executor: QueryExecutor = Arc::new(move |sql, _bindings| {
            let captured_sql = captured_sql_for_executor.clone();
            Box::pin(async move {
                captured_sql.lock().unwrap().push(sql);
                Ok(Vec::<crate::response::RowMap>::new())
            })
        });

        let table_transform: TableNameTransform = Arc::new(|table: &str, ctx: &ChainContext| {
            let table = if ctx.network == "polygon" {
                table.strip_prefix("polygon_")
                    .map(|rest| format!("pol_{rest}"))
                    .unwrap_or_else(|| table.to_string())
            } else {
                table.to_string()
            };

            // Compatibility fallback: dataset is accepted but must not create
            // dataset-suffixed tables.
            let _ = ctx.extra.get("dataset");
            table
        });

        let schema = build_schema(
            CubeRegistry::from_cubes(vec![source, target]),
            Arc::new(ClickHouseDialect::new()),
            executor,
            SchemaConfig {
                chain_groups: vec![ChainGroupConfig {
                    name: "EVM".into(),
                    group: ChainGroup::Evm,
                    networks: vec!["polygon".into()],
                    has_network_arg: true,
                    extra_args: vec![WrapperArg {
                        name: "dataset".into(),
                        type_ref: "Dataset".into(),
                        description: "dataset compat fallback".into(),
                        values: Some(vec!["realtime".into()]),
                    }],
                    network_enum_name: None,
                    network_aliases: HashMap::new(),
                }],
                ...
                stats_callback: None,
                table_name_transform: Some(table_transform),
                extra_types: vec![Enum::new("Dataset").item(EnumItem::new("realtime"))],
            },
        ).expect("schema should build");

        let response = schema
            .execute("{ EVM(network: polygon, dataset: realtime) { Source { Id joinTarget { Id } } } }")
            .await;
        ...
        let sql = captured_sql
            ...
            .first()
            .cloned()
            .expect("query should execute SQL");

        assert!(sql.contains("FROM `db`.`pol_source_alt`"), "main route should be canonicalized: {sql}");
        assert!(sql.contains("JOIN `db`.`pol_target_alt`"), "join route should be canonicalized: {sql}");
        assert!(!sql.contains("polygon_"), "SQL must not use non-canonical polygon tables: {sql}");
        assert!(!sql.contains("_realtime"), "dataset fallback must not use realtime suffix: {sql}");
        assert!(!sql.contains("_archive"), "dataset fallback must not use archive suffix: {sql}");
    }
}
