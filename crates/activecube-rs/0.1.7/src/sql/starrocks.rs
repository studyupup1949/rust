...
impl SqlDialect for StarRocksDialect {
    fn compile(&self, ir: &QueryIR) -> CompileResult {
        ...
        CompileResult { sql, bindings, alias_remap: vec![] }
    }
...
mod tests {
    ...
    fn test_simple_select() {
        ...
        let r = make_dialect().compile(&ir);
        assert_eq!(r.sql, "SELECT `tx_hash`, `buy_amount` FROM `dexes_dwd`.`sol_activities` LIMIT 10");
    ...
    fn test_where_and_order() {
        ...
        let r = make_dialect().compile(&ir);
        assert!(r.sql.contains("WHERE (`buy_amount_usd` > ? AND `success` = ?)"));
        assert!(r.sql.contains("ORDER BY `buy_amount_usd` DESC"));
    ...
    fn test_or_condition() {
        ...
        let r = make_dialect().compile(&ir);
        assert!(r.sql.contains("(`buy_token_symbol` = ? OR `sell_token_symbol` = ?)"));
    ...
    fn test_aggregate_with_having() {
        ...
        let r = make_dialect().compile(&ir);
        assert!(r.sql.contains("GROUP BY `buy_token_symbol`"));
        assert!(r.sql.contains("HAVING SUM(`buy_amount_usd`) > ?"), "HAVING clause should not backtick-wrap aggregate expressions, got: {r}", r = r.sql);
    ...
    fn test_offset() {
        ...
        let r = make_dialect().compile(&ir);
        assert!(r.sql.ends_with("LIMIT 10 OFFSET 20"));
    }

    #[test]
    fn test_from_subquery() {
        let ir = QueryIR {
            cube: "T".into(), schema: "dwd".into(), table: "t".into(),
            selects: vec![SelectExpr::Column { column: "val".into(), alias: None }],
            filters: FilterNode::Empty, having: FilterNode::Empty,
            group_by: vec![], order_by: vec![], limit: 5, offset: 0,
            limit_by: None, use_final: false, joins: vec![], custom_query_builder: None,
            from_subquery: Some("SELECT val FROM dwd.a UNION ALL SELECT val FROM dwd.b".into()),
        };
        let r = make_dialect().compile(&ir);
        assert!(r.sql.contains("FROM (SELECT val FROM dwd.a UNION ALL SELECT val FROM dwd.b) AS _t"),
            "StarRocks should use subquery in FROM, got: {}", r.sql);
        assert!(!r.sql.contains("FROM `dwd`.`t`"),
