                    ...
                    match sel {
                        ...
                        SelectExpr::DimAggregate { agg_type, value_column, compare_column, alias, condition } => {
                            ...
                            let expr = match condition {
                                None => format!("{func}(`{value_column}`, `{compare_column}`) AS `{alias}`"),
                                Some(cond) => format!("{func}If(`{value_column}`, `{compare_column}`, {cond}) AS `{alias}`"),
                            };
                            sub_parts.push(expr);
                        }
                    }
    ...
    fn compile_inner(
        ...
        for sel in &augmented_selects {
            if let SelectExpr::DimAggregate { agg_type, value_column, compare_column, alias, .. } = sel {
                ...
                let expr = format!("{func}(`{value_column}`, `{compare_column}`)");
