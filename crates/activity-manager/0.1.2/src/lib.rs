...
impl<H: ActivityHost> ActivityManager<H> {
    ...
    ///
    /// 如果栈为空则返回 `None`。
    pub fn current_route(&self) -> Option<H::Route> {
        self.stack.last().map(|act| act.route())
    }
...
mod tests {
    ...
    #[test]
    fn test_current_route_empty_stack() {
        let manager = create_manager();
        assert_eq!(manager.current_route(), None);
    }

    #[test]
    fn test_current_route_with_activity() {
        let mut manager = create_manager();
        let mut context = Vec::new();

        manager.start_activity(&mut context, TestRoute::StandardPage);
        assert_eq!(manager.current_route(), Some(TestRoute::StandardPage));

        manager.start_activity(&mut context, TestRoute::SingleTopPage);
        assert_eq!(manager.current_route(), Some(TestRoute::SingleTopPage));
    }

    #[test]
    fn test_current_route_after_back() {
        let mut manager = create_manager();
        let mut context = Vec::new();

        manager.start_activity(&mut context, TestRoute::StandardPage);
        manager.start_activity(&mut context, TestRoute::SingleTopPage);
        manager.back(&mut context);

        assert_eq!(manager.current_route(), Some(TestRoute::StandardPage));
    }

    #[test]
    fn test_current_route_single_task_reuse() {
        let mut manager = create_manager();
        let mut context = Vec::new();

        manager.start_activity(&mut context, TestRoute::StandardPage);
        manager.start_activity(&mut context, TestRoute::SingleTaskPage);
        manager.start_activity(&mut context, TestRoute::StandardPage);
        manager.start_activity(&mut context, TestRoute::SingleTaskPage);

        assert_eq!(manager.current_route(), Some(TestRoute::SingleTaskPage));
        assert_eq!(manager.stack_len(), 2);
    }
