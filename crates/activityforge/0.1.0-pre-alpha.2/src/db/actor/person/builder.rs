...
use crate::db::{
    Db, Follower, Grant, Inbox, Iri, Key, Name, Outbox, Person, TableEntry, TableType, Uuid,
};
use crate::{Error, Result, Role, util};

/// Represents a builder struct for a [Person].
///
/// Allows for users to have a high-level interface for creating all of the
/// sub-record entries that make up a [Person] entry.
...
impl PersonBuilder {
    ...
    pub fn new<I: Into<Iri>, N: Into<Name>>(id: I, name: N) -> Result<Self> {
        ...
        let id = Person::TABLE.id_from_uuid(&id.into(), uuid)?;
        let name = name.into();
        ...
        let inbox = Inbox::new()
            ...
            .with_id(inbox_id)
            .with_actor(TableEntry::create(TableType::Person, uuid));
    ...
    ///
    /// Optional, a random [Uuid] is assigned in [PersonBuilder::new].
    ...
    /// Sets the ID IRI for the [Person].
    ///
    /// Optional, set a different ID than provided in [PersonBuilder::new].
    ...
    ///
    /// Optional, set a different name than provided in [PersonBuilder::new].
    ...
    /// Sets the content for the [Person].
    ...
    /// Sets the summary for the [Person].
    ...
    /// Sets the [Inbox] for the [Person].
    ///
    /// Only needed if the default [Inbox] created by [PersonBuilder::new] is incorrect.
    ...
    /// Sets the [Outbox] for the [Person].
    ///
    /// Only needed if the default [Outbox] created by [PersonBuilder::new] is incorrect.
    ...
    pub fn password(self, val: Password) -> Self {
        Self {
            password: Some(val),
            ..self
        }
    ...
    /// Sets the list of [Person] [Follower]s.
    ...
    /// Builds the [Person] record, and inserts all sub-records into the database.
    pub async fn build(mut self, db: &Db) -> Result<Person> {
        let uuid = self.uuid;
        ...
        if let Some(content) = self.content {
            person.set_content(content);
        }

        if let Some(summary) = self.summary {
            person.set_summary(summary);
        }
        ...
        person.find_or_create_tx(&mut dbtx).await?;

        dbtx.commit().await.map(|_| person).map_err(Error::from)
    }
}
