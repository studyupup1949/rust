...
create_actor! {
    ...
    ///
    /// ```rust
    /// use activityforge::{Roadmap, context};
    ...
    /// assert_eq!(
    ///     serde_json::from_str::<Roadmap>(json_str.as_str()).unwrap(),
    ///     repository
    /// );
    /// # }
    /// ```
    ...
    ///
    /// ```rust
    /// use activityforge::{ActorType, Roadmap, VocabularyTypes, context};
    /// use activitystreams_vocabulary::{
    ///     Iri, MultibaseHeader, MultibasePublicKey, Multikey, MultikeyPublicKey, Name,
    /// };
    ///
    ...
    /// # }
    /// ```
    Roadmap: crate::ActorType::Roadmap {}
}
