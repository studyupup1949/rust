...
macro_rules! create_object {
    ...
    (
        ...
        pub struct $ty<Vocab: $crate::ActivityVocabulary = $crate::VocabularyTypes> {
            ...
            #[serde(skip_serializing_if = "crate::Iri::is_empty")]
            pub(crate) id: $crate::Iri,
        ...
        impl<Vocab: $crate::ActivityVocabulary + From<$vocab_ty>>  $ty<Vocab> {
            $crate::paste! {
                #[doc = "Creates a new [" $ty "]."]
                pub fn new() -> Self {
                    Self::new_kind(Vocab::from($vocab_ty::$ty))
                }
...
/// - `base_field`: only used if for `Object`-derived types with a field containing the `Object`.
#[macro_export]
macro_rules! object_field_access {
    ($ty:ident) => {
        $crate::field_access! {
            $ty {
                /// Provides the ActivityStream Vocabulary `type` of an [Object](crate::Object).
                kind: as_ref { $crate::VocabularyTypes },
    ...
    ($ty:ident $({
        ...
        impl From<$ty> for $crate::Object {
            ...
            fn from(val: $ty) -> Self {
                ...
                Self {
                    context_property: val.context_property,
                    kind: val.kind,
                    id: val.id,
                    name: val.name,
                    name_map: val.name_map,
                    attributed_to: val.attributed_to,
                    audience: val.audience,
                    content: val.content,
                    content_map: val.content_map,
                    summary: val.summary,
                    summary_map: val.summary_map,
                    context: val.context,
...
macro_rules! impl_into_items {
    ($ty:ident) => {
        ...
        impl From<Vec<$ty>> for $crate::Items {
        ...
        impl From<&[$ty]> for $crate::Items {
            fn from(val: &[$ty]) -> Self {
                Self::List(val.into_iter().cloned().map(|i| i.into()).collect())
            }
        }

        impl<const N: usize> From<&[$ty; N]> for $crate::Items {
...
macro_rules! impl_into_ordered_items {
    ($ty:ident) => {
        impl From<$ty> for $crate::OrderedItems {
            fn from(val: $ty) -> Self {
                Self::Single(val.into())
            }
        }
        ...
        impl From<Vec<$ty>> for $crate::OrderedItems {
            fn from(val: Vec<$ty>) -> Self {
                Self::List(val.into_iter().map(|i| i.into()).collect())
            }
        }
        ...
        impl<const N: usize> From<&[$ty; N]> for $crate::OrderedItems {
            fn from(val: &[$ty; N]) -> Self {
                Self::List(val.into_iter().cloned().map(|i| i.into()).collect())
            }
        }
