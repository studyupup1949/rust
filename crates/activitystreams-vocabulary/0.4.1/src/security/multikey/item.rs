...
impl<T: Into<MultikeyItem>> From<T> for MultikeyItems {
    fn from(val: T) -> Self {
        Self::single(val)
    }
}

impl<T: Into<MultikeyItem>> From<Vec<T>> for MultikeyItems {
    fn from(val: Vec<T>) -> Self {
        Self::list(val)
    }
}

impl<T: Into<MultikeyItem> + Clone> From<&[T]> for MultikeyItems {
    fn from(val: &[T]) -> Self {
        Self::list(val.iter().cloned())
    }
}

impl<T: Into<MultikeyItem> + Clone, const N: usize> From<&[T; N]> for MultikeyItems {
    fn from(val: &[T; N]) -> Self {
        Self::list(val.iter().cloned())
    }
}

impl<T: Into<MultikeyItem>, const N: usize> From<[T; N]> for MultikeyItems {
    fn from(val: [T; N]) -> Self {
        Self::list(val)
    }
}
