...
impl<T: Into<KeyItem>> From<T> for KeyItems {
    fn from(val: T) -> Self {
        Self::single(val)
    }
}

impl<T: Into<KeyItem>> From<Vec<T>> for KeyItems {
    fn from(val: Vec<T>) -> Self {
        Self::list(val)
    }
}

impl<T: Into<KeyItem> + Clone> From<&[T]> for KeyItems {
    fn from(val: &[T]) -> Self {
        Self::list(val.iter().cloned())
    }
}

impl<T: Into<KeyItem> + Clone, const N: usize> From<&[T; N]> for KeyItems {
    fn from(val: &[T; N]) -> Self {
        Self::list(val.iter().cloned())
    }
}

impl<T: Into<KeyItem>, const N: usize> From<[T; N]> for KeyItems {
    fn from(val: [T; N]) -> Self {
        Self::list(val)
    }
}
