...
impl Error {
    ...
    pub fn link<S: Into<String>>(err: S) -> Self {
        Self::Link(err.into())
    }
    ...
    pub fn key<S: Into<String>>(err: S) -> Self {
        Self::Key(err.into())
    }

    /// Creates a new Source [Error].
    pub fn source<S: Into<String>>(err: S) -> Self {
        Self::Source(err.into())
    }

    /// Creates a new List [Error].
    pub fn list<S: Into<String>>(err: S) -> Self {
        Self::List(err.into())
    }
}
...
impl core::fmt::Display for Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::Link(err) => write!(f, "link: {err}"),
            Self::Tombstone(err) => write!(f, "tombstone: {err}"),
            Self::Multikey(err) => write!(f, "multikey: {err}"),
            Self::Key(err) => write!(f, "key: {err}"),
            Self::Source(err) => write!(f, "source: {err}"),
            Self::List(err) => write!(f, "list: {err}"),
        }
    }
}
