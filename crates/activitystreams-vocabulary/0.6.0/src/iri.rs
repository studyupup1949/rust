...
use crate::{
    Error, Link, Result, create_list, impl_default, impl_display, impl_into_item, impl_into_items,
};
...
create_list! {
    ...
    ///
    /// ```rust
    /// use activitystreams_vocabulary::{Iri, Iris};
    ///
    /// let iri0 = Iri::try_from("https://example.dev/alice").unwrap();
    /// let iri1 = Iri::try_from("https://example.dev/bob").unwrap();
    ///
    /// let json_str = format!(
    /// r#"[
    ///   "{iri0}",
    ...
    ///         );
    ///
    /// let iris = Iris::list([iri0, iri1]);
    ///
    /// assert_eq!(serde_json::to_string_pretty(&iris).unwrap(), json_str);
    /// assert_eq!(serde_json::from_str::<Iris>(&json_str).unwrap(), iris);
    /// ```
    Iris: Iri
}
...
#[deprecated(
    since = "0.4.2",
    note = "Fails to handle the edge-case of mixed list of IRIs and Links. Use [LinkItems](crate::LinkItems) instead"
)]
