...
macro_rules! impl_into_link {
    ...
    ($ty:ident<$vocab_ty:ty> $({
        ...
        impl From<&$ty<$vocab_ty>> for $crate::Link<$vocab_ty> {
            ...
            fn from(val: &$ty<$vocab_ty>) -> Self {
                let mut link = Self::new()
                    .with_kind(val.kind().clone())
                    .with_href(val.href().clone());

                if let Some(ctx) = val.context_property() {
                    link.set_context_property(ctx.clone());
                } else {
                    link.unset_context_property();
                }

                if let Some(name) = val.name() {
                    link.set_name(name.clone());
                }

                if let Some(name_map) = val.name_map() {
                    link.set_name_map(name_map.clone());
                }
                ...
                if let Some(hreflang) = val.hreflang() {
                    link.set_hreflang(hreflang.clone());
                }

                if let Some(media_type) = val.media_type() {
                    link.set_media_type(media_type.clone());
                }

                if let Some(preview) = val.preview() {
                    link.set_preview(preview.clone());
                }
                ...
                let mut extra_fields = val.extra_fields().map(|i| i.clone());
                $(
        ...
        impl TryFrom<$crate::Link<$vocab_ty>> for $ty<$vocab_ty> {
            type Error = $crate::Error;

            fn try_from(val: $crate::Link<$vocab_ty>) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
        ...
        impl TryFrom<&$crate::Link<$vocab_ty>> for $ty<$vocab_ty> {
            ...
            fn try_from(val: &$crate::Link<$vocab_ty>) -> $crate::Result<Self> {
                ...
                if let Some(field_val) = extra_fiels.get(&[<$field _name>])  {
                    let val = ::serde_json::to_string(&field_val)
                        .and_then(|s| ::serde_json::from_str::<$field_ty>(&s))
