...
macro_rules! create_object {
    ...
    (
        $(#[$doc:meta])*
        $ty:ident: $vocab_path:ident :: $vocab_ty:ident :: $vocab_var:ident {
        $(
            $(#[$field_serde:meta])*
            $field:ident: $field_ty:ty $(,)?
        )*
    ...
    (
        ...
        pub struct $ty<Vocab: $crate::ActivityVocabulary = $crate::VocabularyTypes> {
            ...
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) attachment: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            ...
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) url: Option<$crate::LinkItems>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) preview: Option<Box<$crate::Item>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) replies: Option<Box<$crate::Collection>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) tag: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) to: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) bto: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) cc: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) bcc: Option<Box<$crate::Items>>,
            #[serde(skip_serializing_if = "Option::is_none")]
            ...
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) source: Option<$crate::ContentItem>,
            #[serde(skip_serializing_if = "Option::is_none")]
            pub(crate) proof: Option<$crate::DataIntegrityProof>,
        ...
        impl<Vocab: $crate::ActivityVocabulary> TryFrom<&$ty<Vocab>> for $ty<Vocab> {
            type Error = $crate::Error;

            fn try_from(val: &$ty<Vocab>) -> $crate::Result<Self> {
                Ok(val.clone())
            }
        }
    };
...
macro_rules! field_rename {
    (
        ...
        let mut field_name = stringify!($field).to_lower_camel_case();
        $(
        let fs = stringify!($field_serde);
        if let Some(idx) = fs.find("rename") {
            let (_, rem) = fs.split_at(idx);
            // strip the "rename" prefix
            let r = rem
                .trim_start()
                .strip_prefix("rename")
                .unwrap_or_default();
            ...
            // find the next '"' to extract the field name contents
            if let Some(idx) = r.find('"') {
               let (b, _) = r.split_at(idx);
                                ...
                                match key.as_str() {
                                    ...
                                    "attachment" => {
                                        if attachment.is_some() {
                                            return Err(de::Error::duplicate_field("attachment"));
                                        }
                                        attachment = Some(map.next_value()?);
                                    }
                                    "attributedTo" => {
    ...
    ($ty:ident) => {
        ...
        $crate::field_access! {
            $ty<Vocab> {
                ...
                source: option_ref { $crate::ContentItem },
                /// Represents a [DataIntegrityProof](https://www.w3.org/TR/vc-data-integrity/#dataintegrityproof).
                proof: option_ref { $crate::DataIntegrityProof },
                /// Identifies one or more links to representations of the object
                url: option_ref { $crate::LinkItems },
            }
        }
        ...
        $crate::field_access! {
            $ty<Vocab> {
                ...
                attachment: option_box_deref { $crate::Items },
    ...
    ($ty:ident<$vocab_ty:ty> $({
        ...
        impl From<&$ty<$vocab_ty>> for $crate::Object<$vocab_ty> {
            $crate::paste! {
                ...
                fn from(val: &$ty<$vocab_ty>) -> Self {
                    let mut obj = Self::new().with_kind(val.kind().clone());

                    if let Some(ctx) = val.context_property() {
                        obj.set_context_property(ctx.clone());
                    } else {
                    ...
                    if let Some(name_map) = val.name_map() {
                        obj.set_name_map(name_map.clone());
                    }

                    if let Some(att) = val.attachment() {
                        obj.set_attachment(att.clone());
                    }

                    if let Some(attr) = val.attributed_to() {
                        obj.set_attributed_to(attr.clone());
                    }

                    if let Some(audience) = val.audience() {
                        obj.set_audience(audience.clone());
                    }
                    ...
                    if let Some(content_map) = val.content_map() {
                        obj.set_content_map(content_map.clone());
                    }

                    if let Some(summary) = val.summary() {
                        obj.set_summary(summary.clone());
                    }

                    if let Some(summary_map) = val.summary_map() {
                        obj.set_summary_map(summary_map.clone());
                    }

                    if let Some(context) = val.context() {
                        obj.set_context(context.clone());
                    }

                    if let Some(generator) = val.generator() {
                        obj.set_generator(generator.clone());
                    }

                    if let Some(icon) = val.icon() {
                        obj.set_icon(icon.clone());
                    }

                    if let Some(image) = val.image() {
                        obj.set_image(image.clone());
                    }

                    if let Some(in_reply_to) = val.in_reply_to() {
                        obj.set_in_reply_to(in_reply_to.clone());
                    }

                    if let Some(location) = val.location() {
                        obj.set_location(location.clone());
                    }

                    if let Some(url) = val.url() {
                        obj.set_url(url.clone());
                    }

                    if let Some(preview) = val.preview() {
                        obj.set_preview(preview.clone());
                    }

                    if let Some(replies) = val.replies() {
                        obj.set_replies(replies.clone());
                    }

                    if let Some(tag) = val.tag() {
                        obj.set_tag(tag.clone());
                    }
                    ...
                    if let Some(bcc) = val.bcc() {
                        obj.set_bcc(bcc.clone());
                    }

                    if let Some(media_type) = val.media_type() {
                        obj.set_media_type(media_type.clone());
                    }

                    if let Some(start_time) = val.start_time() {
                        obj.set_start_time(start_time.clone());
                    }

                    if let Some(end_time) = val.end_time() {
                        obj.set_end_time(end_time.clone());
                    }

                    if let Some(published) = val.published() {
                        obj.set_published(published.clone());
                    }

                    if let Some(updated) = val.updated() {
                        obj.set_updated(updated.clone());
                    }

                    if let Some(duration) = val.duration() {
                        obj.set_duration(duration.clone());
                    }

                    if let Some(source) = val.source() {
                        obj.set_source(source.clone());
                    }

                    let mut extra_fields = val.extra_fields().cloned();
                    $(
                    use $crate::heck::ToLowerCamelCase;
        ...
        impl TryFrom<$crate::Object<$vocab_ty>> for $ty<$vocab_ty> {
            type Error = $crate::Error;

            fn try_from(val: $crate::Object<$vocab_ty>) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
        ...
        impl TryFrom<&$crate::Object<$vocab_ty>> for $ty<$vocab_ty> {
            ...
            $crate::paste! {
                ...
                fn try_from(val: &$crate::Object<$vocab_ty>) -> $crate::Result<Self> {
                    ...
                    let obj_kind_str = obj.kind().to_string();
                    let val_kind_str = val.kind().to_string();

                    if obj_kind_str != val_kind_str {
                        return Err($crate::Error::vocabulary(format!("invalid kind: {val_kind_str}, expected: {obj_kind_str}")));
                    }
                    ...
                    if let Some(extra) = val.extra_fields().and_then(|f| f.get(&[<$field _name>]))  {
                        let field_val_str = ::serde_json::to_string(&extra)?;
                        let field_val = ::serde_json::from_str::<$field_ty>(&field_val_str)?;
                        obj.$field = field_val;
                    }
                    )*
                    )?
...
macro_rules! impl_into_item {
    ($ty:ident, object) => {
        ...
        impl TryFrom<$crate::Item> for $ty {
            ...
            fn try_from(val: $crate::Item) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
        ...
        impl TryFrom<&$crate::Item> for $ty {
            ...
            fn try_from(val: &$crate::Item) -> $crate::Result<Self> {
                match val {
                    $crate::Item::Object(obj) => obj.as_ref().try_into().map_err(|err| {
                        $crate::Error::vocabulary(format!("item: invalid object: {err}"))
                    }),
                    _ => Err($crate::Error::vocabulary(format!(
                        "item: invalid item: {val}"
                    ))),
                }
            }
        }
    ...
    ($ty:ident, link) => {
        ...
        impl TryFrom<&$crate::Item> for $ty {
            ...
            fn try_from(val: &$crate::Item) -> $crate::Result<Self> {
                match val {
                    $crate::Item::Link(link) => link.as_ref().try_into().map_err(|err| {
                        $crate::Error::vocabulary(format!("item: invalid link: {err}"))
                    }),
                    _ => Err($crate::Error::vocabulary(format!(
                        "item: invalid item: {val}"
                    ))),
    ...
    ($ty:ident, iri) => {
        ...
        impl TryFrom<&$crate::Item> for $ty {
            ...
            fn try_from(val: &$crate::Item) -> $crate::Result<Self> {
                match val {
                    $crate::Item::Iri(iri) => iri.as_ref().try_into().map_err(|err| {
                        $crate::Error::vocabulary(format!("item: invalid IRI: {err}"))
                    }),
                    _ => Err($crate::Error::vocabulary(format!(
                        "item: invalid item: {val}"
                    ))),
        ...
        impl TryFrom<Box<$crate::Item>> for $ty {
            type Error = $crate::Error;

            fn try_from(val: Box<$crate::Item>) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
...
macro_rules! impl_into_items {
    ($ty:ident) => {
        ...
        impl TryFrom<$crate::Items> for $ty {
            type Error = $crate::Error;

            fn try_from(val: $crate::Items) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
        ...
        impl TryFrom<&$crate::Items> for $ty {
            ...
            fn try_from(val: &$crate::Items) -> $crate::Result<Self> {
                match val {
                    $crate::Items::Single(item) => item.try_into(),
                    $crate::Items::List(list) => {
                        list.iter().filter_map(|i| i.try_into().ok()).next().ok_or(
                            $crate::Error::vocabulary(format!("items: no valid item: {list:?}")),
                        )
                    }
                }
            }
        ...
        impl TryFrom<Box<$crate::Items>> for $ty {
            type Error = $crate::Error;

            fn try_from(val: Box<$crate::Items>) -> $crate::Result<Self> {
                (&val).try_into()
            }
        }
