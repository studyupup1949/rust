// actix-logger, a logging middleware for actix-web.
    ...
    S: Service<ServiceRequest, Response = ServiceResponse<B>, Error = Error>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let response = res.response().status().as_u16();
...
impl Default for Format {
    ...
    fn default() -> Self {
        Self::new(r#"%a "%r" %s %b "%{Referer}i" "%{User-Agent}i" %T"#)
    }
}
...
impl FormatText {
    fn render(
        ...
        match self {
            Self::Str(ref string) => fmt.write_str(string),
            Self::Percent => "%".fmt(fmt),
            Self::ResponseSize => size.fmt(fmt),
            Self::Time => {
    ...
    fn render_response(&mut self, res: &ServiceResponse) {
        match self {
            Self::ResponseStatus => *self = Self::Str(format!("{}", res.status().as_u16())),

            Self::ResponseHeader(ref name) => {
                ...
                *self = Self::Str(s.to_string());
            }

            Self::CustomResponse(_, res_fn) => {
                let text = match res_fn {
                    Some(res_fn) => Self::Str(res_fn.call(res)),
                    None => Self::Str("-".to_owned()),
                };
    ...
    fn render_request(&mut self, now: OffsetDateTime, req: &ServiceRequest) {
        match self {
            ...
            Self::UrlPath => *self = Self::Str(req.path().to_string()),
            Self::RequestTime => *self = Self::Str(now.format(&Rfc3339).unwrap()),
            Self::RequestHeader(ref name) => {
            ...
            Self::RemoteAddr => {
                let s = if let Some(peer) = req.connection_info().peer_addr() {
                    Self::Str((*peer).to_string())
                } else {
                    Self::Str("-".to_string())
                };
                *self = s;
            }
            Self::RealIpRemoteAddr => {
                let s = if let Some(remote) = req.connection_info().realip_remote_addr() {
                    Self::Str(remote.to_string())
                } else {
                    Self::Str("-".to_string())
                };
                *self = s;
            }
            Self::CustomRequest(_, request_fn) => {
                let s = match request_fn {
                    Some(f) => Self::Str(f.call(req)),
                    None => Self::Str("-".to_owned()),
                };
