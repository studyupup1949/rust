...
#[derive(Debug, Clone)]
pub struct RateLimitBuilder<Id, Backend> {
    _p: PhantomData<Id>,
    backend: Backend,
    per_user: LimitType,
    per_ip: LimitType,
...
impl<Id, Backend> RateLimitBuilder<Id, Backend> {
    ...
    pub fn finish(self) -> RateLimit<Id, Backend> {
        RateLimit {
            _p: PhantomData,
            backend: Rc::new(self.backend),
...
fn headers(remaining: LimitType, limit: LimitType) -> Vec<(HeaderName, HeaderValue)> {
    ...
    static LIMIT: Lazy<HeaderName> =
        Lazy::new(|| HeaderName::from_bytes(b"X-RateLimit-Limit").unwrap());
    static RESET: Lazy<HeaderName> =
        Lazy::new(|| HeaderName::from_bytes(b"X-RateLimit-Reset").unwrap());
    static REMAINING: Lazy<HeaderName> =
        Lazy::new(|| HeaderName::from_bytes(b"X-RateLimit-Remaining").unwrap());

    let reset = seconds_elapsed_for_next_hour();

    vec![
        (HeaderName::from(&*REMAINING), HeaderValue::from(remaining)),
        (HeaderName::from(&*LIMIT), HeaderValue::from(limit)),
        (HeaderName::from(&*RESET), HeaderValue::from(reset)),
    ]
}
