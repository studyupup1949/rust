...
impl Connection {
    ...
    // sends message from single connection if given
    // condition is true.
...
impl Room {
    // checks if a connection with given id exist and if it's 
    // not add a connection with given id and Session to a room.
    ...
    // checks if a connection exist and returns it
    // as an option.
    ...
    // broadcastes the message if given condition for connection instances
    // is true.
...
impl Broadcaster {
    ...
    // does all the setup basically. You don't have to use other functions 
    // for all the grouping of rooms and connections. You can give the same 
    // room id for all instances if you don't want to seperate communication
    // groups. But you have to give different connection id's to each session,
    // otherwise it'll introduce bugs.
    ...
    // this function check if a room exist and if it's exist
    // returns it, if it's not then creates it. If you just want to
    // check if a room exist, use .check() instead.
    ...
    // it scans a room with given id and it returns it if it's exist.
    // if there is a risk that room isn't exist than use ".check_room()"
    pub fn room(&mut self, id: String) -> &mut Room {
        return self.rooms.iter_mut().find(|room| room.id == *id).unwrap();
    }

    // checks a room and if it's exist, returns a mutable
    // reference of that room.
    ...
    // it returns room if exist with given ip. Use .handle_room() method if you want
    // to create a room with given ip.
    ...
    // it removes a connection and returns the session struct of it.
    // since async closures not stable yet, we cannot close the
    // actual "Session" implementation in that method. For making that
    // cleanup, we have to get actual Session implementation and close
    // that connection manually - check out the example and readme.
