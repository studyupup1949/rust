...
/// Generate an [`AsyncActor`](crate::util::AsyncActor) type-saftey wrapper.
///
...
///
/// This builds an actor from an asynchronous closure of type `Fn(Context, InboxEvent) ->
/// Future<Item=(Context, Option<OutboxEvent)>`. This closure is passed in an owned context value to
/// avoid some lifetime hell that occurs with async, and the closure must return the (potentially
/// modified) context value back to the executor, so that it can be passed back into the closure
/// when handling the next [`Event`](crate::traits::Event).
///
