...
impl std::fmt::Debug for ClientQuotas {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ClientQuotas")
            .field("count", &self.request_count)
            .field("limit", &self.request_limit)
            .finish()
    }
}
...
impl DeviceId {
    pub fn generate() -> Self {
        let mut builder = machineid_rs::IdBuilder::new(machineid_rs::Encryption::SHA256);
        builder.add_component(machineid_rs::HWIDComponent::SystemID);
        builder.add_component(machineid_rs::HWIDComponent::Username);
        let key = std::env::var("ACTNEL_DEVICE_KEY").unwrap();
        let hwid = builder.build(&key).unwrap();
        DeviceId(hwid)
    }
}