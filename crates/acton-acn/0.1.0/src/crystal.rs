...
impl Crystal {
    ...
    fn find_cell(&mut self, target: u8) -> (usize, usize, usize) {
        loop {
            let x = self.rng.gen_range(0..self.config.size);
            let y = self.rng.gen_range(0..self.config.size);
            let z = self.rng.gen_range(0..self.config.size);
    ...
    pub fn decode(&mut self, coords: &[(usize, usize, usize)], expected_len: usize) -> Option<Vec<u8>> {
        ...
        for (i, &(x, y, z)) in coords.iter().enumerate() {
            if i < total_bits && self.get(x, y, z) == 1 {
                bits[i] = 1;
                self.set(x, y, z, 0);
            }
        }
