...
async fn http_ping_handler(Json(req): Json<HttpPingRequest>) -> std::result::Result<Json<HttpPongResponse>, (StatusCode, String)> {
    ...
    let mut client = PingServiceClient::connect("http://localhost:9090")
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("gRPC connection failed: {}", e)))?;
    ...
    let response = client
        .ping(grpc_request)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("gRPC call failed: {}", e)))?;
