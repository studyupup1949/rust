...
fn write_dac_b() {
    ...
    let spi = MockSpi::new(&trans);

    let mut dac = Ad57xxShared::new_ad57x4(spi);
    dac.set_dac_output(ad57xx::ad57x4::ChannelQuad::DacB, 0xF00F).unwrap();
    dac.destroy().done();
    ...
    dac.set_dac_output(ad57xx::ad57x2::ChannelDual::DacB, 0xF00F).unwrap();
    dac.destroy().done();
}
#[test]
fn read_config() {
