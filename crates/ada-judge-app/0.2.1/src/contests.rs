...
use ada_judge_public_models::contests::{
    ContestRequest, DeleteContestRequest, PublicContestConfig,
};
use anyhow::anyhow;
...
pub async fn load_contests(state: Arc<Mutex<AppState>>, weak: Weak<AJMainWindow>) {
    if let Err(e) = load_contests_impl(state.clone(), weak.clone()).await {
...
pub async fn create_contest_impl(
    ...
    let starts_at = starts_at.try_into()?;
    let ends_at = ends_at.try_into()?;
...
pub async fn update_contest_impl(
    ...
    let starts_at = starts_at.try_into().ok().or(None);
    let ends_at = ends_at.try_into().ok().or(None);
    ...
    let request = ContestRequest {
        ...
        starts_at: if let Some(starts_at) = starts_at {
            starts_at
        } else {
            contest.starts_at.clone()
        },
        ends_at: if let Some(ends_at) = ends_at {
            ends_at
        } else {
            contest.ends_at.clone()
        },
        statements_url: if statements_url.is_empty() {
            contest.statements_url.clone()
        } else {
            statements_url
        },
    };
...
pub async fn delete_contest_impl(
    ...
    let token = state.lock().await.token.clone();
    let base_url = state.lock().await.base_url.clone();
    let request = DeleteContestRequest {
        login,
        password,
        password_confirmation,
        deletion_confirmation,
