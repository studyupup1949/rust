...
impl From<(&PublicContestConfig, &PrivateUserData)> for AJContest {
    fn from((contest, user): (&PublicContestConfig, &PrivateUserData)) -> Self {
        ...
        Self {
            ...
            upsolving_opened: contest.upsolving_opened,
            hide_solutions: contest.hide_solutions,
            ...
            status: match now.cmp(&contest.starts_at) {
                ...
                _ => match now.cmp(&contest.ends_at) {
                    ...
                    _ => {
                        if contest.upsolving_opened {
                            AJContestStatus::Upsolving
                        } else {
                            AJContestStatus::Finished
                        }
                    }
                },
            },
...
impl From<ProblemType> for AJProblemType {
    fn from(problem_type: ProblemType) -> Self {
        match problem_type {
            ...
            ProblemType::RunTwiceInteractive => AJProblemType::RunTwiceInteractive,
            ProblemType::RunTwiceFirstInteractive => AJProblemType::RunTwiceFirstInteractive,
            ProblemType::RunTwiceSecondInteractive => AJProblemType::RunTwiceSecondInteractive,
        }
    }
}
...
impl From<(&PublicProblemConfig, &PrivateUserData)> for AJProblem {
    fn from((problem, user): (&PublicProblemConfig, &PrivateUserData)) -> Self {
        Self {
            ...
            merge_subgroups: problem.merge_subgroups,
...
impl From<(String, &Submission)> for AJSubmission {
    fn from((username, submission): (String, &Submission)) -> Self {
        Self {
            ...
            created_at: DateTime::<Local>::from(submission.created_at).into(),
        }
    }
}
