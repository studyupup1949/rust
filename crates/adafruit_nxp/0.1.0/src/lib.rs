...
pub struct Magnetometer {
    ...
    pub raw_data: Data,
    /// Scaled data from the magnetometer sensor in ***µT***.
    pub scaled_data: ScaledData,
...
impl Magnetometer {
    ...
    /// #### Get Scaled data from the magnetometer sensor on the X-axis.
    /// Returns the magnetic field in µT as a f32.
    pub fn get_scaled_x(&self) -> f32 {
        self.scaled_data.x
    }

    /// #### Get Scaled data from the magnetometer sensor on the Y-axis.
    /// Returns the magnetic field in µT as a f32.
    pub fn get_scaled_y(&self) -> f32 {
        self.scaled_data.y
    }

    /// #### Get Scaled data from the magnetometer sensor on the Z-axis.
    /// Returns the magnetic field in µT as a f32.
    pub fn get_scaled_z(&self) -> f32 {
        self.scaled_data.z
    }

    /// #### Get Scaled data from the magnetometer sensor on the all axis.
    ...
    I2C: Write<Error = E> + WriteRead<Error = E>,
{
    ...
    pub fn read_data(&mut self) -> Result<(), SensorError<E>> {
        ...
        let factor = match self.range_gyro {
            GyroRange::Range250dps => config::GYRO_SENSITIVITY_250DPS * RAD2DEG,
            GyroRange::Range500dps => config::GYRO_SENSITIVITY_500DPS * RAD2DEG,
            GyroRange::Range1000dps => config::GYRO_SENSITIVITY_1000DPS * RAD2DEG,
            GyroRange::Range2000dps => config::GYRO_SENSITIVITY_2000DPS * RAD2DEG,
        };
