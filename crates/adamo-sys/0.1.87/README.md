...
All prebuilts are video-enabled. For other targets (e.g. Intel macOS,
Windows), or for offline builds on the download targets, use
`ADAMO_LIB_DIR` (below).

## Host prerequisites

The prebuilt links GStreamer, so the host needs it installed to build
and run:

- Debian/Ubuntu: `libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libv4l-dev`
- macOS (Apple Silicon): `brew install gstreamer glib`

The download targets additionally need `curl`. See
`prebuilt/README.md` for the full host matrix.
