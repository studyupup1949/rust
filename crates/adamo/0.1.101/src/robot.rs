...
pub struct VideoOptions {
    ...
    /// Keep the robot and other healthy tracks running if this track fails.
...
impl RawVideoOptions {
    fn new(options: &VideoOptions) -> Result<Self> {
        ...
        raw.allow_missing = if options.allow_missing { 1 } else { 0 };
...
pub struct Robot {
    // `pub(crate)` so the `audio` module can add `attach_audio*` methods on the
    // same handle (audio implies video, so this module is always present when
    // audio is).
    pub(crate) raw: NonNull<adamo_sys::adamo_robot_t>,
}
...
impl Robot {
    ...
    /// Block driving the pipeline until [`stop`](Self::stop) is called or a
    /// track fails. The robot may be shared across scoped threads so one
    /// thread can run it while another requests shutdown.
    ...
    /// Request graceful shutdown of a blocked [`run`](Self::run) call.
    pub fn stop(&self) -> Result<()> {
        let rc = unsafe { adamo_sys::adamo_robot_stop(self.raw.as_ptr()) };
        if rc == 0 { Ok(()) } else { Err(last_ffi_error()) }
    }
}
...
/// A video track registered on a [`Robot`]. The independent sender handle can
/// be moved to another thread and used while [`Robot::run`] is blocking.
/// Raw frames or complete encoded access units are pushed via
/// [`VideoTrack::send`].
