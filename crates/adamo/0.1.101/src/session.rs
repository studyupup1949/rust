...
impl Session {
    ...
    /// Declare a robot-owned, timed, looping task run published on
...
/// A robot-owned, timed, looping task run with an idle gap between reps. Tied
/// to its parent [`Session`] by lifetime. Wire the pedal to
/// [`advance`](Self::advance); read the cursor and timing from
/// `{robot}/state/task_run`.
