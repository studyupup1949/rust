    ...
    pub fn new(api_base_url: Url, auth_token: String) -> Self {
        ...
        if let Ok(cert_path) = std::env::var("SSL_CERT_FILE") {
            match std::fs::read(&cert_path) {
                Ok(pem_data) => match reqwest::tls::Certificate::from_pem_bundle(&pem_data) {
                    Ok(certs) => {
                        tracing::info!("Loading {} certificate(s) from SSL_CERT_FILE={cert_path:?}", certs.len());
                        for cert in certs {
                            builder = builder.add_root_certificate(cert);
                        }
                    }
                    Err(e) => {
                        tracing::warn!("SSL_CERT_FILE={cert_path:?} contains invalid PEM: {e}");
                    }
                },
                Err(e) => {
                    tracing::warn!("SSL_CERT_FILE={cert_path:?} could not be read: {e}");
                }
            }
        }
