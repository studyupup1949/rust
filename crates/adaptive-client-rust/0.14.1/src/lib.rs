use std::{fmt::Display, fs::File, io::Read, path::Path, time::Duration, time::SystemTime};
...
impl AdaptiveError {
    /// Short, structured description of a retryable error for logging.
    /// Omits response bodies to avoid noisy/repeated log output during retries.
    fn retryable_summary(&self) -> String {
        match self {
            AdaptiveError::HttpError(e) => {
                if e.is_timeout() {
            ...
            AdaptiveError::HttpMiddlewareError(e) => match e {
            ...
            AdaptiveError::HttpStatusError { status, .. } => {
                format!("HTTP {status}")
            }
            AdaptiveError::ChunkedUploadInitFailed { status, .. } => {
                format!("chunked upload init: HTTP {status}")
            }
            AdaptiveError::ChunkedUploadPartFailed {
                part_number,
                status,
                ..
            } => {
                format!("chunked upload part {part_number}: HTTP {status}")
            }
            other => other.to_string(),
        }
    }

    /// Returns true if this error is transient and the operation can be safely retried.
    pub fn is_retryable(&self) -> bool {
        match self {
            // Network-level: timeouts, connection resets, DNS failures
            AdaptiveError::HttpError(e) => e.is_timeout() || e.is_connect(),
            AdaptiveError::HttpMiddlewareError(e) => match e {
                reqwest_middleware::Error::Reqwest(e) => e.is_timeout() || e.is_connect(),
                reqwest_middleware::Error::Middleware(_) => false,
            },
            // transient and safe-to-retry HTTP codes
            AdaptiveError::HttpStatusError { status, .. }
            | AdaptiveError::ChunkedUploadInitFailed { status, .. }
            | AdaptiveError::ChunkedUploadPartFailed { status, .. } => {
                matches!(parse_status_code(status), Some(408 | 429 | 502 | 503 | 504))
            }
            _ => false,
        }
    }
...
#[graphql(
    schema_path = "schema.gql",
    query_path = "src/graphql/artifact.graphql",
    response_derives = "Debug, Clone"
)]
...
#[graphql(
    schema_path = "schema.gql",
    query_path = "src/graphql/update_role.graphql",
    response_derives = "Debug, Clone"
)]
...
/// Configures exponential backoff retry for transient HTTP errors.
#[derive(Clone, Debug)]
...
async fn retry<T, F, Fut>(policy: Option<&RetryPolicy>, mut op: F) -> Result<T>
where
    F: FnMut() -> Fut,
    ...
    match policy {
        Some(policy) => {
            op.retry(policy.backoff())
                ...
                .notify(|err, dur| {
                    tracing::warn!(
                        error_kind = err.retryable_summary(),
                        retry_in = ?dur,
...
impl AdaptiveClient {
    /// Create an `AdaptiveClient` with a pre-built `ClientWithMiddleware`.
    pub fn new(
        client: ClientWithMiddleware,
        api_base_url: Url,
        auth_token: String,
        ...
        T::ResponseData: DeserializeOwned,
    {
        ...
        retry(self.retry_policy.as_ref(), || async {
            let response = self
                .client
    ...
    pub async fn publish_recipe<P: AsRef<Path>>(
        ...
        config_entrypoint: Option<String>,
    ) -> Result<publish_custom_recipe::PublishCustomRecipeCreateCustomRecipe> {
        let input = publish_custom_recipe::CreateRecipeInput {
            name: name.to_string(),
    ...
    pub async fn update_role(
    ...
    ) -> Result<update_role::UpdateRoleUpdateRole> {
        let variables = update_role::Variables {
            input: update_role::RoleUpdate {
                role: role.to_string(),
        ...
        let response_data = self.execute_query(UpdateRole, variables).await?;
        Ok(response_data.update_role)
    }
    ...
    pub async fn get_artifact(
        ...
        project: &str,
        id: Uuid,
    ) -> Result<Option<get_artifact::GetArtifactArtifact>> {
        let variables = get_artifact::Variables {
            project: project.to_string(),
            id,
        };

        let response_data = self.execute_query(GetArtifact, variables).await?;
    ...
    pub async fn upload_part_simple(
        ...
        // make data cheaply clonable (for retries)
    ...
    async fn init_chunked_upload(&self, total_parts: u64) -> Result<String> {
        self.init_chunked_upload_with_content_type(total_parts, "application/jsonl")
            .await
    }
