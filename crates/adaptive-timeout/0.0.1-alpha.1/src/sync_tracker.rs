...
///
/// let now = Instant::now();
/// let tracker = SyncLatencyTracker::<u32>::with_defaults(now);
///
...
> {
    ...
    histograms: DashMap<D, SlidingWindowHistogram<I, N>>,
}

// DashMap<D, V> is Send + Sync when D: Send + Sync and V: Send + Sync.
// SlidingWindowHistogram is Send (no interior mutability, all fields are Send).
// It is NOT Sync on its own (requires &mut), but DashMap's internal RwLock
// provides synchronized access. So SyncLatencyTracker is Send + Sync.

impl<D, I, const N: usize> SyncLatencyTracker<D, I, N>
where
    ...
    I: clock::Instant,
{
    ...
    /// duration.
    #[inline]
    pub fn record_latency_at(&self, dest: &D, earlier: I, now: I) -> Duration {
...
mod tests {
    ...
    fn make_tracker(now: Instant) -> TestTracker {
        ...
        SyncLatencyTracker::new(config, now)
    }

    #[test]
    fn no_data_returns_none() {
        let now = Instant::now();
        let tracker = make_tracker(now);
    ...
    fn insufficient_samples_returns_none() {
        let now = Instant::now();
        let tracker = make_tracker(now); // min_samples = 5
    ...
    fn concurrent_record_same_destination() {
        ...
        let now = Instant::now();
        let tracker = Arc::new(make_tracker(now));
        let num_threads = 8;
