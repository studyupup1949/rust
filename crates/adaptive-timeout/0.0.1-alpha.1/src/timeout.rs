...
/// let timeout = AdaptiveTimeout::with_defaults();
///
...
impl AdaptiveTimeout {
    ...
    /// Creates a new `AdaptiveTimeout` with default configuration.
    ...
    #[inline]
    pub fn select_timeout<'a, D, I, const N: usize>(
        &self,
        tracker: &mut LatencyTracker<D, I, N>,
    ...
    pub fn select_timeout_ms<'a, D, I, const N: usize>(
        &self,
        tracker: &mut LatencyTracker<D, I, N>,
        destinations: impl IntoIterator<Item = &'a D>,
        attempt: u32,
    ...
    #[cfg(feature = "sync")]
    #[inline]
    pub fn select_timeout_sync<'a, D, I, const N: usize>(
        &self,
        tracker: &crate::sync_tracker::SyncLatencyTracker<D, I, N>,
        destinations: impl IntoIterator<Item = &'a D>,
        attempt: u32,
        now: I,
    ) -> Duration
    ...
    /// [`SyncLatencyTracker`](crate::SyncLatencyTracker).
    #[cfg(feature = "sync")]
    pub fn select_timeout_sync_ms<'a, D, I, const N: usize>(
        &self,
        tracker: &crate::sync_tracker::SyncLatencyTracker<D, I, N>,
        destinations: impl IntoIterator<Item = &'a D>,
        attempt: u32,
...
mod tests {
    ...
    fn make_tracker_and_timeout(now: Instant) -> (LatencyTracker<u32>, AdaptiveTimeout) {
        ...
        (
            LatencyTracker::new(tracker_config, now),
    ...
    fn fallback_exponential_backoff_no_data() {
        let now = Instant::now();
        let (mut tracker, timeout) = make_tracker_and_timeout(now);
    ...
    mod sync_tests {
        ...
        fn make_sync_tracker_and_timeout(
            now: Instant,
        ) -> (SyncLatencyTracker<u32>, AdaptiveTimeout) {
            let tracker_config = TrackerConfig {
                min_samples: 5,
                ..TrackerConfig::default()
            };
            ...
            (
                SyncLatencyTracker::new(tracker_config, now),
                AdaptiveTimeout::new(timeout_config),
            )
        }
        ...
        fn sync_fallback_exponential_backoff_no_data() {
            let now = Instant::now();
            let (tracker, timeout) = make_sync_tracker_and_timeout(now);
        ...
        fn sync_matches_mutable_tracker_results() {
            ...
            let mut mutable_tracker = LatencyTracker::<u32>::new(tracker_config, now);
            let sync_tracker = SyncLatencyTracker::<u32>::new(tracker_config, now);
