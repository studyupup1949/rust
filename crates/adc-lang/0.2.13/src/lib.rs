...
use malachite::base::num::arithmetic::traits::{Factorial, NegAssign, Pow};
use malachite::base::num::basic::traits::{NegativeOne, Zero, One, Two};
use malachite::base::num::conversion::traits::{PowerOf2DigitIterable, RoundingFrom, WrappingFrom};
...
/// Dense map of character bytes to commands
...
#[inline(always)] fn byte_cmd(b: u8) -> Command {
...
#[inline(always)] const fn upper_hex_to_nibble(b: u8) -> Option<u8> {
	match b {
...
#[inline(always)] const fn mixed_ascii_to_digit(b: u8) -> Option<u8> {
	match b {
		b'0'..=b'9' => Some(unsafe{b.unchecked_sub(0x30)}),	//SAFETY: underflow is impossible
...
#[must_use] pub enum ExecResult {
	...
	/// `q` command + exit code (early exit from current set of commands)
	SoftQuit(u8),

	/// `` `q `` command + exit code (early exit from entire interpreter, if applicable)
	HardQuit(u8),
...
/// - `kill`: Optional kill signal, receiving `()` terminates the interpreter as soon as it parses its next command.
...
///
/// # Safety
/// If built without the `no_os` feature (as is the default), passing `restrict = false` enables OS interactions from within the language. This function is marked unsafe as a speedbump.
///
/// When working with untrusted input and/or OS features are not desired, [`interpreter_no_os`] and [`interpreter_simple`] provide safe wrappers.
...
) -> std::io::Result<ExecResult>
{
	...
	macro_rules! debug {
    	($s:expr) => {
			if ll == LogLevel::Debug {
				let mut lock = io.lock().unwrap();
				writeln!(lock.2, "\tDEBUG: {th_name}{}", $s)?;
				lock.2.flush()?;
				drop(lock);
			}
		};
		($f:literal, $($s:expr),*) => {
			if ll == LogLevel::Debug {
				let mut lock = io.lock().unwrap();
				writeln!(lock.2, "\tDEBUG: {th_name}{}", format!($f, $($s),*))?;
				lock.2.flush()?;
				drop(lock);
			}
		};
	...
	let mut rptr: Option<Rational> = None;	//global scope to allow setting by a macro

	let mut rng: Option<RandomPrimitiveInts<u64>> = None;

	let mut call: Vec<(Utf8Iter, Natural)> = vec![(start, Natural::ONE)];
	...
	'mac: while let Some((mac, count)) = call.last_mut() {	//while call stack has contents, macro scope:
					...
					match b {
							...
							if let Some(top) = st.mstk.pop() {
								...
								match Utf8Iter::try_macros(&top, sec.as_deref()) {
									Ok((stk, ret)) => {
										...
										if mac.is_finished() && *count == Natural::ONE {	//tail call optimization: discard current macro if this is the last command
											call.pop();
										}
									...
									match &*va {
										Value::N(r) => {
											match Integer::try_from(r) {
												...
												Ok(i) if let Ok(n) = Natural::try_from(&i) => {	//custom seed
						...
						b'_' => {	//word commands
							...
							match &word[..] {	//word commands:
								...
								b"thlist" => {
									push!(Value::A(st.regs.list_threads().into_iter().map(Value::N).collect()));
								},
								b"trim" => {
									st.trim();
									RE_CACHE.clear();
					...
					match b {
						...
						b'l' => {
							if let Some(rv) = st.regs.try_get(&ri).and_then(|reg| reg.v.last()) {
								...
								synerr!('l', "Register {} is empty", reg_index_pretty(&ri));
							}
						},
						b'L' => {
						...
						b'X' => {
							if let Some(true) = st.regs.try_get(&ri).map(|reg| reg.th.is_some()) {
								valerr!('X', "Register {} is already running a thread", reg_index_pretty(&ri));
							}
							else if let Some(va) = st.mstk.pop() {
								if let Value::S(sa) = &*va {
									let name = format!("{th_name}{}: ", reg_index_pretty(&ri));
									let th_st = if alt {st.clone()} else {State::default()};
									match ThreadHandle::spawn(&name, th_st, sa.to_owned(), Arc::clone(&io), ll, restrict) {
										Ok(th) => {
											st.regs.get_mut(&ri).th = Some(th);
										},
										Err(e) => {
											st.mstk.push(va);
						...
						b'j' => {
							let ri_nice = reg_index_pretty(&ri);
							if let Some(reg) = st.regs.try_get_mut(&ri) && let Some(th) = reg.th.take() {
								if alt {th.kill();}
								let (mut vals, th_res) = th.join();
								match th_res {
									Err(e) => {
						...
						b'J' => {
							if let Some(th) = st.regs.try_get(&ri).and_then(|reg| reg.th.as_ref()) {
								let mut bz = BitVec::new();
								bz.push(th.is_finished());
								push!(Value::B(bz));
							}
							else {
								valerr!('J', "Register {} is not running a thread", reg_index_pretty(&ri));
							}
						},
						_ => unreachable!()
					}
				...
				Lit => {
					match b {
						...
						b'\'' | b'0'..=b'9' | b'.' | b'@' => {	//numbers
							...
							match (b == b'\'', ibase > Natural::const_from(36) || ibase == Natural::ONE) {
								(false, high_base) => {	//plain
									mac.back();
									ibase = if high_base {Natural::const_from(10)} else {ibase};	//if plain but I>36, interpret as I=10
								...
								(true, true) => {	//enclosed
									...
									match ns.split(|b| *b == b'@').collect::<Vec<&[u8]>>()[..] {
										[mpart] => {	//only mantissa
											if let Some((b'`', rest)) = mpart.split_first() {	//remove and apply negative sign
												alt = true;
												ms = rest.to_vec();
											}
											else {
									...
									match frs.split(|b| *b == b'`').collect::<Vec<&[u8]>>()[..] {
										...
										[fpart, rpart] => {
											if ibase != Natural::ONE {
												fs = fpart.to_vec();
												rs = rpart.to_vec();
											}
											else {
												synerr!('\'', "Recurring digits (`) are not allowed in factoradic numbers");
												alt = false;
												continue 'cmd;
											}
										},
									...
									if ibase != Natural::ONE { for d in ipart.iter().chain(fpart.iter()).chain(rpart.iter()) {
										if *d >= ibase {
									...
									else { for (i, d) in ipart.iter().rev().enumerate().chain(fpart.iter().enumerate()) {
										if *d >= i+2 {	//highest allowed values are '... 3 2 1.1 2 3 ...'
											synerr!('\'', "Digit {} is too high for factoradic place {} away from the '.'", d, i);
											alt = false;
											continue 'cmd;
										}
									}}
							...
							let mut r;
							match (m_empty, ibase != Natural::ONE) {
								(true, true) => {	//empty normal
									r = Rational::ZERO;
								},
								(true, false) => {	//empty factoradic
									if let Some(i) = exp.take() {	//consume "exponent" here
										if let Ok(u) = u64::try_from(i) {
											r = Rational::from(Natural::factorial(u+1));
										}
										else {
											r = Rational::from_naturals(Natural::ONE, Natural::factorial(i.unsigned_abs()+1));
										}
									}
									else {
										r = Rational::ZERO;
								...
								(false, false) => {	//non-empty factoradic
									if let Some(i) = exp.take() {	//consume "exponent" here
										if let Ok(u) = u64::try_from(i) {
											let mut fi = fpart.into_iter();
											ipart.extend(fi.by_ref().chain(std::iter::repeat(Natural::ZERO)).take(u as usize));
											fpart = fi.collect();
										}
										else {
											let mut temp = ipart.split_off(ipart.len().saturating_sub(i.unsigned_abs() as usize));
											temp.append(&mut fpart);
											fpart = temp;
										}
									}

									let mut ii = ipart.iter().rev().peekable();
									let mut fi = fpart.iter().peekable();
									let mut d = Natural::TWO;	//current place value

									r = if let Some(n) = ii.next() {n.into()} else {Rational::ZERO};	//1! digit
									//further place values are symmetrically reciprocal
									for n in malachite::natural::exhaustive::exhaustive_natural_range_to_infinity(Natural::const_from(3)) {
										if let Some(id) = ii.next() {
											r += Rational::from(id * &d);
										}
										if let Some(fd) = fi.next() {
											r += Rational::from_naturals_ref(fd, &d);
										}
										if ii.peek().or_else(|| fi.peek()).is_none() {break;}
										d *= n;
									}
		...
		*count -= Natural::ONE;	//finish current repetition
