...
impl Default for Repo {
    fn default() -> Self {
        ...
        repo.gitlab_token = get_from_gitconfig("add-remote.gitLabToken");
        repo.github_token = get_from_gitconfig("add-remote.gitHubToken")
...
impl Repo {
    ...
    pub fn set_remote(&self) {
        println!();
        let remotes_before = git_remote_verbose_output();
        ...
        let remotes_after = git_remote_verbose_output();
        ...
        let mut branches = git_branch_verbose_output(chosen_alias);
        if branches.is_empty() {
            branches = git_branch_verbose_output(&chosen_alias.to_lowercase());
        }
        println!("\n{branches}");
    }
    ...
    fn populate_local_remotes(&mut self) {
        let local_remotes_output = Command::new("git")
            .args(["remote", "show"])
            .output()
            .unwrap();
    ...
    fn suggest_fork(&self) -> Option<u64> {
        ...
        get_from_gitconfig("add-remote.preferredFork")
            .and_then(|preferred| {
                self.available_forks
    ...
    fn suggest_alias(&self) -> String {
        ...
        get_from_gitconfig(&alias_arg).unwrap_or_else(|| {
...
pub fn git_remote_verbose_output() -> String {
    let output = Command::new("git").args(["remote", "-v"]).output().unwrap();
...
fn git_branch_verbose_output(alias: &str) -> String {
...
fn get_from_gitconfig(key: &str) -> Option<String> {
    let output = Command::new("git").args(["config", key]).output().unwrap();
    if output.status.success() {
