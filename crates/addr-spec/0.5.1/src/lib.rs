...
macro_rules! check_atext {
    ($ident:ident, $name:literal) => {
        if $ident.is_empty() {
            return Err(ParseError(concat!("empty ", $name), 0));
        }
        if $ident.starts_with('.') {
            return Err(ParseError(concat!("empty label in ", $name), 0));
        }
        if $ident.ends_with('.') {
            return Err(ParseError(
                concat!("empty label in ", $name),
                $ident.len() - 1,
            ));
        }
        if let Some(index) = $ident.find("..").map(|i| i + 1) {
            return Err(ParseError(concat!("empty label in ", $name), index));
        }
        if let Some(index) = $ident.find(is_not_atext) {
            return Err(ParseError(concat!("invalid character in ", $name), index));
        }
    };
}
...
impl AddrSpec {
    ...
    fn new_impl(local_part: &str, domain: &str, literal: bool) -> Result<Self, ParseError> {
        ...
        if literal {
            ...
            check_atext!(domain, "domain");
        }
        Ok(Self {
            local_part: local_part.nfc().collect(),
            domain: (domain.nfc().collect(), literal),
        })
    }
    ...
    pub fn into_serialized_parts(self) -> (String, String) {
        let local_part = if !self.is_quoted() {
            self.local_part
        } else {
            ["\"", &quote(self.local_part()), "\""].concat()
        };
        #[cfg(feature = "literals")]
        let domain = if !self.domain.1 {
            self.domain.0
        } else {
            ["[", &self.domain.0, "]"].concat()
        };
        #[cfg(not(feature = "literals"))]
...
impl fmt::Display for AddrSpec {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        formatter.write_str(&match (self.is_quoted(), self.is_literal()) {
