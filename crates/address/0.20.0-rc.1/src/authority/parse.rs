...
use crate::{parse_lowercase, parse_port, strip_brackets};
use std::str::FromStr;
...
impl FromStr for Authority {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        parse_lowercase(s, |s| Ok(AuthorityRef::try_from(s)?.to_authority()))
    }
}
...
impl<'a> TryFrom<&'a str> for AuthorityRef<'a> {
    ...
    /// A domain name must already be lowercase, since a borrowed name cannot be normalized. Use `Authority::from_str`
