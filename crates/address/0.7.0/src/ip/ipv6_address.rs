...
mod tests {
    ...
    fn construction() {
        ...
        assert_eq!(IPv6Address::new(address).address, address);
        ...
        let result: [u8; 16] = IPv6Address::new(address).into();
        assert_eq!(result, address);

        let result: [u16; 8] = IPv6Address::new(address).into();
        assert_eq!(result, segments);

        let result: u128 = IPv6Address::new(address).into();
    ...
    fn properties() {
        ...
        assert_eq!(v6.address(), address);
        assert_eq!(v6.segments(), segments);
    }
}
