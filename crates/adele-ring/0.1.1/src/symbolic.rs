...
impl IdentityGraph {
    ...
    fn simplify_add(&self, terms: Vec<SymbolicExpr>) -> SymbolicExpr {
        ...
        // Canonicalize: sort the non-constant terms by a total order so that
        // structurally-equal sums compare equal regardless of input order.
        others.sort_by(canonical_cmp);
    ...
    fn simplify_exp(&self, x: SymbolicExpr) -> SymbolicExpr {
        ...
        // Note: Euler's identity `exp(iπ) + 1 = 0` is deliberately NOT encoded.
        // `SymbolicExpr` has no imaginary unit, so the rule is unrepresentable —
        // and the refactor's policy is to never ship a rule that cannot be
        // expressed. Add a `Complex`/`I` variant first if that identity is wanted.
        Exp(Box::new(x))
    }
...
/// A total order on expressions, used to canonicalize n-ary `Add`/`Mul` operand
/// lists. The exact ordering is unimportant; only that it is total and stable so
/// that equal multisets of terms produce identical trees (enabling identity-table
/// matches like `sin(π/6)`).
fn canonical_cmp(a: &SymbolicExpr, b: &SymbolicExpr) -> std::cmp::Ordering {
