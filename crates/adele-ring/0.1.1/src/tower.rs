...
impl TowerValue {
    ...
    fn elevate_once(&self) -> TowerValue {
        match self {
            TowerValue::Integer(i) => {
                TowerValue::Rational(RnsRational::new(i.to_bigint(), BigInt::from(1), i.basis.clone()))
            }
    ...
    pub fn to_f64(&self) -> Option<f64> {
        match self {
            TowerValue::Integer(i) => {
                Some(RnsRational::new(i.to_bigint(), BigInt::from(1), i.basis.clone()).to_f64())
            }
    ...
    pub fn digits(&self, precision: u64) -> RnsRational {
        match self {
            TowerValue::Integer(i) => {
                RnsRational::new(i.to_bigint(), BigInt::from(1), i.basis.clone())
            }
...
fn symbolic_to_value(e: &SymbolicExpr, ch: &Basis) -> Option<TowerValue> {
    match e {
...
fn symbolic_to_computable(e: &SymbolicExpr, ch: &Basis) -> Option<ComputableReal> {
    match e {
