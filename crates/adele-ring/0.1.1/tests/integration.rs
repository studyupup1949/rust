...
use adele_ring::{
    cpu::CpuBackend, gpu::GpuBackend, AlgebraicNumber, Basis, ComputableReal, Dispatcher,
...
fn cpu_gpu_bit_identical() {
    let basis = ch();
    let a = RnsBatch::from_rns_ints(&vec![RnsInt::from_i64(123, basis.clone()); 256]);
    let b = RnsBatch::from_rns_ints(&vec![RnsInt::from_i64(456, basis.clone()); 256]);

    let cpu = CpuBackend::new();
    let cpu_add = cpu.batch_add(&a, &b);
    let cpu_sub = cpu.batch_sub(&a, &b);
    let cpu_mul = cpu.batch_mul(&a, &b);

    if let Ok(gpu) = GpuBackend::try_init() {
        assert_eq!(cpu_add.data, gpu.batch_add(&a, &b).data);
        assert_eq!(cpu_sub.data, gpu.batch_sub(&a, &b).data);
        assert_eq!(cpu_mul.data, gpu.batch_mul(&a, &b).data);
    }
}
...
fn algebraic_arithmetic() {
    ...
    assert_eq!(s2.annihilating_poly.degree(), 2);
    assert!(s2.to_rational().is_none());

    // √2 · √2 = 2  (degree-1 annihilating poly => rational)
    assert_eq!(s2.mul(&s2).degree(), 1);

    // √2 · √3 = √6
    let s3 = AlgebraicNumber::sqrt(3, ch());
    assert_eq!(
        s2.mul(&s3).annihilating_poly,
        Polynomial::from_int_coeffs(&[-6, 0, 1], ch()).monic()
    );

    // √2 + √2 = 2√2  (annihilating poly x² - 8)
    assert_eq!(
        s2.add(&s2).annihilating_poly,
...
#[test]
fn dispatcher_analysis_and_provisioning() {
    let basis = Basis::standard();
    let dispatcher = Dispatcher::new(basis.clone());
    let sixth = RnsRational::from_fraction(1, 6, basis.clone());
    let quarter = RnsRational::from_fraction(1, 4, basis);
    let plan = dispatcher.plan_add(&sixth, &quarter);
    // Natural base of {1/6, 1/4} is lcm(radical(6), radical(4)) = lcm(6, 2) = 6.
    assert_eq!(plan.natural_base, 6);
    // A 5/12 result needs only a handful of bits; the standard basis fits it.
    assert!(plan.fits());
