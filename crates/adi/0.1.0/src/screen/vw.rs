/**
 * Aldaron's Device Interface - "screen/vw.rs"
...
use screen::ffi::{ NativeWindow, vulkan, string };
...
pub fn make_styles(screen: &mut Screen, extrashaders: &[Shader]) -> Vec<Style> {
	...
	let default_shaders = [
		Shader::new(screen, include_bytes!("shaders/color-vert.spv"),
			include_bytes!("shaders/color-frag.spv"), 0),
		Shader::new(screen, include_bytes!("shaders/texture-vert.spv"),
			include_bytes!("shaders/texture-frag.spv"), 1),
	];
