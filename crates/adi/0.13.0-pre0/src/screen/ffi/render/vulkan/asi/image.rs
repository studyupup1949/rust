...
/// An Image
#[derive(Clone)]
pub struct Image(Rc<ImageContext>);
...
impl Image {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            (vulkan.get().get_imgmemreq)(vulkan.get().device, image, &mut memory_reqs);

            let memory_type_index =
                get_memory_type(vulkan, memory_reqs.memory_type_bits, reqs_mask);
            ...
            (vulkan.get().bind_imgmem)(vulkan.get().device, image, memory, 0).unwrap();

            let view = super::create_img_view(
                vulkan,
                image,
                format.clone(),
