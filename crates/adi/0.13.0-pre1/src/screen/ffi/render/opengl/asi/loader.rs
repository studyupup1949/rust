...
#[cfg(windows)]
use winapi::shared::{minwindef::BOOL, ntdef::LPCSTR};
...
extern "system" {
    ...
    fn ChoosePixelFormat(a: *mut c_void, b: *const PixelFormatDescriptor) -> i32;
    fn SetPixelFormat(a: *mut c_void, b: i32, c: *const PixelFormatDescriptor) -> i32;
}
...
impl Display {
    ...
    pub fn swap(&self) {
        if unsafe { SwapBuffers(self.dc.unwrap().as_ptr()) } == 0 {
...
impl Display {
    ...
    pub fn swap(&self, lib: &Lib) {
        if unsafe { (lib.gl.eglSwapBuffers)(self.display, self.surface.unwrap().as_ptr()) } == 0 {
            panic!("Swapping Failed");
        }
    }
}
...
impl Lib {
    ...
    #[cfg(not(windows))]
    pub fn init(&self) -> (Display, i32) {
        let display = unsafe { (self.gl.eglGetDisplay)(ptr::null_mut()) };
        if display.is_null() {
            panic!("EGL: Couldn't load display.");
        }

        if unsafe { (self.gl.eglInitialize)(display, ptr::null_mut(), ptr::null_mut()) } == 0 {
        ...
        if unsafe {
            (self.gl.eglGetConfigAttrib)(display, config, EGL_NATIVE_VISUAL_ID, &mut visual_id)
        } == 0
        {
    ...
    pub fn init2(&self, display: &mut Display, window: *mut c_void) {
        ...
        let format = unsafe { ChoosePixelFormat(dc, &pixel_format) };
    ...
    pub fn init2(&self, display: &mut Display, window: EGLNativeWindowType) {
        // Create surface
        let surface = unsafe {
            (self.gl.eglCreateWindowSurface)(display.display, display.config, window, ptr::null())
        };
        ...
        if unsafe { (self.gl.eglMakeCurrent)(display.display, surface, surface, display.context) }
            == 0
        {
    ...
    pub fn load<T>(&self, name: &[u8]) -> T {
        let fn_ptr: *const c_void =
            unsafe { (self.gl.eglGetProcAddress)(name as *const _ as *const i8) };
    ...
    pub fn load<T>(&self, name: &[u8]) -> T {
        let fn_ptr: *const c_void =
            unsafe { (self.gl.wglGetProcAddress)(name as *const _ as LPCSTR) };

        if fn_ptr.is_null() {
            if let Ok(n) = unsafe {
                self.gl
                    .__lib
                    .symbol_cstr(::std::ffi::CStr::from_bytes_with_nul(name).unwrap())
            } {
                return n;
            } else {
                panic!(
