...
use screen::ffi::Matrix;

#[derive(Clone)]
#[repr(C)]
struct TransformFullUniform {
    mat4: [f32; 16],
}

#[derive(Clone)]
#[repr(C)]
struct TransformAndFadeUniform {
    mat4: [f32; 16],
    fade: f32,
}
...
#[derive(Clone)]
#[repr(C)]
struct TransformAndColorUniform {
    mat4: [f32; 16],
    vec4: [f32; 4],
}
...
fn swapchain_resize(
    ...
    unsafe {
        ...
        super::asi::create_swapchain(connection, image_count, &mut present_images[0]);
        ...
        depth_image = super::asi::create_depth_buffer(connection, &submit_fence);

        // Create multisampling buffer
        ms_image = super::asi::create_ms_buffer(connection);

        // Link Render Pass to swapchain
        render_pass = super::asi::create_render_pass(connection);
...
fn new_texture(vw: &Vw, width: u16, height: u16) -> Texture {
    ...
    let mappable_image = super::asi::Image::new(
        ...
        if staged {
            VkImageUsage::TransferSrcBit
        } else {
            VkImageUsage::SampledBit
        },
    ...
    let layout = unsafe { super::asi::subres_layout(&vw.connection, &mappable_image) };
...
fn set_texture(vw: &Vw, texture: &mut Texture, writer: &Fn(usize) -> [u8; 4]) {
    ffi::copy_memory_pitched(
        &vw.connection,
        texture
            .image
            .as_ref()
...
impl Vw {
    pub(crate) fn new(rgb: Vector) -> Result<(Vw, ::screen::ffi::Window), String> {
        let (mut connection, window) = super::asi::Gpu::new(rgb)?;

        // END BLOCK 2
        let mut image_count = unsafe { super::asi::get_buffering(&mut connection) };

        // Prepare Swapchain
        let mut present_images: [VkImage; 2] = [unsafe { mem::zeroed() }; 2];
        let mut present_image_views = [unsafe { mem::zeroed() }; 2];
        let mut frame_buffers: [VkFramebuffer; 2] = [unsafe { mem::uninitialized() }; 2];

        let (depth_image, ms_image, render_pass) = swapchain_resize(
            &connection,
            &mut image_count,
            &mut present_images,
...
fn draw_shape(connection: &Gpu, shape: &Shape) {
    unsafe {
        ...
        // Is the major slow down factor 60fps down to 20fps when this is used.
        super::asi::cmd_bind_vb(connection, &shape.buffers[..shape.num_buffers]);
        super::asi::cmd_bind_pipeline(connection, shape.instance.pipeline);
        super::asi::cmd_bind_descsets(
            connection,
        ...
        for i in shape.fans.iter() {
            super::asi::cmd_draw(connection, i.1, 1, i.0, 0);
        }
    }
}
...
impl Renderer {
    pub(crate) fn pitch(&self) -> usize {
        unsafe { &mut *self.gui_texture.get() }.pitch
    }

    pub(crate) fn draw(&self, _wh: (u16, u16), writer: &mut FnMut(*mut u8) -> ()) {
        let texture = unsafe { &mut *self.gui_texture.get() };

        if texture.staged {
            let texmem = texture.image.as_ref().unwrap().memory();

            let mapped: *mut u8 =
                unsafe { super::asi::map_memory(&self.vw.connection, texmem, !0) };

            assert!(mapped.is_null()); // TODO debug assert

            writer(mapped);

            unsafe {
                super::asi::unmap_memory(&self.vw.connection, texmem);
                super::asi::copy_image(
                    &self.vw.connection,
                    &texture.mappable_image,
                    texture.image.as_ref().unwrap(),
            ...
            let texmem = texture.mappable_image.memory();

            let mapped: *mut u8 =
                unsafe { super::asi::map_memory(&self.vw.connection, texmem, !0) };

            assert!(mapped.is_null() == false); // TODO debug assert
            ...
            texture.image = None; // TODO: is needed?
        }
    }

    pub(crate) fn new(rgb: Vector) -> Result<(Renderer, ::screen::ffi::Window), String> {
        let (mut vw, window) = Vw::new(rgb)?;

        let solid_vert = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/solid-vert.spv"),
        );
        let solid_frag = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/solid-frag.spv"),
        );
        let texture_vert = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/texture-vert.spv"),
        );
        let texture_frag = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/texture-frag.spv"),
        );
        let gradient_vert = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/gradient-vert.spv"),
        );
        let gradient_frag = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/gradient-frag.spv"),
        );
        let faded_vert = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/faded-vert.spv"),
        );
        let faded_frag = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/faded-frag.spv"),
        );
        ...
        let gui_frag = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/gui-frag.spv"),
        );
        let gui_vert = super::asi::ShaderModule::new(
            &mut vw.connection,
            include_bytes!("../shaders/res/gui-vert.spv"),
        );
        let style_solid = Style::new(
            &mut vw.connection,
            vw.render_pass,
            &solid_vert,
            &solid_frag,
        ...
        let style_nagradient = Style::new(
            &mut vw.connection,
            vw.render_pass,
            &gradient_vert,
            &gradient_frag,
        ...
        vw.connection
            .projection_set(::screen::shared::projection(ar, 0.5 * PI));

        // Add GUI
        let wh = window.wh();
        let mut gui_texture = new_texture(&mut vw, wh.0, wh.1);
        set_texture(&mut vw, &mut gui_texture, &|_i| [0; 4]);
            ...
            Sprite::new(
                ...
                Some(
                    gui_texture
                        .image
                        .as_ref()
                        .unwrap_or(&gui_texture.mappable_image)
                        .clone(),
                ),
                true, // 1 texure
                true, // gui
        ...
        let shape = super::asi::new_buffer(&crate::screen::shared::GUI_MC);
        let texcoords = super::asi::new_buffer(&crate::screen::shared::GUI_TC);

        let gui = Shape {
            instance,
            num_buffers: 2,
            buffers: [shape.buffer(), texcoords.buffer(), unsafe {
                mem::uninitialized()
            }],
            fans: crate::screen::shared::gui_mc_fans(), // make a rectangle (4 vertices)
            transform: unsafe { ::std::mem::uninitialized() },
        };

        ::std::mem::forget(shape);
        ::std::mem::forget(texcoords);
        ::std::mem::forget(style_gui);
    ...
    pub fn update(&mut self, window: &mut Window) -> f32 {
        let rendering_complete_sem = unsafe { super::asi::new_semaphore(&self.vw.connection) };

        let fence = unsafe { super::asi::fence::new(&self.vw.connection) };

        // Get the next image index.
        let next_image_index = unsafe {
            // May fail if window has just been resized.
            loop {
                if let Some(index) = super::asi::get_next_image(&self.vw.connection, fence) {
                    break index;
                } else {
                    // Check for Resize.
                    if window.update() {
                        self.resize(window.wh());
                    }
                }
            }
        };
...
/*        super::base::zsort(
            &mut self.opaque_ind,
            unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.opaque_vec.get()) },
            true,
            self.xyz,
        );*/
        // TODO no ind:
...
/*        super::base::zsort(
            &mut self.alpha_ind,
            unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.alpha_vec.get()) },
            false,
            self.xyz,
        );*/
        // TODO no ind:
        ...
        unsafe {
            // Drop fence when it's done use
            let fence = super::asi::Fence::new(&self.vw.connection);

            super::asi::queue_submit(
                &self.vw.connection,
                &fence,
        ...
        unsafe {
            ...
            super::asi::queue_present(&self.vw.connection, next_image_index);

            super::asi::fence::wait(&self.vw.connection, fence);
            super::asi::fence::drop(&self.vw.connection, fence);

            super::asi::drop_semaphore(&self.vw.connection, rendering_complete_sem);
        ...
        let new = Instant::now();
        let r = new.duration_since(self.earlier).subsec_nanos() as f32 / 1_000_000_000.0;
        self.earlier = new;
    ...
    pub fn resize(&mut self, size: (u16, u16)) {
        ...
        self.vw
            .connection
            .projection_set(::screen::shared::projection(self.ar, 0.5 * PI));

        // Resize GUI Texture, and set GUI sprite to new texture.
        let mut gui_texture = new_texture(&mut self.vw, size.0, size.1);
        set_texture(&mut self.vw, &mut gui_texture, &|_i| [0; 4]);
        unsafe {
            self.gui.instance.set_texture(
                gui_texture
                    .image
                    .as_ref()
                    .unwrap_or(&gui_texture.mappable_image)
            ...
            *self.gui_texture.get() = gui_texture;
        }
    }

    pub fn texture(&mut self, width: u16, height: u16, rgba: &[u8]) -> usize {
        let mut texture = new_texture(&mut self.vw, width, height);

        set_texture(&mut self.vw, &mut texture, &|mut index| {
            index *= 4;
    ...
    pub fn set_texture(&mut self, texture: usize, rgba: &[u8]) {
        set_texture(&mut self.vw, &mut self.textures[texture], &|mut index| {
            index *= 4;
    ...
    pub fn resize_texture(&mut self, texture_id: usize, width: u16, height: u16, rgba: &[u8]) {
        println!("RESIZE TX");
        let mut texture = new_texture(&mut self.vw, width, height);
        set_texture(&mut self.vw, &mut texture, &|mut index| {
            index *= 4;
    ...
    pub fn model(&mut self, vertices: &[f32], fans: Vec<(u32, u32)>) -> usize {
        let shape = super::asi::new_buffer(vertices);
    ...
    pub fn texcoords(&mut self, texcoords: &[(f32, f32)]) -> usize {
        ...
        let vertex_buffer = super::asi::new_buffer(buffer.as_slice());
    ...
    pub fn colors(&mut self, colors: &[f32]) -> usize {
        let vertex_buffer = super::asi::new_buffer(colors);
    ...
    pub(crate) fn textured(
        ...
        if self.models[model].vertex_count != self.texcoords[texcoords].vertex_count {
        ...
        let instance = unsafe {
            Sprite::new(
                ...
                TransformFullUniform { mat4: mat.into() },
                Some(
                    self.textures[texture]
                        .image
                        .as_ref()
                        .unwrap_or(&self.textures[texture].mappable_image)
                        .clone(),
                ),
                true,  // 1 texure
                false, // not gui
        ...
        if alpha {
            let index = unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.alpha_vec.get()) }
                .len() as u32;
            unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.alpha_vec.get()) }
                .push(shape);
            ...
            let index =
                unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.opaque_vec.get()) }.len()
                    as u32;
            unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.opaque_vec.get()) }
                .push(shape);
    ...
    pub(crate) fn gradient(
        ...
        if self.models[model].vertex_count != self.gradients[colors].vertex_count {
    ...
    pub(crate) fn complex(
        ...
        if self.models[model].vertex_count != self.texcoords[texcoords].vertex_count
            || self.models[model].vertex_count != self.gradients[colors].vertex_count
        {
            panic!("TexCoord length doesn't match vertex length");
        }
    ...
    pub(crate) fn transform(&self, shape: &ShapeHandle, mat: Matrix) {
        let uniform = TransformUniform { mat4: mat.into() };
        ...
        match shape {
            ...
            ShapeHandle::Alpha(x) => {
                let x = *x as usize; // for indexing
                (unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.alpha_vec.get()) })[x]
                    .transform = mat;
                ffi::copy_memory(
                    &self.vw.connection,
                    unsafe { ::std::mem::transmute::<_, &mut Vec<Shape>>(self.alpha_vec.get()) }[x]
