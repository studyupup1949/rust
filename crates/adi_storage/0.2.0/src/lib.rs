...
impl Storage {
	...
	pub fn new(app_name: &str, developer: &str) -> Storage {
		let mut app_root = if cfg!(target_os = "linux") {
			match env::home_dir() {
				...
				None => panic!("couldn't get homedir!")
			}
		} else if cfg!(target_os = "windows") {
			"C:/Program Files/AT Root/~/".to_string()
		...
		let computer = whoami::computer();

		Storage { app_root, user, computer }
	}
	...
	pub fn path_internal(&self) -> String {
		let mut path = self.app_root.to_string();

		path.push_str(&self.user);
		path.push_str("/internal/");
...
impl ::std::fmt::Display for Storage {
	fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
		write!(f, "{{ app_root: \"{}\", user: \"{}\", computer: \"{}\" \
			}}",
			self.app_root, self.user, self.computer)
	}
}
