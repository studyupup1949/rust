...
fn main() -> Result<(), std::io::Error>{
    ...
    passed_args.next();
    let mut result_vector: Vec<(String, String, f64)> = Vec::new();
    let ref_ener: f64;
    ...
    while let Some(o) = passed_args.next() {
        ...
        for l in line_iter {
            if enl.is_match(l) {
                result_vector.push((o.clone(),
                get_irrep_in_line(l).to_owned(),
                get_energy_in_line(l) - ref_ener ));
            }
        }
    }
    for (fln, irrp, val) in result_vector.iter() {
        println!("{}\t{}\t{:.8} Eh \t{:.3} eV", fln, irrp, val, val*27.2114)
    }
    Ok(())
}
