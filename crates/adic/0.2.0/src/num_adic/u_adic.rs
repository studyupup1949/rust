...
use crate::AdicError;
use super::{AdicInteger, RAdic, ZAdic, ZAdicValuation};
...
/// With this, you can represent exactly the non-negative integers.
///
/// ```
...
/// ```
///
/// This representation EXACTLY matches the real number base-p digits for a non-negative integer.
/// `2 = 2._5, 123 = 123._5`
...
impl fmt::Display for UAdic {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let p = self.p;
        let digits = if self.digits().next().is_none() {
            "0".to_string()
        } else {
...
mod tests {
    ...
    fn test_add_u_adic() {
        ...
        assert_eq!(uadic!(5, [0, 1]), truncates_zeros);
