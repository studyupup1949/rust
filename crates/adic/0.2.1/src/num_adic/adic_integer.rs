...
use super::{UAdic, ZAdic, ZAdicValuation};
...
pub trait AdicInteger: Debug + Clone + PartialEq + Eq + Hash + Display {
    ...
    fn approximation(&self, n: usize) -> ZAdic {
        ZAdic::new_approx(self.p(), n as u32, self.digits().copied().take(n).collect())
    }
    ...
    fn into_approximation(self, n: usize) -> ZAdic {
        ZAdic::new_approx(self.p(), n as u32, self.into_digits().take(n).collect())
    }
