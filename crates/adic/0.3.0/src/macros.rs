...
macro_rules! zadic_exact_pos {
    ( $p:expr, [$( $known_digits:expr ),* $(,)?]) => {
        $crate::ZAdic::new_exact_pos($p, vec![$($known_digits,)*])
    };
}
...
macro_rules! qadic {
    ( $a:expr, $v:expr ) => {
        $crate::QAdic::new($a, $crate::QAdicValuation::Finite($v))
    };
}
...
mod test {
    ...
    use num::Rational32;
    use crate::{RAdic, UAdic, QAdic, ZAdic, ZAdicVariety};
    ...
    pub fn macros() {
        ...
        let ze = zadic_exact_pos!(5, [1, 2, 3]);
        assert_eq!(ZAdic::new_exact_pos(5, vec![1, 2, 3]), ze);
        ...
        assert_eq!(QAdic::new(UAdic::new(5, vec![1, 2, 3, 4]), crate::QAdicValuation::Finite(-2)), q);
    }
