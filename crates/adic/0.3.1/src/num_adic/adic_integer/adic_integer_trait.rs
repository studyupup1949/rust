...
where Self: Debug + Clone + PartialEq + Eq + Hash + Display + From<UAdic>
    ...
    fn approximation(&self, n: usize) -> ZAdic {
        let c = match self.certainty() {
            ZAdicValuation::PosInf => n,
            ZAdicValuation::Finite(v) => std::cmp::min(v, n),
        };
        ZAdic::new_approx(self.p(), c, self.digits().copied().take(c).collect())
    }
    ...
    /// let mut z = zadic_approx!(5, 1, [3]);
    /// assert!(!z.has_no_certainty());
    /// let _ = z.pop_digit();
    /// assert!(z.has_no_certainty());
    /// ```
    fn has_no_certainty(&self) -> bool {
        matches!(self.certainty(), ZAdicValuation::Finite(0))
    }
