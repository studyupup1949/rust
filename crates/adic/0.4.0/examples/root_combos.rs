...
use clap::Parser;
use adic::{AdicApproximate, AdicInteger, AdicResult, AdicValuation, IAdic, SignedAdicNumber, ZAdic};
...
where AI: AdicInteger + AdicApproximate {
...
fn printzrt(adic_int: &ZAdic, n: u32, precision: usize) -> AdicResult<()> {
    ...
    trunced_int.set_certainty(std::cmp::min(adic_int.certainty(), AdicValuation::Finite(10)));
    if vars.is_empty() {
