...
use adic::{
    ...
    AdicInteger, AdicNumber, AdicResult, AdicValuation,
    HasDigits, IAdic, SignedAdicNumber, ZAdic, ZAdicVariety,
};
...
fn main() -> AdicResult<()> {
    ...
    for (idx, witt_digit) in representation.into_digits().enumerate() {
        ...
        witt_num.set_certainty(AdicValuation::Finite(precision - idx));
        println!("{witt_num}");
    }
