...
use crate::{
    ...
    Divisible, HasDigits, IAdic, QAdic, RAdic, UAdic, ZAdic
};
use super::{
...
impl HasDigitStr for AdicPower<ZAdic> {
    fn digit_str(&self) -> String {
        let pp = self.p_pow();
        match self.adic_ref().approx_or_exact() {
            Either::Left((c, _)) => {
            ...
            Either::Right(i) => {
                AdicPower::new(i.clone(), self.power()).digit_str()
            },
        }
    }
}
...
mod tests {
    ...
    use crate::{
        apow, iadic_neg, qadic, radic, uadic, zadic_approx, zadic_exact_neg,
    ...
    fn display_adic_power() {
        ...
        assert_eq!("(o)nb._25", apow!(zadic_exact_neg!(5, [1, 2, 3, 4]), 2).to_string());
