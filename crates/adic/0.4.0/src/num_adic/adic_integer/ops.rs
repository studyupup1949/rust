...
use crate::{
    ...
    AdicApproximate, AdicInteger, AdicNumber, AdicSized, AdicValuation,
    Divisible, HasDigits, Prime, Sign,
};
...
macro_rules! impl_inv {
    ( $AdicInt:ty ) => {
        ...
        impl num::traits::Inv for $AdicInt {
            type Output = $crate::LazyDiv<$AdicInt>;
            fn inv(self) -> Self::Output {
                $crate::LazyDiv::new(Self::one(self.p()), self)
            }
        }
...
macro_rules! impl_div {
    ( $AdicInt:ty ) => {
        ...
        impl std::ops::Div<&$AdicInt> for $AdicInt {
            type Output = $crate::LazyDiv<$AdicInt>;
            fn div(self, rhs: &$AdicInt) -> Self::Output {
                $crate::LazyDiv::new(self, rhs.clone())
            }
        }
        ...
        impl std::ops::Div<$AdicInt> for &$AdicInt {
            type Output = $crate::LazyDiv<$AdicInt>;
            fn div(self, rhs: $AdicInt) -> Self::Output {
                $crate::LazyDiv::new(self.clone(), rhs)
            }
        }
        ...
        impl std::ops::Div<&$AdicInt> for &$AdicInt {
            type Output = $crate::LazyDiv<$AdicInt>;
            fn div(self, rhs: &$AdicInt) -> Self::Output {
                $crate::LazyDiv::new(self.clone(), rhs.clone())
            }
        }
...
macro_rules! impl_pow {
    ( $AdicInt:ty ) => {
        ...
        impl Pow<u32> for $AdicInt {
            type Output = $AdicInt;
            fn pow(self, mut power: u32) -> Self::Output {

                // Exponentiation by squaring
                let mut out = <$AdicInt>::one(self.p());
                if power == 0 {
                    return out;
                }
...
overload!((a: ?IAdic) + (b: ?IAdic) -> IAdic {
    ...
    let (s_pos, s_trail) = if matches!(a.sign, Sign::Pos) { (true, 0) } else { (false, p.m1()) };
    let (r_pos, r_trail) = if matches!(b.sign, Sign::Pos) { (true, 0) } else { (false, p.m1()) };
...
overload!((a: ?IAdic) * (b: ?IAdic) -> IAdic {
    ...
    if a.is_zero() || b.is_zero() {
        ...
        let ru = b.abs();
        match new_sign {
            Sign::Pos => IAdic::new_pos(p, su.mul(ru).into_digits_vec()),
            Sign::Neg => -IAdic::new_pos(p, su.mul(ru).into_digits_vec()),
        }
...
overload!( - (a: ?RAdic) -> RAdic {
    ...
    if a.fix_d.iter().chain(a.rep_d.iter()).all(|d| *d == 0) {
        ...
        let new_repeats = if a.rep_d.is_empty() {
            ...
            a.rep_d.iter().copied().map(|d| p.mod_neg(d) - 1).collect::<Vec<_>>()
        };
...
overload!((a: ?RAdic) * (b: ?RAdic) -> RAdic {
    ...
    let o_rs = if m > 0 && n > 0 {
        ...
        let h = carmichael(
            (BigUint::from(p).pow(m32) - BigUint::one()) *
            (BigUint::from(p).pow(n32) - BigUint::one())
        );
...
overload!((a: ?ZAdic) + (b: ?ZAdic) -> ZAdic {
    ...
    match (a.variant.clone(), b.variant.clone()) {
        ...
        (ZAdicVariant::Exact(si), ZAdicVariant::Exact(ri)) => {
            let i = si + ri;
            match i.sign {
                Sign::Pos => ZAdic::new_exact_pos(p, i.into_digits().collect()),
                Sign::Neg => {
        ...
        match a.variant.clone() {
            ...
            ZAdicVariant::Exact(i) => {
                match i.sign {
                    ...
                    Sign::Neg => ZAdic::new_exact_pos(p, (-i).into_digits().collect()),
                }
            }
        }
...
overload!((a: ?ZAdic) * (b: ?ZAdic) -> ZAdic {
    ...
    fn approx_mult<AI1, AI2>(p: Prime, c: AdicValuation<usize>, a1: AI1, a2: AI2) -> ZAdic
    where AI1: AdicInteger, AI2: AdicInteger {
        if let AdicValuation::Finite(cert) = c {
