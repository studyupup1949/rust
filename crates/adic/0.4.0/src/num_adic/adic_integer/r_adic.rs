...
use crate::{adic_valid, AdicInteger, AdicNumber, Divisible, HasDigits, Prime};
use super::UAdic;
...
impl RAdic {
    ...
    /// ```
    /// # use adic::{radic, uadic, AdicInteger};
    /// assert_eq!(vec![2, 1], radic!(5, [2, 1], [3, 4]).fixed_digits().collect::<Vec<_>>());
    /// ```
    ...
    /// assert_eq!(vec![3, 4], radic!(5, [2, 1], [3, 4]).repeat_digits().collect::<Vec<_>>());
    /// ```
    ...
    pub fn rational_value(&self) -> Rational32 {
        ...
        let numerator: u32 = repeat_n(&0, self.fix_d.len()).chain(self.rep_d.iter())
            .zip(0..)
            .map(|(d, k)| *d * u32::from(self.p().pow(k)))
            .sum();
...
mod tests {
    ...
    use crate::{
        radic, uadic, zadic_approx,
        AdicApproximate, AdicError, AdicSized, AdicValuation,
        HasDigits, RationalAdicNumber, SignedAdicNumber, ZAdic, AdicVariety,
    };
    ...
    fn r_adic() {
        ...
        assert_eq!(twenty_five().certainty(), AdicValuation::PosInf);
        assert_eq!(pos_17_6().certainty(), AdicValuation::PosInf);
    }
    ...
    fn from_rational() {
        assert_eq!(
            radic!(5, [], [1]),
            RAdic::from_rational(5, Rational32::new(-1, 4))
        );
        assert_eq!(
            radic!(5, [0, 4], [3]),
            RAdic::from_rational(5, Rational32::new(5, 4))
        );
        assert_eq!(
            radic!(5, [], [2, 1, 4, 2, 3, 0]),
            RAdic::from_rational(5, Rational32::new(-1, 7))
        );
    }
    ...
    fn r_adic_norm() {
        ...
        assert_eq!(AdicValuation::Finite(0), one().valuation());
        assert_eq!(Ratio::new(1, 1), one().norm());
        assert_eq!(AdicValuation::Finite(1), five().valuation());
        assert_eq!(Ratio::new(1, 5), five().norm());
        assert_eq!(AdicValuation::Finite(0), neg_1_4().valuation());
        assert_eq!(Ratio::new(1, 1), neg_1_4().norm());
        assert_eq!(AdicValuation::Finite(1), neg_5_4().valuation());
        assert_eq!(Ratio::new(1, 5), neg_5_4().norm());
        assert_eq!(AdicValuation::Finite(0), neg_1_24().valuation());
        assert_eq!(Ratio::new(1, 1), neg_1_24().norm());
        assert_eq!(AdicValuation::Finite(1), neg_5_24().valuation());
