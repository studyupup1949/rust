...
use crate::{
    ...
    Divisible, HasDigits, SignedAdicNumber, ZAdic, ZAdicVariety,
};
...
pub fn polynomial_variety<T>(f: AdicPolynomial<T>, precision: usize) -> AdicResult<ZAdicVariety>
where
T: AdicNumber + SignedAdicNumber + AdicApproximate,
AdicPolynomial<T>: Into<AdicPolynomial<ZAdic>> {
...
pub fn variety_size<AdicNum>(f: &AdicPolynomial<AdicNum>) -> AdicResult<usize>
where AdicNum: AdicNumber + SignedAdicNumber,
AdicPolynomial<AdicNum>: Into<AdicPolynomial<ZAdic>> {
...
where AdicInt: AdicInteger + AdicApproximate {
    // TODO: We shouldn't need to treat the zero case specially; return here when we have better ZAdic conversion
    if adic_int.is_zero() {
        let p = adic_int.p();
        let nusize = n.try_into()?;
        Ok(ZAdicVariety::new(p, vec![ZAdic::zero(p).into_approximation(precision); nusize]))
    } else {
        // nth_root(adic_int) will have valuation of (valuation / n) and max certainty of ((certainty + valuation) / n)
        let adjusted_precision = usize::try_from(n).unwrap() * precision;
        let adic_int = adic_int.approximation(adjusted_precision);
...
/// # Errors
/// Errors if rootfinding encounters problems, e.g. heavily degenerate roots
pub fn num_nth_roots<AdicNum>(adic_int: &AdicNum, n: u32) -> AdicResult<usize>
where AdicNum: AdicNumber + Into<ZAdic> {
