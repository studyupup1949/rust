...
use num::Rational32;
use crate::{adic_valid, AdicApproximate, AdicInteger, AdicNumber, HasDigits, Prime, SignedAdicNumber, ZAdic};
...
where A: AdicNumber {
    ...
    pub fn from_integer_roots<P>(p: P, precision: usize, roots: Vec<i32>) -> AdicVariety<ZAdic>
    where P: Into<Prime> {
        let p = p.into();
        let roots = roots.into_iter().map(|root| ZAdic::from_i32(p, root).into_approximation(precision)).collect();
    ...
    ///
    /// # Panics
    /// Will panic from inappropriate precision
    pub fn from_rational_roots<P>(p: P, precision: usize, roots: Vec<Rational32>) -> AdicVariety<ZAdic>
    where P: Into<Prime> {
        let p = p.into();
        let roots = roots
            .into_iter()
            // Note: zapprox truncates fractions so watch out!!
            .map(|root| (ZAdic::from_i32(p, *root.numer()) / ZAdic::from_i32(p, *root.denom())).zapprox(precision).unwrap())
            .collect();
        AdicVariety::new(p, roots)
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{zadic_approx, zadic_exact_pos, zadic_variety, AdicVariety, ZAdic};
    ...
    fn display() {
        ...
        let var = AdicVariety::new(5, vec![zadic_exact_pos!(5, [1, 0, 3, 0, 0, 0]), zadic_approx!(5, 6, [1, 2, 3, 0, 1, 2])]);
