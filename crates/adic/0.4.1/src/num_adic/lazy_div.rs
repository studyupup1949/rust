...
VR: AdicValuationRing + Sub<Output = VR> {
    ...
    ///
    /// ```
    /// # use adic::{iadic_neg, iadic_pos, qadic, zadic_approx, AdicError, AdicInteger, AdicFraction, AdicNumber, ZAdic};
    /// let four = ZAdic::from(iadic_pos!(5, [4]));
    /// let neg_one = ZAdic::from(iadic_neg!(5, []));
    ...
    ///
    /// ```
    /// # use adic::{iadic_neg, iadic_pos, qadic, zadic_approx, AdicError, AdicFraction, AdicInteger, AdicNumber, ZAdic};
    /// let four = ZAdic::from(iadic_pos!(5, [4]));
    /// let neg_one = ZAdic::from(iadic_neg!(5, []));
