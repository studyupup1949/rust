...
impl MAdicDigits {
    ...
    where A: AdicPrimitive, PowAdic<A>: HasApproximateDigits<DigitIndex = usize> {
        ...
        // Consider doing the digit calculation manually, outside of MAdicDigits.
    ...
    where P: Into<Prime> {
        ...
        //  Multiply (b/p) back in as an MAdic
    ...
    where P: Into<Prime> {
        ...
        if Composite::from(p) == *c {
            // If the Composite is the same as the single Prime, the idempotent is zero
...
mod tests {
    ...
    fn big_idempotent() {
        ...
        // Too slow:
        // assert_eq!(10000, MAdicDigits::prime_idempotent(2, &c, 10000).unwrap().len());
    }
