...
impl DeserializeADI for Record {
    ...
    fn deserialize_adi(&mut self, adi: impl AsRef<str>) -> Result<(), Error> {
        ...
        let mut end = 0;
        let length = 0;
        let rec_len = adi.as_ref().len();

        while start < rec_len {
            let off = if start == 0 { 0 } else { 1 };
            start = match adi.as_ref()[(end + off + length)..rec_len].find('<') {
                Some(pos) => pos + end + off + length,
                None => break,
            };

            end = match adi.as_ref()[start..rec_len].find('>') {
                Some(pos) => pos + start,
                None => break,
            };
            ...
            match tag_def.next() {
                Some(param) => match tag_def.next() {
                    Some(val_len) => {
                        let val_len: usize = val_len.parse().map_err(|err: ParseIntError| {
                            Error::DeserializeValueLength(err.to_string())
                        })?;
                        let val_start = end + 1;
                        if val_start > rec_len {
                            return Err(Error::DeserializeOutOfBounds(val_start));
                        } else if val_start + val_len > rec_len {
                            return Err(Error::DeserializeOutOfBounds(val_start + val_len));
                        }
                        let val = String::from(&adi.as_ref()[val_start..val_start + val_len]);
...
impl DeserializeADI for Header {
    fn deserialize_adi(&mut self, adi: impl AsRef<str>) -> Result<(), Error> {
        ...
        self.record
            .deserialize_adi(&adi.as_ref()[start..adi.as_ref().len()])
    }
}
...
impl DeserializeADI for Doc {
    fn deserialize_adi(&mut self, adi: impl AsRef<str>) -> Result<(), Error> {
        ...
        for (i, rec) in Regex::split(&RE_RECORD, rec_str).enumerate() {
            ...
            record
                .deserialize_adi(rec)
                .map_err(|err| Error::DeserializeRecord(format!("#{}: {err}", i + 1)))?;
...
mod tests {
    ...
    #[test]
    fn test_112_adi_de_error() {
        let mut rec = Record::new();

        let adi_str = "<QSO_DATE:8>20231008 <TIME_ON:4>1145 <CALL:6>dl4bdf <NAME:7>Walter";
        assert!(rec.deserialize_adi(adi_str).is_err());

        let adi_str = "<QSO_DATE:8>20231008 <TIME_ON:4>1145 <CALL:6>dl4bdf <NAME:a>Walter";
        assert!(rec.deserialize_adi(adi_str).is_err());

        let adi_str = "<QSO_DATE:8>20231008 <TIME_ON:4>1145 <CALL:6>dl4bdf <NAME:1> ";
        assert!(rec.deserialize_adi(adi_str).is_ok());

        let adi_str = "<QSO_DATE:8>20231008 <TIME_ON:4>1145 <CALL:6>dl4bdf <NAME:0>";
        assert!(rec.deserialize_adi(adi_str).is_ok());

        let adi_str = "<QSO_DATE:8>20231008 <TIME_ON:>1145 <CALL:6>dl4bdf <NAME:0>";
        assert!(rec.deserialize_adi(adi_str).is_err());
    }
