...
impl ArchitectAgent {
    ...
    pub async fn generate(&self) -> Result<(crate::models::DesignDocument, crate::models::TaskList)> {
        ...
        let mut stream = runner
            .run(
                adk_rust::UserId::try_from("user").map_err(|e| RalphError::Agent { agent: "architect".to_string(), message: e.to_string() })?,
                adk_rust::SessionId::try_from(session_id.as_str()).map_err(|e| RalphError::Agent { agent: "architect".to_string(), message: e.to_string() })?,
                user_content,
            )
            .await
            .map_err(|e| RalphError::Agent {
