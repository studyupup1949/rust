...
use crate::model_factory::create_model_from_config;
...
use adk_rust::agent::{LlmAgentBuilder, LlmEventSummarizer, LoopAgent};
...
pub struct RalphLoopAgent {
    ...
    agent: Arc<dyn Agent>,
    /// Model for context compaction summarizer
    summarizer_model: Option<Arc<dyn Llm>>,
...
impl RalphLoopAgentBuilder {
    ...
    pub fn build_with_model(self, model: Arc<dyn Llm>) -> Result<RalphLoopAgent> {
        ...
        // Build the inner LlmAgent with all tools and 0.5.0 resilience features
        let mut llm_builder = LlmAgentBuilder::new("ralph-worker")
            ...
            .tool(git_tool)
            .tool(exit_loop_tool)
            // 0.5.0: per-tool execution timeout (5 min for long-running tests/builds)
            .tool_timeout(Duration::from_secs(300))
            // 0.5.0: auto-retry transient tool failures (max 2 retries)
            .default_retry_budget(adk_rust::RetryBudget::new(2, Duration::from_millis(500)))
            // 0.5.0: disable a tool after 5 consecutive failures
        ...
        Ok(RalphLoopAgent {
            ...
            // Keep a reference to the model for context compaction
            summarizer_model: Some(model),
...
impl RalphLoopAgent {
    ...
    pub async fn run(&self) -> Result<CompletionStatus> {
        ...
        // 0.5.0: Context compaction — summarize older events to keep context bounded
        let compaction_config = self.summarizer_model.as_ref().map(|model| {
            use adk_rust::runner::EventsCompactionConfig;
        ...
        // 0.5.0: Use Content builder
        let initial_content = Content::new("user")
            .with_text("Start implementing tasks. Read progress first, then get the next task and implement it.");
        ...
        let mut event_stream = runner
            .run(
                adk_rust::UserId::try_from("ralph-agent").map_err(|e| RalphError::Agent { agent: "ralph-loop".to_string(), message: e.to_string() })?,
                adk_rust::SessionId::try_from(session_id.as_str()).map_err(|e| RalphError::Agent { agent: "ralph-loop".to_string(), message: e.to_string() })?,
                initial_content,
            )
            .await
            .map_err(|e| RalphError::Agent {
...
// Model creation delegated to crate::model_factory::create_model_from_config
