...
impl PrdAgentBuilder {
    ...
    pub async fn build(self) -> Result<PrdAgent> {
        ...
        // 0.5.0: use temperature() and max_output_tokens() builder methods
        let agent = LlmAgentBuilder::new("prd-agent")
            ...
            .max_output_tokens(self.model_config.max_tokens as i32)
...
impl PrdAgent {
    ...
    pub async fn generate(&self, prompt: &str) -> Result<PrdResult> {
        ...
        let user_content = Content::new("user")
            .with_text(format!("Generate a PRD for the following project:\n\n{}", prompt));

        // Run the agent and collect the response
        let mut stream = runner
            .run(
                adk_rust::UserId::try_from("user").map_err(|e| RalphError::Agent { agent: "prd".to_string(), message: e.to_string() })?,
                adk_rust::SessionId::try_from(session_id.as_str()).map_err(|e| RalphError::Agent { agent: "prd".to_string(), message: e.to_string() })?,
                user_content,
            )
            .await
            .map_err(|e| RalphError::Agent {
