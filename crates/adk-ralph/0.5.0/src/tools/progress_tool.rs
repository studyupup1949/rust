...
impl Tool for ProgressTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> AdkResult<Value> {
        ...
        match operation {
            "read" => self.read().await.map_err(|e| adk_rust::AdkError::tool(e)),
            "append" => {
                ...
                self.append(entry).await.map_err(|e| adk_rust::AdkError::tool(e))
            }
            "summary" => {
                let tasks_remaining = args["tasks_remaining"].as_u64().unwrap_or(0) as usize;
                self.summary(tasks_remaining).await.map_err(|e| adk_rust::AdkError::tool(e))
            }
