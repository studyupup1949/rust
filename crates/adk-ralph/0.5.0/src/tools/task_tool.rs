...
impl Tool for TaskTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> AdkResult<Value> {
        ...
        match operation {
            "list" => self.list().await.map_err(|e| adk_rust::AdkError::tool(e)),
            "get_next" => self.get_next().await.map_err(|e| adk_rust::AdkError::tool(e)),
            "get" => {
                let task_id = args["task_id"]
                    .as_str()
                    .ok_or_else(|| adk_rust::AdkError::tool("Missing 'task_id' for get operation".to_string()))?;
                self.get_task(task_id).await.map_err(|e| adk_rust::AdkError::tool(e))
            }
            "update_status" => {
                let task_id = args["task_id"]
                    .as_str()
                ...
                self.update_status(task_id, status).await.map_err(|e| adk_rust::AdkError::tool(e))
            }
            "complete" => {
                let task_id = args["task_id"]
                    .as_str()
                ...
                self.complete(task_id, commit_hash).await.map_err(|e| adk_rust::AdkError::tool(e))
            }
