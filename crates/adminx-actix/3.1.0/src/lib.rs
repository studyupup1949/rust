...
use adminx_core::attach::UploadedFile;
...
fn resource_routes(resource: Box<dyn Resource>, mount: &'static str) -> Scope {
    ...
    scope = scope.route("/history/{id}", {
        let r = r.clone();
        web::get().to(move |req: HttpRequest, path: web::Path<String>| {
            let r = r.clone();
            async move { into_response(r.history_page(&make_ctx(&req, mount), &path.into_inner()).await) }
        })
    });
    ...
    // ---------- File attachments ----------
    // Upload rides a dedicated multipart endpoint; `_csrf` travels as a text
    // part of the same body alongside the file. Mirrors the Axum adapter.
    scope = scope.route("/{id}/attach/{field}", {
        let r = r.clone();
        web::post().to(
            move |req: HttpRequest, path: web::Path<(String, String)>, mut payload: Multipart| {
                let r = r.clone();
                async move {
                    let (id, field) = path.into_inner();
                    let mut csrf = None;
                    let mut file = None;
                    while let Some(Ok(mut item)) = payload.next().await {
                        let name = item
                            .content_disposition()
                        ...
                        let content_type = item.content_type().map(|m| m.to_string());
                        // Drain this field's chunks into one buffer.
                        let mut bytes = Vec::new();
                        while let Some(Ok(chunk)) = item.next().await {
                            bytes.extend_from_slice(&chunk);
                        }
                        match name.as_deref() {
                            Some(n) if n == csrf::FIELD_NAME => {
                                csrf = String::from_utf8(bytes).ok();
                            }
                            Some("file") => {
                    ...
                    match file {
                        Some(file) => {
                            into_response(r.attach_file(&ctx, &id, &field, csrf, file).await)
                        }
                        None => into_response(
                            adminx_core::CoreError::BadRequest("no file in upload".into()).into(),
                        ),
                    }
                }
            },
    ...
    scope = scope.route("/{id}/blob/{field}", {
        let r = r.clone();
        web::get().to(move |req: HttpRequest, path: web::Path<(String, String)>| {
            let r = r.clone();
            async move {
                let (id, field) = path.into_inner();
                into_response(r.serve_file(&make_ctx(&req, mount), &id, &field).await)
            }
        })
    });
    scope = scope.route("/{id}/detach/{field}", {
        let r = r.clone();
        web::post().to(
            move |req: HttpRequest,
                ...
                async move {
                    let (id, field) = path.into_inner();
                    into_response(r.detach_file(&make_ctx(&req, mount), &id, &field, csrf).await)
                }
            },
        )
    });
