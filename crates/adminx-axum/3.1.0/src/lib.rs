...
use axum::body::Body;
use axum::extract::{Form, Multipart, Path, RawQuery};
...
fn resource_router(resource: Box<dyn Resource>, mount: &'static str) -> Router {
    ...
    let history = get({
        ...
        move |headers: HeaderMap, Path(id): Path<String>| {
            let r = r.clone();
            async move { into_response(r.history_page(&make_ctx(&headers, "", mount), &id).await) }
        }
    });
    let edit = get({
        let r = r.clone();
    ...
    // File attachments: multipart in, raw bytes out. The `_csrf` field travels
    // as a text part of the same multipart body alongside the file.
    let attach = post({
        let r = r.clone();
        move |headers: HeaderMap, Path((id, field)): Path<(String, String)>, mut multipart: Multipart| {
            let r = r.clone();
            async move {
                let mut csrf = None;
                let mut file = None;
                while let Ok(Some(part)) = multipart.next_field().await {
                    match part.name() {
                        Some(csrf::FIELD_NAME) => {
                            csrf = part.text().await.ok();
                        }
                        Some("file") => {
                            let filename =
                                part.file_name().unwrap_or("upload").to_string();
                            let content_type = part
                                .content_type()
                                .unwrap_or("application/octet-stream")
                                .to_string();
                            if let Ok(bytes) = part.bytes().await {
    ...
    let serve = get({
        let r = r.clone();
        move |headers: HeaderMap, Path((id, field)): Path<(String, String)>| {
            let r = r.clone();
            async move {
                into_response(r.serve_file(&make_ctx(&headers, "", mount), &id, &field).await)
            }
        }
    });
    let detach = post({
        let r = r.clone();
        move |headers: HeaderMap, Path((id, field)): Path<(String, String)>, Form(mut form): Form<HashMap<String, String>>| {
            let r = r.clone();
            let csrf = form.remove(csrf::FIELD_NAME);
            async move {
                into_response(r.detach_file(&make_ctx(&headers, "", mount), &id, &field, csrf).await)
            }
        }
    });
    ...
    let ui_routes = Router::new()
        ...
        .route("/{id}/attach/{field}", attach)
        .route("/{id}/blob/{field}", serve)
        .route("/{id}/detach/{field}", detach);
