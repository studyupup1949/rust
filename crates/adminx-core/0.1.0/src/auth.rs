...
fn set_cookie_value(token: &str) -> String {
    let secure = config().map(|c| c.secure_cookie).unwrap_or(true);
...
/// Build a request context, verifying the cookie token into `claims` if present.
pub fn build_ctx(mount: &str, query: &str, token: Option<&str>) -> ReqCtx {
...
fn mfa_pending(ctx: &ReqCtx) -> bool {
...
pub fn login_page(ctx: &ReqCtx, error: Option<&str>) -> ApiResponse {
    ...
    ui::render("login.html", &c)
}

/// Validate credentials against the admin table and, on success, redirect to the
/// dashboard with the auth cookie set.
pub async fn handle_login(ctx: &ReqCtx, email: &str, password: &str) -> ApiResponse {
    ...
    let user = match storage().find_one_by(&cfg.admin_table, "email", email).await {
        Ok(Some(u)) => u,
        Ok(None) => return login_page(ctx, Some("Invalid email or password")),
    ...
    if !verify_password(password, hash) {
        return login_page(ctx, Some("Invalid email or password"));
    }
...
pub async fn mfa_setup_page(ctx: &ReqCtx, error: Option<&str>) -> ApiResponse {
    ...
    ui::render("mfa_setup.html", &c)
}

/// Confirm the code, enable MFA, generate backup codes, and show them once.
pub async fn handle_mfa_enable(ctx: &ReqCtx, code: &str) -> ApiResponse {
    let user = match current_user(ctx).await {
        Some(u) => u,
        None => return ApiResponse::redirect(format!("{}/login", ctx.mount)),
    };
...
pub async fn mfa_verify_page(ctx: &ReqCtx, error: Option<&str>) -> ApiResponse {
    ...
    ui::render("mfa_verify.html", &c)
}

/// Verify a TOTP or one-time backup code and, on success, upgrade the session.
pub async fn handle_mfa_verify(ctx: &ReqCtx, code: &str) -> ApiResponse {
    let user = match current_user(ctx).await {
        Some(u) => u,
        None => return ApiResponse::redirect(format!("{}/login", ctx.mount)),
    };
