...
pub fn register_resource(resource: Box<dyn Resource>) {
    RESOURCE_REGISTRY.write().unwrap_or_else(|e| e.into_inner()).push(resource);
}
...
pub fn get_registered_menus() -> Vec<MenuItem> {
    let resources = RESOURCE_REGISTRY.read().unwrap_or_else(|e| e.into_inner());
...
pub fn clear_registry() {
    RESOURCE_REGISTRY.write().unwrap_or_else(|e| e.into_inner()).clear();
}

pub fn resource_count() -> usize {
    RESOURCE_REGISTRY.read().unwrap_or_else(|e| e.into_inner()).len()
}
