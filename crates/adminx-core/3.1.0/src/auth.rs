...
pub struct AuthConfig {
    /// HS256 signing key. **Rotating this instantly invalidates every existing
    /// session** — the only built-in "log everyone out" / revoke-all mechanism,
    /// since sessions are stateless (no server-side store to clear).
    pub jwt_secret: String,
    /// Cookie/JWT lifetime in seconds. This is also the **maximum window a leaked
