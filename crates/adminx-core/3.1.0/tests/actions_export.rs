...
async fn actions_and_export() {
    ...
    // Auth is unconfigured here, so the CSRF guard is a no-op and the token can
    // be `None`; the configured path is covered in the adapter csrf tests.
    let ran = w.run_action(&ctx, "toggle", "42".into(), json!({}), None).await;
    assert_eq!(ran.status, 200);
    assert_eq!(body_str(&ran), json!({"toggled": "42"}).to_string());

    // unknown action -> 404
    let missing = w.run_action(&ctx, "nope", "1".into(), json!({}), None).await;
