...
//
// Note: Mongo's primary key is `_id`. This backend maps the default `"id"` onto
// `_id` on both the query side (`id_filter`) and the read side (`doc_to_json`
// exposes `_id` as `id`), so a resource written for SQL runs unchanged on Mongo
// — no `primary_key()` override needed. Setting `primary_key() = "_id"`
