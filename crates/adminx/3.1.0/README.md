...
<img src="https://srotasspace.s3.ap-south-1.amazonaws.com/srotas.svg" alt="adminx" width="110" />

# adminx

**One `Resource` definition → a complete admin panel.**<br/>
Any web framework. Any database. Pure Rust.

[![crates.io](https://img.shields.io/crates/v/adminx.svg)](https://crates.io/crates/adminx)
[![Downloads](https://img.shields.io/crates/d/adminx.svg)](https://crates.io/crates/adminx)
...
[**Install**](#install) · [**Quick start**](#how-to-use) · [**Docs**](https://docs.rs/adminx) · [**Reference**](#reference) · [**Deploy**](#deployment)
...
(SeaORM) or **MongoDB**. The logic lives in a neutral core; frameworks and
databases are thin, swappable adapters — switching either is a one-line change,
and your resource code never moves.

## Quick taste

Implement four methods on a type, and adminx generates the rest:
...
impl Resource for PostResource {
    ...
    fn table_name(&self)    -> &'static str { "posts" }   // table / collection
    fn clone_box(&self) -> Box<dyn Resource> { Box::new(self.clone()) }
    fn permit_keys(&self) -> Vec<&'static str> { vec!["title", "body", "published"] }
}
```

That single `impl` gives you a **REST/JSON API**, a rendered **HTML admin UI**,
create/edit forms, list filters, CSV/JSON export, and role-gated auth — all
generated, on the framework and database of your choice. Nothing else to wire up.
...
- 📝 &nbsp;**Audit logging** (`adminx-audit`) — every create/update/delete recorded with a per-column before/after diff, browsable as an in-panel history
- 📎 &nbsp;**File attachments** (`adminx-attachments`) — attach files to any resource, on local disk or **AWS S3 · GCS · Azure** (`adminx-cloud`)
- 🔎 &nbsp;**Full-text search** (`adminx-search`) — searchable resources with BM25 relevance, kept in sync on every write (powered by the standalone [`searchez`](https://crates.io/crates/searchez) crate)
- 🌱 &nbsp;**Seeding** and admin-user creation from a **CLI** or from code
- 🔄 &nbsp;The *same* resource code on **Actix or Axum**, over **SQL or Mongo**

## The `adminx` family

You depend on the single **`adminx`** facade and enable what you need with Cargo
features — nothing you don't select is ever compiled. Under the hood it's a family
of small, focused crates:

| Crate | Role |
|---|---|
| [`adminx`](https://crates.io/crates/adminx) | the facade you depend on — re-exports the rest behind features |
| [`adminx-core`](https://crates.io/crates/adminx-core) | framework/DB-neutral core: the `Resource` trait, storage & auth |
| [`adminx-axum`](https://crates.io/crates/adminx-axum) · [`adminx-actix`](https://crates.io/crates/adminx-actix) | web-framework adapters |
| [`adminx-seaorm`](https://crates.io/crates/adminx-seaorm) · [`adminx-mongo`](https://crates.io/crates/adminx-mongo) | storage backends (SQL / MongoDB) |
| [`adminx-rbac`](https://crates.io/crates/adminx-rbac) | DB-backed, per-action RBAC |
| [`adminx-audit`](https://crates.io/crates/adminx-audit) | audit logging with before/after diffs |
| [`adminx-attachments`](https://crates.io/crates/adminx-attachments) · [`adminx-cloud`](https://crates.io/crates/adminx-cloud) | file attachments + S3 / GCS / Azure |
| [`adminx-search`](https://crates.io/crates/adminx-search) | full-text search (via [`searchez`](https://crates.io/crates/searchez)) |
...
- [Reference](#reference)
  ...
  - [Audit logging (`adminx-audit`)](#audit-logging-adminx-audit)
  - [File attachments (`adminx-attachments` / `adminx-cloud`)](#file-attachments-adminx-attachments--adminx-cloud)
  - [Full-text search (`adminx-search`)](#full-text-search-adminx-search)
...
| `file_fields()` | | `[]` — file-upload fields (needs [`adminx-attachments`](#file-attachments-adminx-attachments--adminx-cloud)) |
| `search_fields()` | | `[]` — full-text columns (needs [`adminx-search`](#full-text-search-adminx-search)) |
...
### Audit logging (`adminx-audit`)

Record **who changed what**. Add the optional `adminx-audit` crate and every
create / update / delete through the default CRUD is logged with the acting admin
and a **per-column before/after diff** — in the spirit of Rails' PaperTrail. Leave
it out and adminx issues not one extra query.
...
adminx::seaorm::init(&db_url).await?;                  // 1. storage
adminx::seed(audit::migrate_sql()).await?;             // 2. table (SQL backends only; Mongo skips)
audit::init(AuditConfig::default());                   // 3. register the auditor (before auth)
configure_auth(AuthConfig { /* ... */ });
register_resource(Box::new(PostResource));
audit::register_resources();                           // 4. the in-panel log viewer (optional)
```

- **Storage-agnostic & pluggable**, exactly like `adminx-rbac`: rows go through
  the same `Storage` trait, so it works over SeaORM or Mongo.
- **Diffs are real** — only columns that actually changed are stored, as
  `{"column": [old, new]}`; a no-op save records nothing.
- **A per-record History** page is linked from every detail view; the full log
  lives at `/adminx/adminx-audit-versions/list`, and is **append-only** (edits
  are refused with `405`).
- **Failure policy** is best-effort by default (a log failure never blocks the
  write); opt into `AuditConfig::strict()` for compliance.

See the [`adminx-audit`](https://crates.io/crates/adminx-audit) crate for details.

### File attachments (`adminx-attachments` / `adminx-cloud`)

Attach files to any resource. A resource declares its file fields; the detail
page then shows an upload widget, and adminx exposes attach / serve / detach
routes — an ActiveStorage-shaped layer.
...
```rust
// register a backend — local disk here (swap for cloud below)
adminx::attachments::init_local("./adminx_uploads");
// on a SQL backend, create the metadata table:
adminx::seed(adminx::attachments::migrate_sql()).await?;
```

A resource opts in by listing its file fields:
...
- **One file per field**, replaced on re-upload; **purged when the record is
  deleted**, so blobs never outlive their owner.
- **Metadata over SQL or Mongo** (via the `Storage` trait); bytes go to a
  pluggable `BlobStore`.
- **Cloud backends** — enable `adminx-cloud` for **AWS S3, Google Cloud Storage,
  or Azure Blob**, a one-line swap from local disk:

  ```toml
  adminx = { version = "3", features = ["axum", "seaorm", "cloud-aws"] }
  ```
  ```rust
  adminx::attachments::init(adminx::cloud::s3_from_env("my-bucket")?);
  ```

See [`adminx-attachments`](https://crates.io/crates/adminx-attachments) and
[`adminx-cloud`](https://crates.io/crates/adminx-cloud).

### Full-text search (`adminx-search`)

Make resources searchable, with the index kept in sync automatically. Powered by
the standalone [`searchez`](https://crates.io/crates/searchez) crate — the Rust
analog of Searchkick — which adminx simply depends on.
...
The list page then grows a `?q=` search box, results are **ranked by BM25
relevance**, and every create / update / delete keeps the index current. The
in-memory backend is non-persistent, so backfill it at startup:
...
For production, enable `search-meilisearch` and register a
`searchez::MeilisearchBackend` via `adminx::search::init(..)` — nothing about
your resources changes. See [`adminx-search`](https://crates.io/crates/adminx-search).
...
Optional add-on crates, each verified across all four framework/backend combos:
**audit logging** (`adminx-audit`), **file attachments** (`adminx-attachments`,
with **S3 / GCS / Azure** via `adminx-cloud`), and **full-text search**
(`adminx-search`, powered by the standalone `searchez` crate).
...
- **[Deepak Maurya](https://github.com/deepxmaurya)** - Core Developer & Contributor
  <img src="https://srotasspace.s3.ap-south-1.amazonaws.com/deepx.jpeg" alt="Deepak Maurya" width="80" height="80" style="border-radius: 50%;"> 
