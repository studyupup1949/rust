...
use nom::{
    bytes::complete::take,
    number::complete::{be_i16, be_i32, be_u8},
};
    ...
    U: TryIndex + 'a,
{
    while input.len() > 0 && !s.done {
        ...
        let i = match b0 {
            0 => {
                tracing::warn!("reserved Type2 operator 0");
                //error!("reserved {}", b0);
                i
            }
            1 => {
            ...
            6 => {
                ...
                for (i, &d) in s.stack.iter().enumerate() {
                    let dv = if i % 2 == 0 { v(d, 0.) } else { v(0., d) };
                    let p = s.current + dv;
                    s.outline.line_to(p.x, p.y);
                    s.current = p;
                }
            ...
            7 => {
                ...
                trace!("vlineto");
                for (i, &d) in s.stack.iter().enumerate() {
                    let dv = if i % 2 == 0 { v(0., d) } else { v(d, 0.) };
                    let p = s.current + dv;
                    s.outline.line_to(p.x, p.y);
                    s.current = p;
                }
            ...
            12 => {
                ...
                match b1 {
                    ...
                    34 => {
                        ...
                        let _ = bezier!(s, slice, x xy x  x x x);
                        s.stack.clear();
                        i
                    }
                    35 => {
                        ...
                        debug!("flex");
                        let slice = s.stack.as_slice();
                        let _ = bezier!(s, slice, xy xy xy  xy xy xy);
                        s.stack.clear();
                        i
                    }
                    36 => {
                        ...
                        trace!("hflex1");
                        let slice = s.stack.as_slice();
                        let _ = bezier!(s, slice, xy xy x  x xy x);
                        s.stack.clear();
                        i
                    }
                    37 => {
                        ...
                        s.outline.cubic_to(d4.x, d4.y, d5.x, d5.y, d6.x, d6.y);
                        s.current = d6;
                        s.stack.clear();
                        i
                    }
