use super::eexec::Decoder;
use super::parser::{Token, comment, hex_string, spaces, token};
use crate::FontError;
use decorum::R32;
...
impl<'a> RefDict<'a> {
    ...
    pub fn string_entries(&self) -> impl Iterator<Item = (&'a str, RefItem<'a>)> {
        self.iter().filter_map(|(key, val)| match key {
...
impl<'a> RefArray<'a> {
    pub fn iter(&self) -> impl Iterator<Item = RefItem<'a>> {
        let vm = self.vm;
        self.array.iter().map(move |&item| RefItem::new(vm, item))
    }
...
fn print_string(s: &[u8]) -> Cow<'_, str> {
...
impl<'a> RefItem<'a> {
    fn new(vm: &'a Vm, item: Item) -> RefItem<'a> {
        match item {
            ...
            Item::Dict(key) => RefItem::Dict(RefDict {
                vm,
                dict: vm.get_dict(key),
            }),
            Item::Array(key) => RefItem::Array(RefArray {
                vm,
                array: vm.get_array(key),
            }),
    ...
    pub fn as_bytes(&self) -> Option<&'a [u8]> {
        match *self {
            RefItem::String(bytes) | RefItem::Name(bytes) | RefItem::Literal(bytes) => Some(bytes),
            _ => None,
        }
    }
...
impl Vm {
    pub fn new() -> Vm {
        ...
        let error_dict_key = dicts.insert((
            Dictionary::new(),
            Mode {
                read: true,
                write: true,
        ...
        let internal_dict_key = dicts.insert((
            Dictionary::new(),
            Mode {
                read: true,
                write: true,
        ...
        let mut system_dict: Dictionary = OPERATOR_MAP
            .iter()
            .map(|&(name, op)| {
                (
                    Item::Literal(vm.make_lit(name.as_bytes())),
                    Item::Operator(op),
                )
            })
            .collect();
        ...
        let mut user_dict = Dictionary::new();
        {
            ...
            user_dict.insert(
                Item::Literal(vm.make_lit(b"FontDirectory")),
                Item::Dict(font_dict),
            );

            // StandardEncoding …
            use crate::tables::{STANDARD_ENCODING, STANDARD_STRINGS};
            let arr = STANDARD_ENCODING
                .iter()
                .map(|&sid| Item::Literal(vm.make_lit(STANDARD_STRINGS[sid as usize].as_bytes())))
                .collect();
            let standard_encoding = vm.make_array(
                arr,
                Mode {
                    write: false,
                    execute: false,
            ...
            user_dict.insert(
                Item::Literal(vm.make_lit(b"StandardEncoding")),
                Item::Array(standard_encoding),
            );
        }
        let user_dict_key = vm.make_dict(
            user_dict,
            Mode {
                write: true,
                execute: false,
        ...
        // systemdict fuckery
        {
            let system_dict_key = vm.make_dict(
                system_dict,
                Mode {
                    write: false,
                    execute: false,
            ...
            vm.dicts[system_dict_key]
                .0
                .insert(Item::Literal(lit), Item::Dict(system_dict_key));
        }
    ...
    pub fn fonts<'a>(&'a self) -> impl Iterator<Item = (&'a str, RefDict<'a>)> {
        self.fonts.iter().map(move |(key, &dict)| {
            (
                ...
                RefDict {
                    vm: self,
                    dict: self.get_dict(dict),
                },
            )
        })
    }
    fn pop_tuple<const N: usize>(&mut self) -> Result<[Item; N], FontError> {
        if self.stack.len() < N {
            expect!(None, "not enough data on the stack");
        }
        let mut tuple = [Item::Int(0); N];
        for (x, s) in tuple
            .iter_mut()
    ...
    fn exec_array(&mut self, key: ArrayKey, input: &mut Input) -> Result<(), FontError> {
    ...
    fn transform_token(&mut self, token: Token) -> Item {
        match token {
            ...
            Token::Real(r) => Item::Real(R32::from(r)),
        ...
        match item {
            ...
            Item::Name(key) => {
                let item = match self.resolve(Item::Literal(key)) {
                    ...
                    None => error!(
                        "unimplemented token {:?}",
                        String::from_utf8_lossy(self.get_lit(key))
                    ),
                };
        ...
        match item {
            ...
            Item::Array(key) => {
                ...
                if !expect!(self.arrays.get(key), "no item for key").1.execute {
                    ...
                    loop {
                        match self.arrays.get(key).expect("no item for key") {
                            (items, Mode { execute: true, .. }) => match items.get(pos) {
    ...
    fn exec_operator(&mut self, op: Operator, input: &mut Input) -> Result<(), FontError> {
        match op {
            ...
            Operator::DefineFont => match self.pop_tuple()? {
            ...
            Operator::InternalDict => match self.pop() {
                Item::Int(1183615869) => {
            ...
            Operator::If => match self.pop_tuple()? {
            ...
            Operator::IfElse => match self.pop_tuple()? {
                [Item::Bool(cond), Item::Array(proc_a), Item::Array(proc_b)] => {
                    let proc = if cond { proc_a } else { proc_b };
            ...
            Operator::Known => match self.pop_tuple()? {
                [Item::Dict(dict), key] => {
            ...
            Operator::String => match self.pop() {
                Item::Int(n) if n >= 0 => {
            ...
            Operator::ReadString => match self.pop_tuple()? {
                [Item::File, Item::String(key)] => {
                    let string = self.get_string_mut(key);
                    let flag = input.read_to(string);
                    expect!(input.try_parse(spaces), "Failed to parse space");

                    self.push(Item::String(key));
                    self.push(Item::Bool(flag));
                }
                args => error!(
                    "readstring: invalid arguments {:?}",
                    self.display_tuple(args)
                ),
            },
            Operator::Dup => {
                let v = self.pop();
                    ...
                    _ => {
                        ...
                        match (first, last) {
                            ...
                            (Item::String(a), Item::String(b)) => {
                                ...
                                expect!(self.get_string_mut(b).get_mut(..a.len()), "out of bounds")
                                    .copy_from_slice(&a);
                                self.push(last);
                            }
                            (a, b) => {
                                error!("copy: invalid arguments {:?}", self.display_tuple([a, b]))
                            }
                        }
                    }
                }
            ...
            Operator::Put => {
                ...
                match [a, b, c] {
                    [Item::Array(array), Item::Int(idx), any] => {
                        *expect!(
                            self.get_array_mut(array).get_mut(idx as usize),
                            "out of bounds"
                        ) = any;
                    }
            ...
            Operator::MaxLength => match self.pop() {
                Item::Dict(key) => {
            ...
            Operator::CloseFile => match self.pop() {
                Item::File => {
                    input.open = false;
                }
            ...
            Operator::Add => {
                let out = match self.pop_tuple()? {
                    ...
                    [Item::Int(a), Item::Real(b)] | [Item::Real(b), Item::Int(a)] => {
                        Item::Real(R32::from(a as f32) + b)
                    }
                    [arg1, arg2] => error!(
                        "add: unsupported args {:?} {:?})",
                        self.display(arg1),
                        self.display(arg2)
                    ),
                };
    ...
    pub fn display(&self, item: Item) -> RefItem<'_> {
        RefItem::new(self, item)
    }
    pub fn display_tuple<'a, const N: usize>(&'a self, tuple: [Item; N]) -> [RefItem<'a>; N] {
