...
api_request! {
    ...
    ///
    /// <https://api.adoptium.net/q/swagger-ui/#/Checksum/getChecksumByVersion>
    struct Version {
        endpoint: "/v3/checksum/version/{release_name}/{os}/{arch}/{image_type}/{jvm_impl}/{heap_size}/{vendor}",
...
mod tests {
    ...
    fn version_url_without_query() {
        ...
        let expected = url::Url::parse(&format!("{base}/v3/checksum/version/8/linux/x64/jdk/hotspot/normal/eclipse", base = ApiBase::production()))
    ...
    fn version_url_with_query() {
        ...
        let mut expected = url::Url::parse(&format!("{base}/v3/checksum/version/8/linux/x64/jdk/hotspot/normal/eclipse", base = ApiBase::production()))
            .expect("Failed to create expected value");
