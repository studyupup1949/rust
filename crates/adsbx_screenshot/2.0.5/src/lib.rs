...
impl AdsbxBrowser {
    ...
    pub fn take_screenshot(&mut self, options: &AdsbxBrowserOptions) -> Result<Screenshot, Error> {
        ...
        // Delete the div with class banner-adv.
        self.tab.evaluate("document.querySelector('.banner-adv').remove();", false).map_err(|e| {
            Error::BrowserError(format!("Could not remove banner-adv: {}", e))
        })?;
        ui_update_needed = true;
...
mod tests {
    ...
    fn test_browser_options_to_url() {
        ...
        assert_eq!(
            ...
            "https://globe.adsbexchange.com/?hideButtons&hideSidebar&zoom=1.0"
        );
        let options = AdsbxBrowserOptions {
            no_isolation: false,
            hide_buttons: false,
        ...
        assert_eq!(
            options.to_url().as_str(),
            "https://globe.adsbexchange.com/?icao=000000%2CAE0001&hideButtons&hideSidebar&zoom=13.0&reg=ABC123%2CDEF456&showTrace=2021-12-07&foo&bar=baz"
        );
    }
