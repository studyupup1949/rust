...
fn obstruction_from_input(
    ...
    let mut counterexample = Counterexample::new(input.message.clone()).map_err(hg_err)?;
    for (name, value) in assignment_pairs(&input.metadata) {
        counterexample = counterexample
            .with_assignment(name, value)
            .map_err(hg_err)?;
    }
    for witness_id in &witness_hg_ids {
        counterexample = counterexample.with_path_cell(witness_id.clone());
    }
...
fn assignment_pairs(metadata: &Value) -> Vec<(String, String)> {
    ["from_cell_id", "to_cell_id", "route_path", "http_methods"]
        .into_iter()
        .filter_map(|key| match metadata.get(key) {
...
mod tests {
    ...
    #[test]
    fn counterexample_assignments_include_structural_witness_metadata() {
        let finding = violation_finding(FindingInput {
            space_id: "space:test",
            invariant_id: "invariant:test",
            obstruction_id: "obstruction:test",
            obstruction_type: "api_route_missing_auth",
            severity: "high",
            message: "Route touches storage without auth.".to_owned(),
            witness_ids: vec!["cell:route".to_owned()],
            blocked_ids: vec![json!("cell:route")],
            evidence_ids: vec![json!("source:route")],
            recommended_completion_types: vec!["proposed_auth_guard"],
            resolution: "Add authentication.",
            metadata: json!({
                "route_path": "/api/upload",
                "http_methods": ["POST", "PUT", ""],
                "from_cell_id": "cell:route",
                "to_cell_id": "cell:db"
            }),
        })
        .unwrap();

        let assignments = finding
            .obstruction
            .pointer("/higher_graphen/counterexample/assignments")
            .and_then(Value::as_object)
            .unwrap();
        assert_eq!(assignments["from_cell_id"], json!("cell:route"));
        assert_eq!(assignments["to_cell_id"], json!("cell:db"));
        assert_eq!(assignments["route_path"], json!("/api/upload"));
        assert_eq!(assignments["http_methods"], json!("POST,PUT"));
    }
}
