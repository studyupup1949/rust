...
use higher_graphen_core::{DifferenceSeverity, Id, ParticipantRef};
...
pub fn candidate_gluing_review(
    ...
    for mut correspondence in result.candidates {
        ...
        match gluing.result.kind() {
            "failure" => failure_count += 1,
            "candidate" => review_count += 1,
            "success" => success_count += 1,
            _ => {}
        }
        preserved_structure_ids.extend(
            gluing
                .preservation_report
        ...
        preserved_invariant_ids.extend(
            gluing
                .preservation_report
        ...
        for difference in &correspondence.difference_witnesses {
            if difference.severity == DifferenceSeverity::Blocking {
                blocking_difference_ids.insert(difference.id.as_str().to_owned());
            }
        }
