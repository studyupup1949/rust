...
use advreader::{
    AdvReader, AdvReturnValue, Source,
...
fn new_reader(
    ...
    AdvReader::new(
        Source::File(get_example_path("ASAP2_Demo_V171.a2l")),
                            ...
                            match result {
                                Ok(result) => match ref_result {
                                    Ok(ref_result) => {
                                        if result != ref_result {
                                            ...
                                            if res != ref_res {
                                                ...
                                                eprintln!(
                                                    "ERROR: nr={nr} bs={bs}: Inconsistent result:\n{res:?}\n{ref_res:?}!"
                                                );
                                                eprintln!("");
                                            }
                                            break;
                                    ...
                                    Err(ref_e) => {
                                        ok = false;
                                        eprintln!(
                                            "ERROR: nr={nr} bs={bs}: Inconsistent error: {result:?} != {ref_e}!"
                                        );
                                        break;
                                    }
                                },
                                Err(e) => match ref_result {
                                    Ok(ref_result) => {
                                        ok = false;
                                        eprintln!(
                                            "ERROR: nr={nr} bs={bs}: Inconsistent result: {result:?} != {ref_result:?}!"
                                        );
                                        break;
                                    }
                                    Err(ref_e) => {
                                        if e.kind() != ref_e.kind() {
                                            ok = false;
                                            eprintln!(
                                                "ERROR: nr={nr} bs={bs}: Inconsistent error: {e} != {ref_e}!"
                                            );
                                            break;
                                        }
                                    }
