...
mod tests {
    ...
    use crate::{
        AdvReader, ReaderState, Source,
        test::{Results, parse_file},
    };
    ...
    #[test]
    fn reader_read() -> Result<(), Error> {
        unsafe { std::env::set_var("RUST_TEST_NOCAPTURE", "1") };
        for buf_size in 64..300 {
            let reader = AdvReader::new(
                Source::File(get_example_path("strings.txt")),
                Some(buf_size),                   // Buffer size. Default is 65536.
