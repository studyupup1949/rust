...
fn check_results(
    ...
    assert_eq!(results.line_comments.len(), line_comments_cnt);
...
fn test_example() -> Result<(), Error> {
    let reader = AdvReader::new(
        ...
        Some(false),                      // Double double quote escaping. Default is false.
...
#[test]
fn test_example_double_quote_escape() -> Result<(), Error> {
    let reader = AdvReader::new(
        Source::File(get_example_path("example.txt")),
        None,                             // Buffer size. Default is 65536.
        None,                             // Trim. Default is false.
